...
test("众阳门诊费用明确业务拒绝不误报为响应结构异常", async () => {
	const gateway = createZhongyangOutpatientPaymentGateway({
		baseUrl: "https://zhongyang.example.test",
		authSysCode: "thirdSelfMachine",
		fetcher: async () =>
			new Response(JSON.stringify({ success: false, data: [] }), {
				status: 200,
				headers: { "x-request-id": "payment-business-rejected" },
			}),
	});
	...
	await expect(
		...
		name: "ProviderRequestError",
		requestId: "payment-business-rejected",
		retryable: false,
...
test("众阳门诊费用 adapter 拒绝对象和数组金额，避免隐式类型转换", async () => {
	const createGateway = (amount: unknown, requestId: string) =>
		createZhongyangOutpatientPaymentGateway({
			baseUrl: "https://zhongyang.example.test",
			authSysCode: "thirdSelfMachine",
			fetcher: async () =>
				new Response(
					JSON.stringify({
						...
						data: [
							{
								outTradeOrderId: `invalid-amount-${requestId}`,
								amount,
								tradeStatus: "1",
								billDate: "2026-08-16 09:00:00",
							},
	...
	for (const [amount, requestId] of [
		[{ toString: "12.30" }, "object-amount"],
		[["12.30"], "array-amount"],
	] as const) {
		await expect(
			createGateway(amount, requestId).listRecords(input, context),
		).rejects.toMatchObject({
			name: "ProviderRequestError",
			operation: "outpatient-payment-records",
			requestId,
...
test("众阳门诊费用 adapter 不静默忽略异常的稳定身份字段", async () => {
	...
	for (const [identity, requestId] of [
		["valid-order\n-invalid", "identity-control-character"],
		[{ nested: true }, "identity-object"],
		["x".repeat(257), "identity-too-long"],
	] as const) {
		await expect(
			createGateway(
				{
					outTradeOrderId: identity,
					amount: "1.00",
					tradeStatus: "1",
					billDate: "2026-08-16 09:00:00",
				},
