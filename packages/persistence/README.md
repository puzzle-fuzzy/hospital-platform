...
内存患者仓储只用于单元测试和本地组合测试，但它仍需保持与生产事务相同的租约边界：
没有真实 I/O 的目录快照替换路径不会插入隐藏的 `await`，会在同一个事件循环 turn 内完成租约
校验、患者状态变更和 operation 成功标记。这样旧租约在新代次接管后会在入口被拒绝，不会让
测试 fixture 留下半套患者资料；生产 MySQL 仍以事务回滚和条件更新提供最终并发保证。
