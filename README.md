...
> **当前仓库执行检查点（2026-08-27）**：当前 `main` 已推送但尚未部署到线上 API（具体提交以 `git rev-parse HEAD` 为准）；本轮 API 运行时代码变更来源为 `eb4d2eb4`，线上新 API 仍为 `1bc8b0a8`，旧 Python `8001` 未修改。发布基线会因此阻断，不能把本地测试或代码状态当作线上业务验收。详见 [`docs/migration/current-execution-checkpoint-2026-08-27.md`](docs/migration/current-execution-checkpoint-2026-08-27.md)。
