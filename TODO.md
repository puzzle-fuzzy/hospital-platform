...
| 工作流 | 未完成条目 | 当前判断 |
| --- | ---: | --- |
| P0 数据迁移、身份安全与当前准入 | 5 | 账户、患者、支付和便民/健康历史处置策略已完成；旧库存量、凭据轮换、真机证据和切换窗口仍需外部确认 |
...
| **合计未完成** | **47** | **不包含已经确认“不迁移”的策略项** |

复选框总数为 93 项，其中已完成 46 项、未完成 47 项。上表按工作流归并，`P1/P2 临床、患者、便民与外部能力` 包含第 10.2 节的 4 个产品闭环骨架项和 C/D/E 三个批次的 16 个 contract 项。另按标题优先级统计未完成项为：P0 5、P1 20、P2 16、P3 6；混合标题按标题中最高优先级归类。每次清单变更后，提交前都必须重新计算这组数字。
