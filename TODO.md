...
| P1 运维、恢复、告警与运营配置 | 11 | 代码门禁和人工复核工具已有，生产 runbook、告警接入和存量任务盘点仍缺 |
| P3 支付、医保、退款与 HIS 回写 | 6 | 按约定最后处理，当前保持关闭 |
| **合计未完成** | **55** | **不包含已经确认“不迁移”的策略项** |

复选框总数为 85 项，其中已完成 30 项、未完成 55 项。上表按工作流归并，`P1/P2 临床、患者、便民与外部能力` 包含第 10.2 节的 4 个产品闭环骨架项和 C/D/E 三个批次的 16 个 contract 项。另按标题优先级统计未完成项为：P0 10、P1 23、P2 16、P3 6；混合标题按标题中最高优先级归类。每次清单变更后，提交前都必须重新计算这组数字。
...
- [ ] 提供脱敏旧源快照并放入约定证据目录；当前 `.local/health-knowledge/legacy-source-snapshot.json` 缺失。
