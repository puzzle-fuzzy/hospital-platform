...
2026-08-21 只读复核确认：`docs/提供商接入/` 仍只有上述 3 份登记记录，状态均为 `normalized`，没有新增正式
Provider/HIS contract、确认环境或四类脱敏响应样例。本次没有运行另一个会话的自动获取流程，也没有修改其相关文件；病历、报告、
二维码、患者绑定、支付、医保和 HIS 回写的准入边界保持不变。

2026-09-05 已从众阳官方开放平台核对患者档案相关页面 2.1.51、2.1.52、2.1.53、2.1.55，
核对结果见 [`2026-09-05-众阳患者档案绑定官方文档核对.md`](提供商接入/2026-09-05-众阳患者档案绑定官方文档核对.md)。
这次核对补齐了请求字段和类型边界，但没有取得当前机构的卡类型值、自助绑卡授权、失败样例和最终确认语义，
因此患者绑定仍保持 `normalized` 和 fail-closed，不因文档页面可见就打开真实写入。

临床只读的材料缺口已经拆成门诊记录、住院 episode、医生关系、电子导诊/问诊四条独立队列，见
[`迁移/临床只读接入看板-2026-08-25.md`](迁移/临床只读接入看板-2026-08-25.md)。
...
- `docs/Provider契约-v1.md` 或新的版本化 contract 文档；
- `docs/迁移/API矩阵.md`；
- `docs/迁移/旧页面矩阵.md` 与 `remaining-migration-inventory.md`；
- `docs/日志规范.md`；
- `docs/发布/*-acceptance.md`；
...
| --- | --- | --- | --- | --- |
| 患者新增/绑定家属 | 未收到当前环境的正式绑定 contract | [`迁移/患者绑定契约草案.md`](迁移/患者绑定契约草案.md)；PB-01 至 PB-16 | 绑定/建档 endpoint、身份核验、协议确认、owner 归属、重复/超时/幂等样例、撤销和错误码 | 只维护患者目录读取；“添加就诊人”不伪造成功 |
| 门诊就诊记录/病历目录 | 未收到 `out-visit-records` 的正式确认包 | [`迁移/病案目录契约草案.md`](迁移/病案目录契约草案.md)；MR-01 至 MR-15 | 请求/响应 envelope、`patId` 映射、成功空目录/权限拒绝/暂时失败样例、分页/时区、字段白名单和资源授权 | 不注册 `/api/v2/medical-records`；保持 404 |
| 报告目录与详情 | 代码骨架存在，但当前 Provider gate 和资源授权仍未完成 | [`迁移/报告Provider契约审计-2026-08-19.md`](迁移/报告Provider契约审计-2026-08-19.md) | 当前 LIS/PACS/ECG/体检环境的脱敏成功与失败样例、时间格式、详情/附件权限、短期引用和公网验收入口 | 继续保持报告 gate；不开放下载、影像/心电详情或体检接口 |
| 现金支付/医保/HIS 回写 | 已收到历史规范材料，但当前状态仍为 `normalized`，不是已确认生产 contract | [`提供商接入/2026-08-16-门诊结算医保.md`](提供商接入/2026-08-16-门诊结算医保.md) | 当前环境鉴权、金额守恒、6201/6202/6301/6203/6401 状态机、回调/查单/补偿、HIS 顺序和回滚样例 | 按用户要求最后处理；支付、医保和回写 gate 保持关闭 |
