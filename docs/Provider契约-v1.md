...
| 医保费用上传 | `MbsFsiService.forward_6201` | `MedicalInsuranceGateway.uploadFees` | contract/金额校验和真实服务端 adapter 已实现；默认仍由配置/schema/provider gate 保护 |
| 医保支付下单 | `MbsFsiService.forward_6202` | `MedicalInsuranceGateway.settle` | contract/金额校验和真实服务端 adapter 已实现；默认仍由配置/schema/provider gate 保护 |
| 医保结算查询 | `MbsFsiService.forward_6301` | `MedicalInsuranceGateway.query` | contract/金额校验和真实服务端 adapter 已实现；默认仍由配置/schema/provider gate 保护 |
...
- 报告目录的日期参数目前由平台校验为起止日期差值最多 366 天；当前小程序默认窗口是旧端语义的前一个日历月，月底沿用 JavaScript 日历月溢出；provider `endDate` 的包含规则、同日查询和分页一致性仍未冻结，不能把平台默认窗口写成 provider 的条目数量语义。
...
- 两个范围都由服务端固定请求参数，公网只接受 `scope=online|all`；不能让小程序透传 `requestChannel`，也不能用在线结果伪装全部历史。旧的渠道 4 未开放审计仅作为历史记录保留，详见 [`迁移/请求渠道4全量记录契约审计-2026-08-18.md`](迁移/请求渠道4全量记录契约审计-2026-08-18.md)；
...
- `ZHONGYANG_OUTPATIENT_PAYMENT_READY` 仍只控制门诊费用读取。门诊医保支付另有独立 gate，代码已接入统一授权、6201/6202/6301 和微信支付编排；只有 Provider 合同、幂等、查单、授权、公网和真机验收全部通过后才允许打开，退款/HIS 退费仍不在本阶段；
- 原生小程序门诊缴费详情页已提供“医保支付”和“微信支付”入口；服务端通过 owner-scoped `recordId/patientId` 重查 2.6.33，不能伪造支付成功或调用旧万能转发。
