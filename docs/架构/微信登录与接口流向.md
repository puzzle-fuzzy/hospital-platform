...
> **支付入口说明（2026-09-17）**：`apps/miniprogram-pay` 已停止使用，本文中该目录的支付流向仅作历史追溯，不能作为当前开发、验收或发布依据；现行边界见 [`miniprogram-pay` 退役说明](../迁移/miniprogram-pay退役说明.md)。

> 审计基线:2026-09-03(同日完成二次复核:全部路由路径、请求层机制、页面↔接口映射均已反向与源码逐条比对)。本文按当前工作区实际注册的路由(`apps/api/src/app.ts`)、小程序请求层(`apps/miniprogram/src/services/api-client.ts`)与页面代码逐行核对绘制;"代码存在"不等于"用户入口已开放",未开放项在 §7 单独标注。
...
```mermaid
flowchart LR
    User([用户]) --> MP[微信小程序<br/>42 个页面 / src/pages]
...
- **401 处理规则**(`requestWithSession`):
  ...
  - 患者范围只读请求(`/appointments/records`、`/reports`、`/reports/{reportId}`、`/payments/outpatient/records`)走更严格的 `requestWithStableSession`:仅允许 GET、不自动登录、不重放。
...
| GET | `/payments/outpatient/records/{recordId}?patientId&status` | Bearer | 当前用户/就诊人范围内已核对的单笔费用摘要 | 复用上述众阳门诊费用查询，不新增 Provider 字段 |
...
- 页面共 42 个,真正有业务数据接口的约 21 个;纯静态/占位 8 个(见 §7)。
...
| `confirm-registration` | 确认挂号信息 | 患者上下文(GET `/me`+`/patients`)；提交调用 POST `/appointments/holds` → POST `/appointments/registrations`，成功进入平台挂号详情；支付以主小程序和当前服务端合同为准 |
| `appointment-records` | 我的挂号(在线/全部) | 患者上下文;GET `/appointments/records?patientId&scope=online|all`(在线前后各三个月，月底遵循旧服务日历月溢出) |
...
| 页面 | 用途 | 接口 |
| --- | --- | --- |
| `outpatient-payment` | 门诊缴费(待缴/已缴) | 患者上下文;GET `/payments/outpatient/records?patientId&status=unpaid` 和 `status=paid`；点击记录进入已核对摘要详情。**不调 wx.requestPayment** |

### 报告区

| 页面 | 用途 | 接口 |
| --- | --- | --- |
| `report-directory` | 报告查询 | 患者上下文;GET `/reports?patientId&startDate&endDate`（默认当前日前一个日历月，月底沿用旧端月份溢出） |
...
| `my` | 我的 Tab | 登录链;GET `/patients` + GET `/me`;微信头像昵称授权后、且服务端昵称仍是默认"微信用户"时,经全局资料仓库 PUT `/me/profile` 回写昵称/性别 |
| `hospital` | 互联网医院 Tab | 无(纯导航壳) |
| `hospital-list` | 医院列表 | GET `/me`(恢复会话后导航) |
| `feedback` | 意见反馈 | 无(本地文案) |
| `smart-customer` | 智能客服固定 H5（不交付平台登录态） | 无；固定 `https://html.ydrj.top` WebView |
