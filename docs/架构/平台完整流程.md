...
“我的挂号”覆盖中国标准时间前后各三个月，月底遵循旧服务 JavaScript 日历月溢出语义；“爽约记录”只覆盖过去 90 天。两者复用 Provider 记录接口，但页面和 service 的窗口语义不同，未知窗口不会静默降级为普通历史查询。

### 9.1 “我的挂号”和“爽约记录”的差别

| 页面 | 小程序传入的窗口 | 服务端最终查询窗口 | 页面处理 |
| --- | --- | --- | --- |
| 我的挂号 | 当前日期前后三个自然月 | 过去/未来各三个自然月，按 Asia/Shanghai 生成并保留月底溢出 | 本地分页和展示预约状态 |
...
| 我的挂号 | `GET /appointments/records` → 众阳预约记录 | 只读已实现 | 过去/未来各三个自然月窗口 |
| 爽约记录 | 同一 `GET /appointments/records` | 只读已实现 | 仅过去 90 天；不是另一个 Provider endpoint |
| 报告目录 | `GET /reports` → LIS/PACS/ECG 三路目录 | 只读已实现 | 默认前一个日历月；月底沿用旧端月份溢出；三路聚合整批校验 |
