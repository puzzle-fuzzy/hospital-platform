...
- 当前小程序候选来源为 `b9ce8ae1ccb17a2be80cabdd0211d613e1a975bf`，14 个页面运行包已重新构建并通过 `runtime:verify`；小程序为 122 项测试、1052 个断言；本轮修正了首页目录旧请求在新请求或页面卸载后错误回写的生命周期竞态，并补充“我的”页普通资料慢响应不阻塞患者目录关键路径，保留门诊缴费首次加载期间标签状态快照竞态、普通资料 409 冲突后的强制刷新、报告详情/门诊费用失败态患者上下文、“我的”页未迁移菜单的 WXML key 和根入口全局脚本兼容，未改变服务端或旧服务。
