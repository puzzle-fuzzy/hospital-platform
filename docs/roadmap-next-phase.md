...
- 2026-08-16 16:57 CST 只读复核发现：SSH 主机上的 `current=55fce6c` 进程在 `10.0.0.3:18081` 返回 `database/schema=unavailable`，而公网 `test-hp.meiyi.pro/api/v2/health/ready` 在随机 query 和 `no-cache` 下仍返回三项 `ok`，且缺少 `Cache-Control: no-store`；公网 upstream、外层转发或另一份 release 尚未完成 provenance 对齐，不能把公网 ready 当作当前主机 release 证据。详见 [`release/production-coexistence-readonly-audit-2026-08-16.md`](release/production-coexistence-readonly-audit-2026-08-16.md)。
...
14. 在任何 provider 或真机业务验收前，先取得外层阿里云转发配置或运维权限，使用同一个 `X-Request-Id` 关联公网 `/api/v2/health/ready`、转发日志、Bun 进程和 release；确认公网实际指向经过 readiness 的目标后，才能继续当前只读业务验收。
...
- 2026-08-16：收紧普通个人资料页的并发读取边界；下拉刷新使用最后一次请求获胜守卫，加载/保存期间禁止保存，避免旧 GET 覆盖新 `version` 后制造不必要的 409。真实微信资料读写和真机验收仍未完成。
...
- 2026-08-16 16:57 CST：线上只读复核确认公网 readiness 与 SSH 主机 `55fce6c` 进程的依赖状态不一致；随机 query 和 `no-cache` 仍无法消除差异，暂按公网 upstream/release provenance 未对齐处理，禁止用公网 `200` 推导当前代码已发布或业务已验收。
