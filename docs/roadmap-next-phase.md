> **当前事实源（2026-08-27，优先于本文其余历史段落）**：新 Elysia API 线上 release 为 `0aaa13b53cb6e21b59b332dbd4e2b982a5aba1e7`，旧 Python 服务继续监听 `0.0.0.0:8001`；本轮已完成新 API 的 production preflight、隔离 smoke、原子切换、公网 runtime smoke 和稳定错误码日志投影验证，没有修改或停止旧服务。项目最终固定使用微信原生 `tabBar`。最新小程序运行相关源码和本地 live 运行输入为 `02865d385a9c09876dc51da1ffb71183139a559b`（短提交 `02865d3`），共 40 个页面；该来源已从通过校验的运行包原子发布到本地 live `dist`，发布后 pending 目录已清理。本轮继续保持共享患者外壳、电子锦旗/表扬信、二维码和健康数值工具的安全边界，真实临床 Provider、物流/采血号源/公开记录仍关闭；就诊页今日预约摘要已补齐但实时叫号仍未开放；健康自测保留不带临床结论的 BMI/血压安全数值子集，其规则版本已固定，其余临床入口仍按 contract 关闭；九个真机证据域仍待采集。健康百科正式审核 bundle、协议同意/撤回/审计、二维码真实能力、支付/医保、临床真实读取、实时就诊和外部 web-view 继续关闭；旧端 64 页面逐页落点台账和 34 个冻结入口门禁有效；完整推进队列见 [`migration/full-migration-handoff-2026-08-25.md`](migration/full-migration-handoff-2026-08-25.md)。服务端切换证据见 [`release/candidate-0aaa13b5-production-acceptance-2026-08-27.md`](release/candidate-0aaa13b5-production-acceptance-2026-08-27.md)。
> **当前仓库 API 候选（2026-08-27）**：当前 `main`（候选提交以 `git rev-parse HEAD` 为准）已推送并完成 API-only 远端原子发布；其中 API 运行时代码变更来源为 `0aaa13b5`，将统一错误处理器已知异常投影为稳定日志错误码，线上为 `0aaa13b53cb6e21b59b332dbd4e2b982a5aba1e7`。详细来源和下一步见 [`migration/current-execution-checkpoint-2026-08-27.md`](migration/current-execution-checkpoint-2026-08-27.md)。
> 当前线上服务端 release（2026-08-27）：`0aaa13b53cb6e21b59b332dbd4e2b982a5aba1e7`，已完成候选 preflight、隔离 smoke、原子切换、公网 runtime smoke 和稳定错误码日志投影实测；该运行层证据不等价于真实 Provider 或支付业务成功。
...
> **2026-08-26 owner 作用域复核补充**：普通资料 `normalizeUserProfileReadModel`、患者目录和报告引用的 domain 入口现都会先校验当前会话 owner 的 opaque 标识，再比较仓储归属；预约历史与门诊费用 service 继续校验 owner、患者和临床映射。该修正只加固运行时 fail-closed 边界，不扩大 API、Provider、支付或真机验收范围，详见 [`migration/owner-scope-runtime-invariants-2026-08-26.md`](migration/owner-scope-runtime-invariants-2026-08-26.md)。
> **2026-08-27 审计可靠性补充**：`2580a1fe` 将未开放入口的源码调用扫描改为保留模板 `${...}` 插值中的可执行表达式，避免把模板文案与真实调用混淆；随后 `cbb43040` 又将集中 API client 的导入路径从字符串剥离扫描中分离，补齐了原先可能漏报的静态导入规则；本轮 `02865d3` 在前述基础上修复 App.onLaunch 阶段全局资料初始化对 getApp 注册时序的依赖。定向测试、15 个关闭态目标和全量门禁均通过。本修正只增强静态/客户端边界，不改变线上 API、旧 Python 服务或任何业务开放状态。
...
> 预约适配器错误分类提交 `4404c556` 已属于当前 `main` 的仓库历史；它是否进入线上 release 只以对应的服务端 release 证据为准，不能从 Git 祖先关系推断线上已部署。本轮不修改、不部署旧 Python 服务，也不把仓库代码状态冒充线上业务验收。

> 历史发布基线更新（2026-08-24 19:54 CST）：当时线上服务端 release 为 `8eb51b5ffe85b0b8f8a032783f893117d3df549d`；当时小程序运行包来源为 `13f597ea9ee3f65b9be858117826d948339d904a`（提交 `13f597e`）。服务端独立 adapter 发布已完成，真机业务三层证据仍待；当前版本以本文顶部事实源为准。
 > 历史小程序底栏修正（2026-08-25）：上一候选 custom-tab-bar 在真机扫码后完全未呈现，项目已最终固定为微信原生 `tabBar` 的单一 `list` 声明；页面不再同步 selected，也不再保留第二套底栏。`fcc6630e` 是当时 live dist 的上一候选；`baa31df0` 和 `7627843a` 仅作历史交接。旧 Python `8001`、线上服务、数据库和 Redis 未修改。详见 [`release/candidate-baa31df0-breadth-migration-gate-2026-08-25.md`](release/candidate-baa31df0-breadth-migration-gate-2026-08-25.md)。
 > 历史下一步只读核对（2026-08-25）：通过内网专用 inspection key 对 `192.168.112.172` 做只读检查，新 API `hospital-platform-api-v2.service` 仍为 `active`、监听 `10.0.0.3:18081`，旧 Python Gunicorn 仍监听 `0.0.0.0:8001`；当时 Elysia release 为 `8eb51b5ffe85b0b8f8a032783f893117d3df549d`。当时计划发布 pending 的 `fc70fa0b`，该记录不覆盖当前 `a6319d7` 候选；本次没有重启、写配置、写 MySQL/Redis 或修改旧 Python 服务。
> 本段优先于本文下方旧日期、旧 release 或旧运行包叙述；旧值只作为历史记录，不作为当前验收入口。
> 历史业务修正（2026-08-24）：已确认生产 Provider 的渠道 3 在线记录与渠道 4 全部历史均可只读返回；“我的挂号”使用服务端拥有的 `scope=online|all` 双查询，全部标签重新请求渠道 4 并保留取消记录。该修正已随此前服务端候选进入线上，当时 `8eb51b5f` 仅补强四个只读 adapter 的运行时输入门禁，支付、医保、预约写入、取消和 HIS 回写仍不受影响；当前版本以本文顶部事实源为准。
> 历史健康域复核（2026-08-24）：健康百科工程骨架已具备，但没有真实脱敏内容 bundle、临床审核和 staging 发布/撤回证据；旧端自测评分、BMI/血压分类存在规则版本与适用人群缺口。健康百科、自测、风险评估和计算器继续保持关闭态，不导入 fixture、不挂载患者路由。详见 [`migration/health-content-and-self-test-audit-2026-08-24.md`](migration/health-content-and-self-test-audit-2026-08-24.md)。
> 历史线上观察（2026-08-24 17:37 CST）：对新 API journald 做有界脱敏关联审计，预约历史 8/8、预约科室 1/1、预约排班 1/1、门诊费用只读 4/4 均通过同链请求/业务成功/HTTP 2xx；这只是服务器链路证据，仍缺当时小程序页面和客户端 requestId，不能标记为真机完成。报告目录和普通资料更新没有请求，支付/医保/HIS 继续关闭。详见 [`release/current-business-correlation-observation-2026-08-24-1737.md`](release/current-business-correlation-observation-2026-08-24-1737.md)。
> 历史公网认证边界只读审计（2026-08-24 18:41 CST）：live/ready/ping 均为 200，ready 的 database/redis/schema 均为 `ok`；普通资料、预约记录和门诊费用的未授权 GET 均为 401。该探针不触发登录、患者读取、Provider 或写入，不能替代真机业务证据。详见 [`release/current-api-unauthorized-boundary-audit-2026-08-24.md`](release/current-api-unauthorized-boundary-audit-2026-08-24.md)。
> 历史预约 adapter 门禁（2026-08-24）：直接调用众阳预约记录 adapter 时也会在触网前拒绝未知 `scope`、`all` 混入日期、倒序日期和未知字段，避免组合根绕过 service 后把未定义渠道发送给 Provider。该修正只收紧错误输入，不扩大预约写入、取消、支付、医保或 HIS 能力。
> 当前只读 adapter 门禁（2026-08-24，历史服务端事实）：线上 `8eb51b5f` 已部署的 adapter 门禁和 `118/118` 全量测试仍是线上事实；本地 `4404c556` 新增预约包络错误分类回归后，全量 adapters 为 `119/119`，但尚未部署。费用时间/状态/未知字段、报告目录日期/来源/患者引用、患者 `unionId`、预约科室/排班日期与筛选标识均在触网前 fail-closed。小程序线上运行包仍为 `13f597e`，该段不代表当前 pending；旧 Python `8001` 未修改。新修正详见 [`release/appointment-provider-envelope-error-classification-2026-08-24.md`](release/appointment-provider-envelope-error-classification-2026-08-24.md)。
> 历史线上窗口复核（2026-08-24）：15:50–15:52 CST 的微信登录/患者同步观察仍是历史真机窗口；17:37 CST 的新增只读审计确认预约历史、预约目录和门诊费用形成同链 HTTP 2xx，但没有把日志成功误写成页面验收。真机页面截图、显式第二患者切换和客户端 requestId 仍待补齐，详见 [`release/current-business-correlation-observation-2026-08-24-1737.md`](release/current-business-correlation-observation-2026-08-24-1737.md)。
...
> 当前开发者工具执行要求：当前 `02865d3` 已完成原子发布到 `apps/miniprogram/dist/`，不再执行旧的 `runtime:publish-pending` 或使用 `87ad1092`。现场应直接打开 `apps/miniprogram/dist/` 独立工程并普通编译，确认四项共享底栏只有一份、就诊三标签患者栏正确显示、今日预约摘要不冒充实时状态、快递/采血预约空态、电子锦旗/健康表扬信关闭态和患者切换入口正确、切换后当前图标/文字变蓝；同时确认就诊和互联网医院关闭态的插图、标题、说明纵向居中，并确认 App.onLaunch 不再出现 `globalData` 未定义异常。历史线上 `13f597e`、旧 live `fcc6630e`、`02dbf10`、`d204425`、`87ad1092` 和其它候选不能作为本次修复证据，不能打开父目录、`src/` 或另一个旧 `dist` 工程，也不能新增页面级底栏。

> 本轮后续唯一执行入口：基于已经发布到 live `dist` 的 `02865d3` 重新普通编译并生成二维码，从新二维码开始只读 preflight，采集四 Tab、就诊三标签患者栏、今日预约摘要、患者显式切换、预约历史/爽约、门诊费用只读和普通资料的同链证据；未取得页面、客户端 requestId、服务端 Pino 和 Provider 低敏 requestId 四方关联前，不把代码测试或健康检查写成业务完成。
> 当前小程序会话/导航门禁（2026-08-27，本地未部署）：患者范围 GET 收到 `503 persistence-temporarily-unavailable` 时保留 token、不重新登录、不重放旧 `patientId`；主 Tab 程序化入口强制使用 `switchTab`，当前 Tab 重复导航会 no-op，底栏由微信原生 `tabBar` 统一维护，会话恢复期间患者卡片只显示固定占位。本轮最新候选回归为 `345 pass / 0 fail / 3736 expect()`，运行包来源为已发布 live `02865d3`；本轮选择页刷新并发门禁已补齐，微信资料被拒绝后需要用户点击设置页重试；今日预约摘要只展示预约事实，不展示实时叫号，快递和采血预约页只展示旧端真实空态，电子锦旗和健康表扬信只展示安全关闭态；临床/服务入口和我的问诊共享当前就诊人上下文，患者绑定/签名页可查看协议原文但不写入同意；会话失效时会清理旧 owner 的微信资料缓存；报告/费用底层请求和预约排班/历史底层构造器已增加运行时输入门禁；真机页面证据仍待。
> 历史日志链路审计（2026-08-24，服务端当时已随 `e5d941ae` 部署）：小程序每个 `wx.request` 生成独立 `x-request-id`，服务端归一化后由 Pino HTTP 事件、业务 service 和 Provider 低敏 request id 共用同一 `traceId`；请求/响应正文、Authorization、患者身份和 Provider 原文均不进入日志。API 日志/错误定向回归 `27/27`、requestId/traceId 回归 `7/7`、`pnpm logging:audit` 的 84 个事件登记均通过。真机日志关联仍待；详见 [`release/observability-chain-audit-2026-08-24.md`](release/observability-chain-audit-2026-08-24.md)。当前日志运行层事实以线上 release `0aaa13b5` 对应证据为准。

> 当前服务端与小程序采用分层发布：线上服务端 release 为 `0aaa13b5`，线上历史小程序运行包来源为 `13f597e`，本地开发者工具 live `dist` 已原子发布为 `02865d3`；旧 Python `8001` 继续共存。当前可以从 `02865d3` 重新生成二维码开始真机业务三层证据，但不能把本地 live `dist` 写成微信线上版本，也不能把运行层 smoke 当作业务完成。

> 历史候选刷新（2026-08-22）：服务端 release 曾为 `0e2a366efcca8da25d7edd4a286781f2d3dfdbec`；小程序运行包来源为 `4ba492a3fdae8283409bd2ab4a0a45247c46600c`（提交 `4ba492a`）。本行仅作追溯，不能覆盖上方当前 `02865d3` 的本地 live 边界。

> **历史事实源（2026-08-25，已被 2026-08-26 发布记录覆盖）**：该窗口观察到新 Elysia API release 为
> `8eb51b5ffe85b0b8f8a032783f893117d3df549d`，`hospital-platform-api-v2.service=active`，监听
> `10.0.0.3:18081`；旧 Python 服务继续监听 `0.0.0.0:8001`。当前线上 release、原子切换和旧端共存证据以
> [`release/e5d941ae-production-acceptance-2026-08-26.md`](release/e5d941ae-production-acceptance-2026-08-26.md) 为准。
> 内网 `/health/ready` 返回 `database=ok`、`redis=ok`、`schema=ok`。Worker 继续保持 inactive。
>
> 历史小程序代码候选 `90d5ab0323b33fb620c83b86a8248b31c0ae3341` 和 `fc70fa0b78a29f564c4e3e4b35ab28e4afb3fde` 仅作追溯；当前本地 live 运行输入以本文顶部 `02865d3` 为准。开发者工具锁定导致的历史候选切换阻塞已解除，当前应从 `apps/miniprogram/dist/` 重新生成二维码；线上正式小程序仍未上传。
> 四个主 Tab 由微信原生运行时统一渲染，支付、医保、结算、预约写入、取消、患者绑定、二维码、病历和 HIS 回写均关闭。
>
...
> **最新小程序启动修正（2026-08-27）**：运行相关源码和本地 live 运行包为 `02865d385a9c09876dc51da1ffb71183139a559b`（`02865d3`），App、API、会话代际和资料订阅的 `globalData` 读取已统一经过启动桥，40 个页面入口通过 `runtime:verify`。遇到旧的 `globalData` 未定义异常时，必须关闭新项目开发者工具/真机调试并重新打开 `apps/miniprogram/dist/`、普通编译、重新生成二维码；详见 [`release/miniprogram-globaldata-startup-fix-2026-08-27.md`](release/miniprogram-globaldata-startup-fix-2026-08-27.md)。

# 下一阶段实施路线图

> **待发布小程序启动保护候选（2026-08-27）**：`02865d3`（完整来源
> `02865d385a9c09876dc51da1ffb71183139a559b`）已通过定向回归、类型检查和 pending
> 运行包校验；当前 live `dist` 仍为 `02865d3`。开发者工具释放目录锁后，先原子发布候选并
> 重新普通编译，再继续真机业务证据采集。候选只修复 App 启动资料初始化的同步异常保护，
> 不改变 API、数据库、Redis、Provider 或旧 Python 服务。

> **当前小程序候选（2026-08-27）**：`02865d385a9c09876dc51da1ffb71183139a559b`（`02865d3`）已通过构建、回归和运行包校验并原子发布到 live `dist`；本候选修复 App.onLaunch 全局资料初始化时序，同时保留预约排班和预约历史请求构造器的运行时字段、日期和范围边界。新的九域真机清单为 [`release/device-evidence-02865d38-pending.json`](release/device-evidence-02865d38-pending.json)，当前已在正确的独立工程中完成开发者工具运行包验证，启动阶段未再出现 `globalData` 未定义异常；真实微信真机业务证据尚未采集，所有整域结果继续保持 `pending`。详见 [`release/miniprogram-globaldata-startup-fix-2026-08-27.md`](release/miniprogram-globaldata-startup-fix-2026-08-27.md)。

> **最新小程序候选事实（2026-08-27）**：当前运行相关源码和本地 live 运行输入为 `02865d385a9c09876dc51da1ffb71183139a559b`，40 个页面；该候选已原子发布到 live `dist`，并修复 App.onLaunch 全局资料初始化时序，保留会话失效时旧 owner 微信资料缓存清理和就诊/互联网医院关闭态卡片纵向居中。本轮继续保持共享患者外壳、电子锦旗和表扬信记录区域、二维码安全壳和健康数值工具的安全边界，临床/服务入口的当前就诊人上下文、重试和选择入口、患者签名和消息订阅的原生安全展示层、采血预约、今日预约摘要与安全数值子集继续保持边界，未开放临床分级、采血号源、公开记录或实时叫号；预约排班和预约历史底层构造器拒绝未知字段、非法日期及范围错配。正式健康审核 bundle 缺失时健康百科仍保持 fail-closed，协议同意/撤回/审计仍关闭。支付、医保、临床真实读取、外部 WebView 和预约写入仍按各自 contract 关闭。

> **当前广度事实源（2026-08-27，运行包来源 `02865d385a9c09876dc51da1ffb71183139a559b`）**：旧端 64 个页面已全部登记；新端现有 40 个注册页面，健康自测中的 BMI/血压安全数值工具已进入 `partial`，其余临床页面继续使用 `surface-only` 或关闭态，健康百科三个只读页面、协议原文只读页、旧端“我的快递”和采血预约真实空态、电子锦旗/健康表扬信安全页面结构、患者签名/消息订阅安全展示层、临床/服务入口共享当前就诊人上下文以及我的问诊患者作用域已接入，微信资料拒绝后的设置页重试、选择页刷新并发门禁和我的快递三态已接入，统一状态页已展示迁移阶段与旧入口覆盖范围。当前运行包已完成静态校验和原子发布，并修复 App.onLaunch 全局资料初始化时序，补齐会话失效时旧 owner 微信资料缓存清理、关闭态卡片纵向居中和预约查询底层输入边界；九个真机证据域仍为 pending。正式健康内容、协议同意、临床审核、支付、医保、预约写入和 HIS 回写仍按原顺序关闭。

> **当前仓库事实补充（2026-08-27）**：本地 live 小程序运行包来源为 `02865d385a9c09876dc51da1ffb71183139a559b`，小程序核心回归通过；启动保护候选已完成原子发布且 pending 目录已清理。本轮继续保持共享患者外壳、电子锦旗和表扬信记录区域、二维码安全壳和健康数值工具的安全边界，不改变线上服务、live `dist` 之外的旧服务或其他会话适配器；候选修复 App.onLaunch 全局资料初始化时序，协议同意、撤回、临床真实读取、采血号源、公开记录、实时叫号和支付医保继续关闭，上方旧候选段落保留为历史追溯。服务端切换证据见 [`release/candidate-0aaa13b5-production-acceptance-2026-08-27.md`](release/candidate-0aaa13b5-production-acceptance-2026-08-27.md)。
...
> 当前小程序最新 live 运行包来源为 `02865d385a9c09876dc51da1ffb71183139a559b`，共 40 个注册页面，候选已通过构建、类型检查、旧端 64 页面台账、自动化测试和运行包静态校验，并完成原子发布；启动保护候选已覆盖 live，pending 目录已清理。项目最终固定使用微信原生 `tabBar`。现场应直接打开独立工程并普通编译核对完整来源指纹。线上服务端 release 为 `0aaa13b53cb6e21b59b332dbd4e2b982a5aba1e7`；线上历史小程序运行包 sourceRevision 仍为 `13f597ea9ee3f65b9be858117826d948339d904a`，真实页面与服务端同链业务证据仍待。本页下方带历史日期的候选只作追溯。

## 历史事实源（2026-08-22，仅供追溯）

本节记录当时的发布窗口，已被本文顶部 2026-08-24 交接信息覆盖，不能作为当前线上版本依据。该窗口的 Git `main` 已包含服务端运行候选与线上发布记录；当时服务端已验证为
`0e2a366efcca8da25d7edd4a286781f2d3dfdbec`，小程序运行包来源为
`4ba492a3fdae8283409bd2ab4a0a45247c46600c`。SSH 已通过中转内网地址恢复；本次只重启新 API，旧 Python `8001` 继续共存。

| 事实 | 当前状态 | 证据与限制 |
| --- | --- | --- |
| 新旧服务共存 | 当前已证明新 API 与旧 Python `8001` 共存 | `2a2acd9` 原子切换后新 API 重启，旧 Python PID 集合前后不变；Worker 仍 inactive |
| 服务端生产候选 | `0e2a366e` | preflight、隔离 `18082` smoke、切换后内网/公网探针通过；门诊费用只读引用作用域修正已部署，支付/医保/HIS 未触发 |
| 小程序运行包 | `4ba492a`、14 个页面、无测试 JS | 构建、运行包校验和请求观测回归已通过；患者范围入口门禁已收紧，仍需手机扫码并取得当前服务端的三层业务证据 |
| SSH 发布 | 已恢复 | 使用 `192.168.112.172` 完成受控发布；不在文档写入凭据，公网 `8.130.127.184` 未被直接改动 |
| 真实业务验收 | 未完成 | 微信登录、患者显式切换、预约历史、门诊费用和报告仍缺当前候选页面 + HTTP + Pino 三层证据 |

> 2026-08-22 18:04 CST：针对 `dist/services/single-flight.test.js` ENOENT 已重新构建运行包、重开新项目开发者工具并普通编译；
> `dist/` 测试脚本为 0，当前二维码已重新生成但仍无手机连接。该恢复不增加微信、患者、预约、费用或报告业务证据。

## 2026-08-22 08:30–08:32 切换前候选复核

切换前候选 `49f74e0` / `b0e0935` 已再次通过全仓 `pnpm check`：架构、迁移台账、Provider 文档、日志事件、发布基线、
Biome、9 个 workspace 的类型检查/测试/构建均通过；小程序运行包仍为 14 个页面且不包含测试 JS。线上 `08:23–08:30 CST`
低敏日志只有 1 条 `200` 健康请求，没有微信登录、患者、预约或门诊费用业务事件，因此 P1 真机证据仍为 `pending`，
不能把“工具能生成二维码”或“没有业务日志”解释成业务成功或业务失败。

本轮重新对照旧端 64 页迁移矩阵和当前原生页面/路由，没有发现可以在缺少 Provider/HIS/临床 contract 时安全打开的新业务域。
病历目录、患者新增/绑定、二维码、住院、动态外部入口、健康内容和风险评估继续保持关闭；下一步顺序固定为：
真机微信登录 → 患者目录同步/显式切换 → 我的挂号/爽约 → 门诊费用只读。支付、医保、结算、退款和 HIS 写回仍最后处理。

当前下一步顺序固定为：从已发布 live `dist` 的 `02865d3` 重新普通编译并生成二维码，再对线上 `0aaa13b5` 服务端进行微信登录 → 患者目录同步与显式切换 → 今日预约摘要/预约历史/爽约 →
...
> 当前服务端事实：已验证 release `0aaa13b53cb6e21b59b332dbd4e2b982a5aba1e7`；当前小程序 live 运行输入为 `02865d385a9c09876dc51da1ffb71183139a559b`，线上 `13f597e` 仅作历史运行包记录。生产运行层证据见 [`release/candidate-0aaa13b5-production-acceptance-2026-08-27.md`](release/candidate-0aaa13b5-production-acceptance-2026-08-27.md)，后续重建锁定与发布流程见 [`release/wechat-devtools-dist-lock-2026-08-25.md`](release/wechat-devtools-dist-lock-2026-08-25.md)，当前九域真机清单见 [`release/device-evidence-02865d38-pending.json`](release/device-evidence-02865d38-pending.json)，上一候选清单不能直接作为本轮真机完成证据。

> 下方较早的 `160e7c8533c3a1d42c832184c90e274c6a4a1e9e`、`4e1b2e2` 和其它候选记录均为历史验证证据，不能覆盖上方当前事实源，
> 也未因路线图记录自动部署线上。

> 历史工作树记录（2026-08-22）：患者卡号公共 contract 收口后，本地 `dist/build-info.json` 曾由未部署候选
> `b0e093565493285e07fe549879f8b87eda649cc7` 重建。该记录只证明当时的本地编译和测试脚本边界，已被当前
> `dc8cd5b8` 小程序候选取代；具体停止条件见
> [`release/candidate-b0e0935-local-build-2026-08-22.md`](release/candidate-b0e0935-local-build-2026-08-22.md)。

> 2026-08-22 07:32 CST 当前复核：全仓门禁在 `release:baseline:audit` 按设计停止，唯一原因是
> `packages/contracts/src/index.ts` 尚未进入线上 `7181e99e`；此前的架构、迁移台账、Provider 文档、日志、
> 文档断链、格式、lint、工具测试和小程序运行包门禁均未出现新增失败。使用本机已有的
> `remote-8-130-127-184.pem` 以 `ps@8.130.127.184` 做无写入 SSH 检查仍返回 `Permission denied (publickey)`，
> 因此本轮没有上传候选、切换新 API、读取旧 Python `8001` 或修改任何线上状态。必须先恢复受控 SSH 权限，
> 再按发布手册执行；不能把本地构建或公网健康探针当成部署完成。

> 2026-08-22 07:38 CST 患者卡号边界复核：服务端候选已推进到 `4f2d890d`，众阳 adapter 对超过
> 64 个字符的 Provider 卡号在 `patInfosFind` 前返回 `provider-response-invalid`，不再伪装为“未绑定”；
> adapter `108 pass`，小程序运行包来源仍为 `b0e0935`，因为本轮未改小程序运行输入。该候选尚未部署，
> SSH 仍未获阿里云中转机接受，不能把本地测试当作线上 Provider 证据。详见
> [`release/candidate-4f2d890-local-build-2026-08-22.md`](release/candidate-4f2d890-local-build-2026-08-22.md)。

本文档是新会话继续工作的入口，描述当前真实边界、业务优先级、工程治理和上线验收顺序。
其中“已完成”只表示代码、测试或部署证据，不代表微信、众阳、医保、HIS、支付或真机已经完成真实验收。

## 2026-08-22 08:34–08:41 当前业务观察与日志修正

当前发布前的 `49f74e0` 已收到新小程序候选的微信登录、患者目录同步/读取和预约目录只读请求：低敏聚合为登录成功 `1/1`、
患者读取 `5`、同步 `2`、预约科室/排班目录各 `1/1`，HTTP 完成 `12` 次均为 `200`，`parseErrors=0`、
`systemdWarningCount=0`。这确认预约目录可以正常进入两列级联页面，但由于当前取得的页面层证据来自新开发者工具窗口而非手机截图，
真机三层验收仍保持 `pending`；预约提交、支付、医保和 HIS 写回未触发。

同一窗口的 `correlation.missingCount=1` 已定位为排班快照持久化成功/失败事件没有继承所属 HTTP `traceId`，不是 Provider
业务失败。服务端候选 `c01b1af` 已补齐该字段并完成部署；切换后预约科室和排班两条业务链的同链 HTTP 2xx 审计均通过，
生命周期日志的无 trace 记录不计入业务缺失。下一步继续预约历史/爽约和门诊费用只读链路。旧 Python `8001` 不修改、不停止、不重启。
...
- 2026-08-22（普通资料 service 版本后置条件）：补齐仓储返回快照的 service 级门禁，要求
  ...
  API 全量 `204 pass`、全仓 `pnpm check` 通过；该修正已随当前 release `002acc1be5cdd1b16c2c249f5dbbf9f7c65dbd10` 部署，真实 MySQL 双会话和真机资料
...
- 2026-08-22（候选追踪补齐，切换前记录）：普通资料 service 修正的本地候选固定为
  ...
  该记录创建时与线上 `9f491cb5` 明确分离；后续已生成最终提交 `002acc1b` 并按独立 release 流程完成生产运行层验收，未修改旧 Python、数据库或 Redis。

- 2026-08-22 00:07 CST（切换前运行层只读观察）：新 API `hospital-platform-api-v2.service` 当时为
...
- 2026-08-21（历史候选预约目录展示边界）：提交 `968a587` 将医院日期标签、日期分组和本地号源展示窗口抽为纯展示服务，
  ...
  该修正不代表 Provider 分页、预约写入或真机验收完成，详见历史候选记录 [`release/candidate-968a587-local-build-2026-08-21.md`](release/candidate-968a587-local-build-2026-08-21.md)。

- 2026-08-21（历史候选真机 `single-flight.test.js` ENOENT 复核）：`968a587` 运行包中 `single-flight.js` 存在，
  `single-flight.test.js` 不存在，`*.test.js`/`*.spec.js` 为 0；当前可操作窗口未稳定出现，未操作旧 `mp-weixin` 项目。
  需要重新打开 `apps/miniprogram/`，普通编译后生成新二维码；不得把测试脚本复制到 `dist/`。
...
- 2026-08-21（只读页面跨会话本地事件修正）：本次代码复核基于服务端 `5a31427`、小程序候选 `9c582a1`（完整来源 `9c582a1c38b3b3cdecf7145c6b126b185fe474c2`）。发现预约记录、爽约记录和报告目录虽在组合读取提交前校验会话代际，但页面没有保存已提交读模型所属代际；现已阻断旧会话的本地分页、预约标签、报告详情及静态业务事件，同时保留“更换就诊人”作为修复入口。小程序 `181 pass`、`1444 expects`、typecheck、Biome、构建和 `runtime:verify` 均通过。详见 [`release/miniprogram-readonly-page-session-event-audit-2026-08-21.md`](release/miniprogram-readonly-page-session-event-audit-2026-08-21.md)。

- 2026-08-21（门诊费用患者会话竞态与卡片事件边界修正）：本次代码复核基于服务端 `5a31427`、小程序候选 `9c582a1`（完整来源 `9c582a1c38b3b3cdecf7145c6b126b185fe474c2`）。发现费用页状态切换可能使用上一轮会话的缓存患者对象，且费用卡片事件此前只相信 WXML 状态字符串；现已增加患者卡片所属会话代际、请求前断言，以及会话/患者漂移时重新执行 `/me` → 患者目录 → 当前状态查询，并要求卡片点击用本次查询 `viewKey` 回查当前可见读模型。小程序完整测试 `181 pass`、`1444 expects`；支付、医保、HIS、预约写入和真机证据门禁不变。详见 [`release/appointment-outpatient-readonly-correctness-audit-2026-08-21.md`](release/appointment-outpatient-readonly-correctness-audit-2026-08-21.md)。
...
- 2026-08-21（普通资料与就诊人状态机复审）：复核普通资料的 owner/version/409、非 GET
  401 不重放、会话代际回写门禁，以及患者目录的目录读取 → 临床同步 → 显式切换顺序；
  未发现可以在不猜 Provider 契约的前提下安全修复的逻辑缺口。API 资料测试 55 项、
  小程序测试 176 项、类型检查和运行包验证均通过。当前下一步收敛为最新候选的真实微信
  登录、患者切换、预约历史、门诊费用三层证据，再验收普通资料写入；病历、绑卡和二维码
  继续保持关闭。详见 [`release/readonly-profile-patient-state-audit-2026-08-21.md`](release/readonly-profile-patient-state-audit-2026-08-21.md)。
...
- 2026-08-21（患者页面会话组合边界）：发现预约历史、爽约、报告目录和门诊费用页面虽然有页面令牌和患者 ID 回查，
  ...
  当前小程序候选已重新构建为 `7a6f4df`，完整来源为
  `7a6f4df34fac5975c6012a30d2c137953a892059`；详见
...
- 2026-08-21（当前小程序运行包刷新）：针对真机调试报告的 `dist/services/single-flight.test.js` ENOENT，已用提交
  `9c582a1c38b3b3cdecf7145c6b126b185fe474c2` 重新构建并通过 `runtime:verify`。当前 `dist` 有 14 个页面、真实运行模块
  ...
  [`release/candidate-9c582a1-local-build-2026-08-21.md`](release/candidate-9c582a1-local-build-2026-08-21.md)。
...
- 2026-08-21 10:52 CST（上一轮运行包构建门禁修正，历史追溯）：小程序候选曾更新为 `c86a788`，完整来源
  `c86a788c01760fd5a74ac8c2769871025297a4fc`；构建、类型检查和运行包验证通过，14 个页面齐全，`*.test.js`/`*.spec.js` 均为 0。
...
- 2026-08-21 07:39 CST（开发者工具错误诊断复核）：`miniprogram` 模拟器调试面板唯一错误为未建立平台会话时
  ...
  [`release/candidate-39ad2c5-local-build-2026-08-21.md`](release/candidate-39ad2c5-local-build-2026-08-21.md)。
...
> `7a6f4df`，完整运行包来源为 `7a6f4df34fac5975c6012a30d2c137953a892059`，尚未上传线上。
...
当前执行项绑定服务端 release `0aaa13b53cb6e21b59b332dbd4e2b982a5aba1e7` 与小程序来源
`02865d385a9c09876dc51da1ffb71183139a559b`（提交 `02865d3`）；下方历史 release 不得作为本轮真机或业务证据。
...
3. 使用当前服务端 release `0aaa13b53cb6e21b59b332dbd4e2b982a5aba1e7` 和小程序候选 `02865d3`（完整构建来源：`02865d385a9c09876dc51da1ffb71183139a559b`）重新同步真实账号的患者目录，先运行显式 `patient-sync` smoke，再补做 `his-patient` owner-scoped 记录查询验收；
...
13. 当前服务端 release `0aaa13b5` 已按 [`infra/systemd/api-v2-release-runbook.md`](../infra/systemd/api-v2-release-runbook.md) 完成原子 `current` 切换和新 API 单元重启；`18081`、公网 `/api/v2`、旧 `8001` 已复测通过。下一步进行真实微信登录、患者切换、预约只读和门诊费用的分层验收，任何业务层失败只回滚新 API，不触碰旧 Python 服务。
14. 当前公网 runtime 与 P0 日志 bundle 只证明请求进入 `0aaa13b5` Bun 进程；本轮额外验证无会话 401 日志已记录 `errorCode=unauthorized`，基础路由不再重复作为业务完成证据，下一步只补真实 session、owner 映射、Provider 状态和真机页面证据，并使用 `02865d3` 小程序候选（完整构建来源：`02865d385a9c09876dc51da1ffb71183139a559b`）。
...
- 2026-08-24：修正患者同步期间的选择页导航状态机。统一导航入口现在返回“已发起导航 / 会话仍在验证或已回首页 /
...
> **当前事实源覆盖旧段落（2026-08-27）**：请以 live `02865d3`、40 个原生页面、健康自测安全数值子集、协议原文只读页、今日预约摘要窗口、快递/采血预约真实空态、电子锦旗/健康表扬信安全关闭态、共享当前就诊人上下文、微信资料拒绝后的设置页重试、选择就诊人刷新并发门禁、会话失效时旧 owner 微信资料缓存清理、报告/费用和预约查询底层输入门禁和当前工作树复跑的 `345 pass / 0 fail / 3736 expect()` 为准；本页前置的 `baa31df0`/17 页文字只作历史交接。
> **当前本地源码候选纠正（2026-08-27，优先级高于本文下方旧交接段落）**：最新本地运行输入为 `02865d385a9c09876dc51da1ffb71183139a559b`，live 运行包包含 40 个页面；当前工作树复跑为 `345 pass / 0 fail / 3736 expect()`；当前 live `dist` 已原子发布为 `02865d3`，线上历史配套运行包仍为 `13f597e`。本文下方出现的旧候选只作历史交接，不能生成当前二维码或作为真机证据。
