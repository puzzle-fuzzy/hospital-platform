...
1. 在真机重新验收预约科室和排班，保存公网请求的 `requestId` 与页面证据；
2. 取得预约历史接口要求的患者标识映射，补做 owner-scoped 记录查询验收；
3. 再处理报告目录 gate 和报告真实 provider 只读验收；
4. provider 只读稳定后，才进入预约写入合同和锁号设计；
5. 最后按现金支付 → 医保结算 → HIS 回写顺序做专项验收。
