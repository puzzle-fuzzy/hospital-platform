...
- 2026-08-21（当前运行包重新发布）：新增真机三层证据清单审计工具后，按来源指纹门禁重新构建小程序，当前候选为 `6677671`，完整来源
...
- 2026-08-21 07:39 CST（开发者工具错误诊断复核）：`miniprogram` 模拟器调试面板唯一错误为未建立平台会话时
  ...
  [`release/candidate-6677671-local-build-2026-08-21.md`](release/candidate-6677671-local-build-2026-08-21.md)。
...
> `6677671`，完整运行包来源为 `667767123efdb5b3a0bedbe423ab1797f16b1247`，尚未上传线上。
...
3. 使用当前服务端 release `5a31427` 和最新小程序候选 `6677671`（完整构建来源：`667767123efdb5b3a0bedbe423ab1797f16b1247`）重新同步真实账号的患者目录，先运行显式 `patient-sync` smoke，再补做 `his-patient` owner-scoped 记录查询验收；
...
14. 当前公网 runtime 与 P0 日志 bundle 已能证明请求进入 `5a31427` Bun 进程；基础路由不再重复作为业务完成证据，下一步只补真实 session、owner 映射、Provider 状态和真机页面证据，并始终使用最新本地 `6677671` 小程序候选（完整构建来源：`667767123efdb5b3a0bedbe423ab1797f16b1247`）。
