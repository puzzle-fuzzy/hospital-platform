...
> 当前完整小程序来源校验值：`9eb672b1296f282fc536f72bb897631683e4532f`；当前服务端已验证 release：`9f479c9a`。

## 当前事实源（2026-08-22）

本节优先于下方历史记录。当前 Git `main` 已包含服务端运行候选与线上发布记录；服务端上一次已验证为
`9f479c9a`，小程序运行包来源为
`9eb672b1296f282fc536f72bb897631683e4532f`。SSH 已通过中转内网地址恢复；本次只重启新 API，旧 Python `8001` 继续共存。
...
| 小程序运行包 | `9eb672b1`、14 个页面、无测试 JS | 构建、运行包校验已通过；运行包锁定错误提示已补齐，仍需手机扫码并取得当前服务端的三层业务证据 |
...
当前下一步顺序固定为：从 `9eb672b1` 正确项目重新普通编译/扫码 → 微信登录 → 患者目录同步与显式切换 → 预约历史/爽约 →
...
> 当前候选：服务端已验证 release `9f479c9a`；小程序运行包来源 `9eb672b1296f282fc536f72bb897631683e4532f`（提交 `9eb672b1`）。生产运行层证据见 [`release/9f479c9a-production-acceptance-2026-08-22.md`](release/9f479c9a-production-acceptance-2026-08-22.md)，小程序候选证据见 [`release/candidate-9eb672b-local-build-2026-08-22.md`](release/candidate-9eb672b-local-build-2026-08-22.md)。
...
`9eb672b1296f282fc536f72bb897631683e4532f`；下方历史 release 不得作为本轮真机或业务证据。
...
3. 使用当前服务端 release `9f479c9a` 和最新小程序候选 `9eb672b1`（完整构建来源：`9eb672b1296f282fc536f72bb897631683e4532f`）重新同步真实账号的患者目录，先运行显式 `patient-sync` smoke，再补做 `his-patient` owner-scoped 记录查询验收；
...
14. 当前公网 runtime 与 P0 日志 bundle 只证明请求进入 `9f479c9a` Bun 进程；基础路由不再重复作为业务完成证据，下一步只补真实 session、owner 映射、Provider 状态和真机页面证据，并始终使用最新本地 `9eb672b1` 小程序候选（完整构建来源：`9eb672b1296f282fc536f72bb897631683e4532f`）。
