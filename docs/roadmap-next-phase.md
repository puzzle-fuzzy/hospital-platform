...
> 上一轮原生 Tab 候选（2026-08-25）：用户现场仍观察到闪动和选中态失效，已结束，不再作为当前验收入口。当前候选重新使用官方 `custom-tab-bar`，见 [`release/current-custom-tabbar-runtime-recheck-2026-08-25.md`](release/current-custom-tabbar-runtime-recheck-2026-08-25.md)。
> 当前正确结构包含一个官方 `custom-tab-bar/`，不包含页面级底栏或第二套 selected 状态；会话失效回首页仍收口到 `switchTab`。上一轮原生候选只保留历史记录。
> 当前开发者工具执行要求：必须直接打开 `apps/miniprogram/dist/` 独立工程并普通编译 `0a8567b143d16611a02b693e0602b410ab04d108`，再确认首页和“就诊”页只有一份固定共享底栏，切换后当前图标/文字变蓝。历史线上 `13f597e` 和旧候选不能作为本次 Tab 行为证据，不能打开父目录、`src/` 或另一个旧 `dist/` 工程，也不能新增页面级底栏。

> 本轮后续唯一执行入口：先按新旧服务共存边界完成只读 preflight，再使用小程序运行包 `0dd1b65b` 在独立 `dist/` 工程普通编译并采集四 Tab、患者显式切换、预约历史/爽约、门诊费用只读和普通资料的同链证据；未取得页面、客户端 requestId、服务端 Pino 和 Provider 低敏 requestId 四方关联前，不把代码测试或健康检查写成业务完成。
> 当前小程序会话/导航门禁（2026-08-25，本地未部署）：患者范围 GET 收到 `503 persistence-temporarily-unavailable` 时保留 token、不重新登录、不重放旧 `patientId`；主 Tab 程序化入口强制使用 `switchTab`，当前 Tab 重复导航会 no-op，底栏由微信官方 `custom-tab-bar` 唯一组件维护，四个主页面 onShow 会再次同步完整选中模型，会话恢复期间患者卡片只显示固定占位。本轮小程序回归为 `241/241` 通过、`1934` 个断言，运行包来源为 `0dd1b65b`；真机页面证据仍待。
