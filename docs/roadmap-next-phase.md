...
> **最新小程序候选事实（2026-08-26）**：当前运行相关源码候选和 pending 运行输入为 `0be59f966de2c3a0861cb44e9a526a1ef557f6c7`，40 个页面；本地 live `dist` 仍为上一候选 `02dbf10419740d96c4445493df019021ac22bcfa`，新候选等待释放开发者工具文件锁后原子发布。本轮继续保持共享患者外壳、电子锦旗和表扬信记录区域、二维码安全壳和健康数值工具的安全边界，临床/服务入口的当前就诊人上下文、重试和选择入口、患者签名和消息订阅的原生安全展示层、采血预约、今日预约摘要与安全数值子集继续保持边界，未开放临床分级、采血号源、公开记录或实时叫号。正式健康审核 bundle 缺失时健康百科仍保持 fail-closed，协议同意/撤回/审计仍关闭。支付、医保、临床真实读取、外部 WebView 和预约写入仍按各自 contract 关闭。
 ...
 > **当前仓库事实补充（2026-08-26）**：本轮小程序运行包候选来源为 `0be59f966de2c3a0861cb44e9a526a1ef557f6c7`，小程序核心回归通过。本轮继续保持共享患者外壳、电子锦旗和表扬信记录区域、二维码安全壳和健康数值工具的安全边界，不改变线上 `8eb51b5f`、live `dist` 之外的旧服务或其他会话适配器；候选已完成 pending 构建和校验，尚未替换本地 live，协议同意、撤回、临床真实读取、采血号源、公开记录、实时叫号和支付医保继续关闭，上方旧候选段落保留为历史追溯。
