...
> 当前小程序底栏候选（2026-08-25）：用户反馈原生底栏仍出现闪动且选中态不明显；本轮继续保持微信原生 `tabBar`（不恢复 `custom-tab-bar`），清理旧自定义尺寸字段，并把 8 个导航资源切换到全新的 `*-native-v6.png` 路径以隔离开发者工具/真机缓存。图形内容没有改变，旧 Python `8001`、线上服务、数据库和 Redis 未修改。提交后必须重新构建、确认 `build-info` 新 revision 并扫描新二维码，不能继续使用 `a5ff0001` 旧预览包。
