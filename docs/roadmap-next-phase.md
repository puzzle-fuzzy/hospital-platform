...
- 2026-08-20（普通资料更新链定向回归）：服务端普通资料 service `13/13` 通过，小程序患者选择、会话
  ...
  首次写入、双设备 `409`、公网日志同链或真机页面已经验收；下一步仍需使用当前 `767ed9c` 候选按
...
3. 使用当前服务端 release `0e360d3` 和最新小程序候选 `767ed9c`（完整构建来源：`767ed9c225bf4d329761f6abed7668015a2626b2`）重新同步真实账号的患者目录，先运行显式 `patient-sync` smoke，再补做 `his-patient` owner-scoped 记录查询验收；
...
14. 当前公网 runtime 与 P0 日志 bundle 已能证明请求进入 `0e360d3` Bun 进程；基础路由不再重复作为业务完成证据，下一步只补真实 session、owner 映射、Provider 状态和真机页面证据，并始终使用最新本地 `767ed9c` 小程序候选（完整构建来源：`767ed9c225bf4d329761f6abed7668015a2626b2`）。
