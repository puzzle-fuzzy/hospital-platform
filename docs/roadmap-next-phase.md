...
> 不覆盖顶部最新事实。当前服务端为 `65219e2`，小程序候选为 `b55df37`。
...
3. 使用当前服务端 release `65219e2` 和小程序候选 `b55df37`（完整构建来源：`b55df37b48bbe250e4ebefee3db7739d2fd554e2`）重新同步真实账号的患者目录，先运行显式 `patient-sync` smoke，再补做 `his-patient` owner-scoped 记录查询验收；
...
14. 当前公网 runtime 与 P0 日志 bundle 已能证明请求进入 `65219e2` Bun 进程；基础路由不再重复作为业务完成证据，下一步只补真实 session、owner 映射、Provider 状态和真机页面证据，并始终使用与之配套的 `b55df37` 小程序候选（完整构建来源：`b55df37b48bbe250e4ebefee3db7739d2fd554e2`）。
