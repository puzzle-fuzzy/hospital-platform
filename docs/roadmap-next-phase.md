...
> 当前开发者工具执行要求：先关闭占用 `dist/` 的开发者工具和真机调试会话，执行 `runtime:publish-pending` 发布 `87ad1092`，再直接打开 `apps/miniprogram/dist/` 独立工程并普通编译，确认四项共享底栏只有一份、就诊三标签患者栏正确显示、今日预约摘要不冒充实时状态、快递/采血预约空态、电子锦旗/健康表扬信关闭态和患者切换入口正确、切换后当前图标/文字变蓝。历史线上 `13f597e`、当前旧 live `fcc6630e`、`d204425` 和其它候选不能作为本次修复证据，不能打开父目录、`src/` 或另一个旧 `dist` 工程，也不能新增页面级底栏。

> 本轮后续唯一执行入口：先关闭开发者工具占用并发布 `87ad1092` pending 运行包，再从新二维码开始只读 preflight，采集四 Tab、就诊三标签患者栏、今日预约摘要、患者显式切换、预约历史/爽约、门诊费用只读和普通资料的同链证据；未取得页面、客户端 requestId、服务端 Pino 和 Provider 低敏 requestId 四方关联前，不把代码测试或健康检查写成业务完成。
> 当前小程序会话/导航门禁（2026-08-25，本地未部署）：患者范围 GET 收到 `503 persistence-temporarily-unavailable` 时保留 token、不重新登录、不重放旧 `patientId`；主 Tab 程序化入口强制使用 `switchTab`，当前 Tab 重复导航会 no-op，底栏由微信原生 `tabBar` 统一维护，会话恢复期间患者卡片只显示固定占位。本轮最新候选回归为 `312 pass / 0 fail / 3548 expect()`，运行包来源为 `731c9571` pending；微信资料被拒绝后需要用户点击设置页重试；今日预约摘要只展示预约事实，不展示实时叫号，快递和采血预约页只展示旧端真实空态，电子锦旗和健康表扬信只展示安全关闭态；临床/服务入口和我的问诊共享当前就诊人上下文，患者绑定/签名页可查看协议原文但不写入同意；真机页面证据仍待。
...
> 当前服务端与小程序尚未完成 pending 候选切换：线上服务端 release 为 `8eb51b5f`，线上历史小程序运行包来源为 `13f597e`，本地 pending 为 `731c9571`；旧 Python `8001` 继续共存。释放 `dist` 锁并发布 pending 后，才开始当前候选的真机业务三层证据，不把运行层 smoke 当作业务完成。
...
> **最新小程序候选事实（2026-08-26）**：当前运行相关源码候选和可发布 pending 来源为 `731c95718d26bcf2826987b72f79295413b203d7`，40 个页面，`312 pass / 0 fail / 3548 expect()`；本轮补齐公共日期窗口输入边界和微信资料拒绝后的设置页重试，临床/服务入口的当前就诊人上下文、重试和选择入口、患者签名和消息订阅的原生安全展示层、采血预约、锦旗/表扬信安全页面、今日预约摘要与安全数值子集继续保持边界，未开放临床分级、采血号源、公开记录或实时叫号。正式健康审核 bundle 缺失时健康百科仍保持 fail-closed，协议同意/撤回/审计仍关闭。候选已原子发布到 live `dist`，上一候选及更早候选仅作历史追溯。支付、医保、临床真实读取、外部 WebView 和预约写入仍按各自 contract 关闭。
...
当前下一步顺序固定为：先释放微信开发者工具对 `dist/` 的锁，发布并校验 pending `731c9571`，再对线上 `8eb51b5f` 服务端进行微信登录 → 患者目录同步与显式切换 → 今日预约摘要/预约历史/爽约 →
...
> **当前事实源覆盖旧段落（2026-08-26）**：请以 pending `731c9571`、40 个原生页面、健康自测安全数值子集、协议原文只读页、今日预约摘要窗口、快递/采血预约真实空态、电子锦旗/健康表扬信安全关闭态、共享当前就诊人上下文、微信资料拒绝后的设置页重试和当前工作树复跑的 `312 pass / 0 fail / 3548 expect()` 为准；本页前置的 `baa31df0`/17 页文字只作历史交接。
> **当前本地源码候选纠正（2026-08-26，优先级高于本文下方旧交接段落）**：最新本地运行输入为 `731c95718d26bcf2826987b72f79295413b203d7`，pending 运行包包含 40 个页面；当前工作树复跑为 `312 pass / 0 fail / 3548 expect()`；当前 live `dist` 仍为 `fcc6630ebfa7b0697cbd03a5e376ce6765d1643b`，线上历史配套运行包仍为 `13f597e`。本文下方出现的旧候选只作历史交接，不能生成当前二维码或作为真机证据。
