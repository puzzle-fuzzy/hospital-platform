...
> 当前服务端为 `0e360d3`，最新本地小程序候选为 `ce8d68b`，完整来源为
> `ce8d68b15fc0c6a813fbf7a95d36610167874b8e`；小程序尚未上传线上。`e050fa0` 仅保留为历史候选。
...
3. 使用当前服务端 release `0e360d3` 和最新小程序候选 `ce8d68b`（完整构建来源：`ce8d68b15fc0c6a813fbf7a95d36610167874b8e`）重新同步真实账号的患者目录，先运行显式 `patient-sync` smoke，再补做 `his-patient` owner-scoped 记录查询验收；
...
14. 当前公网 runtime 与 P0 日志 bundle 已能证明请求进入 `0e360d3` Bun 进程；基础路由不再重复作为业务完成证据，下一步只补真实 session、owner 映射、Provider 状态和真机页面证据，并始终使用最新本地 `ce8d68b` 小程序候选（完整构建来源：`ce8d68b15fc0c6a813fbf7a95d36610167874b8e`）。
