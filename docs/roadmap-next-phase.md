...
> 当前候选：服务端 release `002acc1be5cdd1b16c2c249f5dbbf9f7c65dbd10`；小程序运行包来源 `f3074243e136621d64f296a687c0fb2ca8230991`（提交 `f307424`），真机前仍需重新普通编译并核对来源。生产运行层证据见 [`release/002acc1b-production-acceptance-2026-08-22.md`](release/002acc1b-production-acceptance-2026-08-22.md)。

> 本轮本地验证使用的新项目服务端代码为 `160e7c8533c3a1d42c832184c90e274c6a4a1e9e`，小程序运行包来源为
> `f3074243e136621d64f296a687c0fb2ca8230991`；这只是未发布验证证据，不能替代上方发布基线，也未因本路线图记录自动部署线上。
...
- 2026-08-21 23:34–23:38 CST（当前 `9f491cb5` 生产切换与运行包复核）：新 API 已从 `c8eef370` 原子切换到
  ...
  本地小程序候选已更新为 `f307424`，运行包没有任何 `*.test.js`/`*.spec.js`；`single-flight.test.js` ENOENT 已确认来自开发者工具旧增量索引，
  不能把测试脚本复制进 `dist/`。应在正确的 `apps/miniprogram/` 项目中普通编译并生成新二维码。详见
  [`release/candidate-f307424-local-build-2026-08-22.md`](release/candidate-f307424-local-build-2026-08-22.md)。
...
- 2026-08-21 23:53 CST（当前 release 业务窗口复核）：新旧服务监听和 Worker 状态未变化；最近 6 小时日志聚合为
  ...
  当前候选仍没有新的微信、患者、预约、门诊费用或普通资料业务流量，因此不能把运行层健康或空日志窗口计作业务成功；下一步仍是用户在当前 `f307424` 小程序候选中重新编译、扫码并取得三层业务证据。
...
3. 使用当前服务端 release `002acc1be5cdd1b16c2c249f5dbbf9f7c65dbd10` 和最新小程序候选 `f307424`（完整构建来源：`f3074243e136621d64f296a687c0fb2ca8230991`）重新同步真实账号的患者目录，先运行显式 `patient-sync` smoke，再补做 `his-patient` owner-scoped 记录查询验收；
...
13. 当前服务端 release `002acc1be5cdd1b16c2c249f5dbbf9f7c65dbd10` 已按 [`infra/systemd/api-v2-release-runbook.md`](../infra/systemd/api-v2-release-runbook.md) 完成原子 `current` 切换和新 API 单元重启；`18081`、公网 `/api/v2`、旧 `8001` 已复测通过。下一步进行真实微信登录、患者切换、预约只读和门诊费用的分层验收，任何业务层失败只回滚新 API，不触碰旧 Python 服务。
14. 当前公网 runtime 与 P0 日志 bundle 已能证明请求进入 `002acc1b` Bun 进程；基础路由不再重复作为业务完成证据，下一步只补真实 session、owner 映射、Provider 状态和真机页面证据，并始终使用最新本地 `f307424` 小程序候选（完整构建来源：`f3074243e136621d64f296a687c0fb2ca8230991`）。
