...
> 当前小程序会话/导航门禁（2026-08-25，本地未部署）：患者范围 GET 收到 `503 persistence-temporarily-unavailable` 时保留 token、不重新登录、不重放旧 `patientId`；主 Tab 程序化入口强制使用 `switchTab`，当前 Tab 重复导航会 no-op，底栏由微信原生 `tabBar` 统一维护，会话恢复期间患者卡片只显示固定占位。本轮最新候选回归为 `311 pass / 0 fail / 3527 expect()`，运行包来源为 `adf536bf` pending；微信资料被拒绝后需要用户点击设置页重试；今日预约摘要只展示预约事实，不展示实时叫号，快递和采血预约页只展示旧端真实空态，电子锦旗和健康表扬信只展示安全关闭态；临床/服务入口和我的问诊共享当前就诊人上下文，患者绑定/签名页可查看协议原文但不写入同意；真机页面证据仍待。
...
> **最新小程序候选事实（2026-08-26）**：当前运行相关源码候选和可发布 pending 来源为 `adf536bff5b01a2cd27c664f05b7feae2be6ec3f`，40 个页面，`311 pass / 0 fail / 3527 expect()`；本轮补齐公共日期窗口输入边界和微信资料拒绝后的设置页重试，临床/服务入口的当前就诊人上下文、重试和选择入口、患者签名和消息订阅的原生安全展示层、采血预约、锦旗/表扬信安全页面、今日预约摘要与安全数值子集继续保持边界，未开放临床分级、采血号源、公开记录或实时叫号。正式健康审核 bundle 缺失时健康百科仍保持 fail-closed，协议同意/撤回/审计仍关闭。live `dist` 仍被微信开发者工具锁定，上一候选及更早候选仅作历史追溯。支付、医保、临床真实读取、外部 WebView 和预约写入仍按各自 contract 关闭。

> **当前广度事实源（2026-08-26，运行包来源 `adf536bff5b01a2cd27c664f05b7feae2be6ec3f`）**：旧端 64 个页面已全部登记；新端现有 40 个注册页面，健康自测中的 BMI/血压安全数值工具已进入 `partial`，其余临床页面继续使用 `surface-only` 或关闭态，健康百科三个只读页面、协议原文只读页、旧端“我的快递”和采血预约真实空态、电子锦旗/健康表扬信安全页面结构、患者签名/消息订阅安全展示层、临床/服务入口共享当前就诊人上下文以及我的问诊患者作用域已接入，微信资料拒绝后的设置页重试已接入，统一状态页已展示迁移阶段与旧入口覆盖范围。当前候选 `311 pass / 0 fail / 3527 expect()`，pending 已完成构建和运行包校验，仍待释放微信开发者工具锁；正式健康内容、协议同意、临床审核、支付、医保、预约写入和 HIS 回写仍按原顺序关闭。

> **当前仓库事实补充（2026-08-26）**：本轮小程序运行包来源为 `adf536bff5b01a2cd27c664f05b7feae2be6ec3f`，小程序回归为 `311 pass / 0 fail / 3527 expect()`。本轮补齐公共日期窗口边界和微信资料拒绝后的设置页重试，不改变线上 `8eb51b5f`、live `dist` 或任何旧服务；协议同意、撤回、临床真实读取、采血号源、公开记录、实时叫号和支付医保继续关闭，上方旧候选段落保留为历史追溯。
...
> **当前事实源覆盖旧段落（2026-08-26）**：请以 pending `adf536bf`、40 个原生页面、健康自测安全数值子集、协议原文只读页、今日预约摘要窗口、快递/采血预约真实空态、电子锦旗/健康表扬信安全关闭态、共享当前就诊人上下文、微信资料拒绝后的设置页重试和当前工作树复跑的 `311 pass / 0 fail / 3527 expect()` 为准；本页前置的 `baa31df0`/17 页文字只作历史交接。
> **当前本地源码候选纠正（2026-08-26，优先级高于本文下方旧交接段落）**：最新本地运行输入为 `adf536bff5b01a2cd27c664f05b7feae2be6ec3f`，pending 运行包包含 40 个页面；当前工作树复跑为 `311 pass / 0 fail / 3527 expect()`；当前 live `dist` 仍为 `fcc6630ebfa7b0697cbd03a5e376ce6765d1643b`，线上历史配套运行包仍为 `13f597e`。本文下方出现的旧候选只作历史交接，不能生成当前二维码或作为真机证据。
