...
> 当前本地未发布小程序候选（2026-08-24）：页面代码为 `ff931d7cbbc50e18649a38e14cd93f389d7487e3`。本轮统一四个主入口的共享 Tab 架构，并继续保留“我的挂号”卡片、爽约入口、查询状态稳定高度和报告目录患者加载占位；未部署、未替换线上 `13f597e`，也未修改旧 Python、数据库或 Redis。构建和运行包验证见 [`release/candidate-ff931d7c-miniprogram-build-2026-08-24.md`](release/candidate-ff931d7c-miniprogram-build-2026-08-24.md)。

> 本轮后续唯一执行入口：先按 [`release/ff931d7c-real-device-business-acceptance-runbook-2026-08-24.md`](release/ff931d7c-real-device-business-acceptance-runbook-2026-08-24.md) 重新编译本地候选并采集四 Tab、患者显式切换、预约历史/爽约、门诊费用只读和普通资料的同链证据；未取得页面、客户端 requestId、服务端 Pino 和 Provider 低敏 requestId 四方关联前，不把代码测试或健康检查写成业务完成。
