...
- 2026-08-19（当前候选文档指针对齐）：复核发现部分验收手册和迁移总表仍把历史 `48ba22f` 当作当前小程序候选，
  容易让下一次真机验收导入错误运行包。历史 release 记录保留原始来源，当前入口文档已统一指向
  `474b044` / `474b0444736599c848a4cef9f47fd930884e401d`；本轮只修改新项目文档，没有上传小程序、调用 Provider、
  修改旧 Python、数据库或 Redis。
...
14. 当前公网 runtime 与 P0 日志 bundle 已能证明请求进入 `398be8e` Bun 进程；基础路由不再重复作为业务完成证据，下一步只补真实 session、owner 映射、Provider 状态和真机页面证据，并始终使用与之配套的 `474b044` 小程序候选（完整构建来源：`474b0444736599c848a4cef9f47fd930884e401d`）。
