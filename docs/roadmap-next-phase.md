...
> **最新小程序候选事实（2026-08-26）**：当前 pending 来源为 `dee4803fb94ad50c59c9ef8fda996bc0f37427c6`（功能提交 `dee4803f`），40 个页面，`307 pass / 0 fail / 3511 expect()`；本轮继续统一临床/服务入口的当前就诊人上下文、重试和选择入口，并补齐我的问诊患者作用域，患者签名和消息订阅的原生安全展示层、采血预约、锦旗/表扬信安全页面、今日预约摘要与安全数值子集继续保持边界，未开放临床分级、采血号源、公开记录或实时叫号。正式健康审核 bundle 缺失时健康百科仍保持 fail-closed，协议同意/撤回/审计仍关闭。live `dist` 仍被微信开发者工具锁定，上一候选及更早候选仅作历史追溯。支付、医保、临床真实读取、外部 WebView 和预约写入仍按各自 contract 关闭。

> **当前广度事实源（2026-08-26，pending `dee4803fb94ad50c59c9ef8fda996bc0f37427c6`）**：旧端 64 个页面已全部登记；新端现有 40 个注册页面，健康自测中的 BMI/血压安全数值工具已进入 `partial`，其余临床页面继续使用 `surface-only` 或关闭态，健康百科三个只读页面、协议原文只读页、旧端“我的快递”和采血预约真实空态、电子锦旗/健康表扬信安全页面结构、患者签名/消息订阅安全展示层、临床/服务入口共享当前就诊人上下文以及我的问诊患者作用域已接入，统一状态页已展示迁移阶段与旧入口覆盖范围。当前候选 `307 pass / 0 fail / 3511 expect()`，pending 已完成构建和运行包校验，仍待释放微信开发者工具锁；正式健康内容、协议同意、临床审核、支付、医保、预约写入和 HIS 回写仍按原顺序关闭。

> **当前仓库事实补充（2026-08-26）**：本轮功能交接基线为 `dee4803f`，小程序回归已推进到 `307 pass / 0 fail / 3511 expect()`，pending 来源为 `dee4803fb94ad50c59c9ef8fda996bc0f37427c6`。本轮继续统一临床/服务入口的当前就诊人上下文、重试和选择入口，并补齐我的问诊患者作用域，不改变线上 `8eb51b5f`、live `dist` 或任何旧服务；协议同意、撤回、临床真实读取、采血号源、公开记录、实时叫号和支付医保继续关闭，上方旧候选段落保留为历史追溯。
...
> 当前服务端事实：已验证 release `8eb51b5ffe85b0b8f8a032783f893117d3df549d`；当前小程序 pending 运行输入为 `dee4803fb94ad50c59c9ef8fda996bc0f37427c6`，线上 `13f597e` 仅作历史运行包记录。生产运行层证据见 [`release/8eb51b5f-production-acceptance-2026-08-24.md`](release/8eb51b5f-production-acceptance-2026-08-24.md)，候选锁定与发布前置见 [`release/wechat-devtools-dist-lock-2026-08-25.md`](release/wechat-devtools-dist-lock-2026-08-25.md)，真机证据清单见 [`release/device-evidence-dee4803-pending.json`](release/device-evidence-dee4803-pending.json)。
...
> **当前事实源覆盖旧段落（2026-08-26）**：请以 pending `dee4803f`、40 个原生页面、健康自测安全数值子集、协议原文只读页、今日预约摘要窗口、快递/采血预约真实空态、电子锦旗/健康表扬信安全关闭态、共享当前就诊人上下文和当前工作树复跑的 `307 pass / 3511 expect()` 为准；本页前置的 `baa31df0`/17 页文字只作历史交接。
> **当前本地源码候选纠正（2026-08-26，优先级高于本文下方旧交接段落）**：最新本地运行输入为 `dee4803fb94ad50c59c9ef8fda996bc0f37427c6`，pending 运行包包含 40 个页面；当前工作树复跑为 `307 pass / 0 fail / 3511 expect()`；当前 live `dist` 仍为 `fcc6630ebfa7b0697cbd03a5e376ce6765d1643b`，线上历史配套运行包仍为 `13f597e`。本文下方出现的旧候选只作历史交接，不能生成当前二维码或作为真机证据。
