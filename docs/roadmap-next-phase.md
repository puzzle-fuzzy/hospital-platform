...
6. 再处理报告真实 provider 只读验收、医院列表/病历和便民服务逐域迁移；院内导航动态能力必须先取得地图数据与路线 contract；
...
- 患者目录失效回收暂不通过简单删除实现。患者记录可能被报告引用、费用引用或未来订单引用，下一步必须设计带事务的“当前目录快照 + 历史引用保留”状态模型，再进行 migration 和生产回填。
