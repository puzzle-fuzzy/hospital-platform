...
> **2026-08-27 审计可靠性补充**：`2580a1fe` 将未开放入口的源码调用扫描改为保留模板 `${...}` 插值中的可执行表达式，避免把模板文案与真实调用混淆；随后 `cbb43040` 又将集中 API client 的导入路径从字符串剥离扫描中分离，补齐了原先可能漏报的静态导入规则；本轮 `1bc5bf6` 在前述基础上修复 App.onLaunch 阶段全局资料初始化对 getApp 注册时序的依赖。定向测试、15 个关闭态目标和全量门禁均通过。本修正只增强静态/客户端边界，不改变线上 API、旧 Python 服务或任何业务开放状态。
...
> 当前小程序最新 live 运行包来源为 `1bc5bf6f7cc4d38fad29fbf7e8aca3f65c46b916`，共 38 个注册页面，候选已通过构建、类型检查、旧端 64 页面台账、自动化测试和运行包静态校验，并完成原子发布；启动保护候选已覆盖 live，pending 目录已清理。项目最终固定使用微信原生 `tabBar`。现场应直接打开独立工程并普通编译核对完整来源指纹。线上服务端 release 为 `5738a71e0bcddaa8849106754baf5b296427bed7`；线上历史小程序运行包 sourceRevision 仍为 `13f597ea9ee3f65b9be858117826d948339d904a`，真实页面与服务端同链业务证据仍待。本页下方带历史日期的候选只作追溯。
...
`1bc5bf6f7cc4d38fad29fbf7e8aca3f65c46b916`（提交 `1bc5bf6`）；下方历史 release 不得作为本轮真机或业务证据。
