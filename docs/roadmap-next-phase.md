...
- 2026-08-21（当前候选预约目录展示边界）：提交 `968a587` 将医院日期标签、日期分组和本地号源展示窗口抽为纯展示服务，
  ...
  该修正不代表 Provider 分页、预约写入或真机验收完成，详见 [`release/candidate-968a587-local-build-2026-08-21.md`](release/candidate-968a587-local-build-2026-08-21.md)。

- 2026-08-21（当前真机 `single-flight.test.js` ENOENT 复核）：最新 `968a587` 运行包中 `single-flight.js` 存在，
...
3. 使用当前服务端 release `5a31427` 和最新小程序候选 `968a587`（完整构建来源：`968a587158289da6a482b3614907bde0a5ad9581`）重新同步真实账号的患者目录，先运行显式 `patient-sync` smoke，再补做 `his-patient` owner-scoped 记录查询验收；
...
14. 当前公网 runtime 与 P0 日志 bundle 已能证明请求进入 `5a31427` Bun 进程；基础路由不再重复作为业务完成证据，下一步只补真实 session、owner 映射、Provider 状态和真机页面证据，并始终使用最新本地 `968a587` 小程序候选（完整构建来源：`968a587158289da6a482b3614907bde0a5ad9581`）。
