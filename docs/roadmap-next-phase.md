> **当前事实源（2026-08-26，优先于本文其余历史段落）**：新 Elysia API 线上 release 为 `8eb51b5ffe85b0b8f8a032783f893117d3df549d`，旧 Python 服务继续监听 `0.0.0.0:8001`；本轮没有修改或停止旧服务。项目最终固定使用微信原生 `tabBar`。最新小程序运行相关源码候选和 pending 运行输入为 `1d132315dff23c8befdfc867cb62a4227dbb7820`（短提交 `1d132315`），共 40 个页面，回归为 `326 pass / 0 fail / 3622 expect()`；本地 live `dist` 仍为上一候选 `02dbf10419740d96c4445493df019021ac22bcfa`（`02dbf10`），新候选因开发者工具文件锁尚未原子发布。本轮统一电子锦旗和表扬信记录区域的加载、错误、未开放三态及固定高度，保留选择页刷新并发门禁、微信资料被拒绝后的设置页重试和“我的快递”加载/错误/未开放三态，真实临床 Provider、物流/采血号源/公开记录仍关闭；就诊页今日预约摘要已补齐但实时叫号仍未开放；健康自测保留不带临床结论的 BMI/血压安全数值子集，其余临床入口仍按 contract 关闭；九个真机证据域仍待采集。健康百科正式审核 bundle、协议同意/撤回/审计、二维码真实能力、支付医保、临床真实读取、实时就诊和外部 web-view 继续关闭；旧端 64 页面逐页落点台账和 34 个冻结入口门禁有效；完整推进队列见 [`migration/full-migration-handoff-2026-08-25.md`](migration/full-migration-handoff-2026-08-25.md)。
...
# 下一阶段实施路线图
 ...
 > **最新小程序候选事实（2026-08-26）**：当前运行相关源码候选和 pending 运行输入为 `1d132315dff23c8befdfc867cb62a4227dbb7820`，40 个页面，`326 pass / 0 fail / 3622 expect()`；本地 live `dist` 仍为上一候选 `02dbf10419740d96c4445493df019021ac22bcfa`，新候选等待释放开发者工具文件锁后原子发布。本轮统一电子锦旗和表扬信记录区域的加载、错误、未开放三态及固定高度，保留选择就诊人刷新并发门禁、微信资料拒绝后的设置页重试和“我的快递”加载/错误/未开放三态，临床/服务入口的当前就诊人上下文、重试和选择入口、患者签名和消息订阅的原生安全展示层、采血预约、锦旗/表扬信安全页面、今日预约摘要与安全数值子集继续保持边界，未开放临床分级、采血号源、公开记录或实时叫号。正式健康审核 bundle 缺失时健康百科仍保持 fail-closed，协议同意/撤回/审计仍关闭。支付、医保、临床真实读取、外部 WebView 和预约写入仍按各自 contract 关闭。

 > **当前广度事实源（2026-08-26，运行包来源 `1d132315dff23c8befdfc867cb62a4227dbb7820`）**：旧端 64 个页面已全部登记；新端现有 40 个注册页面，健康自测中的 BMI/血压安全数值工具已进入 `partial`，其余临床页面继续使用 `surface-only` 或关闭态，健康百科三个只读页面、协议原文只读页、旧端“我的快递”和采血预约真实空态、电子锦旗/健康表扬信安全页面结构、患者签名/消息订阅安全展示层、临床/服务入口共享当前就诊人上下文以及我的问诊患者作用域已接入，微信资料拒绝后的设置页重试、选择页刷新并发门禁和我的快递三态已接入，统一状态页已展示迁移阶段与旧入口覆盖范围。当前候选 `326 pass / 0 fail / 3622 expect()` 已完成构建和 pending 运行包校验；新候选尚未替换 live `dist`。正式健康内容、协议同意、临床审核、支付、医保、预约写入和 HIS 回写仍按原顺序关闭。

 > **当前仓库事实补充（2026-08-26）**：本轮小程序运行包候选来源为 `1d132315dff23c8befdfc867cb62a4227dbb7820`，小程序回归为 `326 pass / 0 fail / 3622 expect()`。本轮统一电子锦旗和表扬信记录区域的加载、错误、未开放三态及固定高度，保留选择就诊人刷新并发门禁、微信资料拒绝后的设置页重试和我的快递三态，不改变线上 `8eb51b5f`、live `dist` 之外的旧服务或其他会话适配器；候选已完成 pending 构建和校验，尚未替换本地 live，协议同意、撤回、临床真实读取、采血号源、公开记录、实时叫号和支付医保继续关闭，上方旧候选段落保留为历史追溯。
