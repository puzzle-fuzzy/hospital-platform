...
- 2026-08-22（本轮当前小程序候选与患者范围错误态收口）：当前候选已更新为提交 `1b621f07`，运行包来源为 `1b621f07ab8cec04f76fc8b682d0b3114ef6e3a1`；14 个页面入口齐全，`dist/` 中 `*.test.js`/`*.spec.js` 数量为 0，`runtime:verify` 通过。`pnpm check` 全部通过：架构规则 67 条、工具测试 `53 pass / 0 fail`、API 测试 `207 pass / 0 fail`、小程序测试 `212 pass / 0 fail`，9 个 workspace 的类型检查和构建均通过。预约历史、门诊费用、报告目录和爽约记录在依赖失败、会话失效或网络错误时不再落入“暂无数据/请先选择就诊人”空状态，而是提供重试和重新选择就诊人的恢复入口；详见 [`release/candidate-1b621f0-local-build-2026-08-22.md`](release/candidate-1b621f0-local-build-2026-08-22.md) 与 [`release/readonly-page-error-state-2026-08-22.md`](release/readonly-page-error-state-2026-08-22.md)。本轮没有修改旧 Python 服务、数据库或 Redis；真机仍需从该来源重新导入/普通编译后取得新的二维码和三层业务证据。
