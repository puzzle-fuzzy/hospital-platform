...
- 随后本地客户端继续推进到 `c1338a2`：修正选择页“目录读取 + 临床同步”在快速刷新/重复同步下的页面实例并发竞态，
  并同步补充中文业务不变量和验收断言；小程序 106 项测试、935 个断言、typecheck、build 和 14 页面运行包校验均通过。
  该客户端仍未部署到线上，真机候选必须使用 `c1338a2` 重新构建的 `dist/`，不能继续使用 `055a784` 旧包。
