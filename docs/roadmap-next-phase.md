...
> **最新小程序候选事实（2026-08-26）**：当前运行相关源码候选和 pending 运行输入为 `de5dea8df249c2fd1e122df4508bcd6cbb3852a7`，40 个页面，`316 pass / 0 fail / 3574 expect()`；本地 live `dist` 仍为上一候选 `02dbf10419740d96c4445493df019021ac22bcfa`，新候选等待释放开发者工具文件锁后原子发布。本轮补齐选择就诊人刷新并发门禁，临床/服务入口的当前就诊人上下文、重试和选择入口、患者签名和消息订阅的原生安全展示层、采血预约、锦旗/表扬信安全页面、今日预约摘要与安全数值子集继续保持边界，未开放临床分级、采血号源、公开记录或实时叫号。正式健康审核 bundle 缺失时健康百科仍保持 fail-closed，协议同意/撤回/审计仍关闭。支付、医保、临床真实读取、外部 WebView 和预约写入仍按各自 contract 关闭。

> **当前广度事实源（2026-08-26，运行包来源 `de5dea8df249c2fd1e122df4508bcd6cbb3852a7`）**：旧端 64 个页面已全部登记；新端现有 40 个注册页面，健康自测中的 BMI/血压安全数值工具已进入 `partial`，其余临床页面继续使用 `surface-only` 或关闭态，健康百科三个只读页面、协议原文只读页、旧端“我的快递”和采血预约真实空态、电子锦旗/健康表扬信安全页面结构、患者签名/消息订阅安全展示层、临床/服务入口共享当前就诊人上下文以及我的问诊患者作用域已接入，微信资料拒绝后的设置页重试已接入，统一状态页已展示迁移阶段与旧入口覆盖范围。当前候选 `316 pass / 0 fail / 3574 expect()` 已完成构建和 pending 运行包校验；新候选尚未替换 live `dist`。正式健康内容、协议同意、临床审核、支付、医保、预约写入和 HIS 回写仍按原顺序关闭。

> **当前仓库事实补充（2026-08-26）**：本轮小程序运行包候选来源为 `de5dea8df249c2fd1e122df4508bcd6cbb3852a7`，小程序回归为 `316 pass / 0 fail / 3574 expect()`。本轮补齐选择就诊人刷新并发门禁，不改变线上 `8eb51b5f`、live `dist` 之外的旧服务或其他会话适配器；候选已完成 pending 构建和校验，尚未替换本地 live，协议同意、撤回、临床真实读取、采血号源、公开记录、实时叫号和支付医保继续关闭，上方旧候选段落保留为历史追溯。
