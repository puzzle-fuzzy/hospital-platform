...
> **当前广度事实源（2026-08-26，pending `7f7a7a18`）**：旧端 64 个页面已全部登记；新端现有 20 个注册页面，健康百科三个只读页面已接入，统一状态页已展示迁移阶段与旧入口覆盖范围。当前候选 `285 pass / 0 fail / 3211 expect()`，pending 构建等待释放微信开发者工具锁；正式健康内容、临床审核、支付、医保、预约写入和 HIS 回写仍按原顺序关闭。
