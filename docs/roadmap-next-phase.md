...
`9f479c9a`，小程序运行包来源为
`937bb2ad2c71a2e83fac679939d31ed6bb3f0996`。SSH 已通过中转内网地址恢复；本次只重启新 API，旧 Python `8001` 继续共存。
...
当前下一步顺序固定为：从 `937bb2ad` 正确项目重新普通编译/扫码 → 微信登录 → 患者目录同步与显式切换 → 预约历史/爽约 →
...
`937bb2ad2c71a2e83fac679939d31ed6bb3f0996`；下方历史 release 不得作为本轮真机或业务证据。
...
3. 使用当前服务端 release `9f479c9a` 和最新小程序候选 `937bb2ad`（完整构建来源：`937bb2ad2c71a2e83fac679939d31ed6bb3f0996`）重新同步真实账号的患者目录，先运行显式 `patient-sync` smoke，再补做 `his-patient` owner-scoped 记录查询验收；
...
14. 当前公网 runtime 与 P0 日志 bundle 只证明请求进入 `9f479c9a` Bun 进程；基础路由不再重复作为业务完成证据，下一步只补真实 session、owner 映射、Provider 状态和真机页面证据，并始终使用最新本地 `937bb2ad` 小程序候选（完整构建来源：`937bb2ad2c71a2e83fac679939d31ed6bb3f0996`）。
