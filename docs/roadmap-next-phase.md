...
> 历史原生 Tab 候选（2026-08-24）：本轮曾使用 `custom=false` 的微信原生 `tabBar` 和独立 `-native` 资源路径；该候选因真机反馈仍有闪动/选中态问题而结束，不再作为当前验收入口。当前候选改用官方单实例 `custom-tab-bar`，见 [`release/current-tabbar-runtime-recheck-2026-08-25.md`](release/current-tabbar-runtime-recheck-2026-08-25.md)。
> 当前正确结构只包含一份 `custom-tab-bar/`、没有页面级底栏或第二套 selected 状态；会话失效回首页也已从 `reLaunch` 收口到 `switchTab`。上一轮开发者工具复核和原生候选只保留历史记录。
> 当前开发者工具执行要求：必须直接打开 `apps/miniprogram/dist/` 独立工程并普通编译 `c4eb3587d0126458f7210fee134f96b51dce7b6c`，再确认首页和“就诊”页只有一份固定共享底栏，切换后当前图标/文字变蓝。历史线上 `13f597e` 和原生候选不能作为本次 Tab 行为证据，不能打开父目录、`src/` 或另一个旧 `dist/` 工程，也不能新增页面级底栏。
...
> 当前小程序会话/导航门禁（2026-08-25，本地未部署）：患者范围 GET 收到 `503 persistence-temporarily-unavailable` 时保留 token、不重新登录、不重放旧 `patientId`；主 Tab 程序化入口强制使用 `switchTab`，当前 Tab 重复导航会 no-op，底栏由官方 `custom-tab-bar` 单实例维护，会话恢复期间患者卡片只显示固定占位。本轮小程序回归为 `240/240` 通过、`1929` 个断言，运行包来源为 `c4eb358`；真机页面证据仍待。
