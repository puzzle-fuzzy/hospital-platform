> **最新事实源（2026-08-25，优先于本文其余历史段落）**：新 Elysia API 线上 release 为 `8eb51b5ffe85b0b8f8a032783f893117d3df549d`，旧 Python 服务继续监听 `0.0.0.0:8001`；本轮没有修改或停止旧服务。小程序源码本轮恢复微信官方 `custom-tab-bar` 共享组件，并修正切换锁，尚未重新构建发布运行包；真机必须等待新的 `dist/build-info.json` 来源指纹后再验收。线上小程序仍是分层发布的 `13f597ea9ee3f65b9be858117826d948339d904a`，不能用线上包证明本地底栏修正。
...
> 当前小程序底栏修正（2026-08-25）：用户反馈原生底栏仍出现闪动且选中态不明显；本轮恢复微信官方 `custom-tab-bar`，由单一共享组件持有四项底栏，切换期间禁止旧页面路由回写 selected。源码修正后需要重新构建并直接打开 `apps/miniprogram/dist/` 普通编译验收。旧 Python `8001`、线上服务、数据库和 Redis 未修改。详见 [`release/current-native-tabbar-runtime-recheck-2026-08-25.md`](release/current-native-tabbar-runtime-recheck-2026-08-25.md)。
