...
- 2026-08-21（只读页面跨会话本地事件修正）：本次代码复核基于服务端 `5a31427`、小程序候选 `7a6f4df`（完整来源 `7a6f4df34fac5975c6012a30d2c137953a892059`）。发现预约记录、爽约记录和报告目录虽在组合读取提交前校验会话代际，但页面没有保存已提交读模型所属代际；现已阻断旧会话的本地分页、预约标签、报告详情及静态业务事件，同时保留“更换就诊人”作为修复入口。小程序 `178 pass`、`1418 expects`、typecheck、Biome、构建和 `runtime:verify` 均通过。详见 [`release/miniprogram-readonly-page-session-event-audit-2026-08-21.md`](release/miniprogram-readonly-page-session-event-audit-2026-08-21.md)。

- 2026-08-21（门诊费用患者会话竞态修正）：本次代码复核基于服务端 `5a31427`、小程序候选 `7a6f4df`（完整来源 `7a6f4df34fac5975c6012a30d2c137953a892059`）。发现费用页状态切换可能使用上一轮会话的缓存患者对象，已增加患者卡片所属会话代际、请求前断言，以及会话/患者漂移时重新执行 `/me` → 患者目录 → 当前状态查询。小程序完整测试 `177 pass`、API 预约/费用/报告定向测试 `62 pass`；支付、医保、HIS、预约写入和真机证据门禁不变。详见 [`release/miniprogram-outpatient-payment-session-race-audit-2026-08-21.md`](release/miniprogram-outpatient-payment-session-race-audit-2026-08-21.md)。
...
- 2026-08-21（当前小程序运行包刷新）：针对真机调试报告的 `dist/services/single-flight.test.js` ENOENT，已用提交
  `7a6f4df34fac5975c6012a30d2c137953a892059` 重新构建并通过 `runtime:verify`。当前 `dist` 有 14 个页面、真实运行模块
  ...
  [`release/candidate-7a6f4df-local-build-2026-08-21.md`](release/candidate-7a6f4df-local-build-2026-08-21.md)。
...
3. 使用当前服务端 release `5a31427` 和最新小程序候选 `7a6f4df`（完整构建来源：`7a6f4df34fac5975c6012a30d2c137953a892059`）重新同步真实账号的患者目录，先运行显式 `patient-sync` smoke，再补做 `his-patient` owner-scoped 记录查询验收；
...
14. 当前公网 runtime 与 P0 日志 bundle 已能证明请求进入 `5a31427` Bun 进程；基础路由不再重复作为业务完成证据，下一步只补真实 session、owner 映射、Provider 状态和真机页面证据，并始终使用最新本地 `7a6f4df` 小程序候选（完整构建来源：`7a6f4df34fac5975c6012a30d2c137953a892059`）。
