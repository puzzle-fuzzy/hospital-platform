...
`171a8743185fb4ecc1696851662659c1a0ee7ebf`。SSH 已通过中转内网地址恢复；本次只重启新 API，旧 Python `8001` 继续共存。
...
| 小程序运行包 | `171a874`、14 个页面、无测试 JS | 构建、运行包校验和请求观测回归已通过；患者范围入口门禁已收紧，仍需手机扫码并取得当前服务端的三层业务证据 |
...
当前下一步顺序固定为：从 `171a874` 正确项目重新普通编译/扫码 → 微信登录 → 患者目录同步与显式切换 → 预约历史/爽约 →
...
> 当前候选：服务端已验证 release `0e2a366efcca8da25d7edd4a286781f2d3dfdbec`；小程序运行包来源 `171a8743185fb4ecc1696851662659c1a0ee7ebf`（提交 `171a874`）。生产运行层证据见 [`release/0e2a366e-production-acceptance-2026-08-22.md`](release/0e2a366e-production-acceptance-2026-08-22.md)，小程序候选证据见 [`release/candidate-171a874-local-build-2026-08-22.md`](release/candidate-171a874-local-build-2026-08-22.md)。
...
`171a8743185fb4ecc1696851662659c1a0ee7ebf`；下方历史 release 不得作为本轮真机或业务证据。
...
3. 使用当前服务端 release `0e2a366efcca8da25d7edd4a286781f2d3dfdbec` 和最新小程序候选 `171a874`（完整构建来源：`171a8743185fb4ecc1696851662659c1a0ee7ebf`）重新同步真实账号的患者目录，先运行显式 `patient-sync` smoke，再补做 `his-patient` owner-scoped 记录查询验收；
...
14. 当前公网 runtime 与 P0 日志 bundle 只证明请求进入 `0e2a366e` Bun 进程；基础路由不再重复作为业务完成证据，下一步只补真实 session、owner 映射、Provider 状态和真机页面证据，并始终使用最新本地 `171a874` 小程序候选（完整构建来源：`171a8743185fb4ecc1696851662659c1a0ee7ebf`）。
