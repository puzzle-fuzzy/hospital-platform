...
`1e58bb66bf24021d2b680eb5fd03abfec467989a`，小程序运行包来源为
`5e43aed0e026cd48d980d58c468223b9a5ee8744`。SSH 已通过中转内网地址恢复；本次只重启新 API，旧 Python `8001` 继续共存。

| 事实 | 当前状态 | 证据与限制 |
| --- | --- | --- |
| 新旧服务共存 | 当前已证明新 API 与旧 Python `8001` 共存 | `2a2acd9` 原子切换后新 API 重启，旧 Python PID 集合前后不变；Worker 仍 inactive |
| 服务端生产候选 | `1e58bb66` | preflight、隔离 `18082` smoke、切换后内网/公网探针通过；请求关联号安全修正已部署，支付/医保/HIS 未触发 |
| 小程序运行包 | `5e43aed`、14 个页面、无测试 JS | 构建、运行包校验已通过；二维码患者上下文门禁已收紧，仍需手机扫码并取得当前服务端的三层业务证据 |
...
当前下一步顺序固定为：从 `5e43aed` 正确项目重新普通编译/扫码 → 微信登录 → 患者目录同步与显式切换 → 预约历史/爽约 →
...
> 当前候选：服务端已验证 release `1e58bb66bf24021d2b680eb5fd03abfec467989a`；小程序运行包来源 `5e43aed0e026cd48d980d58c468223b9a5ee8744`（提交 `5e43aed`）。生产运行层证据见 [`release/1e58bb66-production-acceptance-2026-08-22.md`](release/1e58bb66-production-acceptance-2026-08-22.md)，小程序候选证据见 [`release/candidate-5e43aed-local-build-2026-08-22.md`](release/candidate-5e43aed-local-build-2026-08-22.md)。
...
当前执行项绑定服务端 release `1e58bb66bf24021d2b680eb5fd03abfec467989a` 与小程序来源
`5e43aed0e026cd48d980d58c468223b9a5ee8744`；下方历史 release 不得作为本轮真机或业务证据。
...
3. 使用当前服务端 release `1e58bb66bf24021d2b680eb5fd03abfec467989a` 和最新小程序候选 `5e43aed`（完整构建来源：`5e43aed0e026cd48d980d58c468223b9a5ee8744`）重新同步真实账号的患者目录，先运行显式 `patient-sync` smoke，再补做 `his-patient` owner-scoped 记录查询验收；
...
13. 当前服务端 release `1e58bb66` 已按 [`infra/systemd/api-v2-release-runbook.md`](../infra/systemd/api-v2-release-runbook.md) 完成原子 `current` 切换和新 API 单元重启；`18081`、公网 `/api/v2`、旧 `8001` 已复测通过。下一步进行真实微信登录、患者切换、预约只读和门诊费用的分层验收，任何业务层失败只回滚新 API，不触碰旧 Python 服务。
14. 当前公网 runtime 与 P0 日志 bundle 只证明请求进入 `1e58bb66` Bun 进程；基础路由不再重复作为业务完成证据，下一步只补真实 session、owner 映射、Provider 状态和真机页面证据，并始终使用最新本地 `5e43aed` 小程序候选（完整构建来源：`5e43aed0e026cd48d980d58c468223b9a5ee8744`）。
