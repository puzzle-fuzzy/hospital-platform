...
> **2026-08-27 审计可靠性补充**：`2580a1fe` 将未开放入口的源码调用扫描改为保留模板 `${...}` 插值中的可执行表达式，避免把模板文案与真实调用混淆；定向测试、15 个关闭态目标和全量 `pnpm check` 均通过。本修正只增强静态门禁，不改变小程序运行包 `99d9f60`、线上 API、旧 Python 服务或任何业务开放状态。
