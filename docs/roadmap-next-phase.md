...
> `6252ea4`，完整运行包来源为 `6252ea41a40a22a6264276b6590750e4bf0ef5bc`，尚未上传线上。
...
- 2026-08-21（普通资料页读写互斥修正）：当前 `6252ea4` 为最新小程序候选；资料页新增页面实例级 `onShow`，
...
- 2026-08-20（普通资料更新链定向回归）：服务端普通资料 service `13/13` 通过，小程序患者选择、会话
  ...
  首次写入、双设备 `409`、公网日志同链或真机页面已经验收；下一步仍需使用当前 `6252ea4` 候选按
...
3. 使用当前服务端 release `0e360d3` 和最新小程序候选 `6252ea4`（完整构建来源：`6252ea41a40a22a6264276b6590750e4bf0ef5bc`）重新同步真实账号的患者目录，先运行显式 `patient-sync` smoke，再补做 `his-patient` owner-scoped 记录查询验收；
...
14. 当前公网 runtime 与 P0 日志 bundle 已能证明请求进入 `0e360d3` Bun 进程；基础路由不再重复作为业务完成证据，下一步只补真实 session、owner 映射、Provider 状态和真机页面证据，并始终使用最新本地 `6252ea4` 小程序候选（完整构建来源：`6252ea41a40a22a6264276b6590750e4bf0ef5bc`）。
