...
- 2026-08-21（只读列表本地加载窗口事件边界）：当前代码提交为 `aafccf5`，完整运行包来源为 `aafccf53d1e0eaac5ddaf98b1d850b084e4f78ed`。预约历史、爽约记录、报告目录和门诊费用的“加载更多”现在要求页面不在加载中、当前患者和会话代际仍有效、结果确实还有未展示记录，并限制展示数量严格递增；刷新、切换患者或会话漂移期间的旧事件直接失效。小程序 `180 pass`、`1438 expects`、typecheck、Biome、构建和 `runtime:verify` 均通过。该修正只保护本地读模型渲染边界，不代表 Provider、支付、医保或真机业务验收完成，详见 [`release/miniprogram-readonly-list-load-more-boundary-audit-2026-08-21.md`](release/miniprogram-readonly-list-load-more-boundary-audit-2026-08-21.md)。

- 2026-08-21（预约目录本地加载窗口事件边界）：当前代码提交为 `aafccf5`，完整运行包来源为 `aafccf53d1e0eaac5ddaf98b1d850b084e4f78ed`。预约目录的“加载更多”只是对已取得读模型做本地分批展示；现在要求当前日期分组仍存在、页面不在加载中、读模型仍有未展示号源，并将展示数量限制在当前分组总数内，刷新或切换期间抵达的旧按钮事件直接忽略。小程序 `180 pass`、`1438 expects`、typecheck、Biome、构建和 `runtime:verify` 均通过。该修正不代表 Provider 分页、预约写入或真机业务验收完成，详见 [`release/candidate-aafccf5-local-build-2026-08-21.md`](release/candidate-aafccf5-local-build-2026-08-21.md)。

- 2026-08-21（只读页面跨会话本地事件修正）：本次代码复核基于服务端 `5a31427`、小程序候选 `aafccf5`（完整来源 `aafccf53d1e0eaac5ddaf98b1d850b084e4f78ed`）。发现预约记录、爽约记录和报告目录虽在组合读取提交前校验会话代际，但页面没有保存已提交读模型所属代际；现已阻断旧会话的本地分页、预约标签、报告详情及静态业务事件，同时保留“更换就诊人”作为修复入口。小程序 `180 pass`、`1438 expects`、typecheck、Biome、构建和 `runtime:verify` 均通过。详见 [`release/miniprogram-readonly-page-session-event-audit-2026-08-21.md`](release/miniprogram-readonly-page-session-event-audit-2026-08-21.md)。

- 2026-08-21（门诊费用患者会话竞态修正）：本次代码复核基于服务端 `5a31427`、小程序候选 `aafccf5`（完整来源 `aafccf53d1e0eaac5ddaf98b1d850b084e4f78ed`）。发现费用页状态切换可能使用上一轮会话的缓存患者对象，已增加患者卡片所属会话代际、请求前断言，以及会话/患者漂移时重新执行 `/me` → 患者目录 → 当前状态查询。小程序完整测试 `180 pass`、`1438 expects`、API 预约/费用/报告定向测试 `62 pass`；支付、医保、HIS、预约写入和真机证据门禁不变。详见 [`release/miniprogram-outpatient-payment-session-race-audit-2026-08-21.md`](release/miniprogram-outpatient-payment-session-race-audit-2026-08-21.md)。
...
- 2026-08-21（当前小程序运行包刷新）：针对真机调试报告的 `dist/services/single-flight.test.js` ENOENT，已用提交
  `aafccf53d1e0eaac5ddaf98b1d850b084e4f78ed` 重新构建并通过 `runtime:verify`。当前 `dist` 有 14 个页面、真实运行模块
  ...
  [`release/candidate-aafccf5-local-build-2026-08-21.md`](release/candidate-aafccf5-local-build-2026-08-21.md)。
...
3. 使用当前服务端 release `5a31427` 和最新小程序候选 `aafccf5`（完整构建来源：`aafccf53d1e0eaac5ddaf98b1d850b084e4f78ed`）重新同步真实账号的患者目录，先运行显式 `patient-sync` smoke，再补做 `his-patient` owner-scoped 记录查询验收；
...
14. 当前公网 runtime 与 P0 日志 bundle 已能证明请求进入 `5a31427` Bun 进程；基础路由不再重复作为业务完成证据，下一步只补真实 session、owner 映射、Provider 状态和真机页面证据，并始终使用最新本地 `aafccf5` 小程序候选（完整构建来源：`aafccf53d1e0eaac5ddaf98b1d850b084e4f78ed`）。
