...
> 当前小程序底栏候选（2026-08-25）：用户反馈原生底栏仍出现闪动且选中态不明显；本轮继续保持微信原生 `tabBar`（不恢复 `custom-tab-bar`），清理旧自定义尺寸字段，并把 8 个导航资源切换到全新的 `*-native-v6.png` 路径以隔离开发者工具/真机缓存。图形内容没有改变，旧 Python `8001`、线上服务、数据库和 Redis 未修改。当前运行包 revision 为 `04e238e454091691a5aab2d244b4abea43c61d0a`，新二维码为 `.local/hospital-miniprogram/tabbar-preview-04e238e.png`；不能继续使用 `a5ff0001` 旧预览包。
...
> 上一轮自定义 Tab 候选（2026-08-25）：本轮曾使用官方 `custom-tab-bar` 组件维护 selected；该候选因真机反馈仍有闪动/选中态问题而结束，不再作为当前验收入口。当前候选恢复微信原生 `tabBar`，见 [`release/current-tabbar-runtime-recheck-2026-08-25.md`](release/current-tabbar-runtime-recheck-2026-08-25.md)。
