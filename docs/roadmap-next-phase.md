...
- 2026-08-21（当前运行包重新发布）：个人资料保存会话竞态修复后，按来源指纹门禁重新构建小程序，当前候选为 `f085d06`，完整来源
  `f085d06d8e4b9695274f93b5a6b56f2af3faac91`；`dist/` 仍为 14 个页面且不包含测试脚本，`single-flight.test.js` 不会进入运行包。
...
- 2026-08-21 07:39 CST（开发者工具错误诊断复核）：`miniprogram` 模拟器调试面板唯一错误为未建立平台会话时
  ...
  [`release/candidate-f085d06-local-build-2026-08-21.md`](release/candidate-f085d06-local-build-2026-08-21.md)。
...
> `f085d06`，完整运行包来源为 `f085d06d8e4b9695274f93b5a6b56f2af3faac91`，尚未上传线上。
...
3. 使用当前服务端 release `5a31427` 和最新小程序候选 `f085d06`（完整构建来源：`f085d06d8e4b9695274f93b5a6b56f2af3faac91`）重新同步真实账号的患者目录，先运行显式 `patient-sync` smoke，再补做 `his-patient` owner-scoped 记录查询验收；
...
14. 当前公网 runtime 与 P0 日志 bundle 已能证明请求进入 `5a31427` Bun 进程；基础路由不再重复作为业务完成证据，下一步只补真实 session、owner 映射、Provider 状态和真机页面证据，并始终使用最新本地 `f085d06` 小程序候选（完整构建来源：`f085d06d8e4b9695274f93b5a6b56f2af3faac91`）。
