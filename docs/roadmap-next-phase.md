...
> **最新小程序候选事实（2026-08-26）**：当前 pending 来源为 `c7220d733b95d393030af9826c2ca305a7fc1f8e`（功能提交 `c7220d7`），40 个页面，`301 pass / 0 fail / 3435 expect()`；本轮补齐患者签名和消息订阅的原生安全展示层，既有今日预约摘要与安全数值子集继续保持边界，未开放临床分级或实时叫号。正式健康审核 bundle 缺失时健康百科仍保持 fail-closed，协议同意/撤回/审计仍关闭。live `dist` 仍被微信开发者工具锁定，上一候选及更早候选仅作历史追溯。支付、医保、临床真实读取、外部 WebView 和预约写入仍按各自 contract 关闭。

> **当前广度事实源（2026-08-26，pending `c7220d733b95d393030af9826c2ca305a7fc1f8e`）**：旧端 64 个页面已全部登记；新端现有 40 个注册页面，健康自测中的 BMI/血压安全数值工具已进入 `partial`，其余临床页面继续使用 `surface-only` 或关闭态，健康百科三个只读页面、协议原文只读页、旧端“我的快递”真实空态以及患者签名/消息订阅安全展示层已接入，统一状态页已展示迁移阶段与旧入口覆盖范围。当前候选 `301 pass / 0 fail / 3435 expect()`，pending 已完成构建和运行包校验，仍待释放微信开发者工具锁；正式健康内容、协议同意、临床审核、支付、医保、预约写入和 HIS 回写仍按原顺序关闭。

> **当前仓库事实补充（2026-08-26）**：本轮功能交接基线为 `c7220d7`，小程序回归已推进到 `301 pass / 0 fail / 3435 expect()`，pending 来源为 `c7220d733b95d393030af9826c2ca305a7fc1f8e`。本轮补齐患者签名和消息订阅安全展示层，不改变线上 `8eb51b5f`、live `dist` 或任何旧服务；协议同意、撤回、临床真实读取、实时叫号和支付医保继续关闭，上方旧候选段落保留为历史追溯。
