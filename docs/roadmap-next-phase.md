...
> **当前仓库 API 候选（2026-08-27）**：当前 `main`（候选提交以 `git rev-parse HEAD` 为准）已推送但尚未部署；其中 API 运行时代码变更来源为 `eb4d2eb4`，新增健康知识服务直调关系查询白名单，线上仍为 `1bc8b0a8`。因此发布基线当前应保持阻断，不能把本地代码或测试结果写成线上 API 事实。详细来源和下一步见 [`migration/current-execution-checkpoint-2026-08-27.md`](migration/current-execution-checkpoint-2026-08-27.md)。
