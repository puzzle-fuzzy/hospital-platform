# 下一阶段实施路线图
> 当前候选更新（2026-08-22 17:35 CST）：服务端 release 为 `84370077`；小程序运行包来源为 `7f09bbb2cf32d4753795bcbc91fe23ec05eeeee6`（提交 `7f09bbb`）。历史候选仅作追溯。
...
`7f09bbb2cf32d4753795bcbc91fe23ec05eeeee6`。SSH 已通过中转内网地址恢复；本次只重启新 API，旧 Python `8001` 继续共存。
...
| 小程序运行包 | `7f09bbb`、14 个页面、无测试 JS | 构建、运行包校验已通过；患者范围入口门禁已收紧，仍需手机扫码并取得当前服务端的三层业务证据 |
...
> 2026-08-22 17:35 CST：针对 `dist/services/single-flight.test.js` ENOENT 已重新构建运行包、重开新项目开发者工具并普通编译；
...
当前下一步顺序固定为：从 `7f09bbb` 正确项目重新普通编译/扫码 → 微信登录 → 患者目录同步与显式切换 → 预约历史/爽约 →
...
> 当前候选：服务端已验证 release `84370077024762d92050cf077c27f3c60302e8f8`；小程序运行包来源 `7f09bbb2cf32d4753795bcbc91fe23ec05eeeee6`（提交 `7f09bbb`）。生产运行层证据见 [`release/84370077-production-acceptance-2026-08-22.md`](release/84370077-production-acceptance-2026-08-22.md)，小程序候选证据见 [`release/candidate-7f09bbb-local-build-2026-08-22.md`](release/candidate-7f09bbb-local-build-2026-08-22.md)。
...
`7f09bbb2cf32d4753795bcbc91fe23ec05eeeee6`；下方历史 release 不得作为本轮真机或业务证据。
...
3. 使用当前服务端 release `84370077024762d92050cf077c27f3c60302e8f8` 和最新小程序候选 `7f09bbb`（完整构建来源：`7f09bbb2cf32d4753795bcbc91fe23ec05eeeee6`）重新同步真实账号的患者目录，先运行显式 `patient-sync` smoke，再补做 `his-patient` owner-scoped 记录查询验收；
...
14. 当前公网 runtime 与 P0 日志 bundle 只证明请求进入 `84370077` Bun 进程；基础路由不再重复作为业务完成证据，下一步只补真实 session、owner 映射、Provider 状态和真机页面证据，并始终使用最新本地 `7f09bbb` 小程序候选（完整构建来源：`7f09bbb2cf32d4753795bcbc91fe23ec05eeeee6`）。
