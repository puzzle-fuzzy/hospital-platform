...
> **2026-08-27 审计可靠性补充**：`2580a1fe` 将未开放入口的源码调用扫描改为保留模板 `${...}` 插值中的可执行表达式，避免把模板文案与真实调用混淆；随后 `cbb43040` 又将集中 API client 的导入路径从字符串剥离扫描中分离，补齐了原先可能漏报的静态导入规则；本轮 `805c54e` 在前述基础上修复 App.onLaunch 阶段全局资料初始化对 getApp 注册时序的依赖。定向测试、15 个关闭态目标和全量门禁均通过。本修正只增强静态/客户端边界，不改变线上 API、旧 Python 服务或任何业务开放状态。
...
> 当前开发者工具执行要求：当前 `805c54e` 已完成原子发布到 `apps/miniprogram/dist/`，不再执行旧的 `runtime:publish-pending` 或使用 `87ad1092`。现场应直接打开 `apps/miniprogram/dist/` 独立工程并普通编译，确认四项共享底栏只有一份、就诊三标签患者栏正确显示、今日预约摘要不冒充实时状态、快递/采血预约空态、电子锦旗/健康表扬信关闭态和患者切换入口正确、切换后当前图标/文字变蓝；同时确认就诊和互联网医院关闭态的插图、标题、说明纵向居中，并确认 App.onLaunch 不再出现 `globalData` 未定义异常。历史线上 `13f597e`、旧 live `fcc6630e`、`02dbf10`、`d204425`、`87ad1092` 和其它候选不能作为本次修复证据，不能打开父目录、`src/` 或另一个旧 `dist` 工程，也不能新增页面级底栏。

> 本轮后续唯一执行入口：基于已经发布到 live `dist` 的 `805c54e` 重新普通编译并生成二维码，从新二维码开始只读 preflight，采集四 Tab、就诊三标签患者栏、今日预约摘要、患者显式切换、预约历史/爽约、门诊费用只读和普通资料的同链证据；未取得页面、客户端 requestId、服务端 Pino 和 Provider 低敏 requestId 四方关联前，不把代码测试或健康检查写成业务完成。
> 当前小程序会话/导航门禁（2026-08-27，本地未部署）：患者范围 GET 收到 `503 persistence-temporarily-unavailable` 时保留 token、不重新登录、不重放旧 `patientId`；主 Tab 程序化入口强制使用 `switchTab`，当前 Tab 重复导航会 no-op，底栏由微信原生 `tabBar` 统一维护，会话恢复期间患者卡片只显示固定占位。本轮最新候选回归为 `341 pass / 0 fail / 3728 expect()`，运行包来源为已发布 live `805c54e`；本轮选择页刷新并发门禁已补齐，微信资料被拒绝后需要用户点击设置页重试；今日预约摘要只展示预约事实，不展示实时叫号，快递和采血预约页只展示旧端真实空态，电子锦旗和健康表扬信只展示安全关闭态；临床/服务入口和我的问诊共享当前就诊人上下文，患者绑定/签名页可查看协议原文但不写入同意；会话失效时会清理旧 owner 的微信资料缓存；报告/费用底层请求和预约排班/历史底层构造器已增加运行时输入门禁；真机页面证据仍待。
...
> 当前服务端与小程序采用分层发布：线上服务端 release 为 `b44421cd`，线上历史小程序运行包来源为 `13f597e`，本地开发者工具 live `dist` 已原子发布为 `805c54e`；旧 Python `8001` 继续共存。当前可以从 `805c54e` 重新生成二维码开始真机业务三层证据，但不能把本地 live `dist` 写成微信线上版本，也不能把运行层 smoke 当作业务完成。

> 历史候选刷新（2026-08-22）：服务端 release 曾为 `0e2a366efcca8da25d7edd4a286781f2d3dfdbec`；小程序运行包来源为 `4ba492a3fdae8283409bd2ab4a0a45247c46600c`（提交 `4ba492a`）。本行仅作追溯，不能覆盖上方当前 `805c54e` 的本地 live 边界。
...
>
> 历史小程序代码候选 `90d5ab0323b33fb620c83b86a8248b31c0ae3341` 和 `fc70fa0b78a29f564c4e3e4b35ab28e4afb3fde` 仅作追溯；当前本地 live 运行输入以本文顶部 `805c54e` 为准。开发者工具锁定导致的历史候选切换阻塞已解除，当前应从 `apps/miniprogram/dist/` 重新生成二维码；线上正式小程序仍未上传。
...
> **当前小程序候选（2026-08-27）**：`805c54ea9fa943385ad6feebed1401d521fbad3c`（`805c54e`）已通过构建、回归和运行包校验并原子发布到 live `dist`；本候选修复 App.onLaunch 全局资料初始化时序，同时保留预约排班和预约历史请求构造器的运行时字段、日期和范围边界。新的九域真机清单为 [`release/device-evidence-805c54ea-pending.json`](release/device-evidence-805c54ea-pending.json)，当前开发者工具已恢复并已生成 iOS 二维码，但手机尚未连接，因此所有真机结果仍保持 `pending`。
...
> 当前最新小程序代码和 live 运行包来源为 `805c54ea9fa943385ad6feebed1401d521fbad3c`，共 40 个注册页面，候选已通过构建、类型检查、旧端 64 页面台账、自动化测试和运行包静态校验，并完成原子发布；项目最终固定使用微信原生 `tabBar`。现场应直接打开独立工程并普通编译核对完整来源指纹。线上服务端 release 为 `b44421cd321ff9ff23eeb49b12641d1772d2bdc1`；线上历史小程序运行包 sourceRevision 仍为 `13f597ea9ee3f65b9be858117826d948339d904a`，真实页面与服务端同链业务证据仍待。本页下方带历史日期的候选只作追溯。
...
当前下一步顺序固定为：从已发布 live `dist` 的 `805c54e` 重新普通编译并生成二维码，再对线上 `b44421cd` 服务端进行微信登录 → 患者目录同步与显式切换 → 今日预约摘要/预约历史/爽约 →
...
> 当前服务端事实：已验证 release `b44421cd321ff9ff23eeb49b12641d1772d2bdc1`；当前小程序 live 运行输入为 `805c54ea9fa943385ad6feebed1401d521fbad3c`，线上 `13f597e` 仅作历史运行包记录。生产运行层证据见 [`release/candidate-b44421cd-production-acceptance-2026-08-27.md`](release/candidate-b44421cd-production-acceptance-2026-08-27.md)，后续重建锁定与发布流程见 [`release/wechat-devtools-dist-lock-2026-08-25.md`](release/wechat-devtools-dist-lock-2026-08-25.md)，当前九域真机清单见 [`release/device-evidence-805c54ea-pending.json`](release/device-evidence-805c54ea-pending.json)，上一候选清单不能直接作为本轮真机完成证据。
...
`805c54ea9fa943385ad6feebed1401d521fbad3c`（提交 `805c54e`）；下方历史 release 不得作为本轮真机或业务证据。
...
3. 使用当前服务端 release `b44421cd321ff9ff23eeb49b12641d1772d2bdc1` 和小程序候选 `805c54e`（完整构建来源：`805c54ea9fa943385ad6feebed1401d521fbad3c`）重新同步真实账号的患者目录，先运行显式 `patient-sync` smoke，再补做 `his-patient` owner-scoped 记录查询验收；
...
14. 当前公网 runtime 与 P0 日志 bundle 只证明请求进入 `b44421cd` Bun 进程；基础路由不再重复作为业务完成证据，下一步只补真实 session、owner 映射、Provider 状态和真机页面证据，并使用 `805c54e` 小程序候选（完整构建来源：`805c54ea9fa943385ad6feebed1401d521fbad3c`）。
...
> **当前事实源覆盖旧段落（2026-08-27）**：请以 live `805c54e`、40 个原生页面、健康自测安全数值子集、协议原文只读页、今日预约摘要窗口、快递/采血预约真实空态、电子锦旗/健康表扬信安全关闭态、共享当前就诊人上下文、微信资料拒绝后的设置页重试、选择就诊人刷新并发门禁、会话失效时旧 owner 微信资料缓存清理、报告/费用和预约查询底层输入门禁和当前工作树复跑的 `341 pass / 0 fail / 3728 expect()` 为准；本页前置的 `baa31df0`/17 页文字只作历史交接。
> **当前本地源码候选纠正（2026-08-27，优先级高于本文下方旧交接段落）**：最新本地运行输入为 `805c54ea9fa943385ad6feebed1401d521fbad3c`，live 运行包包含 40 个页面；当前工作树复跑为 `341 pass / 0 fail / 3728 expect()`；当前 live `dist` 已原子发布为 `805c54e`，线上历史配套运行包仍为 `13f597e`。本文下方出现的旧候选只作历史交接，不能生成当前二维码或作为真机证据。
