> **最新事实源（2026-08-25，优先于本文其余历史段落）**：新 Elysia API 线上 release 为 `8eb51b5ffe85b0b8f8a032783f893117d3df549d`，旧 Python 服务继续监听 `0.0.0.0:8001`；本轮没有修改或停止旧服务。项目最终固定使用微信原生 `tabBar`。最新小程序候选为 `baa31df08f63af30266664f9fef9224653cf52bb`，已完成类型检查、自动化测试和静态门禁，构建候选已安全保存在 pending，但因微信开发者工具锁定 live `dist/`，尚未原子发布；当前 live dist 仍为 `fcc6630e`，不能用旧二维码证明本候选。候选新增就诊页未来/历史预约摘要只读展示和分批展开，今日实时队列仍关闭；旧端 64 页面逐页落点台账和七个业务域覆盖仍有效；候选记录详见 [`release/candidate-baa31df0-breadth-migration-gate-2026-08-25.md`](release/candidate-baa31df0-breadth-migration-gate-2026-08-25.md)。
