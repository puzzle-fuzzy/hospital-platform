...
> 当前本地未发布小程序候选（2026-08-24）：`356705e41852e585b07296c5e6e3dec52bce1381`。它收口“我的挂号”卡片、爽约入口和查询状态稳定高度，补齐预约目录科室/排班状态容器，并固定在线/全部标签的请求范围快照，未部署、未替换线上 `13f597e`，也未修改旧 Python、数据库或 Redis；本地构建和运行包验证见 [`release/candidate-356705e-miniprogram-build-2026-08-24.md`](release/candidate-356705e-miniprogram-build-2026-08-24.md)。
> 当前小程序会话门禁（2026-08-24，本地未部署）：新增回归锁定患者范围 GET 收到 `503 persistence-temporarily-unavailable` 时保留 token、不重新登录、不重放旧 `patientId`；全套小程序回归 `228/228` 通过，共 `1706` 个断言。`401` 的一次受控恢复和同代二次 401 清理规则不变，详见 [`release/miniprogram-session-dependency-error-boundary-2026-08-24.md`](release/miniprogram-session-dependency-error-boundary-2026-08-24.md)。
