...
| `provider.request.raw` / `provider.response.raw` | `PROVIDER_RAW_LOGGING=true` 的受控 Provider 联调窗口 | 记录真实 URL、完整请求头、完整请求体和完整响应体，便于核对 Provider `code/message` 与字段值；仅允许测试环境/短期使用，日志可能包含医保凭证和患者信息，联调结束必须关闭该环境变量并重启服务 |
| `provider.request.logical.raw` / `provider.response.logical.raw` | 加密 FSI（6201/6202/6203/6301/6401）解密前后的业务报文 | 在同一开关下记录 FSI 实际业务请求和解密后的真实响应；通用 raw 事件只看到 relay/envelope，不能替代此处的 logical raw；联调结束必须关闭该环境变量并重启服务 |
