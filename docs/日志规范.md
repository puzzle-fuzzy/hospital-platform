...
本项目服务端使用 [Pino](https://github.com/pinojs/pino) 作为唯一日志实现；微信小程序客户端使用 `apps/miniprogram/src/services/telemetry.ts` 统一遥测层（见下文「小程序客户端遥测」）。业务代码不自行拼接文本日志，也不直接输出请求体、令牌或第三方原始报文。Pino 负责 JSONL 输出、日志级别、时间戳、子 logger 和敏感字段脱敏；项目只负责补充业务事件字段。
...
| `provider.request.dispatched` / `provider.response.received` / `provider.request.invalid` / `provider.request.failed` | 所有 adapter 共用的 Provider HTTP 边界 | 记录 provider、operation、trace、HTTP 状态、请求/响应结构、字段名、长度、数组数量和不可逆摘要；禁止记录请求 URL 查询值、Authorization、身份证、姓名、医保 token、授权码、支付参数和原始报文 |
