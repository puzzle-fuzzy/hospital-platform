...
| `medical-insurance.cancellation.2.6.65.4.completed` / `medical-insurance.cancellation.2.6.65.11.completed` / `medical-insurance.cancellation.2.6.65.6.completed` | 支付查单、支付关单、取消结算 | 记录 provider request id、`paymentStatusCode`、`revokeStatus`、`cancelStatus`、有限状态分类、响应字段结构和完成结果；不记录请求/响应值、支付凭证或医保原文 |
...
| `medical-insurance.reauthorization.cancellation.requested` / `medical-insurance.reauthorization.cancellation.completed` | 重新授权前关闭旧医保结算 | 记录内部订单、固定阶段和低敏 Provider 请求号；不记录授权码、医保凭证或原始报文 |
