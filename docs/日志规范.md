...
| `medical-insurance.plugin-preorder.ready-for-wechat-mix` / `medical-insurance.plugin-prepay.persisted` / `medical-insurance.plugin-settlement.completed` / `medical-insurance.plugin-settlement.completed-for-wechat-mix` | 云健康预支付与结算 | 记录内部订单、固定阶段、低敏 Provider 请求号和 trace；不记录患者身份、支付凭证或原始报文 |
| `medical-insurance.plugin.2.27.2.29.persisted` / `medical-insurance.plugin.2.27.2.29.storage-skipped` | 存量云健康流水兼容 | 只记录内部订单、是否持久化及固定原因；不记录第三方流水原文 |
| `medical-insurance.pre-payment-component.succeeded` | 微信医保支付前的非 HIS 收款分项 `.2` 已落库 | 记录内部医保订单、分项类型、金额（分）、payModel/payTypeId 和低敏 Provider 请求号 |
