...
| `provider.request.raw` / `provider.response.raw` | `PROVIDER_RAW_LOGGING=true` 的 3090 永久原始日志输出 | 记录真实 URL、完整请求头、完整请求体和完整响应体，便于核对 Provider `code/message` 与字段值；日志可能包含医保凭证和患者信息，只能在受控服务器上按 traceId/订单排查，永远不得关闭该开关 |
