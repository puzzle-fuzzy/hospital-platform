> 当前事实（2026-08-26）：线上服务端 release 为 `8eb51b5ffe85b0b8f8a032783f893117d3df549d`；线上历史小程序运行包来源为 `13f597ea9ee3f65b9be858117826d948339d904a`，当前本地 pending 运行输入为 `dee4803fb94ad50c59c9ef8fda996bc0f37427c6`，共 40 个页面，回归为 `307 pass / 0 fail / 3511 expect()`。pending 尚未发布，真机业务三层证据仍待。

> 当前候选已继续推进到 `dee4803f`：在全量入口覆盖、患者签名/消息订阅安全展示层、采血预约和锦旗/表扬信安全页面、临床/服务入口当前就诊人上下文基础上，补齐“我的问诊”的患者作用域、错误重试和选人入口；不调用未知外部小程序，不调用微信订阅授权，不伪造号源、预约写入、公开记录或临床 Provider；正式健康审核 bundle 缺失时仍保持 fail-closed，旧 Python 服务和线上服务均未修改。详见 [`release/candidate-dee4803f-miniprogram-runtime-2026-08-26.md`](release/candidate-dee4803f-miniprogram-runtime-2026-08-26.md)。
> 本段优先于本文下方旧日期、旧 release 或旧运行包叙述；旧值只作为历史记录，不作为当前验收入口。
# 微信授权登录实施与验收手册
> 当前验收配套基线：线上服务端 release 为 `8eb51b5ffe85b0b8f8a032783f893117d3df549d`；线上历史小程序运行包来源为 `13f597ea9ee3f65b9be858117826d948339d904a`，当前本地 pending 运行输入为 `dee4803fb94ad50c59c9ef8fda996bc0f37427c6`（功能提交 `dee4803f`），共 40 个页面。pending 尚未发布，真实微信业务三层证据仍待；29 个跨域页面当前只处于 `surface-only`，健康自测另有安全数值子集。候选详情见 [`release/candidate-dee4803f-miniprogram-runtime-2026-08-26.md`](release/candidate-dee4803f-miniprogram-runtime-2026-08-26.md)。
...
当前本地 pending 运行输入为 `dee4803f`，完整指纹为
`dee4803fb94ad50c59c9ef8fda996bc0f37427c6`；线上历史小程序包为 `13f597e`。本候选包含运行包 test/spec 文件边界、迁移入口覆盖展示、健康自测安全数值子集、临床与外部入口安全页面、预约 Provider 入口、患者签名和消息订阅安全展示页、旧端“我的快递”和采血预约真实空态、电子锦旗和健康表扬信安全页面、共享页面工厂构建校验、统一当前就诊人上下文、“我的问诊”患者作用域、患者切换链路、今日预约摘要边界和成功请求低敏 requestId 观测，
并保留认证命令会话代际边界，
...
本地尚未发布的页面候选运行输入为 `dee4803fb94ad50c59c9ef8fda996bc0f37427c6`；它只用于本地回归和后续真机验收准备，不能与线上历史 `13f597e` 的运行包证据混写。当前候选已完成 staging 构建和 `runtime:verify:pending`，原子替换因 `dist` 锁保留 pending，真机验收仍需先释放 `dist` 锁，再按独立手册执行。
...
全量小程序测试为 `307 pass / 0 fail / 3511 expect()`，当前 pending 运行输入为 `dee4803fb94ad50c59c9ef8fda996bc0f37427c6`，登录后患者初始化边界见
