...
当前线上 API release 为 `3ab0a6c`，并已应用 `0016_patient_directory_sync_owner_index`。该版本已经完成生产 env preflight、原子切换、MySQL/Redis/schema 探针、公网
`/api/v2` 健康检查、6/6 readiness、未登录认证边界和旧 Python 服务共存验收；此前 release 的真实微信登录、单患者同步、预约科室和排班读取证据仍只能作为历史证据，不能直接当作 `3ab0a6c` 的真机验收。完整发布边界见
[`release/3ab0a6c-production-acceptance-2026-08-17.md`](release/3ab0a6c-production-acceptance-2026-08-17.md)。

### 当前 release 切换后的业务边界（2026-08-17）

`3ab0a6c` 只增加患者目录空快照的 fail-closed 保护：已有 HIS 就诊人时，外部目录“完成但为空”会返回
