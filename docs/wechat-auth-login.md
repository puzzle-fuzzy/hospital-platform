> 当前事实（2026-08-26）：线上服务端 release 为 `8eb51b5ffe85b0b8f8a032783f893117d3df549d`；线上历史小程序运行包来源为 `13f597ea9ee3f65b9be858117826d948339d904a`，当前本地 pending 运行输入为 `896a83cfb9d8b4350664cfe97f8bee643cbca434`。pending 尚未发布，真机业务三层证据仍待。

> 当前候选已继续推进到 `896a83cf`：首页二维码入口已迁移为安全展示壳，旧端第三方二维码因协议和隐私边界不完整保持关闭；旧 Python 服务和线上服务均未修改。详见 [`migration/patient-qr-safe-shell-audit-2026-08-26.md`](migration/patient-qr-safe-shell-audit-2026-08-26.md)。
> 本段优先于本文下方旧日期、旧 release 或旧运行包叙述；旧值只作为历史记录，不作为当前验收入口。
# 微信授权登录实施与验收手册
> 当前验收配套基线：线上服务端 release 为 `8eb51b5ffe85b0b8f8a032783f893117d3df549d`；线上历史小程序运行包来源为 `13f597ea9ee3f65b9be858117826d948339d904a`，当前本地 pending 运行输入为 `896a83cfb9d8b4350664cfe97f8bee643cbca434`。pending 尚未发布，真实微信业务三层证据仍待。
...
当前本地 pending 运行输入为 `896a83cf`，完整指纹为
`896a83cfb9d8b4350664cfe97f8bee643cbca434`；线上历史小程序包为 `13f597e`。本候选包含运行包 test/spec 文件边界、迁移入口覆盖展示、首页二维码安全展示壳和成功请求低敏 requestId 观测，
并保留认证命令会话代际边界，
...
本地尚未发布的页面候选运行输入为 `896a83cfb9d8b4350664cfe97f8bee643cbca434`；它只用于本地回归和后续真机验收准备，不能与线上历史 `13f597e` 的运行包证据混写。当前候选已完成 staging 构建和 `runtime:verify:pending`，原子替换因 `dist` 锁保留 pending，真机验收仍需先释放 `dist` 锁，再按独立手册执行。
...
全量小程序测试为 `288 pass / 0 fail / 3229 expect()`，当前 pending 运行输入为 `896a83cfb9d8b4350664cfe97f8bee643cbca434`，登录后患者初始化边界见
