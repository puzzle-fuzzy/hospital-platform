...
当前页面真机验收应使用已原子发布的 live 候选 `34f0fd21`；必须从当前 live `dist` 重新生成二维码，不能把历史 `1691f2d`、`02dbf10` 或旧二维码的页面、日志证据混写。
