> 当前事实（2026-08-26）：线上服务端 release 为 `8eb51b5ffe85b0b8f8a032783f893117d3df549d`；线上历史小程序运行包来源为 `13f597ea9ee3f65b9be858117826d948339d904a`，当前本地 pending 运行输入为 `3b42b867ae19f6dd23bacd88648d1f5917dabf26`，共 21 个页面，回归为 `293 pass / 0 fail / 3237 expect()`。pending 尚未发布，真机业务三层证据仍待。
...
# 微信授权登录实施与验收手册
> 当前验收配套基线：线上服务端 release 为 `8eb51b5ffe85b0b8f8a032783f893117d3df549d`；线上历史小程序运行包来源为 `13f597ea9ee3f65b9be858117826d948339d904a`，当前本地 pending 运行输入为 `25cd7c4a323c9767b9367d319be14a82f681b37b`（功能提交 `b4120bac`），共 25 个页面。pending 尚未发布，真实微信业务三层证据仍待；四个临床页面当前只处于 `surface-only`。
...
当前本地 pending 运行输入为 `b4120bac`，完整指纹为
`25cd7c4a323c9767b9367d319be14a82f681b37b`；线上历史小程序包为 `13f597e`。本候选包含运行包 test/spec 文件边界、迁移入口覆盖展示、四个临床页面外壳、共享页面工厂构建校验、协议静态页契约关联修正和成功请求低敏 requestId 观测，
并保留认证命令会话代际边界，
...
本地尚未发布的页面候选运行输入为 `25cd7c4a323c9767b9367d319be14a82f681b37b`；它只用于本地回归和后续真机验收准备，不能与线上历史 `13f597e` 的运行包证据混写。当前候选已完成 staging 构建和 `runtime:verify:pending`，原子替换因 `dist` 锁保留 pending，真机验收仍需先释放 `dist` 锁，再按独立手册执行。
...
全量小程序测试为 `293 pass / 0 fail / 3237 expect()`，当前 pending 运行输入为 `3b42b867ae19f6dd23bacd88648d1f5917dabf26`，登录后患者初始化边界见
