...
本地尚未发布的页面候选运行输入为 `ed20c525de0f0ae0ed3b047b95b7365b39c4dec9`；它只用于本地回归和后续真机验收准备，不能与线上历史 `13f597e` 的运行包证据混写。当前候选已完成 staging 构建和 `runtime:verify:pending`，原子替换因 `dist` 锁保留 pending，真机验收仍需先释放 `dist` 锁，再按独立手册执行。
...
全量小程序测试为 `299 pass / 0 fail / 3407 expect()`，当前 pending 运行输入为 `ed20c525de0f0ae0ed3b047b95b7365b39c4dec9`，登录后患者初始化边界见
