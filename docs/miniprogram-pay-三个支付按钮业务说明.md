...
> **历史文档（2026-09-17）**：`miniprogram-pay` 已停止使用。本文件仅用于追溯当时的测试端流程，不得作为现行支付开发、验收或发布依据；现行边界见 [`迁移/miniprogram-pay退役说明.md`](迁移/miniprogram-pay退役说明.md)。
...
页面加载后按以下顺序准备固定预约条件。日期不取当天，先请求后天；如果后天没有上午可约排班，再请求大后天：
...
页面展示的排班和号源是进入页面时的观察快照。用户真正点击任一支付按钮时，会再次读取后天/大后天排班和
当前可用号源，再调用预约占位；“取消后重新挂号”在取消成功后也会重新读取号源。这样既保持固定业务条件，
又不会把多人同时测试时已经变化的旧序号直接提交给服务端。服务端仍会在占位接口内再次校验，最终以服务端
实时结果为准。
...
用户明确取消授权或退出微信支付时，页面调用统一的支付退出接口；服务端确认支付未成功后作废订单并取消预约、
释放号源。只有退出处理成功后才清除待支付上下文：

```text
POST /payments/appointments/{appointmentId}/payment-exit
  → 查询微信/医保支付状态
  → 未支付：关闭微信单（如有）并作废平台订单
  → 取消预约并释放号源
  → 清除 pendingPayment
```
...
用户需要回到原支付方式继续确认，不能用另一种支付方式覆盖当前订单。
...
4. 服务端校验预约归属和订单状态，然后调用众阳取消预约 `2.10.4.3`：
...
取消失败时，页面保留原预约信息，不会误显示成“没有重复预约”。普通预约取消接口仍会拒绝存在活动支付流水的预约；
支付退出接口只有在支付结果明确未成功且关联订单已经失效后，才会进入预约取消。
