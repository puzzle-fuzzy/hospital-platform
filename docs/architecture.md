...
`0010_health_knowledge` 初始 schema、`0011_health_knowledge_versioned_keys` 升级和 MySQL fail-closed repository；尚未把旧内容直接复制到新库，
...
支付状态表的查询和迁移函数在运行时也必须重新校验状态值；未知状态没有合法后继，
只能返回 `false`/空集合或进入统一领域错误，不能因为 TypeScript 类型声明而触发
`undefined.includes`，更不能被解释成支付成功、失败或已完成。支付状态机本身不代表
真实医保/微信支付已经接通，正式 Provider contract 和查单证据完成前仍保持关闭。
