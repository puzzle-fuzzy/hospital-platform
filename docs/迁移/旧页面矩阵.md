...
> 当前事实（2026-09-16）：旧端来源为 `/Users/yxswy/Documents/GitHub/hospital/hospital-app/src`，共扫描 64 个 Vue 页面；新端 `app.json` 已注册 47 个 TypeScript 页面，当前源码运行输入的最近一次相关提交为 `47574b9acbe79203d23a7acbfe034ffd5fee3c31`。状态分布为 `partial=34`、`replaced=10`、`surface-only=18`、`blocked-provider=0`、`blocked-external=1`、`excluded=1`；使用条款已新增原文只读页，采血预约已补齐旧端真实空态；`feature-status` 仍是迁移期间的统一状态页，健康百科三个页面和协议页都不代表真实业务 contract 已全部完成。
