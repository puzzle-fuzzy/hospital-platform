...
> 当前事实（2026-09-16）：旧端来源为 `/Users/yxswy/Documents/GitHub/hospital/hospital-app/src`，共扫描 64 个 Vue 页面；新端 `app.json` 已注册 47 个 TypeScript 页面，当前源码运行输入的最近一次相关提交为 `07ab94c3c3cb9979313b32c50b695943d028e841`。状态分布为 `partial=35`、`replaced=10`、`surface-only=17`、`blocked-provider=0`、`blocked-external=1`、`excluded=1`；住院信息已形成独立只读候选链路但仍待 Provider/真机验收；使用条款已新增原文只读页，采血预约已补齐旧端真实空态；`feature-status` 仍是迁移期间的统一状态页，健康百科三个页面和协议页都不代表真实业务 contract 已全部完成。
> 本矩阵用于防止页面遗漏，不把“有旧代码”或“新端有占位入口”当作业务完成证据。
>
> `pnpm migration:audit` 需要通过 `LEGACY_HOSPITAL_ROOT` 显式提供旧仓库，并把 `src/pages`、`src/pagesB` 下的实际 `.vue`
> 文件与本矩阵逐项比对；没有旧仓库、路径不可读或事实文件缺失时会失败，不把跳过当作迁移完成。
>
> 机器可校验的逐页落点同时维护在
...
| `pagesB/patient/` | `agreement.vue`、`doctor.vue`、`express.vue`、`patient_signature.vue`、`patientAdd.vue`、`patientChange.vue` | `agreement` 已迁移原文只读页，`patientChange` 已被安全的患者选择页替换；`express` 和 `patient_signature` 已完成安全子集；`doctor` 已由“我的医生”列表/详情和未来排班页替换 | 协议版本/同意/撤回/审计、新增/绑定、签名真实会话和地址仍需分别确认 owner、授权与审计规则；我的医生已使用新 `/my/doctors*` 关系模型，不复用旧客户端快照；患者绑定见 [`patient-binding-contract-draft.md`](患者绑定契约草案.md)，绑卡/协议/签名总边界见 [`patient-center-and-external-entry-boundaries.md`](患者中心与外部入口边界.md) |
...
| `pagesB/hospital/` | `confirm_registration.vue`、`registration.vue`、`registration_detail.vue` | 部分迁移 | 新端确认页已接入服务端占位/预约写入，详情页支持平台预约取消；支付、最终 Provider 状态、医保和 HIS 回写仍由独立支付项目或后续 contract 承接 |
| `pagesB/hospital/` | `department_select.vue`、`doctor_card.vue`、`timeslot_source.vue` | 部分迁移 | 新端已覆盖科室/排班、医生详情、未来七天排班和分时段号源；号源进入确认页后由服务端重新核验并执行占位/预约写入，Provider 原始号源 ID 和费用仍不出客户端 |
...
| `pagesB/health/` | `outpatient_pay.vue` | 部分迁移 | 新端已接入门诊费用只读目录；点击记录进入 owner/patient 作用域的已核对摘要详情；支付、医保授权、结算回写和退费未开放 |
| `pagesB/health/` | `outpatient_pay_detail.vue` | 部分迁移 | 已接入 `GET /api/v2/payments/outpatient/records/{recordId}` 的摘要详情；项目级费用明细、医保分摊和电子票据仍需正式 Provider contract |
| `pagesB/health/` | `electronic_bill.vue` | 待 provider contract | 需电子票据资源、账单归属、短期授权和下载边界 |
...
| `pagesB/health/` | `electronic_consultation.vue` | 旧服务兼容版 partial | 已读取预约历史并筛选近 30 天，复用预约摘要保持旧端连续性；不代表独立电子导诊 Provider contract，实时导诊/执行状态仍关闭 |
...
| `pagesB/health/` | `admission_preconsultation.vue`、`pre_visit.vue` | 待 provider contract | 旧端分别走本地 JSON 问卷和 `saveBeforeVisitRecord` provider 写入；必须绑定问卷版本、预约/住院任务、患者授权、幂等、医护读取和撤回规则；详见 [`convenience-service-boundaries.md`](便民服务边界.md) |
...
| `pagesB/health/` | `bmi_calc.vue`、`blood_pressure_calc.vue` | 待临床审核 | 虽是本地计算，但旧端 BMI 分类与参考表存在版本差异、血压页仍写 1998 年标准且缺少完整输入边界；迁移前必须确认阈值、适用人群、规则版本和免责声明，详见 [`health-calculator-contract-draft.md`](健康计算器契约草案.md) |
...
| `pagesB/user/` | `edit_profile.vue` | 普通资料子集已迁移 | 原生 `pages/profile` 已迁移昵称、性别、年龄、邮箱并使用版本并发；头像、实名、微信身份和患者绑定仍关闭；详见 [`user-profile-contract.md`](用户资料契约.md) |
...
| `pagesB/user/` | `my_consultation.vue` | 部分迁移 | 新端已提供当前就诊人的过去 120 天历史摘要和患者切换；旧端演示数据不复制，外部问诊会话、正文、附件和实时能力仍待 contract/验收；账单/病历/住院预约/就诊码按钮原本只是 Toast |
