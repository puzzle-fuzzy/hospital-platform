...
| `pagesB/hospital/` | `department_select.vue`、`doctor_card.vue`、`timeslot_source.vue` | 部分迁移 | 新端已覆盖科室/排班、医生详情、未来七天排班和分时段号源只读；号源进入确认信息页，写入前的锁号、费用和预约执行仍待 appointment-write contract |
