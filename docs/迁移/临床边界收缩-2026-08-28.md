...
- 因此，门诊病历不能用预约、报告或费用读模型拼出一个“看起来可用”的页面；“我的问诊”也不能把预约历史冒充为旧外部问诊事实。本轮新增的页面只允许作为显式标注的兼容历史摘要，不改变外部问诊会话仍未迁移的结论。

旧端源码的具体证据和指纹见：

- [`clinical-readonly-source-audit-2026-08-26.md`](临床只读来源审计-2026-08-26.md)
- [`medical-record-directory-contract-draft.md`](病案目录契约草案.md)
- [`consult-and-internet-hospital-boundary-audit-2026-08-25.md`](问诊与互联网医院边界审计-2026-08-25.md)
...
“我的问诊”仍需要独立的外部入口 contract，包括外部主体、受众、短期会话、回跳、退出、撤销、内容保留和失败回退；当前兼容页可以读取受限的 `online` 历史摘要，但不能把该摘要直接宣称为外部问诊会话或替代独立 contract。
