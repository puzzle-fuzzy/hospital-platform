...
> [`公共API-v2.md`](../公共API-v2.md) 和源码 OpenAPI 门禁为准。
...
| `GET /msun-middle-business-amc-server/v1/schedulings/scheduling-doctors` | `api/modules/appointment.ts` | 代码已迁移（只读；待 provider contract） | 新端 `/api/v2/appointments/doctor-schedules` 固定渠道 4，仅展开医生卡和嵌套排班的受控字段；医生电话、医保编码等不公开，缺少 `scheduleStatus` 时保留 `unknown`。 |
...
| `GET /msun-middle-aggregate-clinic/v1/out-emrs` | `api/modules/medicalRecord.ts` | 待 provider contract | 门诊电子病历目录需要单独确认患者归属、就诊记录 ID、分页/排序和可公开字段，不能由预约历史或报告目录代替；边界审计见 [`medical-record-and-hospital-boundary.md`](病案与医院边界.md)。 |
...
| `POST /common/yunhealth/registration/plugin-settlement-complete` | `api/modules/payment.ts`、旧 Python `module_common/yunhealth_settle` | 最后处理 | HIS 收款结果回写后才能调用；必须绑定平台订单、微信支付终态、幂等键和最终结算审计，不能作为小程序“支付成功”回调。 |
| `POST /common/yunhealth/registration/plugin-payment-order` | `api/modules/payment.ts`、旧 Python `module_common/yunhealth_settle` | 最后处理 | 旧端用于创建医院微信 JSAPI 预支付单；金额、HIS 收款流水、openid 和患者医保字段必须由服务端从已落库订单及当前会话编排，不能把旧请求体直接暴露给小程序。 |
...
| `POST /common/yunhealth/registration/plugin-refund` | `api/modules/payment.ts`、旧 Python `module_common/yunhealth_settle` | 最后处理 | HIS 收款的混合支付退款会分别触发医保 6203、微信退款和 HIS 回写；外部退款不是本地事务，必须绑定原订单、退款金额、退款类型、幂等键和补偿状态。 |
...
| `POST /msun-yb-app-miop/thirdPartPay/start` | `api/modules/payment.ts` | 最后处理 | 旧端注释明确该接口用于 HIS 收款结果登记/回写；请求体含证件、患者、医保人员、金额和结算标识，不能由小程序直接调用，也不能把它当作第二笔支付或虚拟成功。 |
...
| `GET /convenience/my-doctor/list` | `api/modules/user.ts` | 代码已具备，待实证 | 新端为 `GET /api/v2/my/doctors`；静态代码使用 Bearer owner 隔离并返回受控关系快照，但旧端筛选/分页、21 条旧关系是否导入、Provider/公网/真机仍待核对。 |
| `POST /convenience/my-doctor/create` | `api/modules/user.ts` | 代码已具备，待实证 | 新端为 `POST /api/v2/my/doctors`；静态代码只接受 `doctorId`，由排班目录确认医生资料，数据库唯一键和幂等请求防重复；旧存量和当前 Provider 目录尚未验收。 |
| `GET /convenience/my-doctor/delete?doctor_id=xxx` | `api/modules/user.ts` | 代码已具备，待实证 | 新端为 `DELETE /api/v2/my/doctors/{doctorId}`；静态代码改为明确的幂等删除命令，不复用旧端破坏性 GET，但尚缺 owner A/B、公网和真机证据。 |
...
| `PUT /system/user/current/info/update` | `api/modules/user.ts` | 普通资料子集已迁移 | 新端 `PUT /api/v2/me/profile` 只接收昵称、性别、年龄、邮箱和 version；实名资料、微信身份和患者字段仍关闭，不能接收旧端的 `openid`/`unionid`/身份证字段；详见 [`user-profile-contract.md`](用户资料契约.md)。 |
...
只有显式设置 `LEGACY_HOSPITAL_ROOT` 指向旧仓库时，`pnpm migration:audit` 才会核对各模块数量、
已挂载总数和这个未挂载文件。环境变量未设置、路径不可读或任一事实文件缺失时，审计必须失败；
不能把外部扫描跳过当成已验证。

旧小程序的非 HTTP 行为不在上述路由数量中，包括 `src/api/ws.ts` 的 WebSocket、`httpZy.ts` 的直连
provider、Pinia 患者状态和健康问卷组件；它们的迁移边界单独记录在
[`legacy-client-infrastructure-boundaries.md`](旧客户端基础设施边界.md)，不能因未出现在
...
| `/convenience` | 13 个表扬信、锦旗、风险评估、出院随访、入院预问诊的 list/create/submit/delete 路由（我的医生已拆入新公共模块） | 其余便民能力仍待各自 contract、内容审核和权限确认；我的医生由 `/api/v2/my/doctors*` 静态链路承接，但真实验收仍待完成 |
...
[`infrastructure-and-operations-boundaries.md`](基础设施与运维边界.md)。
...
| 预约目录 | `GET /api/v2/appointments/departments`、`GET /api/v2/appointments/doctor-schedules`、`GET /api/v2/appointments/schedules` | 只读目录、按医生/按日期排班已实现代码；Provider/真机验收以及号源锁定、预约写入、取消和挂号费仍关闭 |
...
| 门诊费用目录 | `GET /api/v2/payments/outpatient/records`、`GET /api/v2/payments/outpatient/records/{recordId}` | 只读列表和已核对费用摘要详情已实现；项目级明细、支付、医保和 HIS 回写关闭 |
| 微信支付基础设施 | `/api/v2/payments/orders*`、`/api/v2/payments/wechat/notifications` | 代码和 fail-closed gate 已实现；不能据此宣称真实支付完成 |

新 API 的完整输入、输出、错误码和认证规则见 [`../公共API-v2.md`](../公共API-v2.md)。
