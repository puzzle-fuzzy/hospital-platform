...
| `GET /convenience/my-doctor/list` | `api/modules/user.ts` | 已迁移 | 新端为 `GET /api/v2/my/doctors`；使用 Bearer owner 隔离，返回服务端校验后的医生关系快照。 |
| `POST /convenience/my-doctor/create` | `api/modules/user.ts` | 已迁移 | 新端为 `POST /api/v2/my/doctors`；只接受 `doctorId`，由排班目录确认医生资料，数据库唯一键和幂等请求防重复。 |
| `GET /convenience/my-doctor/delete?doctor_id=xxx` | `api/modules/user.ts` | 已迁移 | 新端为 `DELETE /api/v2/my/doctors/{doctorId}`；已改为明确的幂等删除命令，不复用旧端破坏性 GET。 |
...
| `/convenience` | 13 个表扬信、锦旗、风险评估、出院随访、入院预问诊的 list/create/submit/delete 路由（我的医生已拆入新公共模块） | 其余便民能力仍待各自 contract、内容审核和权限确认；我的医生已由 `/api/v2/my/doctors*` 承接 |
