...
> 状态：`draft`，当前已注册平台候选路由和实名表单，但真实 Provider 写入仍由配置门禁关闭。
>
...
因此本草案继续保持 `draft`；候选的 `POST /api/v2/patient-binding/*` 仍不注册。当前代码另有平台路由
`POST /api/v1/patients/bind`（公网由 `/api/v2` 转发），但只有独立配置、旧服务用户认证和 Provider 真实验收全部满足时才注入真实
gateway；默认/不完整配置由 `DependencyNotConfiguredError` 保持关闭。不能把“有科室/院内用户文档”
误判成“患者绑定 Provider contract 已到达”，也不能根据旧端 `patInfosFind` 的一次成功响应打开写入。

## 0.3 2026-09-05 众阳官方患者档案文档核对

本轮通过众阳官方开放平台文档页核对了 2.1.51 创建档案、2.1.52 绑卡、2.1.53 档案查询和 2.1.55 卡类型字典。
脱敏核对记录见 [`2026-09-05-众阳患者档案绑定官方文档核对.md`](../提供商接入/2026-09-05-众阳患者档案绑定官方文档核对.md)。
这次核对把以下字段从“推测”提升为“文档已确认”：

- `patInfosFind` 的 `type=2` 使用姓名、证件类型、证件号，并要求 `org-id`；
- `patients` 的 `sex`、`idCardType`、`cardType`、`hospitalId`、`orgId` 是数值语义；
- `patCards` 使用数值 `patId` 和真实 `cardNo`；
- `cardType` 必须来自当前机构卡类型字典，不能在代码中固定为经验值。

因此适配器已经移除“身份证号复制到 `cardNo`”的错误，并把机构、院区和卡类型改为独立配置；
但 2.1.55 的当前机构实际值、2.1.52 是否允许患者端自助调用、无档案/重复/超时/补偿样例仍未取得。
本轮仍不打开 `ZHONGYANG_PATIENT_BINDING_READY`，不注册真实绑定页面，不把文档页面的存在当成写入授权。
...
### 2.2 编辑能力结论（2026-09-16）

重新核对旧端后，`patientAdd.vue:258-264` 的编辑分支只有“修改就诊人信息”的标题设置，回填代码仍是 TODO；旧
`ZY.ts:17-75` 只声明 `patInfosFind`、`patients` 和 `patCards`，没有患者资料更新接口。旧
`user.ts:113-135` 的 `PUT /system/user/current/info/update` 是普通用户资料接口，不能据此推导医院患者档案更新能力。

因此 P1-03 已关闭为“无旧服务可迁移行为”：新端不新增患者编辑 API、不把普通 profile PUT 复用为患者编辑，后续若业务
确实需要修改患者资料，必须另取得院方/provider 的字段白名单、版本冲突、审计和 owner contract。
...
## 4. 候选 API 边界（不作为公网 contract）

以下 `patient-binding/*` 形态只是内部设计草案，不是已冻结的公共接口；当前实际接入的是受门禁保护的
`/patients/bind` 平台候选路由。
...
- 候选 `POST /api/v2/patient-binding/*` 不注册；现有 `POST /api/v1/patients/bind` 虽已注册，但仅在
  `ZHONGYANG_PATIENT_BINDING_READY`、患者目录、机构/院区/卡类型以及
  `LEGACY_PATIENT_AUTH_BASE_URL` 均满足时注入 gateway，默认和不完整配置仍拒绝写入；
- `pages/patient-select` 的“添加就诊人”只进入固定 `patient-binding` 表单；表单可以收集旧端同样的
  姓名、手机号、身份证和协议确认，但未完成真实 contract 时提交仍由服务端保持关闭；
