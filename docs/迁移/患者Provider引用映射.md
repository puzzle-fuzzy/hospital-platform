...
``patient-archive-identity-correlation-2026-08-19.md``。
...
- 目录 `thirdPatientId` 按旧端类型允许在 `Number.isSafeInteger` 范围内无损字符串化；档案 `patInfosFind.data.patId` 则严格只接受字符串。即使数字形式的 `patId` 没有超出 JavaScript 安全整数范围，也属于 Provider schema 错误，adapter 必须拒绝整次同步，不能把错误类型伪装成有效的 `his-patient` 映射。实现记录见 ``../release/patient-archive-patid-string-contract-2026-08-20.md``。
