...
旧通用页面同时承载的 `path=outpatient-guide`、患者绑定/解绑完整 URL 和 `/system/auth/ticket` 不随本次恢复。智能导诊仍等待独立的固定入口确认；患者绑定/解绑需要患者归属、短期会话、回跳和最终结果 contract；支付收银台需要订单 owner、支付回调、动态地址 allowlist、查单和失败回退 contract。新端不接受任意 URL，也不把平台 token 交给智能客服网页。微信真机使用前还必须将 `html.ydrj.top` 配置为小程序业务域名。
...
本轮结论：互联网医院、智能客服和医保电子凭证均已按旧端确认的固定外部目标恢复；它们不共享 URL、AppID、票据或业务参数。协议同意、订阅保存、患者签名、医生关系、智能导诊和医保支付结算仍按各自业务实现，不能因为外部入口已能打开就合并为同一链路。该结论与本文件的迁移矩阵和 `pnpm migration:audit` 门禁保持一致。
