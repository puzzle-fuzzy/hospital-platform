...
[`patient-binding-contract-draft.md`](患者绑定契约草案.md)。本文负责总边界，
...
| `pagesB/user/my_consultation.vue` | 查询陪诊历史；账单、病历、住院预约、就诊码按钮均只弹 Toast | `GET /intelligent/treatment_companion/history`；旧代码另有直连队列位置接口 | 已提供显式标注的当前患者过去 120 天兼容历史摘要和患者切换；不复制旧演示数据，独立 AI/陪诊会话、正文、附件、实时队列和二维码 contract 仍未迁移 |
...
[`patient-binding-contract-draft.md`](患者绑定契约草案.md)。这里保留安全原则：
...
小程序只筛选服务端预约历史读模型返回的 `missed` 状态，使用中国标准时间过去 90 天窗口，并支持重新选择就诊人；“我的挂号”则使用当前日前后各三个月，不能从空列表推导“没有爽约”，
...
## 7. 2026-08-17 旧页面行为复核补充

本节记录本轮对旧端源码的定向复核结果。复核目标不是判断页面“能不能打开”，而是判断页面是否产生了可被新端承诺的业务事实。以下行为均来自旧端源码，不能作为新端接口或数据模型的设计依据。

### 7.1 协议页：有展示和跳转，没有同意事实

旧 `pagesB/patient/agreement.vue` 的协议正文是静态模板，底部“不同意”只弹窗后返回，“同意”只显示成功 Toast，延迟跳转首页，没有提交协议版本、用户主体、用途、患者关系、客户端来源或同意时间的请求。

因此新端可以在未来复用“协议正文展示”这一视觉能力，但不能把旧页面迁移为真实授权页，也不能因为用户点击过按钮就认为患者绑定、签名、支付或报告查看已获得法律/业务授权。正式开放前必须先确定：

- 法律文本的唯一版本号、发布日期、生效/失效时间和适用业务；
- 同意、拒绝、撤回、重新同意的服务端事实模型和幂等规则；
- 用户主体与患者关系的绑定范围，以及未成年人/代理人场景；
- 任何需要同意的业务在缺失或版本过期时的明确拒绝码。

### 7.2 订阅消息页：所有开关均为本地状态

旧 `pagesB/user/subscription_message.vue` 将提醒开关写在页面内存中，搜索和折叠也只是本地状态；“确定修改”直接显示“设置已保存”并返回，没有调用 `wx.requestSubscribeMessage`，没有模板 ID、业务事件、授权结果、服务端保存或撤回逻辑。

这意味着该页面不能迁移为“消息订阅管理”业务。新端在没有模板清单和业务事件契约时，必须使用建设中/暂未开放状态，不能显示成功保存，也不能把开关值写入用户或患者订阅表。待资料齐全后，至少要分别建模微信授权结果、用户选择、患者/家庭成员作用域、发送结果、失败重试和撤回事实。

### 7.3 患者签名页：跨小程序参数不可信且缺少回执

旧 `pagesB/patient/patient_signature.vue` 从 `BOUND_PATIENTS` 本地缓存读取患者列表；没有服务端 owner 校验，缺少真实患者时还使用本地示例患者。点击后通过 `navigateToMiniProgram` 向固定 AppID `wx0b76c9904392518f` 传递 `patientId` 和姓名，成功回调为空，失败只在前端弹 Toast，也没有签名结果回跳或服务端确认。

该页面不能作为新端患者签名的兼容实现。必须先取得目标小程序的正式协议，并由服务端生成限定受众、短期、一次性的 `signatureSessionRef`；目标小程序只能接收引用，回跳结果必须由服务端验签并落库。未满足这些条件前，入口保持迁移提示，严禁透传内部患者 ID、provider 患者号或姓名。

### 7.4 我的医生：服务端受控静态链路已具备，真实验收未完成

旧 `pagesB/patient/doctor.vue` 调用旧的 `getMyDoctorListApi`，将返回字段映射为展示模型；缺少医生 ID 时使用数组索引作为 ID，头像还会经过旧的 `proxyForward`，点击后拼接旧医生详情路径。数组索引不能作为跨请求稳定标识，旧接口字段也没有证明 owner、医生关系状态和详情权限。

新端源码已具备 `我的医生` 列表、医生名片、关注/取消关注、未来七天排班和分时段入口：关注请求只提交 opaque `doctorId`，服务端从当前排班目录确认姓名、职称、简介、擅长、科室位置和头像后写入 owner-scoped 关系表，并由唯一键保证关系幂等；小程序不接收旧 `user_id`、provider 医生号或客户端伪造的医生资料。以上是静态代码结论，不是 Provider 或真机完成结论。旧库中的 21 条快照关系没有可验证的旧 openid 到新 owner 映射，因此当前不自动导入关系表；如需保留历史关注，必须另行执行受控 owner 映射、当前医生目录复核、失效规则和回滚演练。

### 7.5 WebView：两条旧链路都不能直接复制

旧 `pages/hospital/hospital.vue` 直接打开硬编码的互联网医院 URL；旧 `pagesB/health/webview.vue` 同时支持路由传入完整 URL，并通过旧 `/system/auth/ticket` 生成票据后拼到 URL。后者会把任意 URL、旧票据消费方和外部页面身份混在一起，前者则没有新端 audience、登录态隔离和失败回退证明。

新端必须按外部服务拆分 `resourceKey`、固定 origin/path allowlist、受众、最小权限短期会话、一次性消费、回调和审计事件。未完成 contract diff 前，不注册可传任意 URL 的页面，也不复用旧 token exchange。

#### 7.5.1 用户确认的互联网医院固定 H5 入口（2026-09-03）

用户确认互联网医院底部 Tab 在旧小程序中的实际行为是直接展示网页，因此新端恢复为唯一固定的 `<web-view>` 入口：

`https://cx.o2o.bailingjk.net/wechat/#/bluser/userCard/index?publicNoCode=gzh-048400_0001`

该入口只在 `pages/hospital/hospital` 中存在，不接受路由传入完整 URL，不调用旧 `/system/auth/ticket`，不向外部页面拼接平台 token；切回 Tab 时仅追加时间戳以重新加载旧端页面。它是经源码核对后的固定兼容入口，不等于通用 WebView contract 或互联网医院服务端迁移完成。微信真机使用前还必须将 `cx.o2o.bailingjk.net` 配置为小程序业务域名。

#### 7.5.2 用户确认的智能客服默认 H5 入口（2026-09-03）

旧 `pagesB/health/webview.vue` 的默认智能客服入口来自旧端 `env/.env` 的 `VITE_AI_WEBVIEW=https://html.ydrj.top`；新端已将“我的 → 智能客服”和首页悬浮客服入口收敛到固定 `https://html.ydrj.top` WebView，并仅追加时间戳刷新。

旧通用页面同时承载的患者绑定/解绑完整 URL 和 `/system/auth/ticket` 不随本次恢复。`path=outpatient-guide` 已由原生智能导诊页替换，使用固定平台文字/音频接口和 owner-scoped 会话引用，不再向 H5 传 ticket 或 token；患者绑定/解绑需要患者归属、短期会话、回跳和最终结果 contract；支付收银台需要订单 owner、支付回调、动态地址 allowlist、查单和失败回退 contract。新端不接受任意 URL，也不把平台 token 交给智能客服网页。`html.ydrj.top` 仅剩智能客服 WebView 需要配置为小程序业务域名。

#### 7.5.3 用户确认的医保电子凭证固定小程序入口（2026-09-03）

旧端 `src/pages/user/user.vue` 中“医保电子凭证”不是通用 WebView，也不是医保支付页，实际行为是使用固定 AppID `wx81ce904580cc0ff1`、空 `path` 和空 `extraData` 调用 `navigateToMiniProgram`。新端已在 `apps/miniprogram/src/services/insurance-voucher-navigation.ts` 收敛该跳转，并在 `app.json` 登记目标白名单；不会把患者、订单或平台会话参数交给目标小程序。

旧端 manifest 目前只登记了医保支付授权流程使用的 `wxe183cd55df4b4369`，没有登记菜单实际使用的 `wx81ce904580cc0ff1`，这是旧端构建配置与菜单代码之间的差异。新端按菜单代码的实际目标补齐白名单；两个 AppID 分属“电子凭证入口”和“医保支付授权”两条链路，不能互相替换。

本轮结论：互联网医院、智能客服和医保电子凭证均已按旧端确认的固定外部目标恢复；智能导诊则已改为独立原生文字/语音链路。它们不共享 URL、AppID、票据或业务参数。协议同意、订阅保存、患者签名、医生关系和医保支付结算仍按各自业务实现，不能因为外部入口或导诊页面已能打开就合并为同一链路。该结论与本文件的迁移矩阵和 `pnpm migration:audit` 门禁保持一致。
