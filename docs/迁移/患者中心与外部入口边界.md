...
新端已完成 `我的医生` 列表、医生名片、关注/取消关注、未来七天排班和分时段入口：关注请求只提交 opaque `doctorId`，服务端从当前排班目录确认姓名、职称、简介、擅长、科室位置和头像后写入 owner-scoped 关系表，并由唯一键保证关系幂等；小程序不接收旧 `user_id`、provider 医生号或客户端伪造的医生资料。旧库中的 21 条快照关系没有可验证的旧 openid 到新 owner 映射，因此不自动导入当前关系表；如需保留历史关注，必须另行执行受控 owner 映射和当前医生目录复核。
