...
[`legacy-api-endpoint-inventory.md`](旧接口清单.md)。本矩阵负责按业务域
...
当前新 Elysia 已实际注册的患者端公共接口，以 [`docs/公共API-v2.md`](../公共API-v2.md) 和
...
> `ce1c217`，完整 sourceRevision 为 `ce1c2179b57fe2783066b51f8621220224982928`，已通过
...
| 旧端 `pagesB/health/electronic_record.vue` / `POST /msun-middle-aggregate-clinic/v1/out-visit-records` | `GET /api/v1/medical-records` | 服务端按 owner 解析内部 `patientId` 到 `his-patient`，固定近 30 天和 `type=5`，只返回白名单后的门诊就诊摘要 | contract/domain/adapter/API/原生页面已接回；`ZHONGYANG_MEDICAL_RECORDS_READY` 默认关闭，病历正文、附件、住院病历、真实 Provider、公网和真机验收仍未完成 |
...
[`legacy-api-endpoint-inventory.md`](旧接口清单.md) 的“旧 FastAPI 路由基线”小节。
...
| `/convenience/*` | 锦旗、表扬信、风险评估、我的医生、出院随访、入院预问诊 | 未注册 | 旧端共 13 个路由；按 [`convenience-service-boundaries.md`](便民服务边界.md) 拆为反馈、临床问卷、医生关系和预问诊四个领域；必须先完成 owner、版本、幂等、审核和医护侧权限 contract |
...
字段、状态、金额和依赖矩阵见 [`../提供商接入/2026-08-16-预约挂号支付退款.md`](../提供商接入/2026-08-16-预约挂号支付退款.md)。
