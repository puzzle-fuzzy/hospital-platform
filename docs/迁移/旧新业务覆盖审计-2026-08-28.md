...
| 我的医生 | `api/modules/user.ts` 的 `/convenience/my-doctor/list/create/delete` | `pages/my-doctor/my-doctor`、`pages/my-doctor-detail/my-doctor-detail` | 新端已具备 `/api/v2/my/doctors*` 的 owner-scoped 列表、详情、关注/取消关注和未来七天排班静态链路；关注时由排班目录确认医生展示字段，关系表有唯一约束 | 21 条旧关系导入决定、目录失效规则、旧 channel=4 与新 adapter channel=3 的 Provider 对照，以及公网/真机证据；不复用旧客户端医生快照 |
| 智能导诊 | 旧端 `webview?path=outpatient-guide`，文字 `/chat_text`、语音 `/chat_audio` | `pages/smart-guide/smart-guide`；`POST /api/v2/intelligent-guide/messages`、`audio` | 原生文字/语音、多轮续接、免责声明、科室推荐和挂号跳转已实现；旧 JWT 和 Provider 会话号留在服务端，原始症状/音频不写日志 | 验证旧模型、知识版本、语音识别、真实账号、公网和真机链路 |
| 电子导诊/智能陪诊/我的问诊 | 旧端电子导诊、陪诊和问诊模块，部分入口依赖实时队列或外部会话 | `pages/electronic-consultation/electronic-consultation`、`pages/consult/consult` 或统一状态页 | 预约摘要和智能导诊会话都不等于电子导诊单、陪诊队列或问诊历史；这些真实 API 仍未注册 | 取得专用来源、受众、保留周期、退出和回跳 contract |
