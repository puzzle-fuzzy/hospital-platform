...
命令输出中的 `featureIntakeRows` 会把 22 个已暴露入口逐条展开，用于材料到达后的逐入口核对；
它只生成审计摘要，不会改变入口状态。
...
| --- | ---: | --- | --- | --- |
| C：临床只读契约 | 3 | `awaiting-formal-contract` | Provider/HIS 业务责任人 | 三条临床线分别提供版本、请求、响应、错误、患者映射和字段白名单 |
...
> 因此不计入当前 11 个冻结入口和 22 个 `featureIntakeRows`。它只能在后续出现真实入口并补齐
...
- 22 个已暴露 FeatureKey 全部覆盖且不能重复归属；
...
因此“22 个已暴露入口全部列出”只代表迁移任务可执行和可追踪，不代表 22 个入口已经开放；
