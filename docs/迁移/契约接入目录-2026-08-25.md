...
命令输出中的 `featureIntakeRows` 会把 23 个已暴露入口逐条展开，用于材料到达后的逐入口核对；
它只生成审计摘要，不会改变入口状态。
...
| D：患者与便民写入 | 11 | `awaiting-formal-contract` | 患者服务、临床内容、平台安全 | owner、版本化同意、幂等、最终查询、撤回、文件安全和医护读取规则 |
...
> D 的命令领域基础目前覆盖 12 个计划能力；其中 `patient-address` 尚未暴露旧页面或 action，
> 因此不计入当前 11 个冻结入口和 23 个 `featureIntakeRows`。它只能在后续出现真实入口并补齐
> owner、字段、版本和删除 contract 后，单独加入冻结准入目录。
...
- 23 个已暴露 FeatureKey 全部覆盖且不能重复归属；
- 每个批次必须有非空责任人、下一项材料说明，以及不重复的正式材料、实现顺序和未确认禁止项；
- 每个 FeatureKey 至少保留一个旧页面或 action 入口，禁止出现只有名称却无法追溯来源的准入条目；
...
因此“23 个已暴露入口全部列出”只代表迁移任务可执行和可追踪，不代表 23 个入口已经开放；
尚未暴露的计划能力也不能因为已有领域状态机就被误报为已迁移。
