> **当前候选同步（2026-09-06）**：服务端候选 commit `f3f2bc665cb0ed5d727936e7918b1dacbd2eccfc`，尚待 3090 发布验收；本地小程序当前源码来源同为 `f3f2bc665cb0ed5d727936e7918b1dacbd2eccfc`。本机旧 `8182a8774bf0d6ad6f62d928693add952ddff034` pending 与当前来源不一致，`runtime:verify:pending` 已拒绝，待开发者工具关闭后清理；历史段落只作追溯。

# 全项目迁移 readiness 报告说明

> 本文说明 `pnpm migration:readiness` 的数据来源和判定边界。报告用于广度迁移交接，不是上线批准单，也不替代 Provider、公网、真机或临床审核证据。

> **当前运行事实（2026-09-06）**：源码已注册 43 个页面、4 个原生 Tab；“我的医生”列表、医生名片、关注/取消关注和未来七天排班入口已接入新 Node API 与数据库关系表。当前候选源码已固定为 `f3f2bc665cb0ed5d727936e7918b1dacbd2eccfc`，需在发布前重新构建运行包并执行来源校验。支付医保、预约写入、实时就诊和真机证据仍按各自 contract/验收边界处理。
...
| `migrationBreadth` | `migration-breadth-audit.mjs` 的结果 | 入口广度审计是否已经纳入总结构准入；任一可见 action 缺少固定分发，或当前 42 个页面的 WXML 事件缺少 TS 方法/共享页面工厂注册时，`structuralAuditPassed` 直接为 `false` | 不代表入口背后的 Provider、临床或支付业务已经完成 |
...
- 旧页面共 64 个，其中 `replaced=9`、`partial=31`、`surface-only=21`、`blocked-provider=1`、`blocked-external=1`、`excluded=1`；“我的医生”已进入真实列表/名片/关注/排班链路，其余支付、医保、预约写入、临床、患者写入和外部高风险能力仍按各自 contract 与证据门禁控制。
...
- 33 个冻结入口 gate 已由 `tools/migration-boundary-catalog.mjs` 统一登记，共覆盖 38 个旧页面入口和 12 个 action-only 引用（合计 50 个入口来源）；“我的医生”由新关系模型和受控排班目录直接承载，不再占用冻结 gate。其余 gate 仍各有独立 contract 家族、页面状态、材料和关闭能力；readiness 的 `entryCoverage.frozenBoundary.passed=true` 只表示入口边界没有漂移，不表示其它业务可以调用 Provider。
- gate 批次覆盖当前为 A `5/4/1`、B `0/0/0`、C `4/4/0`、D `11/22/1`、E `8/3/6`、F `7/7/4`（依次为 gate/旧页面/action-only）；B 的 0 是因为健康内容由独立审核 bundle 队列控制，不允许把内容发布误归入其它业务 gate。`consultation` 属于外部问诊会话，按 `external-session` 归入 E，不计入临床只读 C。
...
- 原生小程序注册 42 个页面，四个主入口继续使用微信原生 `tabBar`；已完成的“我的医生”不再计入 `surface-only` 页面。
