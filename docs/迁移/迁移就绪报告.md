...
> **当前运行事实（2026-09-03）**：源码已注册 42 个页面、4 个原生 Tab；“我的医生”列表、医生名片、关注/取消关注和未来七天排班入口已接入新 Node API 与数据库关系表。当前工作区仍有未提交改动，因此不能生成稳定 sourceRevision；需在提交后重新构建运行包并执行来源校验。支付医保、预约写入、实时就诊和真机证据仍按各自 contract/验收边界处理。
