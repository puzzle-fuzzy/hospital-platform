...
- 预约历史（我的挂号）：当前中国标准时间日前后各三个月，必须覆盖未来预约；月底按旧服务 JavaScript 日历月溢出语义计算；
- 爽约记录：当前中国标准时间日前 90 天到当前日，只用于从预约历史派生 `missed`；
- 报告目录：默认按旧端语义取当前中国标准时间日往前一个日历月到当前日；月底沿用 JavaScript 日历月溢出规则；用户仍可调整日期；
...
任何变更都必须同时更新 `docs/Provider契约-v1.md`、[`公共API-v2.md`](../公共API-v2.md)、对应 adapter 测试、
小程序日期窗口测试和公网/真机验收记录。
