...
> 最新复核日期：2026-09-15；下方 0.0–0.6 为历史审计快照，若与 0.8、公共 API 契约和测试冲突，以后者为准。
>
...
## 0.8 2026-09-15 门诊就诊摘要链路重新接回

本轮确认当前小程序仍把“门诊病历”导向未迁移状态页，而仓库已保留 0.7 阶段的
contract、domain、众阳 adapter 与默认关闭的配置闸门。按旧端
`electronic_record.vue` 的真实调用重新接回 `GET /api/v1/medical-records`、应用组合根、
客户端白名单校验和原生页面；页面查询当前 owner 明确选择的患者及近 30 天记录。

这次只恢复门诊就诊摘要，且 `ZHONGYANG_MEDICAL_RECORDS_READY=false` 仍是默认值。
`out-emrs` 病历正文、详情引用、附件、下载和住院病历继续未注册；没有执行真实 Provider、
公网、生产发布或微信开发者工具/真机验收。因此当前结论是“代码已接回、运行待构建、
真实业务未验收”，不能写成门诊病历已经上线。
...
截至本次复核，`docs/提供商接入/` 仍没有 `out-visit-records` 的正式确认包、患者映射说明、成功空目录/权限
...
当前 `docs/提供商接入/` 仍只有已登记的预约/支付/结算材料，没有新增 `out-visit-records` 的正式确认包、
...
2026-09-15 复核：上述“我的问诊”是 0.7 时期的历史实现记录，不属于本次门诊就诊摘要恢复范围，也不据此改写其当前应用入口或验收状态。2026-09-16 更新：首页和“我的”入口已接入独立的兼容历史摘要页；该页不改变本段对外部问诊会话、正文和附件仍未开放的边界，且不作为真实问诊业务验收证据。
...
文档接收后先登记 [`../Provider文档接入流程.md`](../Provider文档接入流程.md) 的 `documentId`、版本、环境和内容
...
- 文档记录已按 [`../Provider文档接入流程.md`](../Provider文档接入流程.md) 登记，包含版本、环境和内容指纹；
