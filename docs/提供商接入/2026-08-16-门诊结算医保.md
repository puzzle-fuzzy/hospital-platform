...
| 2.6.65.4 支付结果查询 | `POST /msun-middle-open-settlepay/api/v2/open/payment/pay-query` | 查询支付结果；外层 `code=0000` 表示接口调用成功，`data.code=3` 为支付成功、`data.code=5` 为支付失败可取消后重试、其他 `data.code` 均为支付中 | 重复查单/重试策略和最终权威来源仍需结合受控真实返回确认 |
...
| 2.6.65.6 取消结算 | `POST /msun-middle-open-settlepay/api/v2/open/settle/cancel-settle` | 用户主动取消、超时或异常时取消结算；`data.cancelStatus=1` 为取消成功，`data.cancelStatus=0` 表示没有待取消结算单；若存在支付流水，必须先关支付流水 | 取消是否允许覆盖各终态、医保撤销顺序和重复取消仍需结合受控真实返回确认 |
| 2.6.65.11 支付关单 | `POST /msun-middle-open-settlepay/api/v2/open/payment/pay-close` | 产生支付撤销/关单流程；外层 `success=true` 且 `data.revokePayRecords[*].status=3` 才视为撤销成功 | 已支付订单的真实撤销结果、退款边界和最终查单 |
...
`2.6.65.4`、`2.6.65.11` 和 `2.6.65.6`不是正常医保支付链路的固定连续步骤。正常支付应按本节前述顺序进入
6201、6202、6301、2.27.2.32 和 2.6.65.5；`.4/.11/.6`只用于已有 Provider 支付/结算需要安全关闭的恢复分支。

```text
支付中恢复或明确重新授权
  → 有完整 payingId/businessId/hospitalId 上下文
  → 2.6.65.4 pay-query（仅 payment_in_progress 标准分支）
  → 已支付或结果不确定：blocked/manual_review，不得关单重开
  → 可安全关闭：2.6.65.11 pay-close
  → 仅撤销明确成功（success=true 且 revokeStatus=3）
  → 2.6.65.6 cancel-settle
  → 仅 cancelStatus=1 或明确的 cancelStatus=0 才能允许重开
```

旧端门诊纯医保流程图中的“4.1 查询支付结果”没有直接写出 `.4`，只能结合本台账中的
`2.6.65.4 支付结果查询`材料进行接口映射；图中的`.11`明确属于写回异常后的恢复分支。当前 3090 发布版本对
`reauthorization`暂时跳过`.4`并记录 `medical-insurance.cancellation.2.6.65.4.skipped`，但`.11`仍是关单确认门槛，
`.11`未确认时不得调用`.6`或创建新的支付订单。该例外需要 Provider 成功/失败/已支付/处理中样例后再决定是否恢复
`.4`，不能把临时跳过当成标准流程。
...
| SET-09 | 正常支付与 `.4/.11/.6` 恢复分支的触发条件、状态矩阵和重开边界 | 同一订单的支付中、已支付、失败、处理中、关单成功/失败和重复恢复样例 |
