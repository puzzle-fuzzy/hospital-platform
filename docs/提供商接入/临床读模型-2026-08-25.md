...
> 2026-09-16 更新：准入材料总状态仍为 `normalized`，且各域 `contractStatus` 仍为 `pending`；门诊就诊摘要和住院 episode 已基于旧端明确请求事实登记为 `registered-fail-closed` 安全候选。这不代表 Provider 正式 contract 已确认或生产业务已开放。
> 当前状态：`normalized`（Provider contract 未确认；门诊摘要和住院摘要仅以默认关闭闸门登记，其他临床接口未注册，生产业务未开放）
> 结构化准入状态见 [`clinical-read-models-contract-gate.json`](临床读模型契约门禁.json)。该文件只记录各域证据材料是否到位，不是患者端 fixture，也不代表 API 已获得生产准入。
...
| --- | --- | --- | --- |
| 门诊就诊记录目录 | `POST /msun-middle-aggregate-clinic/v1/out-visit-records`；旧页面只展示摘要 | `registered-fail-closed`：新 API/小程序代码已接入，生产 gate 默认关闭 | provider 当前请求/响应/空/拒绝/超时样例和 `patId` 映射确认 |
...
| 住院信息/住院日费用 | `GET /msun-middle-aggregate-hsz/v1/patients`；费用使用独立 settlepay 接口 | `registered-fail-closed`：住院 episode 只读候选已接入，gate 默认关闭 | episode 权威来源、住院患者映射、金额单位和账单状态样例 |
