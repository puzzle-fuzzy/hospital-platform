...
> 2026-09-15 更新：准入材料总状态仍为 `normalized`，且各域 `contractStatus` 仍为 `pending`；但门诊就诊摘要已基于旧端明确请求事实实现 `registered-fail-closed` 安全子集。这不代表 Provider 正式 contract 已确认或生产业务已开放。
> 当前状态：`normalized`（Provider contract 未确认；门诊摘要仅以默认关闭闸门注册，其余临床接口未注册，生产业务未开放）
