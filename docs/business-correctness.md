...
> **当前本地源码与运行包（2026-08-27）**：当前业务代码和 live 运行输入均为 `6f47c6408fe5b62025bd74fa66893f306eb7b9aa`，原生页面 40 个，核心回归、共享患者外壳会话边界、会话失效时旧 owner 微信资料缓存清理、二维码会话门禁、健康数值规则版本和关闭态布局测试通过；该来源已原子发布到本地 live `dist`，`runtime:verify` 通过。线上配套小程序仍是历史 `13f597e`，真机业务证据仍待采集。本文下方旧候选只作历史追溯。

> 当前生产配套基线（2026-08-27）：服务端 release 为 `1107a78a47ac2fbe0557958251d66da9effc66de`，线上配套小程序运行包仍为历史 `13f597ea`；当前本地 live 运行输入为 `6f47c64`。线上上传、真机证据和业务完成仍是独立门禁。
