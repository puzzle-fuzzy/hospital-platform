...
跨幂等键活跃租约索引。当前服务器新 API 的线上 release 是 `4ae2a31`，schema marker、索引列和 schema probe 均已核对通过；发布与共存证据见
[`release/4ae2a31-production-acceptance-2026-08-18.md`](release/4ae2a31-production-acceptance-2026-08-18.md)。此前 release 的运行记录只作为历史证据保留。真实患者并发、provider 和真机证据仍缺，
