...
> **当前本地源码与运行包（2026-08-28）**：当前业务代码和 live 运行输入均为 `cac6561b3f4ebbae2de8c632b052837fe7bc28b6`，原生页面 38 个，核心回归、App.onLaunch 全局资料初始化时序、共享患者外壳会话边界、会话失效时旧 owner 微信资料缓存清理、二维码会话门禁、健康数值规则版本、关闭态布局和预约请求运行时边界测试通过；该来源已原子发布到本地 live `dist`，`runtime:verify` 通过。线上配套小程序仍是历史 `13f597e`，真机业务证据仍待采集。本文下方旧候选只作历史追溯。

> 当前生产配套基线（2026-08-27）：服务端 release 为 `5738a71e0bcddaa8849106754baf5b296427bed7`，线上配套小程序运行包仍为历史 `13f597ea`；当前本地 live 运行输入为 `cac6561`。线上小程序上传、真机证据和业务完成仍是独立门禁。
> 上述三段是本文件当前事实源；下方仍出现的 `6f47c64`、`1107a78a` 等候选编号均为历史快照，只供追溯，不得作为当前运行包或服务端依据。
>
> 下方旧候选说明保留用于解释历史变更；业务规则正文从“患者上下文”开始，继续以本文件当前事实源和 `current-project-baseline-2026-08-27.md` 为准。
>
> **历史候选说明（不作为当前依据）**：下面的旧候选正文仅解释当时的实现范围，不能覆盖上方 `5738a71e` 服务端和 `cac6561` 小程序运行包事实。
> 历史候选 `6f47c64` 已包含旧端 64 页面逐页迁移台账、全入口阻塞类型状态页、迁移阶段/覆盖范围/业务域/所属 A–F 批次/下一步展示、预约/报告/门诊缴费/新增就诊人二级动作收口、可见 action 分发门禁以及患者绑定/签名页查看协议原文的安全入口，并补齐公共日期窗口边界、微信资料拒绝后的设置页重试、选择页刷新并发门禁、预约历史 `scope=online|all` 的显式客户端编码、就诊二维码有效会话门禁、健康数值工具规则版本、共享患者外壳的 owner/会话代际清理和会话失效时旧 owner 微信资料缓存清理。互联网医院旧入口目前只迁移安全壳，不把外部 web-view 或万能 ticket 误记为完成；实时队列、支付医保和写入类链路仍关闭。`custom-tab-bar` 仅作为已撤回的历史候选，不再重新引入。四个主 Tab 由微信运行时统一维护固定位置和 selectedIconPath，页面不渲染自定义底栏，也不手动同步 selected。
> 当前本地 live 候选包含四个正式 Tab 入口、内容滚动视口、挂号卡片、爽约入口、患者上下文错误态分流、查询状态容器、报告目录加载占位收口和协议原文安全入口，以及公共日期窗口边界；预约排班和预约历史底层构造器还会拒绝未知字段、非法日期和范围错配；App.onLaunch 通过显式 App 实例初始化全局资料，避免启动时 getApp 注册时序导致异常；代码回归不能替代生产运行包或真机三层证据。运行包来源和发布边界见 [`release/candidate-cac6561-miniprogram-runtime-2026-08-28.md`](release/candidate-cac6561-miniprogram-runtime-2026-08-28.md)。

> 当前验收基线（2026-08-27）：线上服务端为
> `5738a71e0bcddaa8849106754baf5b296427bed7`（`5738a71e`），线上小程序运行包来源为
> `13f597ea9ee3f65b9be858117826d948339d904a`（历史 `13f597e`）；当前本地 live 运行输入为 `cac6561`（完整来源 `cac6561b3f4ebbae2de8c632b052837fe7bc28b6`）。本文中的历史候选只用于追溯，
...
MySQL 的 `INT`/`BIGINT` 在不同 `mysql2` 连接配置下可能返回 JavaScript number 或十进制字符串，
所以 repository 类型允许这两种真实形态；但这不等于可以使用宽松的 `Number(value)`。数组、布尔值、
空字符串和带小数/指数的文本都不是数据库整数事实，`Number([])`、`Number(false)` 等隐式转换可能把
坏行伪装成合法的 0。所有进入领域层的数值读模型必须先经过严格整数投影：

- 金额分、排班总号源/可用号源、患者同步租约代次、个人资料年龄/版本、支付订单版本和预支付尝试的
  版本/查单次数，只接受安全整数；字符串只接受十进制数字，并按各字段的最小值校验。
- 预支付 claim 在条件更新和返回快照中必须使用同一个已经归一化的数值版本；不能直接对数据库字符串做 `+ 1`，
  否则会把版本 `"3"` 变成 `"31"`，破坏乐观锁和过期 worker 隔离。
- 数值读模型不完整或类型异常时必须在持久化边界失败，不能把异常行转成空列表、0 金额、0 号源或可继续
  运行的版本；真实支付虽然仍关闭，这条内部状态机门禁也必须保持有效。

本规则对应 [`release/persistence-numeric-read-boundary-2026-08-26.md`](release/persistence-numeric-read-boundary-2026-08-26.md)。
...
8. 患者目录、报告、挂号记录和门诊费用页面使用“最后一次请求获胜”规则；旧的异步响应即使晚返回，也不能回写当前页面。
   ...
   预约记录和门诊费用的患者选择行还必须区分“本轮事实仍在加载”和“本轮已经结束但没有患者”：
   前者展示加载中，后者才展示“请先选择就诊人”。不能因为页面为隔离旧患者而暂时清空
   `selectedPatient`，就把网络等待误报成登录或患者选择失败。
   四个患者范围列表页（预约历史、爽约、报告目录、门诊费用）还必须把 `error` 分支放在成功空列表
   分支之前；依赖未配置、会话失效、患者映射失败、Provider 失败和网络异常只能进入明确错误态，
   不能同时显示“暂无数据”或“请先选择就诊人”。错误态的“重新加载”必须重新执行该页完整的
   `/me` → 患者目录 → 患者范围查询链路；错误态中的“选择就诊人”动作只允许由
   `patient-selection-required`、`patient-selection-stale`、`patient-not-bound` 或
   `patient-clinical-unavailable` 触发，网络、Provider、持久化和依赖配置错误只能重试，
   不能把服务故障误导成用户需要换人。用户明确点击“选择就诊人”后才重新进入患者选择页，
   不能只清空页面文案。
   爽约页是预约历史的派生子视图，不把“选择就诊人”模块当作查询结果空态：它从“我的”页只要求已验证会话，
   缺少患者上下文或患者上下文明确失效时留在本页稳定错误态并允许重试；只有用户明确点击“更换就诊人”
   才进入统一患者选择页。Provider、持久化和网络错误仍留在本页错误态，不能把未知故障误导成换人。
   四个患者范围记录页以及患者选择页的 loading、成功空结果
   和错误状态还必须使用同一组固定高度与图片尺寸，避免“正在查询”替换为“未查询记录”时把下方内容突然推移。
   报告详情页虽然不是列表页，但同样必须让加载、错误、无检测项和无云影像状态共享稳定的状态容器；
   详情读模型收敛时不能因为展示分支不同而突然改变卡片高度。
...
11. 会话恢复没有拿到当前 principal 时，首页不得继续把旧患者派生数据交给预约、报告或费用页面；
   ...
   预约记录、报告目录和门诊费用在患者为空、stale 或读取失败时，空态必须提供可点击的患者选择入口；
   不能只显示依赖 `selectedPatient` 卡片的“上方更换”，否则页面虽然安全地清除了旧患者，却把用户留在无法恢复的死路。
   爽约记录是预约历史的派生只读结果，页面不得自动跳转或嵌入“选择就诊人”模块；缺少患者上下文时只展示
   稳定错误态并允许重试，用户如需换人必须先在其他页面完成显式选择。这样不会把入口门禁误认为爽约查询结果，
   也避免用户点击“爽约记录”后被意外带到另一个业务模块。
12. 小程序任意页面的患者同步正在进行时，所有进入患者选择页的入口都必须停止导航并提示稍后重试。
   ...
   统一导航函数必须返回明确的 `PatientSelectorNavigationResult`，至少区分“已发起导航”、
   “会话仍在验证/已回首页”和“同步进行中”；调用方不能把函数返回当作必然已打开选择页。
   爽约页不承担自动跳转患者选择页的职责，因此不维护 `redirectingToPatientSelector`；患者同步中的入口只在
   用户明确点击“更换就诊人”时由统一导航门禁处理，爽约查询本身收到患者上下文错误时回到本页可重试错误态。
   ...
   禁止恢复本地“当前”标记；只有完整同步成功后才允许恢复选择，避免渲染批次短暂出现未经确认的患者上下文。
    ...
    首页目录读取还必须把 `loaded` 与 `superseded` 作为不同的生命周期结果：旧请求即使收到 200，
    只要在回调落地前已经被新一轮读取或页面卸载淘汰，就不能返回 `loaded`，也不能让登录恢复链继续启动
    `onSyncPatients()`。`superseded` 不是成功空目录，必须安静结束，避免旧链路制造重复 Provider 请求和同步租约竞争。
...
全局用户资料状态还必须区分“资料请求失败”和“微信资料授权被拒绝”：`status=error` 只表示普通资料
读取暂时失败，不等于当前 owner 或登录会话已经失效。只要 owner 与会话代次仍然有效，用户仍可主动发起
微信资料授权；用户拒绝授权时应保留 `wechatProfileState=declined` 及“未授权，可点击此处重新获取”
的可重试状态，下一次点击再进入 `wx.openSetting` 后重试的链路。只有明确的 `unauthorized` 或会话代次
变化才允许清理 owner 并要求重新登录，不能因为资料接口失败或用户拒绝授权而误清 token、误跳登录页。

会话失效时还必须在发布空的全局资料快照前保存旧 owner，并删除该 owner 绑定的本机微信昵称/头像缓存；
不能因为缓存 key 按 owner 隔离，就把旧账号隐私和过期头像长期留在设备上。患者选择缓存属于独立的
owner/会话代际边界，不能被这一步无条件删除，但它也不能越过当前患者范围门禁继续参与业务请求。
