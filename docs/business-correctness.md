...
> **当前本地源码候选（2026-08-26）**：当前业务代码候选和 pending 运行输入为 `0be59f966de2c3a0861cb44e9a526a1ef557f6c7`，原生页面 40 个，核心回归、共享患者外壳会话边界、二维码会话门禁和健康数值规则版本测试通过；pending 运行包校验已通过但尚未原子发布，当前 live `dist` 仍为 `02dbf10`，线上配套小程序仍是历史 `13f597e`。本文下方旧候选只作历史追溯。

> 当前生产配套基线（2026-08-26）：服务端 release 为 `8eb51b5ffe85b0b8f8a032783f893117d3df549d`，线上配套小程序运行包仍为历史 `13f597ea`；当前 live 运行输入为 `02dbf10`，最新业务候选为 pending `0be59f96`。线上上传、真机证据和业务完成仍是独立门禁。
...
> 当前验收基线（2026-08-26）：线上服务端仍为
> `8eb51b5ffe85b0b8f8a032783f893117d3df549d`（`8eb51b5f`），线上小程序运行包来源为
> `13f597ea9ee3f65b9be858117826d948339d904a`（历史 `13f597e`）；当前本地 live 运行输入为 `02dbf104`（短值 `02dbf10`），最新 pending 候选为 `0be59f96`。本文中的历史候选只用于追溯，
