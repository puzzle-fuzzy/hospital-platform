...
7. 如果本地已有的 `patientId` 已不在当前 owner 的有效目录中，页面必须进入“请重新选择就诊人”状态；只有本地从未保存过选择时才允许默认目录第一位，不能用 `patients[0]` 静默替换已失效的患者。
   ...
   公共对象。读模型异常不能降级成 `items: []`，服务端只记录固定 `readModelViolation`，返回
   `persistence-invalid`，避免损坏数据触发首页把用户误判成“没有就诊人”或静默默认到另一位患者。
...
跨幂等键活跃租约索引。当前服务器新 API 的线上 release 是 `1b94c46`，schema marker、索引列和 schema probe 均已核对通过；发布与共存证据见
[`release/1b94c46-production-acceptance-2026-08-18.md`](release/1b94c46-production-acceptance-2026-08-18.md)。此前 release 的运行记录只作为历史证据保留。真实患者并发、provider 和真机证据仍缺，
