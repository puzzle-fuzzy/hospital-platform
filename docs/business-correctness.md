...
> 当前生产配套基线（2026-08-24）：服务端 release 为 `8eb51b5ffe85b0b8f8a032783f893117d3df549d`，线上配套小程序运行包来源为 `13f597ea9ee3f65b9be858117826d948339d904a`（提交 `13f597e`）。
 > 当前可运行底栏仍是 `fcc6630e` 对应的微信原生 TabBar；最新本地代码候选 `90d5ab03` 已包含全局启动资料初始化、授权点击修复、全入口阻塞类型状态页以及预约/报告/门诊缴费/新增就诊人二级动作收口，但因开发者工具锁定 `dist/` 尚未发布。`custom-tab-bar` 仅作为已撤回的历史候选，不再重新引入。四个主 Tab 由微信运行时统一维护固定位置和 selectedIconPath，页面不渲染自定义底栏，也不手动同步 selected。
