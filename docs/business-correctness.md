...
> **当前本地源码与运行包（2026-08-27）**：当前业务代码和 live 运行输入均为 `90d8910bdc54d48dde66c4ff03a7434c182ebd92`，原生页面 40 个，核心回归、共享患者外壳会话边界、会话失效时旧 owner 微信资料缓存清理、二维码会话门禁、健康数值规则版本和关闭态布局测试通过；该来源已原子发布到本地 live `dist`，`runtime:verify` 通过。线上配套小程序仍是历史 `13f597e`，真机业务证据仍待采集。本文下方旧候选只作历史追溯。

> 当前生产配套基线（2026-08-27）：服务端 release 为 `1bc8b0a85f21cb58205a99ce4de0de6afe9bf240`，线上配套小程序运行包仍为历史 `13f597ea`；当前本地 live 运行输入为 `90d8910b`。线上上传、真机证据和业务完成仍是独立门禁。
...
>
> **历史候选说明（不作为当前依据）**：下面的旧候选正文仅解释当时的实现范围，不能覆盖上方 `1bc8b0a8` 服务端和 `90d8910b` 小程序运行包事实。
...
> 当前本地 live 候选包含四个正式 Tab 入口、内容滚动视口、挂号卡片、爽约入口、患者上下文错误态分流、查询状态容器、报告目录加载占位收口和协议原文安全入口，以及公共日期窗口边界；代码回归不能替代生产运行包或真机三层证据。运行包来源和发布边界见 [`release/candidate-90d8910b-miniprogram-runtime-2026-08-27.md`](release/candidate-90d8910b-miniprogram-runtime-2026-08-27.md)。

> 当前验收基线（2026-08-27）：线上服务端为
> `1bc8b0a85f21cb58205a99ce4de0de6afe9bf240`（`1bc8b0a8`），线上小程序运行包来源为
> `13f597ea9ee3f65b9be858117826d948339d904a`（历史 `13f597e`）；当前本地 live 运行输入为 `90d8910b`（完整来源 `90d8910bdc54d48dde66c4ff03a7434c182ebd92`）。本文中的历史候选只用于追溯，
