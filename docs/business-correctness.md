...
11. 首页恢复、下拉刷新或登录后的患者同步正在进行时，所有进入患者选择页的入口都必须停止导航并提示稍后重试。
   不能让首页和选择页用不同的幂等键同时同步同一 owner/provider；服务端返回处理中冲突虽然是正确的，
   但页面会把它误显示成加载失败。只有当前快照收敛后，才允许进入选择页继续读取和切换患者。
...
跨幂等键活跃租约索引；当前公网 `18081` 的线上 release 是 `3ab0a6c`，schema marker、索引列和 schema probe 均已核对通过。此前 release 的运行记录只作为历史证据保留。真实患者并发、provider 和真机证据仍缺，
