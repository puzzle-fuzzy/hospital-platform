...
> **当前本地源码候选纠正（2026-08-25）**：健康百科迁移后最新候选为 `99c7e8fd76bd7b38de50d1c5cfdbc7002cba4a15`，原生页面 20 个，当前工作树复跑为 `259 pass / 0 fail / 2525 expect()`；运行包仍在 pending，线上配套小程序仍为 `13f597e`。本文下方 `485c0892` 等候选只作历史追溯。
...
> 当前生产配套基线（2026-08-24）：服务端 release 为 `8eb51b5ffe85b0b8f8a032783f893117d3df549d`，线上配套小程序运行包来源为 `13f597ea9ee3f65b9be858117826d948339d904a`（提交 `13f597e`）。
 > 当前可运行底栏仍是 `fcc6630e` 对应的微信原生 TabBar；最新本地代码候选 `485c0892` 已包含旧端 64 页面逐页迁移台账、全入口阻塞类型状态页、预约/报告/门诊缴费/新增就诊人二级动作收口、可见 action 分发门禁以及“就诊”主 Tab 患者栏/三标签迁移壳，但因开发者工具锁定 `dist/` 尚未发布。实时队列、支付医保和写入类链路仍关闭。`custom-tab-bar` 仅作为已撤回的历史候选，不再重新引入。四个主 Tab 由微信运行时统一维护固定位置和 selectedIconPath，页面不渲染自定义底栏，也不手动同步 selected。
