...
> 当前本地未发布候选为小程序页面代码 `68279dad7ef8e855070bf20abb2205696f50c584`（提交 `68279dad`），包含共享主导航、四个正式 Tab 入口、内容滚动视口、挂号卡片、爽约入口、查询状态容器和报告目录加载占位收口，不能替代生产运行包或真机三层证据。上一次成功构建记录见 [`release/candidate-ff931d7c-miniprogram-build-2026-08-24.md`](release/candidate-ff931d7c-miniprogram-build-2026-08-24.md)。
...
8. 患者目录、报告、挂号记录和门诊费用页面使用“最后一次请求获胜”规则；旧的异步响应即使晚返回，也不能回写当前页面。
   ...
   `/me` → 患者目录 → 患者范围查询链路；“选择就诊人”必须重新进入患者选择页，不能只清空页面文案。
