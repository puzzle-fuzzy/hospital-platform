...
> **当前本地源码候选（2026-08-26）**：当前 pending 运行输入为 `ad7bd1f7148463be5b2f48e6b389108e7ce43531`，原生页面 40 个，当前工作树复跑为 `307 pass / 0 fail / 3513 expect()`；运行包仍在 pending，线上配套小程序仍是历史 `13f597e`。本文下方旧候选只作历史追溯。

> 当前生产配套基线（2026-08-26）：服务端 release 为 `8eb51b5ffe85b0b8f8a032783f893117d3df549d`，线上配套小程序运行包仍为历史 `13f597ea`；最新本地 pending 运行输入为 `ad7bd1f`，当前 live `dist` 为 `fcc6630e`。pending 因开发者工具锁定 `dist/` 尚未发布。
> 当前候选 `ad7bd1f` 已包含旧端 64 页面逐页迁移台账、全入口阻塞类型状态页、迁移阶段/覆盖范围/业务域/所属 A–F 批次/下一步展示、预约/报告/门诊缴费/新增就诊人二级动作收口、可见 action 分发门禁以及患者绑定/签名页查看协议原文的安全入口。互联网医院旧入口目前只迁移安全壳，不把外部 web-view 或万能 ticket 误记为完成；实时队列、支付医保和写入类链路仍关闭。`custom-tab-bar` 仅作为已撤回的历史候选，不再重新引入。四个主 Tab 由微信运行时统一维护固定位置和 selectedIconPath，页面不渲染自定义底栏，也不手动同步 selected。
> 当前本地未发布候选包含四个正式 Tab 入口、内容滚动视口、挂号卡片、爽约入口、患者上下文错误态分流、查询状态容器、报告目录加载占位收口和协议原文安全入口；代码回归不能替代生产运行包或真机三层证据。详见 [`release/candidate-ad7bd1f-miniprogram-runtime-2026-08-26.md`](release/candidate-ad7bd1f-miniprogram-runtime-2026-08-26.md)。

> 当前验收基线（2026-08-26）：线上服务端仍为
> `8eb51b5ffe85b0b8f8a032783f893117d3df549d`（`8eb51b5f`），线上小程序运行包来源为
> `13f597ea9ee3f65b9be858117826d948339d904a`（历史 `13f597e`）；当前本地 pending 为 `ad7bd1f`。本文中的历史候选只用于追溯，
