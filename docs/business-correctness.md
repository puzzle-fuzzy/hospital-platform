...
7. 如果本地已有的 `patientId` 已不在当前 owner 的有效目录中，页面必须进入“请重新选择就诊人”状态；只有本地从未保存过选择时才允许默认目录第一位，不能用 `patients[0]` 静默替换已失效的患者。
   ...
   患者 service 读取仓储结果时还会执行第二道读模型门禁：重新确认每条记录属于当前 owner、
...
- 当前患者端列表响应的 `total` 必须等于本次返回的 `items.length`；它不是 provider 的隐藏总数，
  也不能被页面当作分页总页数。
- 小程序 `dashboard-service.ts` 在患者目录、预约科室/排班/历史、报告目录和门诊费用的统一读取边界
  重新校验 `items` 数组、非负安全整数 `total` 以及两者相等关系；网关、缓存或前后端版本错配时返回
  `provider-response-invalid`，不能把协议异常降级成空列表或继续展示不一致的“加载更多”。
