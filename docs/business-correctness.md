...
> **当前本地源码与运行包（2026-08-28）**：当前业务代码和 live 运行输入均为 `1bc5bf6f7cc4d38fad29fbf7e8aca3f65c46b916`，原生页面 38 个，核心回归、App.onLaunch 全局资料初始化时序、共享患者外壳会话边界、会话失效时旧 owner 微信资料缓存清理、二维码会话门禁、健康数值规则版本、关闭态布局和预约请求运行时边界测试通过；该来源已原子发布到本地 live `dist`，`runtime:verify` 通过。线上配套小程序仍是历史 `13f597e`，真机业务证据仍待采集。本文下方旧候选只作历史追溯。

> 当前生产配套基线（2026-08-27）：服务端 release 为 `5738a71e0bcddaa8849106754baf5b296427bed7`，线上配套小程序运行包仍为历史 `13f597ea`；当前本地 live 运行输入为 `1bc5bf6`。线上小程序上传、真机证据和业务完成仍是独立门禁。
...
>
> **历史候选说明（不作为当前依据）**：下面的旧候选正文仅解释当时的实现范围，不能覆盖上方 `5738a71e` 服务端和 `1bc5bf6` 小程序运行包事实。
