...
> **当前本地源码候选（2026-08-26）**：当前业务代码候选和 pending 运行输入为 `aae1297baae5fc3c35b38472ce724f10ba82336b`，原生页面 40 个，核心回归、二维码会话门禁和健康数值规则版本测试通过；pending 运行包校验已通过但尚未原子发布，当前 live `dist` 仍为 `02dbf10`，线上配套小程序仍是历史 `13f597e`。本文下方旧候选只作历史追溯。

> 当前生产配套基线（2026-08-26）：服务端 release 为 `8eb51b5ffe85b0b8f8a032783f893117d3df549d`，线上配套小程序运行包仍为历史 `13f597ea`；当前 live 运行输入为 `02dbf10`，最新业务候选为 pending `aae1297b`。线上上传、真机证据和业务完成仍是独立门禁。
> 当前候选 `aae1297b` 已包含旧端 64 页面逐页迁移台账、全入口阻塞类型状态页、迁移阶段/覆盖范围/业务域/所属 A–F 批次/下一步展示、预约/报告/门诊缴费/新增就诊人二级动作收口、可见 action 分发门禁以及患者绑定/签名页查看协议原文的安全入口，并补齐公共日期窗口边界、微信资料拒绝后的设置页重试、选择页刷新并发门禁、预约历史 `scope=online|all` 的显式客户端编码、就诊二维码有效会话门禁和健康数值工具规则版本。互联网医院旧入口目前只迁移安全壳，不把外部 web-view 或万能 ticket 误记为完成；实时队列、支付医保和写入类链路仍关闭。`custom-tab-bar` 仅作为已撤回的历史候选，不再重新引入。四个主 Tab 由微信运行时统一维护固定位置和 selectedIconPath，页面不渲染自定义底栏，也不手动同步 selected。
...
> `13f597ea9ee3f65b9be858117826d948339d904a`（历史 `13f597e`）；当前本地 live 运行输入为 `02dbf104`（短值 `02dbf10`），最新 pending 候选为 `aae1297b`。本文中的历史候选只用于追溯，
