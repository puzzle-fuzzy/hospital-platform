...
> **当前本地源码候选（2026-08-26）**：当前 pending 运行输入为 `de9c5b996c6735ced9684bce72e493834fe9325e`，原生页面 40 个，当前工作树复跑为 `309 pass / 0 fail / 3522 expect()`；运行包仍在 pending，线上配套小程序仍是历史 `13f597e`。本文下方旧候选只作历史追溯。

> 当前生产配套基线（2026-08-26）：服务端 release 为 `8eb51b5ffe85b0b8f8a032783f893117d3df549d`，线上配套小程序运行包仍为历史 `13f597ea`；最新本地 pending 运行输入为 `de9c5b99`，当前 live `dist` 为 `fcc6630e`。pending 因开发者工具锁定 `dist/` 尚未发布。
...
> 当前本地未发布候选包含四个正式 Tab 入口、内容滚动视口、挂号卡片、爽约入口、患者上下文错误态分流、查询状态容器、报告目录加载占位收口和协议原文安全入口，以及公共日期窗口边界；代码回归不能替代生产运行包或真机三层证据。详见 [`release/candidate-de9c5b99-miniprogram-runtime-2026-08-26.md`](release/candidate-de9c5b99-miniprogram-runtime-2026-08-26.md)。
