...
> 当前底栏候选为 `04e238e454091691a5aab2d244b4abea43c61d0a`：保持原生共享 `tabBar`，删除旧自定义尺寸字段，并使用 `*-native-v6.png` 作为全新资源路径；该路径切换只为隔离缓存，不改变图形内容或业务路由。新预览二维码为 `.local/hospital-miniprogram/tabbar-preview-04e238e.png`，真机仍需重新扫码验收。
> 当前本地未发布候选为小程序运行包来源 `a5ff0001edf20beab29583e5101ee34bb9ff0c5c`，包含原生主导航、四个正式 Tab 入口、内容滚动视口、挂号卡片、爽约入口、患者上下文错误态分流、查询状态容器和报告目录加载占位收口；本轮移除了仍会产生生命周期闪动的 `custom-tab-bar` 组件，恢复微信原生 `tabBar`，并由构建门禁确认运行包不含自定义底栏或测试脚本。运行包已通过构建和来源校验，仍不能替代生产运行包或真机三层证据。详见 [`release/current-tabbar-runtime-recheck-2026-08-25.md`](release/current-tabbar-runtime-recheck-2026-08-25.md)。
...
- 重新打开 `apps/miniprogram/dist/` 的正确开发者工具项目并使用当前本地候选 `a5ff0001`，或明确记录线上运行包 `13f597e`，完成真机三层证据；
