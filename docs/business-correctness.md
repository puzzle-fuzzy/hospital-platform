...
> 当前可运行底栏仍是 `fcc6630e` 对应的微信原生 TabBar；最新本地代码候选 `d59dc8fe` 已包含全局启动资料初始化、授权点击修复、未迁移入口统一状态页和首页悬浮客服入口收口，但因开发者工具锁定 `dist/` 尚未发布。`custom-tab-bar` 仅作为已撤回的历史候选，不再重新引入。四个主 Tab 由微信运行时统一维护固定位置和 selectedIconPath，页面不渲染自定义底栏，也不手动同步 selected。
