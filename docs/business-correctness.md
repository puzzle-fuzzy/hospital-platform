...
> 当前发布基线（2026-08-24）：服务端与小程序运行包均为 `13f597ea9ee3f65b9be858117826d948339d904a`（提交 `13f597e`）。本文历史候选只用于追溯，真实业务仍须绑定当前真机三层证据。
...
8. 患者目录、报告、挂号记录和门诊费用页面使用“最后一次请求获胜”规则；旧的异步响应即使晚返回，也不能回写当前页面。
   ...
   爽约页是预约历史的派生子视图，不保留“请先选择就诊人”的结果空态：患者上下文明确失效时，
   页面必须在当前请求仍有效且会话已验证的前提下直接进入统一患者选择页；Provider、持久化和网络错误
   仍留在本页错误态，不能把未知故障误导成换人。四个患者范围记录页以及患者选择页的 loading、成功空结果
