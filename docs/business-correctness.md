...
患者同步当前采用按 provider 目录引用幂等 upsert：重复同步不会因为随机生成新的内部 ID 而重复创建相同目录患者；数据库唯一键和 repository 负责最终并发保护。

但“provider 当前目录中已不存在的患者”不能直接物理删除。报告引用、费用记录和未来支付订单可能仍然依赖该内部患者。下一阶段必须使用事务设计以下状态模型：
...
在该模型上线前，真实环境不得把“同步返回空列表”解释为已经安全清理历史患者，也不得人工直接删除 `hp_patients`。
