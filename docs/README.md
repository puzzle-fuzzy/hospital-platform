...
> **源码同步备注（2026-08-26）**：后续源码又提交了 `e3356e50` 的就诊患者卡片脱敏纠偏和患者展示审计；pending `e1adbf7` 未包含这次修正。释放 `dist` 锁后必须重新构建并更新运行包来源，不能用旧 pending 包验收这次源码改动。
