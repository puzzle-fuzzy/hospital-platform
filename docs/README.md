...
> **候选切换记录**：本页下方仍保留旧候选文件名和历史验收段落，均不再代表当前运行包；当前源码以 `de9c5b99` 为准，当前小程序运行包仍以 `ad7bd1f` pending 候选为准。源码和 pending 尚未重新构建/发布前，不能把日期边界修正写成真机证据；40 个页面仍无本轮真机证据，不能沿用旧候选清单。
