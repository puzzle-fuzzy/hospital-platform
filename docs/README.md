> 历史小程序配套运行包来源（2026-08-31）：本地 live 运行输入为 ce1c2179b57fe2783066b51f8621220224982928（提交 ce1c217）。本行只作历史追溯。

> **历史候选同步（2026-09-01）**：服务端 release `5738a71e0bcddaa8849106754baf5b296427bed7`；本地小程序 live 运行包 sourceRevision `ce1c2179b57fe2783066b51f8621220224982928`。本机另有旧候选 `8182a8774bf0d6ad6f62d928693add952ddff034` 暂存在 pending，和当时候选不一致，`runtime:verify:pending` 会拒绝它；本段只作历史追溯。
...
> 历史事实（2026-09-01）：线上服务端 release 为 `5738a71e0bcddaa8849106754baf5b296427bed7`；小程序运行相关源码和本地 live 运行输入为 `ce1c2179b57fe2783066b51f8621220224982928`（`ce1c217`），共注册 38 个页面，本地回归为 `356 pass / 0 fail / 3826 expect()`。本段只作历史追溯。
> **历史仓库 API 候选（2026-08-28）**：当时 `main` 包含请求日志稳定错误码投影，线上 release 为 `5738a71e0bcddaa8849106754baf5b296427bed7`，已完成 API-only 远端发布；旧 Python `8001`、旧数据库和旧 Redis 未修改。本段只作历史追溯。
> 历史线上服务端 release（2026-08-27）：`5738a71e0bcddaa8849106754baf5b296427bed7`，当时已完成候选 preflight、隔离 smoke、原子切换、公网 runtime smoke、稳定 `unauthorized` 日志投影和 HTTPS 证书校验；该运行层证据不等价于真实 Provider 或支付业务成功。
...
> **历史源码同步备注（2026-08-31）**：`ce1c217` 是当时运行相关源码和本地 live 运行包。本段只作历史追溯。
> 服务端与小程序继续采用分层发布；线上历史小程序 `13f597e` 与本地 live `ce1c217` 不得互相替代。本文以下旧候选编号均只作历史追溯。

> **历史真机会话状态（2026-09-01）**：当时检测到微信开发者工具进程仍在运行，但尚未取得手机扫码后的页面和业务同链取证；本段只作历史记录。

> **历史候选修正**：当时在全量入口覆盖基础上新增 BMI/血压安全数值子集，并保留临床内容、外部入口和预约 Provider 的关闭态页面外壳；本段只作历史追溯。

> **历史候选切换记录**：本页下方保留的旧候选文件名和验收段落均不代表当前运行包；当时 `ce1c217` 候选已通过静态校验并完成原子发布，但不能沿用为当前候选清单。

> **历史候选覆盖（2026-08-31）**：当时小程序源码和本地 live 运行输入为 `ce1c217`，38 页，回归 `356 pass / 0 fail / 3826 expect()`；九个真机证据域仍为 pending。本段只作历史追溯。

> **历史小程序启动修正（2026-08-31）**：当时运行相关源码和本地 live 运行包为 `ce1c2179b57fe2783066b51f8621220224982928`（`ce1c217`）；本段只作历史追溯。

> **历史启动保护候选（2026-08-27）**：`9354104`（完整构建来源 `935410473e5a7c1be125a85834f957f53a833d8f`）及其发布记录只作历史追溯。

# 项目文档导航

医保支付统一核心、挂号/门诊入口边界、文档证据和迁移索引见
[`架构决策/0005-医保统一核心与业务入口.md`](架构决策/0005-医保统一核心与业务入口.md)。
...
> **历史全量迁移交接单（2026-09-01）**：请优先以本文顶部当前事实和 [`迁移/广度优先迁移计划-2026-08-25.md`](迁移/广度优先迁移计划-2026-08-25.md) 为准；本段的 `ce1c217`、38 页面和旧候选证据只作追溯。

> **历史仓库交接基线（2026-09-01）**：本地 live 运行输入为 `ce1c2179b57fe2783066b51f8621220224982928`；当时真机证据域仍为 `pending`，本段不代表当前运行包。

> **历史 live 运行包**：`apps/miniprogram/dist/build-info.sourceRevision=ce1c2179b57fe2783066b51f8621220224982928`，38 页；本段只作历史运行包记录。
...
> **历史公网探针复核（2026-08-25 23:35 CST）**：当时公网 `/api/v2/health/live`、`/api/v2/health/ready` 和 `/api/v2/system/ping` 均返回 200；本段不替代线上进程、旧服务共存或业务真机证据。
...
> **历史事实源（2026-08-28）**：`1bc5bf6` 及其 356 项回归只作历史候选记录；健康内容、临床 Provider、外部入口和支付/医保的当时关闭结论不因该候选改变。

> **历史发布事实入口（2026-08-27）**：[`发布/当前项目基线-2026-08-27.md`](发布/当前项目基线-2026-08-27.md) 只记录当时的服务端和小程序候选，不代表当前运行包。
...
| [`发布/日志查询与3090快速发布-2026-09-16.md`](发布/日志查询与3090快速发布-2026-09-16.md) | 单笔日志定向导出、3090 分面发布、不可省略的安全门禁和操作记录模板 |
...
| [`提供商接入/2026-09-03-医保测试凭据.md`](提供商接入/2026-09-03-医保测试凭据.md) | 医保测试环境密钥反馈单与国标 V2.2.5 规范的指纹登记、脱敏事实、旧凭据使用决策和 crypto 解冻顺序；当前为 `normalized` |
| [`提供商接入/2026-09-03-旧端链路流向图与生产拓扑.md`](提供商接入/2026-09-03-旧端链路流向图与生产拓扑.md) | 旧端 98% 可信链路流向图的服务器复核版：中转/拦截器/forward 三目标拓扑、医保五段链数据依赖、旧服务分叉清单；当前为 `normalized` |
| [`医保契约-v1.md`](医保契约-v1.md) | 医保金额、状态和回写契约 |
| [`迁移/医保FSI接口规范整理-2026-09-02.md`](迁移/医保FSI接口规范整理-2026-09-02.md) | 山西医保规范 v1.3.35 的协议事实沉淀：通用 FSI envelope、6xxx 族方向与字典、SM2/SM4 两层密码规则、与 legacy-fsi-contract 的对照差异 |
...
| [`提供商接入/2026-09-05-众阳患者档案绑定官方文档核对.md`](提供商接入/2026-09-05-众阳患者档案绑定官方文档核对.md) | 众阳 2.1.51/2.1.52/2.1.53/2.1.55 官方页面核对、适配器差异和患者绑定配置闸门；当前仍为 normalized |
...
> **当前事实（2026-09-16）**：非支付迁移主线以根目录 [`TODO.md`](../TODO.md) 和 [`迁移/广度优先迁移计划-2026-08-25.md`](迁移/广度优先迁移计划-2026-08-25.md) 为准。旧服务 64 个页面、新端 47 个注册页面；当前小程序源码候选为 `07ab94c3c3cb9979313b32c50b695943d028e841`，最近一次已验证的 release 运行包来源为 `cba13c71a74022b756144b9c7a61965cd542b1af`，两者尚未同源发布，development 运行包另有 workspace snapshot。页面、代码、构建和 HTTP 200 不等于 Provider、真实数据、公网、真机或业务结果完成；支付、医保和支付相关 HIS 回写继续排除。

> 当前状态语义统一为“安全静态子集”“代码已具备，待实证”“页面外壳与关闭态”或明确 contract 阻塞；下方 2026-08-27 至 2026-09-01 的候选和运行包记录均只作历史追溯。
