...
> **源码同步备注（2026-08-27）**：`1bc5bf6` 是最新运行相关源码和本地 live 运行包，在既有共享患者外壳、会话代际、二维码会话门禁和安全关闭态基础上，收紧预约排班和预约历史请求构造器的运行时字段、日期和范围边界，并修复 App.onLaunch 全局资料初始化时序。该候选已原子发布到本地 live `dist`，后续真机验收必须从 live 重新生成二维码并绑定本候选证据。
> 服务端与小程序继续采用分层发布；线上历史小程序 `13f597e` 与本地 live `1bc5bf6` 不得互相替代。本文以下旧候选编号均只作历史追溯。

> **真机会话状态（2026-08-27）**：当前没有运行中的微信开发者工具或真机会话；下一次操作必须直接打开 `apps/miniprogram/dist/`，从 `1bc5bf6` 运行包普通编译并生成新的预览二维码。手机尚未完成扫码后的页面和业务同链取证。九个真机证据域继续保持 `pending`；更早的 `device-observation-1bc5bf6` 仅作历史记录，不能绑定当前候选。
...
> **当前全量迁移交接单（2026-08-27）**：请优先阅读 [`migration/full-migration-handoff-2026-08-25.md`](migration/full-migration-handoff-2026-08-25.md)。最新运行相关源码和本地 live 运行输入均为 `1bc5bf6`；64 个旧页面均有明确落点，其中 8 个已替换、23 个安全 `partial` 子集、23 个 `surface-only` 页面外壳、7 个支付/回写入口仍按阻断原因关闭；健康自测仅开放 BMI/血压安全数值子集，今日预约摘要不冒充实时状态，协议正文已可读，但同意/撤回/审计仍关闭。本轮收紧预约排班和预约历史请求构造器的运行时边界并修复 App.onLaunch 全局资料初始化时序，其他安全边界继续保持；本页下方历史候选只作追溯。完整候选证据以 [`release/candidate-1bc5bf6-miniprogram-runtime-2026-08-28.md`](release/candidate-1bc5bf6-miniprogram-runtime-2026-08-28.md)、[`release/device-evidence-1bc5bf6-pending.json`](release/device-evidence-1bc5bf6-pending.json) 和 live 的 `build-info.json` 为准。

> **当前仓库交接基线（2026-08-27）**：本地 live 运行输入为 `1bc5bf6f7cc4d38fad29fbf7e8aca3f65c46b916`，运行包校验通过并完成原子发布；启动保护候选已不再暂存于 pending。全部 9 个真机证据域仍为 `pending`，所以总体仍返回 `passed=false`。任何真实 `passed/failed` 证据仍必须绑定通过 `release:baseline:audit` 的线上 release。不要把本行当作新的线上服务部署记录。

> **当前 live 运行包**：`apps/miniprogram/dist/build-info.sourceRevision=1bc5bf6f7cc4d38fad29fbf7e8aca3f65c46b916`，38 页；live `runtime:verify` 已通过。回归为 `356 pass / 0 fail / 3826 expect()`；当前没有 pending 运行包，后续源码变化仍须按发布和回滚步骤执行，详见 [`release/pending-runtime-publication-runbook-2026-08-26.md`](release/pending-runtime-publication-runbook-2026-08-26.md)。
...
`1bc5bf6f7cc4d38fad29fbf7e8aca3f65c46b916`，当前 live `dist` 已完成原子切换。
...
`1bc5bf6f7cc4d38fad29fbf7e8aca3f65c46b916`。不能沿用线上来源或旧二维码。每次小程序源码/构建输入提交后都必须重新执行 build 和
...
| [`release/current-5a31427-real-business-event-window-2026-08-21.md`](release/current-5a31427-real-business-event-window-2026-08-21.md) | 服务器真实微信登录/患者同步事件窗口；因早于当前 `1bc5bf6` 完整小程序构建 `1bc5bf6f7cc4d38fad29fbf7e8aca3f65c46b916` 且来源未匹配，只作为历史观察；当前 live 另见 `1bc5bf6`，不计入当前候选验收 |
...
> 当前统一发布基线（2026-08-28）：服务端 release 为 `5738a71e0bcddaa8849106754baf5b296427bed7`；小程序本地 live 运行包来源为 `1bc5bf6f7cc4d38fad29fbf7e8aca3f65c46b916`（`1bc5bf6`），共 38 个页面。本文更早版本仅作历史追溯，真机证据仍为 pending；旧 Python `8001` 未修改。
