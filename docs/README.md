> 最新事实（2026-08-26）：线上服务端 release 仍为 `8eb51b5ffe85b0b8f8a032783f893117d3df549d`；最新小程序运行相关源码候选和 pending 运行输入为 `aae1297baae5fc3c35b38472ce724f10ba82336b`，共注册 40 个页面，本地回归为 `329 pass / 0 fail / 3633 expect()`。本轮收紧就诊二维码有效会话门禁，并为健康数值工具固定非诊断规则版本；此前已统一电子锦旗和表扬信记录区域的加载、错误、未开放三态及固定高度，保留就诊人选择页刷新并发门禁、微信资料拒绝后的设置页重试，以及“我的快递”加载/错误/未开放三态；pending 运行包校验已通过，但因开发者工具锁定 `dist` 尚未原子发布，当前 live `dist` 仍为 `02dbf10`。真实真机业务证据仍待采集。25 个 `surface-only` 页面及 4 个安全 `partial` 子集仍未开放对应真实业务，支付/医保入口保持关闭，健康自测仅开放安全数值子集，今日预约摘要已补齐但实时叫号仍关闭。本轮继续横向迁移，不伪造临床 Provider、物流、采血号源或公开记录数据。

> **源码同步备注（2026-08-26）**：`aae1297b` 是最新运行相关源码候选，收紧就诊二维码有效会话门禁并固定健康数值工具规则版本；同时保留电子锦旗和表扬信记录区域三态、微信资料被拒绝后的设置页重试链路、就诊患者卡片脱敏纠偏、患者展示审计、首次加载/同步期间的刷新并发门禁与“我的快递”状态门禁。候选已通过 pending 运行包校验，但尚未替换 live `dist`，后续真机验收必须从该候选原子发布后重新生成二维码并绑定本候选证据。
> 服务端与小程序继续采用分层发布；历史候选、线上 live `13f597e`、当前 live `02dbf10` 和 pending `aae1297b` 不得互相替代。本文以下旧候选编号均只作历史追溯。
...
> **候选切换记录**：本页下方仍保留旧候选文件名和历史验收段落，均不再代表当前运行包；最新运行相关源码候选为 `aae1297b`，当前 live 来源仍为 `02dbf10`。新候选已通过 pending 静态校验但尚未原子发布；40 个页面仍无本轮真机证据，不能沿用旧候选清单。

> **当前候选覆盖（2026-08-26）**：最新小程序源码和 pending 运行输入为 `aae1297b`（完整来源 `aae1297baae5fc3c35b38472ce724f10ba82336b`），40 页，回归 `329 pass / 0 fail / 3633 expect()`；就诊二维码会话门禁和健康数值工具规则版本已补齐，便民记录区域仍保持加载、失败、未开放三态和固定高度。pending 校验通过但尚未原子发布，当前 live 仍为 `02dbf10`，九个真机证据域仍为 pending。候选证据见 [`release/candidate-aae1297-miniprogram-runtime-2026-08-26.md`](release/candidate-aae1297-miniprogram-runtime-2026-08-26.md) 和 [`release/device-evidence-aae1297b-pending.json`](release/device-evidence-aae1297b-pending.json)。
...
> **当前全量迁移交接单（2026-08-26）**：请优先阅读 [`migration/full-migration-handoff-2026-08-25.md`](migration/full-migration-handoff-2026-08-25.md)。最新运行相关源码候选为 `ded78c58`，pending 静态验证已通过，当前 live 运行输入仍为 `02dbf10`；64 个旧页面均有明确落点，其中 8 个已替换、23 个安全 `partial` 子集、25 个 `surface-only` 页面外壳、7 个支付/回写入口仍按阻断原因关闭；健康自测仅开放 BMI/血压安全数值子集，今日预约摘要不冒充实时状态，协议正文已可读，但同意/撤回/审计仍关闭。本轮统一便民记录三态和固定高度，保留公共日期窗口、微信资料拒绝后的设置页重试、选择页刷新并发门禁和我的快递三态，并保留患者绑定/签名页查看协议原文安全入口；本页下方历史候选只作追溯。完整候选证据以 [`release/candidate-ded78c58-miniprogram-runtime-2026-08-26.md`](release/candidate-ded78c58-miniprogram-runtime-2026-08-26.md)、[`release/device-evidence-ded78c58-pending.json`](release/device-evidence-ded78c58-pending.json) 和 live 的 `build-info.json` 为准。

> **当前仓库交接基线（2026-08-26）**：pending 候选运行输入为 `ded78c58c53923ecf5232a8035b3e790e5959216`，当前 live 运行输入仍为 `02dbf10419740d96c4445493df019021ac22bcfa`。pending 包已完成独立静态校验但尚未原子发布；全部 9 个真机证据域仍为 `pending`，所以总体仍返回 `passed=false`。任何真实 `passed/failed` 证据仍必须绑定通过 `release:baseline:audit` 的线上 release。不要把本行当作新的线上服务部署记录。

> **当前 live 运行包**：`apps/miniprogram/dist/build-info.json.sourceRevision=02dbf10419740d96c4445493df019021ac22bcfa`，40 页；live `runtime:verify` 已通过。最新 pending 候选为 `ded78c58`，回归为 `329 pass / 0 fail / 3632 expect()`；原子切换仍等待释放 `dist` 锁，发布和回滚步骤见 [`release/pending-runtime-publication-runbook-2026-08-26.md`](release/pending-runtime-publication-runbook-2026-08-26.md)。
...
> **当前事实源（2026-08-26，优先于本页旧候选段落）**：最新小程序源码候选为 pending `ded78c58`，当前 live 运行输入为 `02dbf10`；pending 回归为 `329 pass / 0 fail / 3632 expect()`，已通过 pending 校验但尚未替换 live。健康百科目录、症状查疾病结果、疾病/药品详情已接入，但正式审核 bundle 未发布前健康内容仍保持 fail-closed；25 个跨域页面为 `surface-only`，另有 4 个安全 `partial` 子集，健康自测只提供安全数值子集，今日预约摘要只展示预约事实，协议同意能力、外部互联网医院能力和支付医保能力继续关闭。旧端“我的快递”和采血预约已迁移真实空态，电子锦旗和健康表扬信已迁移患者上下文与关闭态；本轮统一便民记录区域的加载、错误、未开放三态及固定高度，保留公共日期窗口、微信资料拒绝后的设置页重试、选择页刷新并发门禁、我的快递三态和患者绑定/签名页查看协议原文安全入口，但真实临床 Provider、物流/采血号源、公开记录 provider 仍关闭。完整交接和 64 页推进队列见 [`migration/full-migration-handoff-2026-08-25.md`](migration/full-migration-handoff-2026-08-25.md)。

> 历史广度候选：`baa31df08f63af30266664f9fef9224653cf52bb`。四个入口由微信原生 `tabBar` 统一渲染；`custom-tab-bar` 仅作为已撤回的历史候选，不再重新引入。本段只保留入口台账、患者栏和预约摘要的历史交接信息；当前 pending 候选以本页顶部 `ded78c58` 为准，当前 live 仍为 `02dbf10`。

> 历史运行包观察：当时 live `dist/build-info.sourceRevision` 为上一候选 `fcc6630ebfa7b0697cbd03a5e376ce6765d1643b`，`baa31df0` 在 pending staging 中完成 17 个页面脚本、类型检查、旧端 64 页面台账、迁移状态路由回归和就诊记录分批展示回归。当前 live 运行包请以本页上方 `02dbf10` 事实为准，最新 pending 候选请以 `ded78c58` 事实为准；开发者工具只能打开 `apps/miniprogram/dist/` 独立运行包。
...
> 历史候选更新（2026-08-24）：线上服务端当时为 `8eb51b5ffe85b0b8f8a032783f893117d3df549d`；小程序运行包当时为 `13f597ea9ee3f65b9be858117826d948339d904a`（提交 `13f597e`）。生产切换记录见 [`release/8eb51b5f-production-acceptance-2026-08-24.md`](release/8eb51b5f-production-acceptance-2026-08-24.md)。当前 pending 以顶部 `ded78c58` 为准。
...
| [`roadmap-next-phase.md`](roadmap-next-phase.md) | 业务、工程、运行和验收的下一阶段统一路线图；线上服务端仍为 `8eb51b5f`，线上历史小程序包为 `13f597e`，当前本地 live `dist` 为 `02dbf10`，最新 pending 候选为 `ded78c58` |
...
| [`release/pending-runtime-publication-runbook-2026-08-26.md`](release/pending-runtime-publication-runbook-2026-08-26.md) | 当前 `ded78c58` pending 运行包发布、dist 锁定保护和真机证据入口；发布后的 live 运行包以 `build-info.json` 为准 |
| [`release/candidate-ded78c58-miniprogram-runtime-2026-08-26.md`](release/candidate-ded78c58-miniprogram-runtime-2026-08-26.md) | 当前 `ded78c58` 候选的便民记录三态、固定高度、pending 校验、原子发布阻塞和真机证据边界 |
...
| [`release/miniprogram-runtime-enoent-recovery-2026-08-22.md`](release/miniprogram-runtime-enoent-recovery-2026-08-22.md) | 历史 `single-flight.test.js` ENOENT 恢复记录；当前候选以 pending `ded78c58` 和最新开发者工具准入记录为准 |
...
| [`release/patient-context-consistency-audit-2026-08-22.md`](release/patient-context-consistency-audit-2026-08-22.md) | 当前候选患者范围页面的 owner、会话代际、显式选择和旧结果淘汰审计；该记录中的测试数字属于历史候选，当前构建以 `ded78c58` 为准 |
...
| [`release/candidate-8eb51b5f-server-release-2026-08-24.md`](release/candidate-8eb51b5f-server-release-2026-08-24.md) | 当前服务端独立候选、Bun bundle 校验、生产 preflight，以及线上历史小程序 `13f597e` 与 pending `ded78c58` 的分层配套边界 |
...
| [`release/current-5a31427-real-business-event-window-2026-08-21.md`](release/current-5a31427-real-business-event-window-2026-08-21.md) | 服务器真实微信登录/患者同步事件窗口；因早于当前 `13f597e` 完整小程序构建且来源未匹配，只作为历史观察；当前 pending 另见 `ded78c58`，不计入当前候选验收 |
...
| [`migration/migration-gap-audit-2026-08-17.md`](migration/migration-gap-audit-2026-08-17.md) | 历史迁移差距、证据等级、未迁移分层、新文档接收门禁和下一阶段顺序；当前服务端以 `8eb51b5f`、小程序 pending 以 `ded78c58` 为准 |
...
> 历史发布基线更新（2026-08-24 19:54 CST）：线上服务端 release 当时为 `8eb51b5ffe85b0b8f8a032783f893117d3df549d`；小程序运行包当时为 `13f597ea9ee3f65b9be858117826d948339d904a`（提交 `13f597e`）。本轮只重启新 API，旧 Python `8001` 未修改；普通资料 PUT、支付、医保和 Provider 真机证据仍待。当前事实以顶部 `ded78c58` pending 与 `8eb51b5f` 服务端为准。
> **当前候选覆盖（2026-08-26）**：最新小程序源码和 pending 运行输入为 `ded78c58`（完整来源 `ded78c58c53923ecf5232a8035b3e790e5959216`），40 页，回归 `329 pass / 0 fail / 3632 expect()`；电子锦旗/表扬信记录区域已统一加载、失败、未开放三态和固定高度。pending 校验通过但尚未原子发布，当前 live 仍为 `02dbf10`，九个真机证据域仍为 pending。候选证据见 [`release/candidate-ded78c58-miniprogram-runtime-2026-08-26.md`](release/candidate-ded78c58-miniprogram-runtime-2026-08-26.md) 和 [`release/device-evidence-ded78c58-pending.json`](release/device-evidence-ded78c58-pending.json)。
