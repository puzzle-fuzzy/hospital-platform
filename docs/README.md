> 最新事实（2026-08-26）：线上服务端 release 仍为 `8eb51b5ffe85b0b8f8a032783f893117d3df549d`；最新小程序运行相关源码候选和 pending 运行输入为 `ded78c58c53923ecf5232a8035b3e790e5959216`，共注册 40 个页面，本地回归为 `329 pass / 0 fail / 3632 expect()`。本轮统一电子锦旗和表扬信记录区域的加载、错误、未开放三态及固定高度，保留就诊人选择页刷新并发门禁、微信资料拒绝后的设置页重试，以及“我的快递”加载/错误/未开放三态；pending 运行包校验已通过，但因开发者工具锁定 `dist` 尚未原子发布，当前 live `dist` 仍为 `02dbf10`。真实真机业务证据仍待采集。25 个 `surface-only` 页面及 4 个安全 `partial` 子集仍未开放对应真实业务，支付/医保入口保持关闭，健康自测仅开放安全数值子集，今日预约摘要已补齐但实时叫号仍关闭。本轮继续横向迁移，不伪造临床 Provider、物流、采血号源或公开记录数据。

> **源码同步备注（2026-08-26）**：`ded78c58` 是最新运行相关源码候选，统一电子锦旗和表扬信记录区域的加载、错误、未开放三态及固定高度；同时保留微信资料被拒绝后的设置页重试链路、就诊患者卡片脱敏纠偏、患者展示审计、首次加载/同步期间的刷新并发门禁与“我的快递”状态门禁。候选已通过 pending 运行包校验，但尚未替换 live `dist`，后续真机验收必须从该候选原子发布后重新生成二维码并绑定本候选证据。
> 服务端与小程序继续采用分层发布；历史候选、线上 live `13f597e`、当前 live `02dbf10` 和 pending `ded78c58` 不得互相替代。本文以下旧候选编号均只作历史追溯。
...
> **候选切换记录**：本页下方仍保留旧候选文件名和历史验收段落，均不再代表当前运行包；最新运行相关源码候选为 `ded78c58`，当前 live 来源仍为 `02dbf10`。新候选已通过 pending 静态校验但尚未原子发布；40 个页面仍无本轮真机证据，不能沿用旧候选清单。
