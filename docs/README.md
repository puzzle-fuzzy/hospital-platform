> 最新事实（2026-08-27）：线上服务端 release 为 `1bc8b0a85f21cb58205a99ce4de0de6afe9bf240`；最新小程序运行相关源码和本地 live 运行输入均为 `34f0fd21aac33214e991de561d37dfd7071013bf`，共注册 40 个页面，本地回归为 `338 pass / 0 fail / 3707 expect()`。本轮只更新患者范围请求的底层输入门禁和门诊费用只读卡片层级，未修改服务端；旧 Python `8001` 未修改；公网 `test-hp.meiyi.pro` HTTPS 证书已恢复并启用自动续期，运行包已原子发布到 live `dist`，真实真机业务证据仍待采集。25 个 `surface-only` 页面及 4 个安全 `partial` 子集仍未开放对应真实业务，支付/医保入口保持关闭，健康自测仅开放安全数值子集，今日预约摘要已补齐但实时叫号仍关闭。本轮继续横向迁移，不伪造临床 Provider、物流、采血号源或公开记录数据。
...
> **源码同步备注（2026-08-27）**：`34f0fd21` 是最新运行相关源码和本地 live 运行包，在既有共享患者外壳、会话代际、二维码会话门禁和安全关闭态基础上，收紧报告/费用底层请求的患者范围、日期、详情引用和费用状态输入，并恢复门诊费用卡片的旧端信息层级。该候选已原子发布到本地 live `dist`，后续真机验收必须从 live 重新生成二维码并绑定本候选证据。
> 服务端与小程序继续采用分层发布；线上历史小程序 `13f597e` 与本地 live `34f0fd21` 不得互相替代。本文以下旧候选编号均只作历史追溯。
...
> **候选切换记录**：本页下方仍保留旧候选文件名和历史验收段落，均不再代表当前运行包；最新运行相关源码和当前 live 来源均为 `34f0fd21`。该候选已通过静态校验并完成原子发布；40 个页面仍无本轮真机业务证据，不能沿用旧候选清单。

> **当前候选覆盖（2026-08-27）**：最新小程序源码和本地 live 运行输入为 `34f0fd21`（完整来源 `34f0fd21aac33214e991de561d37dfd7071013bf`），40 页，回归 `338 pass / 0 fail / 3707 expect()`；共享患者外壳已补齐 owner 证明、会话代际和账号切换清理，会话失效时会清理旧 owner 微信资料缓存，报告/费用底层请求的患者范围门禁和门诊费用卡片层级已补齐。候选已完成原子发布，九个真机证据域仍为 pending。候选证据见 [`release/candidate-34f0fd21-miniprogram-runtime-2026-08-27.md`](release/candidate-34f0fd21-miniprogram-runtime-2026-08-27.md) 和 [`release/device-evidence-34f0fd21-pending.json`](release/device-evidence-34f0fd21-pending.json)。
...
> **当前全量迁移交接单（2026-08-27）**：请优先阅读 [`migration/full-migration-handoff-2026-08-25.md`](migration/full-migration-handoff-2026-08-25.md)。最新运行相关源码和本地 live 运行输入均为 `34f0fd21`；64 个旧页面均有明确落点，其中 8 个已替换、23 个安全 `partial` 子集、25 个 `surface-only` 页面外壳、7 个支付/回写入口仍按阻断原因关闭；健康自测仅开放 BMI/血压安全数值子集，今日预约摘要不冒充实时状态，协议正文已可读，但同意/撤回/审计仍关闭。本轮补齐报告/费用底层请求门禁和门诊费用卡片层级，其他安全边界继续保持；本页下方历史候选只作追溯。完整候选证据以 [`release/candidate-34f0fd21-miniprogram-runtime-2026-08-27.md`](release/candidate-34f0fd21-miniprogram-runtime-2026-08-27.md)、[`release/device-evidence-34f0fd21-pending.json`](release/device-evidence-34f0fd21-pending.json) 和 live 的 `build-info.json` 为准。

> **当前仓库交接基线（2026-08-27）**：本地 live 运行输入为 `34f0fd21aac33214e991de561d37dfd7071013bf`，运行包校验通过并完成原子发布；当前无 pending 目录。全部 9 个真机证据域仍为 `pending`，所以总体仍返回 `passed=false`。任何真实 `passed/failed` 证据仍必须绑定通过 `release:baseline:audit` 的线上 release。不要把本行当作新的线上服务部署记录。

> **当前 live 运行包**：`apps/miniprogram/dist/build-info.sourceRevision=34f0fd21aac33214e991de561d37dfd7071013bf`，40 页；live `runtime:verify` 已通过。回归为 `338 pass / 0 fail / 3707 expect()`；当前无 pending 目录，发布和回滚步骤仍见 [`release/pending-runtime-publication-runbook-2026-08-26.md`](release/pending-runtime-publication-runbook-2026-08-26.md)。
...
`34f0fd21aac33214e991de561d37dfd7071013bf`，当前 live `dist` 已完成原子切换。
...
`34f0fd21aac33214e991de561d37dfd7071013bf`。不能沿用线上来源或旧二维码。每次小程序源码/构建输入提交后都必须重新执行 build 和
...
| [`roadmap-next-phase.md`](roadmap-next-phase.md) | 业务、工程、运行和验收的下一阶段统一路线图；线上服务端为 `1bc8b0a8`，线上历史小程序包为 `13f597e`，当前本地 live `dist` 为 `34f0fd21` |
...
| [`release/current-5a31427-real-business-event-window-2026-08-21.md`](release/current-5a31427-real-business-event-window-2026-08-21.md) | 服务器真实微信登录/患者同步事件窗口；因早于当前 `34f0fd21` 完整小程序构建 `34f0fd21aac33214e991de561d37dfd7071013bf` 且来源未匹配，只作为历史观察；当前 live 另见 `34f0fd21`，不计入当前候选验收 |
