...
> **当前仓库交接基线（2026-08-27）**：本地 live 运行输入为 `935410473e5a7c1be125a85834f957f53a833d8f`，运行包校验通过并完成原子发布；启动保护候选已不再暂存于 pending。全部 9 个真机证据域仍为 `pending`，所以总体仍返回 `passed=false`。任何真实 `passed/failed` 证据仍必须绑定通过 `release:baseline:audit` 的线上 release。不要把本行当作新的线上服务部署记录。

> **当前 live 运行包**：`apps/miniprogram/dist/build-info.sourceRevision=935410473e5a7c1be125a85834f957f53a833d8f`，38 页；live `runtime:verify` 已通过。回归为 `356 pass / 0 fail / 3826 expect()`；当前没有 pending 运行包，后续源码变化仍须按发布和回滚步骤执行，详见 [`release/pending-runtime-publication-runbook-2026-08-26.md`](release/pending-runtime-publication-runbook-2026-08-26.md)。
