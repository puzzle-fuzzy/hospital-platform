# 服务端独立候选 `1bc8b0a8` 发布记录（2026-08-27）

> 本候选只更新新 Bun/Elysia API 运行包，不上传微信线上小程序版本。小程序配套使用本地
> live `34f0fd21`；服务端与小程序来源不同是正常的分层发布，不代表微信真机、众阳、HIS、支付或医保业务已经完成。
...
| 小程序客户端 | `34f0fd21` |
| 小程序构建来源 | `34f0fd21aac33214e991de561d37dfd7071013bf` |
...
| 产物 | SHA-256 |
| --- | --- |
| `apps/api/dist/index.js` | `86d40a26056ed5fa6e40ea1b15e614bfff240f717d7514d83cf48844a5e3f770` |
...
2026-08-27 约 13:12 CST，切换前确认 `current=90d8910bdc54d48dde66c4ff03a7434c182ebd92`、新 API active、旧 Python `8001` 监听、Worker inactive，且目标 release 与 `current.next` 均不存在。随后只执行同目录原子切换并重启新 API：
...
- `current` 指向 `/home/ps/code/hospital-platform/releases/1bc8b0a85f21cb58205a99ce4de0de6afe9bf240`；
...
使用同一 `1bc8b0a8` bundle 访问 `https://test-hp.meiyi.pro/api/v2`，HTTPS 证书校验通过：
...
`releases/90d8910bdc54d48dde66c4ff03a7434c182ebd92` 并只重启 `hospital-platform-api-v2.service`；不得停止旧 Python、删除 release、清理 Redis 或回滚 schema。
