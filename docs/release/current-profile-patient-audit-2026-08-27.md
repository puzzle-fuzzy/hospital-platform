...
1. `App.onLaunch` 通过 `ensureGlobalUserProfile(this)` 启动唯一的全局资料初始化；“我的”页和资料页通过 `waitForGlobalUserProfile()` 消费同一份快照，“我的”页还通过订阅接收昵称、头像和授权状态变化。
...
| `pnpm --filter @hospital/miniprogram runtime:verify:pending` | 通过 | 历史发布前 pending 来源为 `0be59f966de2c3a0861cb44e9a526a1ef557f6c7`，历史运行包页数不作为当前事实；当前 live 候选为 `3f8274ec5435779c0603ce8475a4f4e86d292cbd` |
| `pnpm --filter @hospital/miniprogram build` | 发布阶段曾被 `EBUSY` 阻断 | TypeScript 检查已通过；随后释放项目进程锁并通过 `runtime:publish-pending` 完成原子发布 |
| `runtime:publish-pending` + `runtime:verify` | 通过 | 历史 pending 已清理；当前 live 来源为 `3f8274ec5435779c0603ce8475a4f4e86d292cbd`，38 页 |
