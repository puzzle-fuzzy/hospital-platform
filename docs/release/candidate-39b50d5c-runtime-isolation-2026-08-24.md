...
## 2026-08-24 21:31 CST 工具实例复核

本次现场复核确认，底栏闪动不应继续通过页面代码或新增自绘底栏处理。Windows
进程检查曾同时发现两个 `apps/miniprogram` 文件监听实例，以及一个误打开的
上层工程实例；这种状态会让开发者工具把不同工程/增量页面图混在同一次编译中，
从而出现底栏短暂重建、四项都显示未选中和旧页面脚本 404。

已执行以下恢复动作：

1. 关闭上层 `E:\__Super_Core__\hospital-platform` 和重复的小程序工程实例；
2. 退出开发者工具后，只重新打开 `E:\__Super_Core__\hospital-platform\apps\miniprogram`；
3. 重置该工程文件工具缓存；
4. 重新构建当前候选并通过原生 Tab 定向门禁。

当前检查结果：只有一个 `apps/miniprogram` 运行工程监听实例；运行根仍为
`dist/`，`dist/build-info.json.sourceRevision` 仍为完整的
`39b50d5c4287f54ecc24e8564e2dc811a55c1d1b`。本次没有修改旧 Python 服务、服务器、
数据库、Redis 或线上小程序。请在新打开的工程中执行一次“普通编译”后再进行真机
扫码；若仍能看到闪动，必须同时提供编译窗口工程路径和真机画面，才能区分缓存/工具
边界问题与微信基础库自身的过渡动画。
