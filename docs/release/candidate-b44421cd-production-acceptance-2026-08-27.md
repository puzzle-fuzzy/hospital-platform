> 当前配套小程序运行包（2026-08-27）：本地 live `dist` 的 sourceRevision 为 `d4f67485a34195a2e1e392071502cf2a7006dd27`（`d4f6748`），共 40 个页面；开发者工具已打开并已生成本候选预览二维码，但手机业务证据尚未采集，九个真机证据域仍为 `pending`。本文下方历史候选仅作追溯。
...
本记录不产生真机业务成功结论。当前开发者工具已打开并已从 `apps/miniprogram/dist/` 生成本候选预览二维码，但手机和业务证据尚未完成；扫码后仍需逐域采集页面、客户端 requestId、服务端 Pino 和适用 Provider 低敏 requestId。
