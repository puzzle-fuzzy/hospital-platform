# 小程序候选 `39e2caa` 本地构建记录（2026-08-21）
...
| 服务端 release | `0e360d3` |
| 小程序客户端 | `39e2caa` |
| 小程序构建来源 | `39e2caac27ec4bf569f5debc8edb47947bc398d2` |
...
选择就诊人页增加页面实例级 `onShow` 生命周期门禁：首次展示不重复 `onLoad`
读取；从页面栈返回时先清除旧的 owner-scoped 患者目录，再重新执行目录读取和临床
映射同步；如果平台会话已经失效，则清理医疗派生数据并回首页重新确认微信账号。
这样可以避免页面停留期间发生会话代际变化时，旧姓名、关系、脱敏卡号继续显示。

本候选没有修改 Provider 请求参数、支付/医保、患者新增绑定、旧 Python 服务、线上配置、
数据库表结构或众阳自动化获取任务。同步仍使用既有客户端单飞和服务端幂等边界。

## 构建与门禁

- `pnpm --filter @hospital/miniprogram typecheck`：通过。
- 小程序完整测试：169 项通过，0 项失败，1342 个断言。
