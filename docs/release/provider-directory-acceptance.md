...
当前生产 `current` 为 `9833a01`，对应新 API `hospital-platform-api-v2.service`；旧 Python
服务仍由原端口独立提供，未被本次发布替换。`9833a01` 发布后的 SSH、候选启动、公网健康检查、
认证边界和旧服务共存证据，统一见 [`9833a01-production-acceptance-2026-08-17.md`](9833a01-production-acceptance-2026-08-17.md)。

截至 `9833a01` 切换窗口结束，当前 release 取得了服务启动、schema migration、健康探针、system-ping 和未登录边界请求，
但尚未在该版本取得新的微信登录、患者目录、预约、报告或门诊费用业务请求。`ca5a372` 及更早 release 的真实微信登录、患者同步、
预约科室和排班读取属于历史证据，不能直接标记当前 `9833a01` 的业务 gate；后续真机或公网业务验收必须重新记录当前 release、`traceId`、`requestId` 和脱敏响应摘要。
...
当前 `9833a01` 验收窗口尚未取得预约历史业务请求，因此预约历史不得标记为完整验收。
...
- 迁移阶段 release：`b1b84d7`、`ca3a877`；当日历史生产 release：`b186098`。该段记录用于解释
  ...
  [`9833a01-production-acceptance-2026-08-17.md`](9833a01-production-acceptance-2026-08-17.md)为准。
