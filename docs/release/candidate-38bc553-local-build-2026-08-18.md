...
2026-08-18 14:14 CST 点击首页患者卡二维码图标后，模拟器展示“二维码暂未开放”弹窗，说明医院扫码字段和有效期协议
...
2026-08-18 14:22 CST，从“我的 → 家庭成员管理”进入独立 `patient-select` 页面，患者目录读取完成，页面展示当前
就诊人的脱敏电子就诊卡、当前标记和“刷新就诊人”入口。本轮没有新增、绑定或切换患者；当前会话只有 1 条已同步患者，
因此该证据只覆盖独立选择页导航和单患者当前态，不覆盖多患者切换、患者失效恢复或新增绑定。

2026-08-18 14:26 CST，重新打开 `apps/miniprogram/` 项目并进入爽约记录页，患者卡、页面标题、空态卡片和底部说明
均按 `9ef8c85` 补齐的页面级 WXSS 正常显示。该结果完成了开发者工具缓存清理后的视觉复核，但仍不是微信真机验收。
