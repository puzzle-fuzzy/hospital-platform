...
| `packages/persistence/src/repositories.ts` | `283dabb5` | 暴露人工复核状态更新仓储操作 | 仓库候选有，线上 release 无 |
...
- 仓库中的 9 个文件属于下一次服务端候选，当前不可作为线上日志字段、人工复核状态或 `0017` schema 已存在的证据。
...
1. 由明确的发布窗口负责人确认是否需要这 9 个运行时代码文件及 `0017` migration 进入线上。
