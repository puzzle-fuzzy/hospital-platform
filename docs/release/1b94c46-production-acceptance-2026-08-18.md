...
- 原生小程序当前配套候选为 `77588b5`，构建来源指纹为
  `77588b566d98facfac7b1d952e41d8db875278d4`；本次 API 发布没有替代或上传小程序运行包。
...
下一步使用 `77588b5` 小程序候选完成真实微信会话、患者切换和预约/费用只读三层证据；随后用专用账号完成普通资料首次 PUT、版本递增和 409 冲突验收。支付、医保、HIS 和报告继续最后处理。
