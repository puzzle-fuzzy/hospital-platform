...
- 支付、医保、HIS 写入、预约写入、报告 gate 和 Worker 继续关闭；
 - 原生小程序当前配套候选为 `9b88cf1`，构建来源指纹为
   `9b88cf105314965837260b2d77671939ac22c828`；本次 API 发布没有替代或上传小程序运行包。
...
下一步使用 `9b88cf1` 小程序候选完成真实微信会话、患者切换和预约/费用只读三层证据；随后用专用账号完成普通资料首次 PUT、版本递增和 409 冲突验收。支付、医保、HIS 和报告继续最后处理。
