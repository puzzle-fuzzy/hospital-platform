...
- 原生小程序当前配套候选为 `3f0f9e0`，构建来源指纹为
  `3f0f9e05f697364c9a69163da6a4f718302516e2`；本次 API 发布没有替代或上传小程序运行包。
...
下一步使用 `3f0f9e0` 小程序候选完成真实微信会话、患者切换和预约/费用只读三层证据；随后用专用账号完成普通资料首次 PUT、版本递增和 409 冲突验收。支付、医保、HIS 和报告继续最后处理。
