...
`379eae2a528265df900030cea6e8dc45a82902c1`（当前本地候选，尚未上传线上）；线上只证明运行层和认证边界，域级 Provider/真机证据仍需独立闭环。
...
本次交付候选固定为小程序 `379eae2`，完整运行包来源为
`379eae2a528265df900030cea6e8dc45a82902c1`；本节如保留更早审计窗口的来源文字，仅用于追溯，不能作为当前真机包。
...
本次重新构建的小程序候选为 `379eae2`，完整来源指纹为
`379eae2a528265df900030cea6e8dc45a82902c1`；运行输入在构建和 `runtime:verify` 前必须处于已提交状态，预约记录错误态会关闭院区弹层，报告详情页与其它患者范围页面共用患者上下文错误翻译入口，报告目录错误态也会清空列表派生计数和分页标记；首页会话恢复期间先隐藏旧患者派生展示。
...
- `pnpm --filter @hospital/miniprogram build`：类型检查通过，14 个页面脚本生成；候选 `dist/build-info.json` 来源指纹为
  `379eae2a528265df900030cea6e8dc45a82902c1`；
...
1. 使用与服务端 `b7c9451` 配套、来源指纹为 `379eae2a528265df900030cea6e8dc45a82902c1` 的小程序运行包，在有效微信会话下完成登录、患者刷新/显式切换、我的挂号、
   爽约记录和门诊待缴/已缴页面操作；
