...
> `13f597ea9ee3f65b9be858117826d948339d904a`。历史 contract 审计仍保留，但业务证据必须重新绑定当前候选。

> 历史候选：服务端 release `7181e99e3a352244102f5591279528b3b66332c9`；小程序运行包来源 `4e1b2e224964797c103eba832323ee7074c7ad2b`（提交 `4e1b2e2`）。

> 当前基线更新：服务端 `28a5c0c131794ce9dcc5f94bd3809402188ac87a`；小程序候选 `13f597e`；完整运行包来源 `13f597ea9ee3f65b9be858117826d948339d904a`。下文更早候选只作历史追溯。

本文记录当前源码对“预约历史 / 爽约记录 / 门诊缴费只读查询”的逻辑审计结果。
它用于新会话、代码评审和真机验收前的边界复核；“代码和测试通过”不等于
真实微信会话、Provider、公网 HTTPS 或真机业务已经验收。

当前线上服务端 release 为 `28a5c0c1`，小程序构建来源为
`13f597ea9ee3f65b9be858117826d948339d904a`（当前候选 `13f597e`）；线上只证明运行层和认证边界，域级 Provider/真机证据仍需独立闭环。
...
`cac6561b3f4ebbae2de8c632b052837fe7bc28b6`（提交 `cac6561`），服务端为 `b44421cd321ff9ff23eeb49b12641d1772d2bdc1`。新 Bun/Elysia API 与旧 Python API 共存，旧服务没有被停止。当前只读发布已完成，历史 release 的窄观察窗口没有新的
...
`cac6561b3f4ebbae2de8c632b052837fe7bc28b6`（提交 `cac6561`）为准。当前真机候选以
`cac6561` 小程序候选和服务端 `b44421cd` 的分层配套及完整来源指纹 `cac6561b3f4ebbae2de8c632b052837fe7bc28b6` 为准。
...
本次重新构建的小程序候选为 `d2086d8`，完整来源指纹为
`d2086d819b3e393da2e8c5c39d7704012854214b`；运行输入在构建和 `runtime:verify` 前必须处于已提交状态，预约记录错误态会关闭院区弹层，报告详情页与其它患者范围页面共用患者上下文错误翻译入口，报告目录错误态也会清空列表派生计数和分页标记；首页会话恢复期间先隐藏旧患者派生展示。

- `pnpm --filter @hospital/miniprogram test`：126 项通过，1066 个断言；
- `pnpm --filter @hospital/miniprogram build`：类型检查通过，14 个页面脚本生成；候选 `dist/build-info.json` 来源指纹为
  `d2086d819b3e393da2e8c5c39d7704012854214b`；
...
1. 使用与服务端 `5a31427` 配套、来源指纹为 `f9833c11ef0d49591fd14f99ce60ad92d800156a` 的小程序运行包，在有效微信会话下完成登录、患者刷新/显式切换、我的挂号、
   爽约记录和门诊待缴/已缴页面操作；
