...
`29029175a5064aa4480359255e74d73dc010b3cd`；线上只证明运行层和认证边界，域级 Provider/真机证据仍需独立闭环。
...
本次交付候选固定为小程序 `2902917`，完整运行包来源为
`29029175a5064aa4480359255e74d73dc010b3cd`；本节如保留更早审计窗口的来源文字，仅用于追溯，不能作为当前真机包。
...
本次重新构建的小程序候选为 `2902917`，完整来源指纹为
`29029175a5064aa4480359255e74d73dc010b3cd`；预约记录错误态现在会关闭院区弹层，报告详情页与其它患者范围页面共用患者上下文错误翻译入口，报告目录错误态也会清空列表派生计数和分页标记。

- `pnpm --filter @hospital/miniprogram test`：124 项通过，1060 个断言；
- `pnpm --filter @hospital/miniprogram build`：类型检查通过，14 个页面脚本生成；候选 `dist/build-info.json` 来源指纹为
  `29029175a5064aa4480359255e74d73dc010b3cd`；
...
1. 使用与服务端 `c26e696` 配套、来源指纹为 `29029175a5064aa4480359255e74d73dc010b3cd` 的小程序运行包，在有效微信会话下完成登录、患者刷新/显式切换、我的挂号、
   爽约记录和门诊待缴/已缴页面操作；
