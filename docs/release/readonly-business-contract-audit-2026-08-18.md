...
`39e2caac27ec4bf569f5debc8edb47947bc398d2`（当前本地候选 `39e2caa`，尚未上传线上）；线上只证明运行层和认证边界，域级 Provider/真机证据仍需独立闭环。
...
`39e2caac27ec4bf569f5debc8edb47947bc398d2`（提交 `39e2caa`）。新 Bun/Elysia API 与旧 Python API 共存，旧服务没有被停止。当前 `0e360d3` release 的窄观察窗口没有新的
...
`0e360d3` 配套的 `39e2caa` 和完整来源指纹 `39e2caac27ec4bf569f5debc8edb47947bc398d2` 为准。
...
1. 使用与服务端 `0e360d3` 配套、来源指纹为 `39e2caac27ec4bf569f5debc8edb47947bc398d2` 的小程序运行包，在有效微信会话下完成登录、患者刷新/显式切换、我的挂号、
   爽约记录和门诊待缴/已缴页面操作；
