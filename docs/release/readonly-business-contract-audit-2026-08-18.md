...
为准；配套小程序构建来源为 `07d39882dd7bbd71f7b9d5ea83c27ca0c1e8c7af`：新 Bun/Elysia API 与旧 Python API 共存，旧服务没有被停止。当前 release 的窄观察窗口没有新的
...
服务端生产候选固定为 `1b94c46`，小程序当前运行输入来源为 `07d39882dd7bbd71f7b9d5ea83c27ca0c1e8c7af`，包含微信身份边界修正、患者引用 fail-closed 修正、空目录下已有选择的 stale 修正、同步回写不能覆盖 stale/unavailable 的状态门禁、精确运行包来源输入校验、选择页手动刷新事件边界、会话代际隔离修正、预约目录两阶段查询边界、刷新期间科室/日期事件校验、挂号卡片操作事件 key 回查、报告目录详情事件 key 回查、报告详情患者范围复核、“我的”页未迁移菜单稳定 key 和根入口全局脚本门禁；小程序修正未改变旧服务。
本节计数于 2026-08-18 当前工作树重新执行取得，不把更早审计窗口的测试数字继续当作当前证据：

- `pnpm --filter @hospital/miniprogram test`：121 项通过，1035 个断言；
- `pnpm --filter @hospital/miniprogram build`：类型检查通过，14 个页面脚本生成；候选 `dist/build-info.json` 来源指纹为
  `07d39882dd7bbd71f7b9d5ea83c27ca0c1e8c7af`；
...
1. 使用与服务端 `1b94c46` 配套、来源指纹为 `07d39882dd7bbd71f7b9d5ea83c27ca0c1e8c7af` 的小程序运行包，在有效微信会话下完成登录、患者刷新/显式切换、我的挂号、
   爽约记录和门诊待缴/已缴页面操作；
