...
当前线上服务端 release 为 `0e360d3`，小程序构建来源为
`6252ea41a40a22a6264276b6590750e4bf0ef5bc`（当前本地候选 `6252ea4`，尚未上传线上）；线上只证明运行层和认证边界，域级 Provider/真机证据仍需独立闭环。
...
当前线上 release 以 [`0e360d3-production-acceptance-2026-08-20.md`](0e360d3-production-acceptance-2026-08-20.md)
为准；本文记录的 2026-08-18 历史审计使用小程序来源
`d2086d819b3e393da2e8c5c39d7704012854214b`，不作为当前真机候选。当前配套候选为
`6252ea41a40a22a6264276b6590750e4bf0ef5bc`（提交 `6252ea4`）。新 Bun/Elysia API 与旧 Python API 共存，旧服务没有被停止。当前 `0e360d3` release 的窄观察窗口没有新的
...
`0e360d3` 配套的 `6252ea4` 和完整来源指纹 `6252ea41a40a22a6264276b6590750e4bf0ef5bc` 为准。

本节对应的当前服务端生产候选为 `0e360d3`；下方保留的 `398be8e`、`687690e`、`c26e696` 和 `1b94c46` 说明仅用于追溯当时的代码摘要，不能覆盖当前发布基线。
...
1. 使用与服务端 `0e360d3` 配套、来源指纹为 `6252ea41a40a22a6264276b6590750e4bf0ef5bc` 的小程序运行包，在有效微信会话下完成登录、患者刷新/显式切换、我的挂号、
   爽约记录和门诊待缴/已缴页面操作；
