...
`69e6cdb28f2996095b8647f82b1b90afd061b918`（当前本地候选 `69e6cdb`，尚未上传线上）；线上只证明运行层和认证边界，域级 Provider/真机证据仍需独立闭环。
...
1. 使用与服务端 `b7c9451` 配套、来源指纹为 `69e6cdb28f2996095b8647f82b1b90afd061b918` 的小程序运行包，在有效微信会话下完成登录、患者刷新/显式切换、我的挂号、
   爽约记录和门诊待缴/已缴页面操作；
