...
`ac238c6156f085fdb56f5806fefac3613e5f85be`（当前本地候选 `ac238c6`，尚未上传线上）；线上只证明运行层和认证边界，域级 Provider/真机证据仍需独立闭环。
...
`ac238c6156f085fdb56f5806fefac3613e5f85be`（提交 `ac238c6`）。新 Bun/Elysia API 与旧 Python API 共存，旧服务没有被停止。当前 `0e360d3` release 的窄观察窗口没有新的
...
`0e360d3` 配套的 `ac238c6` 和完整来源指纹 `ac238c6156f085fdb56f5806fefac3613e5f85be` 为准。
...
1. 使用与服务端 `0e360d3` 配套、来源指纹为 `ac238c6156f085fdb56f5806fefac3613e5f85be` 的小程序运行包，在有效微信会话下完成登录、患者刷新/显式切换、我的挂号、
   爽约记录和门诊待缴/已缴页面操作；
