...
当前线上服务端 release 为 `398be8e`，小程序构建来源为
`474b0444736599c848a4cef9f47fd930884e401d`（当前本地候选 `474b044`，尚未上传线上）；线上只证明运行层和认证边界，域级 Provider/真机证据仍需独立闭环。
...
当前线上 release 以 [`398be8e-production-acceptance-2026-08-19.md`](398be8e-production-acceptance-2026-08-19.md)
为准；本文记录的 2026-08-18 历史审计使用小程序来源
`d2086d819b3e393da2e8c5c39d7704012854214b`，不作为当前真机候选。当前配套候选为
`474b0444736599c848a4cef9f47fd930884e401d`（提交 `474b044`）。新 Bun/Elysia API 与旧 Python API 共存，旧服务没有被停止。当前 `398be8e` release 的窄观察窗口没有新的
...
`398be8e` 配套的 `474b044` 和完整来源指纹为准。

本节对应的当前服务端生产候选为 `398be8e`；下方保留的 `687690e`、`c26e696` 和 `1b94c46` 说明仅用于追溯当时的代码摘要，不能覆盖当前发布基线。
...
1. 使用与服务端 `398be8e` 配套、来源指纹为 `474b0444736599c848a4cef9f47fd930884e401d` 的小程序运行包，在有效微信会话下完成登录、患者刷新/显式切换、我的挂号、
   爽约记录和门诊待缴/已缴页面操作；
