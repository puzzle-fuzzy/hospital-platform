...
> 当前完整小程序来源校验值：`a33416d8417661fa5256deb22df55a97456cc608`；当前服务端 release：`9f479c9a`。

> 当前发布基线（2026-08-22）：服务端 `9f479c9a`；小程序运行包来源
> `a33416d8417661fa5256deb22df55a97456cc608`。历史 contract 审计仍保留，但业务证据必须重新绑定当前候选。
...
`a33416d8417661fa5256deb22df55a97456cc608`（当前本地候选 `a33416d8`，尚未上传线上）；线上只证明运行层和认证边界，域级 Provider/真机证据仍需独立闭环。
...
`a33416d8417661fa5256deb22df55a97456cc608`（提交 `a33416d8`）。新 Bun/Elysia API 与旧 Python API 共存，旧服务没有被停止。当前 `9f479c9a` 只读发布已完成，历史 `0e360d3` release 的窄观察窗口没有新的
...
`a33416d8417661fa5256deb22df55a97456cc608`（提交 `a33416d8`）为准。当前真机候选以
`9f479c9a` 配套的 `a33416d8` 和完整来源指纹 `a33416d8417661fa5256deb22df55a97456cc608` 为准。
