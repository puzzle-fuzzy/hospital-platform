...
`457d9aee567bc77c33279a9b61db921e3011f1c1`（当前本地候选 `457d9ae`，尚未上传线上）；线上只证明运行层和认证边界，域级 Provider/真机证据仍需独立闭环。
...
`457d9aee567bc77c33279a9b61db921e3011f1c1`（提交 `457d9ae`）。新 Bun/Elysia API 与旧 Python API 共存，旧服务没有被停止。当前 `0e360d3` release 的窄观察窗口没有新的
...
`0e360d3` 配套的 `457d9ae` 和完整来源指纹 `457d9aee567bc77c33279a9b61db921e3011f1c1` 为准。
...
1. 使用与服务端 `0e360d3` 配套、来源指纹为 `457d9aee567bc77c33279a9b61db921e3011f1c1` 的小程序运行包，在有效微信会话下完成登录、患者刷新/显式切换、我的挂号、
   爽约记录和门诊待缴/已缴页面操作；
