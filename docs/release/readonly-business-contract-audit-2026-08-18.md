...
`482288496c6de90ff86fb2f2eb54db3b9ae0bae5`（提交 `4822884`）。新 Bun/Elysia API 与旧 Python API 共存，旧服务没有被停止。当前 `65219e2` release 的窄观察窗口没有新的
...
`65219e2` 配套的 `4822884` 和完整来源指纹为准。

本节对应的当前服务端生产候选为 `65219e2`；下方保留的 `687690e`、`c26e696` 和 `1b94c46` 说明仅用于追溯当时的代码摘要，不能覆盖当前发布基线。
...
1. 使用与服务端 `65219e2` 配套、来源指纹为 `482288496c6de90ff86fb2f2eb54db3b9ae0bae5` 的小程序运行包，在有效微信会话下完成登录、患者刷新/显式切换、我的挂号、
   爽约记录和门诊待缴/已缴页面操作；
