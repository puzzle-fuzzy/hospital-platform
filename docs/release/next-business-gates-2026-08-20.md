...
> 当前服务端候选为 `5a31427`，当前本地小程序候选为 `cdb27e50`，完整运行包来源为
> `cdb27e5023a188ab36a340497cebe18f1e274013`。小程序尚未上传线上。
>
...
这不是 Provider 失败，也不是业务成功；下一步仍必须使用当前 `cdb27e50` 候选重新扫码，取得同一会话的页面、客户端请求和服务端日志三层证据。
...
本次代码复核基于服务端 `5a31427`、小程序候选 `cdb27e50`（完整来源 `cdb27e5023a188ab36a340497cebe18f1e274013`）；本节只检查代码、领域 contract、adapter、页面状态机和本地测试，
