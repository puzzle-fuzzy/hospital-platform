...
> 当前服务端候选为 `5a31427`，当前本地小程序候选为 `ec3e5f09`，完整运行包来源为
> `ec3e5f094ecd0a85bbf98d4fc6ff9f14b27bb9ac`。小程序尚未上传线上。
>
...
这不是 Provider 失败，也不是业务成功；下一步仍必须使用当前 `ec3e5f09` 候选重新扫码，取得同一会话的页面、客户端请求和服务端日志三层证据。
...
三层证据仍未变化，下一步应使用当前 `ec3e5f09` 重新编译并生成二维码后开始人工操作。
...
后续真机二维码必须重新基于当前 `ec3e5f09` 来源生成；逻辑审计见
...
[`candidate-ec3e5f0-local-build-2026-08-21.md`](candidate-ec3e5f0-local-build-2026-08-21.md)。
...
本次代码复核基于服务端 `5a31427`、小程序候选 `ec3e5f09`（完整来源 `ec3e5f094ecd0a85bbf98d4fc6ff9f14b27bb9ac`）；本节只检查代码、领域 contract、adapter、页面状态机和本地测试，
