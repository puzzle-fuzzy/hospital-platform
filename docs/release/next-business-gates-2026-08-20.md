...
该窗口发生在 `9f491cb5` 切换前，下面的旧候选只作为历史观察；当前下一步必须使用顶部声明的 `13b86a5` 候选重新扫码，取得同一会话的页面、客户端请求和服务端日志三层证据。
...
本次代码复核基于服务端 `9f491cb5ac813acf89ed1f2f4afb361517e82324`、小程序候选 `13b86a5`（完整来源 `13b86a5a400ca0ccbee67abdfed726476a4749d4`）；本节只检查代码、领域 contract、adapter、页面状态机和本地测试，
