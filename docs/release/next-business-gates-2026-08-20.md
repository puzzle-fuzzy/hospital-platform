...
> 当前服务端候选为 `5a31427`，当前本地小程序候选为 `4f80bc2`，完整运行包来源为
> `4f80bc2f313fbcac9fc4e976006ae2c356ce2729`。小程序尚未上传线上。
>
...
这不是 Provider 失败，也不是业务成功；下一步仍必须使用当前 `4f80bc2` 候选重新扫码，取得同一会话的页面、客户端请求和服务端日志三层证据。
...
三层证据仍未变化，下一步应使用当前 `4f80bc2` 重新编译并生成二维码后开始人工操作。
...
后续真机二维码必须重新基于当前 `4f80bc2` 来源生成；逻辑审计见
...
必须使用当前 `4f80bc2` 重新普通编译并生成新二维码后，才可以开始真机业务证据采集。详见
历史证据见 [`candidate-9340846-local-build-2026-08-21.md`](candidate-9340846-local-build-2026-08-21.md)；当前候选请使用
[`candidate-4f80bc2-local-build-2026-08-21.md`](candidate-4f80bc2-local-build-2026-08-21.md)。
...
本次代码复核基于服务端 `5a31427`、小程序候选 `4f80bc2`（完整来源 `4f80bc2f313fbcac9fc4e976006ae2c356ce2729`）；本节只检查代码、领域 contract、adapter、页面状态机和本地测试，
...
- 当前候选证据记录模板：[`miniprogram-real-device-evidence-template-4f80bc2.md`](miniprogram-real-device-evidence-template-4f80bc2.md)
- 只读业务不变量：[`readonly-business-chain-audit-2026-08-20.md`](readonly-business-chain-audit-2026-08-20.md)
- 当前候选来源：[`candidate-4f80bc2-local-build-2026-08-21.md`](candidate-4f80bc2-local-build-2026-08-21.md)
