...
> 当前服务端候选为 `c8eef370c82e358205ee032af41ba2b23576af06`，当前本地小程序候选为 `f488c6f3`，完整运行包来源为
> `f488c6f3270514af10b19fdf3c45a47519e1736b`。小程序尚未上传线上。
>
> 本轮只维护新项目文档和执行顺序；不修改旧 Python 服务、不中断旧 `8001`、不写线上 MySQL/Redis，
> 也不触碰并行会话正在维护的众阳自动化代码。

## 2026-08-21 `c8eef370` 当前候选运行层复核

服务端已从 `5a31427` 原子切换到 `c8eef370`，只重启新 API；新 API `18081`、旧 Python `8001` 共存，Worker 保持 inactive。
真实 production preflight、隔离 `18082` runtime smoke 和公网 `/api/v2` smoke 均通过；完整发布证据见
[`c8eef370-production-acceptance-2026-08-21.md`](c8eef370-production-acceptance-2026-08-21.md)。
...
该窗口发生在 `c8eef370` 切换前，下面的旧候选只作为历史观察；当前下一步必须使用顶部声明的 `f488c6f3` 候选重新扫码，取得同一会话的页面、客户端请求和服务端日志三层证据。
...
本次代码复核基于服务端 `c8eef370c82e358205ee032af41ba2b23576af06`、小程序候选 `f488c6f3`（完整来源 `f488c6f3270514af10b19fdf3c45a47519e1736b`）；本节只检查代码、领域 contract、adapter、页面状态机和本地测试，
