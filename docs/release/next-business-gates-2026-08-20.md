...
> 当前执行基线（2026-08-24）：线上服务端 release 为 `6db3217bd3c990b009571ffd85b7da55d9ea7338`；本地待切换服务端候选为
...
> 当前执行基线（2026-08-24）：线上服务端 release 为 `6db3217b`；本地待切换服务端候选为 `13f597ea`；小程序运行包来源为
> `4ba492a3fdae8283409bd2ab4a0a45247c46600c`（提交 `4ba492a`）。历史候选仅作追溯。


> 当前线上候选：服务端 release `6db3217bd3c990b009571ffd85b7da55d9ea7338`；小程序运行包来源 `4ba492a3fdae8283409bd2ab4a0a45247c46600c`（提交 `4ba492a`）。预约历史修正候选 `13f597ea` 尚未切换。

> 本文是新会话继续工作的短入口，不替代各业务域的详细 contract、代码测试或真实验收记录。
> 当前线上服务端为 `6db3217bd3c990b009571ffd85b7da55d9ea7338`，当前小程序候选为 `4ba492a`，完整运行包来源为
> `4ba492a3fdae8283409bd2ab4a0a45247c46600c`。服务端已切换；小程序运行包已通过构建与运行包门禁，等待从当前候选重新生成二维码并扫码。
>
...
该窗口发生在 `9f479c9a` 切换前，下面的旧候选只作为历史观察；当前下一步必须使用顶部声明的 `4ba492a` 候选重新扫码，取得同一会话的页面、客户端请求和服务端日志三层证据。
...
当前验收基线为服务端 `6db3217bd3c990b009571ffd85b7da55d9ea7338`、小程序候选 `4ba492a`（完整来源 `4ba492a3fdae8283409bd2ab4a0a45247c46600c`）；
...
- 当前候选来源：[`candidate-4ba492a-local-build-2026-08-22.md`](candidate-4ba492a-local-build-2026-08-22.md)
