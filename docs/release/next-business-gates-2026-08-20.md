...
> 当前执行基线（2026-08-27）：线上新 API release 为
> `b44421cd321ff9ff23eeb49b12641d1772d2bdc1`；本地 live 小程序运行包来源为
> `76ca0137ea9a57b8b7ed9c8797bb718040535922`（`76ca013`），线上微信小程序仍为历史 `13f597e`。
> 线上服务端已完成 production preflight、隔离 smoke、原子切换、公网 runtime smoke 和 HTTPS 校验；
> 这只证明运行层，不代表 Provider、真机或支付业务完成。

> 本文是新会话继续工作的短入口，不替代各业务域的详细 contract、代码测试或真实验收记录。
> 当前线上服务端为 `b44421cd321ff9ff23eeb49b12641d1772d2bdc1`，当前本地小程序运行包来源为 `76ca0137ea9a57b8b7ed9c8797bb718040535922`。服务端和小程序已分别通过发布/运行包门禁，等待从当前项目重新生成二维码并扫码。
>
> 本轮只维护新项目文档和执行顺序；不修改旧 Python 服务、不中断旧 `8001`、不写线上 MySQL/Redis，
> 也不触碰并行会话正在维护的众阳自动化代码。

## 历史候选 `2a2acd9` 运行层复核（2026-08-22）
...
上面的 `2a2acd9` 仅是历史运行层事实；此前 `49f74e0`、`7181e99e` 的日志和小程序 `4e1b2e2` 均只作为历史追溯，不能与当前候选混用。
...
该窗口发生在 `9f479c9a` 切换前，下面的旧候选只作为历史观察；当前下一步必须使用顶部声明的 `13f597e` 运行包重新扫码，取得同一会话的页面、客户端请求和服务端日志三层证据。
...
三层证据仍未变化，下一步应使用当前 `cdb27e50` 重新编译并生成二维码后开始人工操作。
...
当前验收基线为服务端 `b44421cd321ff9ff23eeb49b12641d1772d2bdc1`、小程序候选 `76ca013`（完整来源 `76ca0137ea9a57b8b7ed9c8797bb718040535922`）；
...
### 本地验证证据（2026-08-22 当前工作树）

| 范围 | 结果 |
| --- | --- |
| API 全量测试（包含认证、患者、预约、费用、资料、报告、支付 gate 和日志） | 207 项通过，854 个断言 |
...
`dist/build-info.json` 为准。这些结果证明当前 fail-closed 代码边界一致，不证明当前 release 已取得 Provider、HTTPS、页面和真机三层业务证据。
...
> 当前服务端发布基线（2026-08-27）：`b44421cd321ff9ff23eeb49b12641d1772d2bdc1`；本执行板禁止使用历史 release 作为当前验收依据。
> 当前小程序配套运行包来源（2026-08-27）：`76ca0137ea9a57b8b7ed9c8797bb718040535922`（`76ca013`）；本文中更早候选和真机窗口仅作历史追溯，当前无真机/开发者工具会话。
> 当前统一发布基线（2026-08-27）：服务端 release 为 `b44421cd321ff9ff23eeb49b12641d1772d2bdc1`；小程序本地 live 运行包来源为 `76ca0137ea9a57b8b7ed9c8797bb718040535922`，共 40 个页面。本文更早版本仅作历史追溯，真机证据仍为 pending；旧 Python `8001` 未修改。
