...
同一时刻控制台出现一次未登录状态下预期的 `GET https://test-hp.meiyi.pro/api/v2/me 401`，但截图中仍能看到
患者姓名和脱敏卡号。按照当前小程序会话门禁，`/me` 失败后旧患者派生展示必须清空；由于这次只是模拟器窗口观察，
没有配套的成功 HTTP 链和服务端日志，不能把这张患者卡解释为当前会话已确认，也不能直接判断是代码问题还是开发者
工具热重载保留的页面状态。本条证据标记为 `待清洁重载/真机复核`，不计入微信登录或患者同步成功。
