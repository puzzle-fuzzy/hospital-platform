...
后续真机验收只接受小程序源码/运行包来源 `dc8cd5b8bbf99411831cc5112bc39683108cd990`，服务端配套基线为
`2a2acd9bcc89c35988b75fc03304dbd48078c9d5`。前文 `b0e0935`、`4e1b2e2` 和 `7181e99e` 的具体时间段均为历史证据，
不覆盖本节当前事实。

当前 `dist/build-info.json` 已验证为 `dc8cd5b8bbf99411831cc5112bc39683108cd990`，14 个页面齐全，运行包没有
`*.test.js`/`*.spec.js`。开发者工具中仍需关闭旧“真机调试”会话、清理编译缓存、普通编译并重新生成二维码；当前尚无手机页面与
服务端同链业务证据，不能标记真机验收完成。

## 2026-08-22 08:08 CST 当前候选运行包恢复

当前服务端已部署 `2a2acd9bcc89c35988b75fc03304dbd48078c9d5`，小程序当前运行包来源为
`b0e093565493285e07fe549879f8b87eda649cc7`。针对再次出现的 `dist/services/single-flight.test.js` ENOENT，
