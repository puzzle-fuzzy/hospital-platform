...
## 当前业务事件复核（2026-08-22 16:55:59 CST）

再次通过 SSH 只读检查当前 `current`、监听和新 API journald：

- `current` 仍指向本 release；新 API 监听 `10.0.0.3:18081`，旧 Python 继续监听 `0.0.0.0:8001`；
- 从本次切换时间 `16:37:00 CST` 到 `16:55:59 CST` 的新 API journald 共 13 行；
- 未检出 `auth.wechat`、`patient.*`、`appointment.*`、`outpatient.*`、`user.profile.*` 或 `report.*` 业务事件。

因此当前候选仍没有可绑定到手机页面的业务 trace。后续必须由真实微信会话产生操作，再按页面、客户端
requestId/HTTP、服务端低敏日志三层同链规则验收；不能用本窗口的“无事件”推断业务成功或失败。
