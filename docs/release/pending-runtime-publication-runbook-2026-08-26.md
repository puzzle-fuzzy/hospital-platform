...
| pending 候选来源 | `a9963729f905030c0982f332625289ff1cd51216`（`a9963729`），40 个页面；已通过 `runtime:verify:pending` |
...
> 当前 live 仍是 `02dbf10`，待发布候选是 `a9963729`。根目录文档提交不会改变运行包来源；
> 新候选构建已按
...
发布只证明运行包切换，不证明业务完成。当前仍需先将 `a9963729` 原子切换到 live；切换后
重新生成二维码，再按以下顺序逐域采证：
