...
| pending 候选来源 | `d27bddf04a5535520eed2770d7befd447bf61f9b`（`d27bddf0`），40 个页面；已通过 `runtime:verify:pending` |
...
> 当前 live 仍是 `02dbf10`，待发布候选是 `d27bddf0`。根目录文档提交不会改变运行包来源；
> 新候选构建已按
...
发布只证明运行包切换，不证明业务完成。当前仍需先将 `d27bddf0` 原子切换到 live；切换后
重新生成二维码，再按以下顺序逐域采证：
