...
| pending 候选来源 | `6795c2c3f240d6ec092000d34cf71509d81217ff`（`6795c2c3`），40 个页面；已通过 `runtime:verify:pending` |
...
> 当前 live 仍是 `02dbf10`，待发布候选是 `6795c2c3`。根目录文档提交不会改变运行包来源；
> 新候选构建已按
...
发布只证明运行包切换，不证明业务完成。当前仍需先将 `6795c2c3` 原子切换到 live；切换后
重新生成二维码，再按以下顺序逐域采证：
