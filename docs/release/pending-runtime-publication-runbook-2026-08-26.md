...
| pending 候选来源 | `9e94dc6732a032b1e2f19da21047b49845e08a91`（`9e94dc6`），40 个页面；已通过 `runtime:verify:pending` |
...
> 当前 live 仍是 `02dbf10`，待发布候选是 `9e94dc6`。根目录文档提交不会改变运行包来源；
> 新候选构建已按
...
发布只证明运行包切换，不证明业务完成。当前仍需先将 `9e94dc6` 原子切换到 live；切换后
重新生成二维码，再按以下顺序逐域采证：
