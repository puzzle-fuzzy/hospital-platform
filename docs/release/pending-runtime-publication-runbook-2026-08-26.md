...
| pending 候选来源 | `e059788e7cd9270547077e7d6e6890aa168aacf0`（`e059788e`），40 个页面；已通过 `runtime:verify:pending` |
...
> 当前 live 仍是 `02dbf10`，待发布候选是 `e059788e`。根目录文档提交不会改变运行包来源；
> 新候选构建已按
...
发布只证明运行包切换，不证明业务完成。当前仍需先将 `e059788e` 原子切换到 live；切换后
重新生成二维码，再按以下顺序逐域采证：
