...
>
> **当前仓库候选补充**：`main=43514fcf` 已推送但尚未部署到线上 API；本轮 API 运行时代码变更来源为 `eb4d2eb4`，新增健康知识服务直调关系查询白名单。因此 `pnpm release:baseline:audit` 当前应阻断，直到完成 API-only 原子发布并取得远端证据。本文的线上 release 事实不能反向覆盖当前工作树，也不能把本地测试结果写成线上业务验收。
