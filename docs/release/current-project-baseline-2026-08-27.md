...
- 九个真机证据域仍为 `pending`，见 [`device-evidence-413cbea1-pending.json`](device-evidence-413cbea1-pending.json)。清单结构通过不等于业务通过。
...
pnpm device:evidence:audit --file docs/release/device-evidence-413cbea1-pending.json
pnpm check
```
