...
>
> 时间窗口说明：本文记录的是 `2026-08-19 16:57 CST` 的历史运行复核。当时配套小程序来源仍为
> `48ba22f`；随后当前本地真机候选已更新为 `474b044`，完整来源为
> `474b0444736599c848a4cef9f47fd930884e401d`。本文保留原始运行层事实，不得作为当前二维码或真机验收的候选来源。
...
- 小程序构建：`14` 个 `app.json` 页面脚本均生成；本条只记录该历史窗口当时观察到的来源 `48ba22f`，不覆盖当前 `474b044` 候选。
