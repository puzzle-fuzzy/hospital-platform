# 当前小程序 pending 运行包候选（cd26a01）

> 本记录描述 `cd26a01` 生成的 pending 运行包。它尚未发布到微信开发者工具、线上服务或真机，不代表真实业务验收通过。

## 候选事实

| 项目 | 当前值 |
| --- | --- |
| 小程序源码提交 | `cd26a01febf81729516185c79ee76f250b4541c0` |
| pending 位置 | `.local/hospital-miniprogram/pending/` |
| 页面数量 | 20 |
| pending 来源 | `build-info.json.sourceRevision` |
| 真机证据清单 | [`device-evidence-cd26a01-pending.json`](device-evidence-cd26a01-pending.json)，9 个域均为 `pending` |
| 服务端候选 | `b42922f`，尚未与线上旧 release 切换 |
| 当前 live `dist` | `fcc6630ebfa7b0697cbd03a5e376ce6765d1643b`，仍被微信开发者工具占用 |

## 本批迁移覆盖变更

- 统一状态页现在展示迁移阶段、覆盖的旧端入口、业务域和下一步准入材料；
- 状态入口继续返回共享的“医疗服务”主 Tab；
- 迁移覆盖聚合仍只读取旧端台账，不复制业务事实，也不打开任何阻断能力；
- 没有打开 Provider、临床、患者绑定、外部 WebView、支付、医保或 HIS 回写能力。

## 本地验证

- `pnpm --filter @hospital/miniprogram typecheck`：通过；
- 当前小程序测试：`277 pass / 0 fail / 2918 expect()`；
