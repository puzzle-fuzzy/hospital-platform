...
> 当前本地候选（2026-08-24）：服务端线上 release 为 `6db3217bd3c990b009571ffd85b7da55d9ea7338`；本地完整代码候选为 `13f597ea9ee3f65b9be858117826d948339d904a`；小程序运行包来源为 `4ba492a3fdae8283409bd2ab4a0a45247c46600c`（提交 `4ba492a`）。候选已完成隔离检查但尚未切换线上，真实资料读写真机证据仍待。

# 普通资料当前候选读写验收协议（2026-08-22）
> 当前执行基线（2026-08-24）：线上服务端为 `6db3217bd3c990b009571ffd85b7da55d9ea7338`；本地待切换服务端候选为 `13f597ea9ee3f65b9be858117826d948339d904a`；小程序来源为 `4ba492a3fdae8283409bd2ab4a0a45247c46600c`。本协议仍要求真实微信/真机证据。

> 线上配套服务端 release：`6db3217bd3c990b009571ffd85b7da55d9ea7338`；当前小程序运行包来源：
> `4ba492a3fdae8283409bd2ab4a0a45247c46600c`（提交 `4ba492a`）。本协议只适用于重新构建并通过
> `runtime:verify` 的新项目运行包，不能使用旧 `mp-weixin` 项目、旧二维码或历史 token。若验收本地候选
> `13f597ea`，必须先完成受控切换，再重新获取服务端和真机证据。
