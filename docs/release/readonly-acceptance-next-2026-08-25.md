...
> **当前执行事实（2026-08-27）**：线上服务端 release 为 `5738a71e0bcddaa8849106754baf5b296427bed7`；小程序当前 live 运行包来源为 `cac6561b3f4ebbae2de8c632b052837fe7bc28b6`，包含 40 个页面；公网 HTTPS 已通过无 `-k` TLS 校验。当前没有运行中的微信开发者工具或真机会话，九个真机证据域仍为 `pending`，本手册不能据此宣称真实微信、Provider 或业务完成。当前候选已经原子发布到 live `dist`，不再存在待发布目录。
...
> 当前小程序运行相关源码和 live 运行包来源为 `cac6561b3f4ebbae2de8c632b052837fe7bc28b6`，包含 40 个页面；服务端线上 release 为 `5738a71e0bcddaa8849106754baf5b296427bed7`。
...
| 普通资料 | `GET /me/profile` 的安全默认值和字段校验；“我的”页用户主动授权后的昵称/头像本机展示 | `PUT`、真实资料写入、409 冲突，除非另有受控测试值授权；不把头像当作永久上传 |
...
4. 当前 live `d4f6748` 已通过 `runtime:verify`；从该运行包重新生成预览/真机二维码，记录运行包候选
   `d4f6748` 和微信开发者工具显示的项目根，不能使用旧候选二维码验证本次修复。
...
4. 进入“我的”，确认页面加载不会自动弹出微信资料授权；点击头像或“点击获取头像和昵称”，同意后核对真实昵称和头像，拒绝后核对可重试提示。年龄/邮箱不能被授权流程伪造填充。
5. 依次从首页进入“我的挂号”的“在线挂号”和“全部挂号”，确认标签切换与服务端查询范围一致；进入“就诊”页时，今日有预约应展示预约摘要但不显示叫号/排队结论；再进入爽约记录，确认不会自动弹出“选择就诊人”。
6. 进入门诊缴费，分别观察待缴和已缴两个只读列表；只验证脱敏展示、加载壳、空态和错误重试，不点击任何付款入口。
7. 进入报告查询，确认当前关闭 gate 的文案明确表示暂未开放，不出现“未查询到报告”的假空态。
8. 从四个主 Tab 互相切换多次，确认底部栏只有一份、选中图标随当前主页面变化、内容区域滚动而底部栏不滚动。
...
| 验收 wrapper | 已存在，远端 SHA-256 与本地受控 bundle 校验一致 |
