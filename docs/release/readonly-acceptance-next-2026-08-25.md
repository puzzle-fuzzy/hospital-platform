...
> 当前小程序运行相关源码和 live 运行包来源为 `02dbf10419740d96c4445493df019021ac22bcfa`，包含 40 个页面；服务端线上 release 仍为 `8eb51b5f`，本轮未替换服务端。
...
4. 当前 live `02dbf10` 已通过 `runtime:verify`；从该运行包重新生成预览/真机二维码，记录运行包候选
   `02dbf10` 和微信开发者工具显示的项目根，不能使用旧 `fcc6630e` 或上一候选二维码验证本次修复。
