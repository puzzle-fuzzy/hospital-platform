...
> 当前小程序功能候选来源为 `296516a5f255c563ec5eac40f2a3439632b143b8`，pending 运行包来源也为该提交，已构建到 `.local/hospital-miniprogram/pending/`，包含 20 个页面；服务端本地候选为 `b42922f4f83d5018c20f7abc5f9734625306d5a6`，尚未替换线上 `8eb51b5f`。
> 当前 live `dist` 仍为上一份完整运行包 `fcc6630ebfa7b0697cbd03a5e376ce6765d1643b`，因为微信开发者工具仍锁定目录而尚未发布。
> 必须关闭开发者工具后先发布 `296516a5` pending，再从新运行包生成二维码；历史 `eef55001`、`99c7e8fd`、`b587c7ea`、`baa31df0`、`c4dc00b9`、`b3436c24`、`90d5ab03`、`485c0892`、`a002353b` 和 `fcc6630e` 不能作为本候选证据。
...
3. 检查底部栏：四个主页面必须是 `pages/index/index`、`pages/consult/consult`、`pages/hospital/hospital`、
  ...
  4. 只有发布 pending 候选并重新执行 `runtime:verify` 后才生成新的预览/真机二维码；记录运行包候选
    `296516a5` 和微信开发者工具显示的项目根，不能使用旧 `fcc6630e` 二维码验证本次修复。
