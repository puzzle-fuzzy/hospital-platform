# 小程序当前候选真机三层证据记录模板（`4f067e7b`）

> 当前服务端 release 为 `9f479c9a`；小程序完整运行包来源为
> `4f067e7bb8b04e41a734ae4e6605f30c28a9c790`。空白模板不代表真机或业务验收通过。
...
页面、客户端 HTTP 和服务端日志必须来自同一次扫码会话，并且使用本表声明的两份来源指纹。只看到二维码、
模拟器或健康检查，不足以完成真实业务验收。不得记录 openid、session_key、完整身份证、完整卡号、token、
支付密钥或 Provider 原始报文。
