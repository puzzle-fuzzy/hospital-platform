# 小程序运行包候选 `ed63a8e`（2026-08-26）

## 当前结论

本候选已完成 TypeScript 编译、页面边界校验、运行包依赖校验、来源指纹写入和
...
| 项目 | 结果 |
| --- | --- |
| 运行输入来源 | `ed63a8ef570de7f4563f8c1821ce351612888efb`（`ed63a8e`） |
| 页面数量 | 40 |
| pending 运行文件 | 391 |
| `runtime:verify:pending` | 通过，40 个页面和根文件完整 |
| 真机证据清单 | [`device-evidence-ed63a8e-pending.json`](device-evidence-ed63a8e-pending.json)，9 个域均为 `pending` |
