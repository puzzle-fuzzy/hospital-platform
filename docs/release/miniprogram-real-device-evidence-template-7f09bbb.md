> 当前候选事实（2026-08-25）：服务端候选为 `b42922f`，小程序 pending 来源为 `689026770a9f68837ba60f1d7bf1ad9f073d547e`（提交 `68902677`）。服务端候选尚未替换线上 release，小程序 pending 尚未替换微信开发者工具正在使用的 live `dist`；本文不把历史运行包、静态检查或健康检查当作真机业务通过。
> 本段优先于本文下方旧日期、旧 release 或旧运行包叙述；旧值只作为历史记录，不作为当前验收入口。

# 小程序当前候选真机三层证据记录模板（`68902677`）
> 当前服务端候选：`b42922f`；小程序来源：`689026770a9f68837ba60f1d7bf1ad9f073d547e`。模板不把运行层 smoke 当作业务验收。
...
| --- | --- |
| 服务端候选 | `b42922f`（未部署） |
| 小程序客户端 | `68902677`（pending） |
| 小程序构建来源 | `689026770a9f68837ba60f1d7bf1ad9f073d547e` |
