...
> **本轮补充**：收紧小程序底层费用请求的患者标识和状态校验，并恢复旧端费用卡片的院区、患者姓名、科室、医生、金额、就诊时间层级；这些调整不新增 Provider 请求，不改变支付/医保关闭态。
...
- 小程序底层费用请求再次校验患者标识和 `unpaid/paid` 状态，费用卡片恢复旧端的院区、患者、科室、医生、金额和就诊时间层级；
- 待缴记录和已缴记录点击后都进入明确的迁移状态页，不调用 `wx.requestPayment`，不发起医保授权，不执行 6201/6202/6301 或 HIS 写回。

因此，本轮只收紧了底层请求的输入边界并完成了已确认的卡片布局缺口，没有打开支付或医保能力。
...
| 小程序页面 | `apps/miniprogram/src/pages/outpatient-payment/` | 先确认会话和当前就诊人；患者/会话变化时清空旧费用；请求失败不降级为空列表；卡片事件回查当前渲染批次；卡片按旧端层级展示 |
...
小程序服务层针对性回归另通过 `53 pass / 0 fail / 207 expect()`，其中新增底层报告/费用请求的非法患者、日期、来源和详情引用输入门禁。
