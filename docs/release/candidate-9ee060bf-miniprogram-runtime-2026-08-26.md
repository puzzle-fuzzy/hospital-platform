...
| 项目 | 结果 |
| --- | --- |
| 运行输入来源 | `9ee060bf00c4b5c8c67f0e39e2d5493043a33bb5`（`9ee060bf`） |
| 页面数量 | 40 |
| pending 运行文件 | 391 |
| 小程序回归 | `327 pass / 0 fail / 3629 expect()` |
| `runtime:verify:pending` | 通过，40 个页面和根文件完整 |
| 真机证据清单 | [`device-evidence-9ee060bf-pending.json`](device-evidence-9ee060bf-pending.json)，9 个域均为 `pending` |
...
## 2026-08-26 14:23 CST 接续复核

- 迁移宽度审计通过：64 个旧页面与迁移矩阵一致，40 个原生页面均已注册；首页/“我的”可见 action、四个主 Tab 和 40 个交互页面均通过入口门禁；
- 低风险业务域、日志事件和患者展示审计通过：5 个低风险域、8 个页面、10 条公网路由、35 个语义状态，以及 84 个静态日志事件均有代码与文档落点；
- 再次执行 `runtime:publish-pending` 仍因微信开发者工具占用 `apps/miniprogram/dist` 返回 `EBUSY`；发布器没有删除 live，也没有改变 pending 候选；
- 本轮未发现可以在缺少正式 Provider/临床 contract 时安全开放的下一项业务。下一步仍是释放文件锁后，使用同一候选采集 A 批次真实只读业务证据；支付、医保、临床读取、外部会话和患者写入继续保持关闭。

## 候选包含的业务边界修正

- 健康百科查询结果和详情页增加页面级请求竞态门禁，旧响应和旧 WXML 事件不能覆盖或打开当前内容；症状查询入口按服务端最多 10 项、唯一 ID 和 URI 完整性 fail-closed；
- 健康百科目录页统一使用页面生命周期守卫，分类模式、左栏 ID 和疾病详情引用先回查当前读模型；发布目录刷新时清除上一内容版本的症状选择，避免旧 ID 跨版本查询；
