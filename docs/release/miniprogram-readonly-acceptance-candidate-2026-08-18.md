...
| 小程序客户端 | `d1366b0` | 与服务端 `b7c9451` 配套的当前本地验收候选包；本次新增资料、门诊费用和挂号只读响应运行时契约门禁 |
| 小程序构建结果 | 14 个页面脚本 | `pnpm --dir apps/miniprogram build`、`runtime:verify`；小程序包的 Turbo build cache 已关闭，避免 Git 来源指纹被提交前缓存污染 |
| 小程序构建来源 | `d1366b02184b26c56183ee97e30a4e3232128715` | `dist/build-info.json` 的 `sourceRevision` |
| 小程序回归 | 145 项 / 1159 个断言 | `pnpm --filter @hospital/miniprogram test`；运行包来源固定为 `d1366b0` |
...
客户端候选的 `dist/` 必须由 `d1366b0` 运行输入工作树重新构建，并核对 `dist/build-info.json` 的完整 `sourceRevision`；不能使用旧聊天、旧开发者工具缓存或其他 release 的运行包推导本次结果。
`d1366b0` 在既有患者上下文、服务端 canonical 资料快照回写和会话命令重放门禁基础上，新增普通资料、门诊缴费列表和我的挂号列表的运行时响应契约校验：状态、日期、金额、展示字段和列表总数不符合约束时统一 fail-closed。它不改变 Provider、数据库或旧服务。当前真机包的来源指纹必须是完整的 `d1366b02184b26c56183ee97e30a4e3232128715`。
