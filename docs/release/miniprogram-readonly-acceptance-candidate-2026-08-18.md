...
| 小程序客户端 | `e4d9de6` | 与服务端 `1b94c46` 配套的当前候选包，包含选择页手动刷新、会话代际隔离、认证响应回写门禁、预约目录级联查询边界、科室/日期旧事件门禁、挂号卡片操作事件门禁、报告目录详情事件门禁、报告详情的当前患者范围校验与失败态临床读模型清理、“我的”页菜单稳定 key、根入口全局脚本门禁、门诊费用失败态患者上下文清理、普通资料 409 后强制刷新边界和门诊缴费首次加载期间标签状态快照边界 |
| 小程序构建结果 | 14 个页面脚本 | `pnpm --dir apps/miniprogram build`、`runtime:verify`；小程序包的 Turbo build cache 已关闭，避免 Git 来源指纹被提交前缓存污染 |
| 小程序构建来源 | `e4d9de6ef88ca9d24e89682f51f69ce292030ae2` | `dist/build-info.json` 的 `sourceRevision` |
| 小程序回归 | 122 项 / 1044 个断言 | `pnpm --filter @hospital/miniprogram test`；运行包来源固定为 `e4d9de6` |
