...
| 小程序客户端 | `1858e6c` | 与服务端 `b7c9451` 配套的当前本地验收候选包；本次统一患者上下文标识校验 |
| 小程序构建结果 | 14 个页面脚本 | `pnpm --dir apps/miniprogram build`、`runtime:verify`；小程序包的 Turbo build cache 已关闭，避免 Git 来源指纹被提交前缓存污染 |
| 小程序构建来源 | `1858e6cee2efe34e36f5648ee8e53a03bee5543d` | `dist/build-info.json` 的 `sourceRevision` |
| 小程序回归 | 141 项 / 1134 个断言 | `pnpm --filter @hospital/miniprogram test`；运行包来源固定为 `1858e6c` |
...
客户端候选的 `dist/` 必须由 `1858e6c` 运行输入工作树重新构建，并核对 `dist/build-info.json` 的完整 `sourceRevision`；不能使用旧聊天、旧开发者工具缓存或其他 release 的运行包推导本次结果。
`1858e6c` 在既有患者上下文、服务端 canonical 资料快照回写和会话命令重放门禁基础上，将患者标识形状校验集中到公共患者上下文服务，并让默认选择、异步回写、查询请求和显式选择共同拒绝空白、控制字符和超长值；患者目录读取/同步、预约、报告和门诊费用列表的 `total === items.length` 运行时校验以及二维码关闭态门禁仍保持不变。它不改变 Provider、数据库或旧服务。当前真机包的来源指纹必须是完整的 `1858e6cee2efe34e36f5648ee8e53a03bee5543d`。
