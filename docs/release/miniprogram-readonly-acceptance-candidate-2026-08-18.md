...
| 小程序客户端 | `f110a59` | 与服务端 `b7c9451` 配套的当前本地验收候选包；本次阻止损坏 storage 缓存默认切换患者 |
| 小程序构建结果 | 14 个页面脚本 | `pnpm --dir apps/miniprogram build`、`runtime:verify`；小程序包的 Turbo build cache 已关闭，避免 Git 来源指纹被提交前缓存污染 |
| 小程序构建来源 | `f110a5907f3f3dce15b3649abea1302d2986936d` | `dist/build-info.json` 的 `sourceRevision` |
| 小程序回归 | 142 项 / 1140 个断言 | `pnpm --filter @hospital/miniprogram test`；运行包来源固定为 `f110a59` |
...
客户端候选的 `dist/` 必须由 `f110a59` 运行输入工作树重新构建，并核对 `dist/build-info.json` 的完整 `sourceRevision`；不能使用旧聊天、旧开发者工具缓存或其他 release 的运行包推导本次结果。
`f110a59` 在既有患者上下文、服务端 canonical 资料快照回写和会话命令重放门禁基础上，将损坏的非字符串 storage 值与“从未选择”明确区分；缓存损坏时进入 `stale`，不得在目录同步后默认切换到第一位患者。患者标识形状校验、患者目录读取/同步、预约、报告和门诊费用列表的 `total === items.length` 运行时校验以及二维码关闭态门禁仍保持不变。它不改变 Provider、数据库或旧服务。当前真机包的来源指纹必须是完整的 `f110a5907f3f3dce15b3649abea1302d2986936d`。
