...
| 层级 | 固定值 | 证据 |
| --- | --- | --- |
| 服务端 release | `687690e` | 服务器 `/home/ps/code/hospital-platform/releases/687690e` |
...
| 小程序客户端 | `01b184d` | 与服务端 `687690e` 配套的当前候选包，包含报告详情统一患者上下文错误语义、报告目录失败态清理列表派生计数和对应静态门禁，并保留选择页手动刷新、会话代际隔离、认证响应回写门禁、预约目录级联查询边界、科室/日期旧事件门禁、挂号卡片操作事件门禁、报告目录详情事件门禁、报告详情的当前患者范围校验与失败态临床读模型清理、“我的”页菜单稳定 key、根入口全局脚本门禁、门诊费用失败态患者上下文清理、普通资料 409 后强制刷新边界、门诊缴费首次加载期间标签状态快照边界、首页目录请求生命周期边界、“我的”页患者关键路径/资料降级边界，以及首页不得绕过独立选择页直接写入患者的入口门禁 |
...
| 全仓回归 | 9/9 package、API 131/599、工具 14/14 | 当前工作树 `pnpm check` 已通过；服务端线上 release 为 `687690e`，旧 Python 保持运行 |
...
服务端当前 release `687690e` 包含 `98e091b` 门诊费用患者引用二次门禁：仓储返回的非法或跨患者/Provider
...
本候选文件只固定验收方法，不宣称业务已经通过。当前 release 已切换为 `687690e`，仍缺：有效微信会话下的页面截图、HTTP trace、真实预约/费用 Provider 字段、
