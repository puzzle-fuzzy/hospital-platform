...
| 小程序客户端 | `055a784` | 首页会话生命周期修正后的 `main` |
| 小程序构建结果 | 14 个页面脚本 | `pnpm --dir apps/miniprogram build`、`runtime:verify` |
| 小程序回归 | 106 项 / 932 个断言 | `pnpm --dir apps/miniprogram test` |
| 公网 API | `https://test-hp.meiyi.pro/api/v2` | 只允许 HTTPS，客户端不直连 Provider |

客户端候选的 `dist/` 必须由 `055a784` 工作树重新构建；不能使用旧聊天、旧开发者工具缓存或其他 release 的运行包推导本次结果。
