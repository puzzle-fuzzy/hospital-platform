# 原生小程序只读业务验收候选（历史候选记录）

本文保留历史候选的“客户端候选 + 服务端 release”组合，避免把不同版本的页面、API 和日志混在同一份证据中；它不再是当前验收入口。
本文只覆盖微信会话、患者目录、预约历史、爽约记录和门诊费用只读查询；预约写入、支付、医保、退款、报告详情和 HIS 继续关闭。

> 历史候选说明：本文记录的 `b451cc6` 是当时验收窗口的事实，现已被当前候选 `48ba22f` 取代。当前构建来源、门禁和真机前置条件以
> [`candidate-48ba22f-local-build-2026-08-19.md`](candidate-48ba22f-local-build-2026-08-19.md) 为准；本文中的历史测试数字和来源指纹不应重新用于验收。

> 当前组合为服务端 `968af78` + 小程序来源 `48ba22f16cb0f1d1098895772e660a3ed96761bb`；本历史文件中的
> `65219e2`、`b451cc6` 和旧测试数字只用于追溯，不能作为当前业务、真机或日志证据。
...
| 全仓回归 | 9/9 package、API 163/713、Worker、工具与 9/9 构建 | `b451cc6` 干净 worktree 的 `pnpm check` 已通过；服务端线上 release 为 `65219e2`，旧 Python 保持运行 |
| 公网 API | `https://test-hp.meiyi.pro/api/v2` | 只允许 HTTPS，客户端不直连 Provider |

客户端候选的 `dist/` 必须由 `b451cc6` 运行输入工作树重新构建，并核对 `dist/build-info.json` 的完整 `sourceRevision`；不能使用旧聊天、旧开发者工具缓存或其他 release 的运行包推导本次结果。
`b451cc6` 在既有登录后患者初始化、显式就诊人选择、报告目录旧患者事件阻断和预约记录状态门禁基础上，进一步拒绝绕过响应校验的未知挂号状态。它不改变 Provider、数据库或旧服务。当前真机包的来源指纹必须是完整的 `b451cc6df6959df3155e1ffaf1ef3c8dcd0c6df8`。
...
服务端当前 release `65219e2` 包含 `98e091b` 门诊费用患者引用二次门禁：仓储返回的非法或跨患者/Provider
...
1. 在微信开发者工具导入 `apps/miniprogram/`，确认运行根目录为 `dist/`；重新编译当前 `dist/`，并在扫码前核对 `dist/build-info.json.sourceRevision` 必须等于 `b451cc6df6959df3155e1ffaf1ef3c8dcd0c6df8`。确认网络请求只指向公网 Hospital API。
...
本候选文件只固定验收方法，不宣称业务已经通过。当前 release 已切换为 `65219e2`，仍缺：有效微信会话下的页面截图、HTTP trace、真实预约/费用 Provider 字段、
