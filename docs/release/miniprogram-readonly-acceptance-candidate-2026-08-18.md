...
| 全仓回归 | 9/9 package、API 127/577、工具 14/14 | 当前工作树 `pnpm check` 已通过；服务端线上 release 仍为 `1b94c46`，旧 Python 保持运行 |
