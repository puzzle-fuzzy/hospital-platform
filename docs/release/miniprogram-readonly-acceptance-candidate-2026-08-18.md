...
| 小程序客户端 | `59d76cf` | 与服务端 `b7c9451` 配套的当前本地验收候选包；本次补强患者端列表总数契约门禁 |
| 小程序构建结果 | 14 个页面脚本 | `pnpm --dir apps/miniprogram build`、`runtime:verify`；小程序包的 Turbo build cache 已关闭，避免 Git 来源指纹被提交前缓存污染 |
| 小程序构建来源 | `59d76cf8f8904cb74bd9cd2bd13df0920f0b99f4` | `dist/build-info.json` 的 `sourceRevision` |
| 小程序回归 | 136 项 / 1114 个断言 | `pnpm --filter @hospital/miniprogram test`；运行包来源固定为 `59d76cf` |
...
客户端候选的 `dist/` 必须由 `59d76cf` 运行输入工作树重新构建，并核对 `dist/build-info.json` 的完整 `sourceRevision`；不能使用旧聊天、旧开发者工具缓存或其他 release 的运行包推导本次结果。
`59d76cf` 在既有患者上下文和会话命令重放门禁基础上，补充患者目录读取/同步、预约、报告和门诊费用列表的 `total === items.length` 运行时校验；协议错配统一 fail-closed，不能伪装成空列表、成功同步快照或错误的本地“加载更多”。它不改变 Provider、数据库或旧服务。当前真机包的来源指纹必须是完整的 `59d76cf8f8904cb74bd9cd2bd13df0920f0b99f4`。
