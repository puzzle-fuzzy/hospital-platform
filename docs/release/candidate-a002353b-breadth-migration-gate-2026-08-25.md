# 小程序候选 `a002353b`：就诊只读子集与全量入口门禁（2026-08-25）
...
| 项目 | 值 |
| --- | --- |
| Git 来源 | `a002353b813d5c5a58945f02aa31089efc4df3f4` |
...
pnpm docs:audit                                 679 docs，无断链
pnpm architecture:audit                         68 条规则通过
pnpm format:check                               通过
pnpm lint                                       通过
```

根 `pnpm test` 和 `release:baseline:audit` 仍会对另一会话尚未部署的
`packages/adapters/src/zhongyang-appointments.ts` fail-closed；本会话没有修改、暂存或部署该文件，
也没有通过放宽发布基线来掩盖漂移。小程序构建的类型阶段通过，发布阶段因微信开发者工具锁定
...
发布后必须从新的 `a002353b` 运行包重新编译、扫码，并按
