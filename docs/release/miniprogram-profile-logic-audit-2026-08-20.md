...
本次审计起始窗口使用小程序 `6e6604f8089e45ceeaaf4bcbbd57065174a59a31` 与服务端 `6038560`；普通资料读写 contract 已通过服务层和页面状态审计，本次继续发现并修正了资料页页面栈复用时缺少 `onShow` 重新读取以及读写并发互斥的生命周期缺口。修正后的候选仍保持“代码已实现、真实写入待验收”。当前真机验收必须使用服务端 `5a31427` 与小程序候选 `1d161b7`，完整来源为 `1d161b701384ad8073ed907fe14f611ab3166fcd`。本次只审计新项目，未修改旧 Python 服务、线上服务、数据库或 Redis。
...
在服务端 `5a31427`、小程序运行包来源 `1d161b701384ad8073ed907fe14f611ab3166fcd` 上重新复核普通资料相关源码、测试和运行包门禁：没有发现需要放宽或改写业务语义的问题。全仓 `pnpm check` 通过，包含架构、迁移清单、Provider intake、文档链接、发布基线、Biome 格式/Lint、类型检查、各包测试和构建；小程序 `runtime:verify` 通过，14 个注册页面脚本齐全，`dist/` 中没有 `*.test.js` 或 `*.spec.js`。
