...
> 当前基线更新：服务端 `5a31427`；小程序候选 `ec3e5f09`；完整运行包来源 `ec3e5f094ecd0a85bbf98d4fc6ff9f14b27bb9ac`。本文件的运行观察早于该客户端构建，仅作运行层历史观察。
...
> 当前发布基线补充（2026-08-21）：本地小程序候选为 `ec3e5f09`，完整运行包来源为 `ec3e5f094ecd0a85bbf98d4fc6ff9f14b27bb9ac`；本文件的 10:38 运行观察早于该次本地构建，不能作为该候选的真机证据。
...
10:38 观察窗口早于后续构建；当前发布基线已更新为 `ec3e5f09` / `ec3e5f094ecd0a85bbf98d4fc6ff9f14b27bb9ac`。该来源只用于当前发布文档一致性，不代表本服务器窗口已经运行小程序或完成真机验收。下一步必须由用户使用该候选重新普通编译并扫码，保留页面、客户端 requestId/traceId 和服务器同链低敏事件；在真实业务请求出现前，不升级任何 P0 业务验收等级。
