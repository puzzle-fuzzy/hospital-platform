...
> 状态：姓名/卡片身份关联规则已在 `968af78` 验收并随新 API release `398be8e` 重新部署；本轮新增的“卡号必须保持字符串”门禁仅完成本地候选验证，尚未部署；旧 Python 服务、生产数据库、Redis 和公网路由未修改。
...
- 当前定向测试结果：20 项通过，46 个断言；Biome 与 TypeScript 检查通过；卡号字符串门禁仍属于本地待发布候选，
