...
> 状态：姓名/卡片身份关联规则已在 `968af78` 验收并随新 API release `398be8e` 重新部署；卡号字符串门禁以及本轮新增的“卡片数组独立证据”门禁仅完成本地候选验证，尚未部署；旧 Python 服务、生产数据库、Redis 和公网路由未修改。
...
3. 如果响应包含顶层卡号，返回的顶层卡号必须包含本次查询卡号；如果响应明确包含
   `patCardVOList` 卡片列表，则卡片列表本身必须独立包含本次查询卡号，不能用顶层卡号
   与列表做并集绕过；明确返回空数组，或卡片项全部缺少可比卡号，也视为无法关联并拒绝；
...
6. 字段格式错误、姓名不一致、卡号不一致、卡片归属不一致或卡片列表结构异常，整次患者同步失败，
   ...
   卡号或原始档案。
7. 目录输入和档案响应中的卡号字段必须是字符串；即使 JSON 数字没有超出安全整数范围，也可能
   丢失医院卡号的前导零，因此不能进入 `patInfosFind` 查询或 `his-patient` 映射。
...
- 测试覆盖姓名/卡号不一致时拒绝绑定、卡片列表不包含查询卡号时拒绝绑定、19 位字符串 `patId`、
  超出安全整数范围的数字 `patId`、目录/档案数字卡号拒绝和敏感字段不进入公共结果；
- 当前患者 adapter 定向测试结果：22 项通过，48 个断言；适配器全套测试为 95 项通过、207 个断言；新增回归覆盖
  “顶层卡号匹配但卡片列表指向另一张卡”和“卡片列表没有可比较卡号”两类绕过路径。Biome 与 TypeScript 检查通过；
  卡号字符串及卡片列表独立证据门禁仍属于本地待发布候选，
  不等同于线上 Provider 或真机验收；
