...
> 审计对象：服务端 release `5a31427`、当前小程序运行包来源 `cdb27e5023a188ab36a340497cebe18f1e274013`（候选 `cdb27e50`）。本记录只覆盖代码、测试和本地运行包证据；不把静态检查替代为微信真机业务验收。
...
这里不能改成 `Promise.all`：普通资料 GET 可能在过期 GET 会话下触发安全恢复，完成资料读取后必须重新确认 `/me` 的 owner 并采用最新代际，再读取患者目录；否则会把旧账号患者目录和新账号资料拼成混合快照。

每次页面读取同时受两层守卫约束：

1. 页面实例请求守卫：新的 `onShow`、下拉刷新或页面卸载后，旧 Promise 不能再 `setData`；
2. 会话代际与 owner 守卫：同一 owner 的 GET 会话恢复可以在 `/me` 重验证后采用最新代际；owner 变化或无法证明时，整轮组合失败并清理派生展示，不自动拼接或重放旧快照。

资料响应之后的 `revalidateCurrentOwner(expectedOwnerId)` 是有意保留的额外 GET：资料 contract
只返回展示字段，不包含 owner 标识，不能用资料响应自身证明账号归属。患者目录完成后再由
`loadPatientsForOwner(expectedOwnerId)` 重验一次 owner，保证“我的”页的患者数量、当前选择和资料标签不会跨 owner 组合。
