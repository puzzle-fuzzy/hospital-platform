> 当前发布基线（2026-08-24）：线上服务端 release 为 `28a5c0c131794ce9dcc5f94bd3809402188ac87a`；当前小程序运行包来源为 `13f597ea9ee3f65b9be858117826d948339d904a`（提交 `13f597e`）。本文下方旧候选只作历史追溯。
...
- 服务端已验证 release：`28a5c0c131794ce9dcc5f94bd3809402188ac87a`（提交 `28a5c0c1`）；
...
| 旧项目、旧 Python 服务、数据库、Redis | 未操作 |

这组证据只证明当前候选的运行包边界正确。如果微信开发者工具仍请求
`dist/services/single-flight.test.js`，应按旧增量模块索引处理：结束真机调试、关闭并重新打开
`apps/miniprogram/`，确认 `miniprogramRoot=dist/`，普通编译后重新生成二维码；不能复制测试脚本
到 `dist/`，也不能把运行包问题转嫁给旧服务。
