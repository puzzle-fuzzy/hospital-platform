...
| `pnpm --filter @hospital/miniprogram runtime:verify` | 通过；运行包来源为 `f66514d`，无 `*.test.js`/`*.spec.js` |
| `pnpm docs:audit` | 通过；371 个 Markdown 文档无断链 |
...
1. 当前小程序候选 `f66514d` 的真实微信扫码、登录、患者同步和页面结果三层同链证据。
