...
> `7f09bbb2cf32d4753795bcbc91fe23ec05eeeee6`（提交 `7f09bbb`）。本文只记录代码级不变量，
...
| `pnpm --filter @hospital/miniprogram runtime:verify` | 通过；运行包来源为 `7f09bbb`，无 `*.test.js`/`*.spec.js` |
...
1. 当前小程序候选 `7f09bbb` 的真实微信扫码、登录、患者同步和页面结果三层同链证据。
