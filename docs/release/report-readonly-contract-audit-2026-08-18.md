...
- 当前线上服务端 release 为 `b7c9451`，配套小程序构建来源为
  `65657a0bb289fb25f17f14c0b8fec22270f964b2`（当前本地候选，尚未上传线上）；本文件的 Provider 材料门禁不因 release 切换而放宽。
...
`65657a0bb289fb25f17f14c0b8fec22270f964b2`（当前本地候选，尚未上传线上）；本次只切换了新 API，报告 Provider gate 仍保持关闭。
...
- 本轮 service 二次校验提交为 `133e94e`，引用写入范围回验提交为 `62e1dac`，引用读取范围回验提交为 `56c73af`；
  这些提交只完成本地代码、定向测试和类型证据，当前已随 `687690e` 部署，但不能据此增加 Provider 或真机验收结论。

当前线上发布和运行观察见 [`687690e-production-acceptance-2026-08-18.md`](687690e-production-acceptance-2026-08-18.md)；
