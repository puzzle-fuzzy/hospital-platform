...
- 当前线上服务端 release 为 `5a31427`，配套小程序构建来源为
  `aafccf53d1e0eaac5ddaf98b1d850b084e4f78ed`（当前本地候选 `aafccf5`，尚未上传线上）；本文件的 Provider 材料门禁不因 release 切换而放宽。
...
`aafccf53d1e0eaac5ddaf98b1d850b084e4f78ed`（当前本地候选 `aafccf5`，尚未上传线上）；本次只切换了新 API，报告 Provider gate 仍保持关闭。
