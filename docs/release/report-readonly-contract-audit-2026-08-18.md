...
- 当前线上服务端 release 为 `0e360d3`，配套小程序构建来源为
  `ac238c6156f085fdb56f5806fefac3613e5f85be`（当前本地候选 `ac238c6`，尚未上传线上）；本文件的 Provider 材料门禁不因 release 切换而放宽。
...
`ac238c6156f085fdb56f5806fefac3613e5f85be`（当前本地候选 `ac238c6`，尚未上传线上）；本次只切换了新 API，报告 Provider gate 仍保持关闭。
