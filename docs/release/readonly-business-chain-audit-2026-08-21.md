...
> 本记录以前次 `9f491cb5` 的业务审计为基础，当前运行基线已更新为 `002acc1b`；预约历史、爽约记录、门诊费用和小程序运行包边界仍需按当前候选重新取得真机三层证据。当前完整运行包来源为 `341524ac345d9cfae3f6e0f8258aed3e9457f458`；文中更早候选只作历史追溯。本地适配器审计修正已提交为 `313e903`，本轮个人资料合同修正已随 `002acc1b` 发布；没有修改旧 Python 服务、线上配置、MySQL、Redis 或并行会话维护的众阳自动化代码。
>
