...
> `b0e093565493285e07fe549879f8b87eda649cc7`。下方更早候选只作历史追溯；真实业务仍需当前候选三层证据。
...
> 本记录以前次 `9f491cb5` 的业务审计为基础，当前运行基线已更新为 `7181e99e`；预约历史、爽约记录、门诊费用和小程序运行包边界仍需按当前候选重新取得真机三层证据。当前完整运行包来源为 `4e1b2e224964797c103eba832323ee7074c7ad2b`；文中更早候选只作历史追溯。本地适配器审计修正已提交为 `313e903`，本轮会话失效与资料读模型失效边界修正已提交为 `4e1b2e2`；没有修改旧 Python 服务、线上配置、MySQL、Redis 或并行会话维护的众阳自动化代码。
>
