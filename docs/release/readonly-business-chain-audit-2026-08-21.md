...
> 本记录以前次 `9f491cb5` 的业务审计为基础，当前运行基线已更新为 `84fac75c`；预约历史、爽约记录、门诊费用和小程序运行包边界仍需按当前候选重新取得真机三层证据。当前完整运行包来源为 `90fd7832e3ad1031c9c916f118f90cc0f284aff`；文中更早候选只作历史追溯。本地适配器审计修正已提交为 `313e903`，报告引用窗口修正已随 `84fac75c` 发布；没有修改旧 Python 服务、线上配置、MySQL、Redis 或并行会话维护的众阳自动化代码。
>
