...
> 本记录以服务端 `5a31427` 和小程序本地候选 `d4d65b7` 为当前运行基线，复核预约历史、爽约记录、门诊费用和小程序运行包边界。当前完整运行包来源为 `d4d65b735da8630e9b6795d9e105192713297474`；文中更早候选只作历史追溯。本地适配器审计修正已提交为 `313e903`，本轮个人资料合同修正尚未部署；没有修改旧 Python 服务、线上配置、MySQL、Redis 或并行会话维护的众阳自动化代码。
>
...
| 小程序运行包 | `pnpm --filter @hospital/miniprogram runtime:verify` | 通过；14 个页面齐全，来源 `d4d65b7`，不含 test/spec 脚本 |
