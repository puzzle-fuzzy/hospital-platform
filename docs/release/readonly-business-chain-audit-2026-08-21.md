...
> 本记录以服务端 `c8eef370` 和小程序本地候选 `f488c6f3` 为当前运行基线，复核预约历史、爽约记录、门诊费用和小程序运行包边界。当前完整运行包来源为 `f488c6f3270514af10b19fdf3c45a47519e1736b`；文中更早候选只作历史追溯。本地适配器审计修正已提交为 `313e903`，本轮个人资料合同修正尚未部署；没有修改旧 Python 服务、线上配置、MySQL、Redis 或并行会话维护的众阳自动化代码。
>
