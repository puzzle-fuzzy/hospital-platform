...
> `d4f67485a34195a2e1e392071502cf2a7006dd27`（`d4f6748`），40 个页面，当前无 pending 目录；
...
| 检查项 | 结果 |
| --- | --- |
| 当前小程序候选 | live 为 `d4f6748`（来源 `d4f67485a34195a2e1e392071502cf2a7006dd27`）；当前无 pending 目录 |
...
| 小程序核心测试 | 全量回归 `344 pass / 0 fail / 3733 expect()`；本候选新增 App.onLaunch 全局资料初始化时序回归，既有共享患者会话边界、二维码会话门禁、健康规则版本和关闭态布局测试均通过 |
...
当前 `docs/release/device-evidence-d4f67485-pending.json` 的九个业务域仍为 `pending`。本地测试只能证明客户端状态机和响应校验，不足以证明微信真机、公网 HTTPS、Nginx、Elysia 日志和 Provider 请求号已经形成同一条证据链。
...
1. 在有真实微信开发者工具/真机会话时，从当前 live `d4f6748` 运行包普通编译并生成新二维码。
