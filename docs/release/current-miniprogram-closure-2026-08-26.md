...
> `ce1c2179b57fe2783066b51f8621220224982928`（`ce1c217`），38 个页面，当前无 pending 目录；
...
| 检查项 | 结果 |
| --- | --- |
| 当前小程序候选 | live 为 `ce1c217`（来源 `ce1c2179b57fe2783066b51f8621220224982928`）；当前无 pending 目录 |
...
| 小程序核心测试 | 当前复跑全量回归 `366 pass / 0 fail / 3890 expect()`；本候选新增 App.onLaunch 全局资料初始化时序回归，既有共享患者会话边界、二维码会话门禁、健康规则版本和关闭态布局测试均通过 |
...
当前 `docs/release/device-evidence-ce1c2179b57fe2783066b51f8621220224982928-pending.json` 的九个业务域仍为 `pending`。本地测试只能证明客户端状态机和响应校验，不足以证明微信真机、公网 HTTPS、Nginx、Elysia 日志和 Provider 请求号已经形成同一条证据链。
...
1. 在有真实微信开发者工具/真机会话时，从当前 live `ce1c217` 运行包普通编译并生成新二维码。
