...
> `76ca0137ea9a57b8b7ed9c8797bb718040535922`（`76ca013`），40 个页面，当前无 pending 目录；
...
| 检查项 | 结果 |
| --- | --- |
| 当前小程序候选 | live 为 `76ca013`（来源 `76ca0137ea9a57b8b7ed9c8797bb718040535922`）；当前无 pending 目录 |
...
| 小程序核心测试 | 全量回归 `341 pass / 0 fail / 3730 expect()`；本候选新增 App.onLaunch 全局资料初始化时序回归，既有共享患者会话边界、二维码会话门禁、健康规则版本和关闭态布局测试均通过 |
...
当前 `docs/release/device-evidence-76ca0137-pending.json` 的九个业务域仍为 `pending`。本地测试只能证明客户端状态机和响应校验，不足以证明微信真机、公网 HTTPS、Nginx、Elysia 日志和 Provider 请求号已经形成同一条证据链。
...
1. 在有真实微信开发者工具/真机会话时，从当前 live `76ca013` 运行包普通编译并生成新二维码。
