...
| 检查项 | 结果 |
| --- | --- |
| 当前小程序候选 | pending `1d132315`（来源 `1d132315dff23c8befdfc867cb62a4227dbb7820`）；live `02dbf10` |
...
| 小程序核心测试 | `326 pass / 0 fail / 3622 expect()` |
...
当前 `docs/release/device-evidence-1d132315-pending.json` 的九个业务域仍为 `pending`。本地测试只能证明客户端状态机和响应校验，不足以证明微信真机、公网 HTTPS、Nginx、Elysia 日志和 Provider 请求号已经形成同一条证据链。
