...
| 检查项 | 结果 |
| --- | --- |
| 当前小程序候选 | pending `bab96de9`（来源 `bab96de9be2ae76e371f09ea7a734d0f078df2a9`）；live `02dbf10` |
...
| 小程序核心测试 | `324 pass / 0 fail / 3611 expect()` |
...
当前 `docs/release/device-evidence-bab96de9-pending.json` 的九个业务域仍为 `pending`。本地测试只能证明客户端状态机和响应校验，不足以证明微信真机、公网 HTTPS、Nginx、Elysia 日志和 Provider 请求号已经形成同一条证据链。
