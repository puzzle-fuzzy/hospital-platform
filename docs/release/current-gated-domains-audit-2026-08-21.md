...
> `7f09bbb2cf32d4753795bcbc91fe23ec05eeeee6`。本次只更新运行层基线，支付、医保、报告、绑定和 HIS 仍关闭。
...
> 上述历史基线仅作追溯；当前执行以本页顶部 `84370077 + 7f09bbb` 为准。

> 本文正文中的旧门禁快照仅作历史追溯；当前服务端和小程序基线以本页顶部 `84370077 + 7f09bbb` 为准。本文用于区分“已有代码骨架”“已注册路由”“具备 Provider 契约”和“真实业务已验收”，不能把其中任一层单独当作迁移完成。旧 Python 服务、旧数据库、旧 Redis 和旧域名本轮均未修改。
...
| 小程序本地候选 | `7f09bbb2cf32d4753795bcbc91fe23ec05eeeee6`（`7f09bbb`），尚未上传线上 |
...
| `pnpm release:baseline:audit` | 当前服务端 `84370077` 与小程序候选 `7f09bbb` 指针已分别核对；服务端运行层发布证据见 [`84370077-production-acceptance-2026-08-22.md`](84370077-production-acceptance-2026-08-22.md)，小程序运行包证据见 [`candidate-7f09bbb-local-build-2026-08-22.md`](candidate-7f09bbb-local-build-2026-08-22.md) |
