...
> 2026-08-25 后续修正：上一份 `a5ff0001` 运行包的源码门禁仍正确，但用户在设备上继续观察到闪动/选中态不明显。本轮不恢复 `custom-tab-bar`，只清理原生配置中的旧自定义尺寸字段，并将普通/选中图标换成全新的 `*-native-v6.png` 路径。提交后的新 `build-info.sourceRevision` 和二维码是本轮唯一验收入口。
