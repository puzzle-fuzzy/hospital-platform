...
> **2026-08-27 复核补充**：重新检查预约历史、爽约、门诊费用、患者切换和全局资料启动链，未发现需要放宽业务边界或修改 Provider 映射的缺陷。针对曾出现的 `globalData` 启动时序异常，新增 `app-runtime-context.test.ts` 运行时回归：覆盖 `getApp()` 尚不可用、页面运行阶段优先真实 App、不完整 App 回退启动容器三种场景；旧 Python 服务、旧数据库、旧 Redis 和并行会话的众阳自动化均未修改。
...
本次用户反馈的 `Cannot read property 'globalData' of undefined` 属于小程序启动阶段的容器时序问题，
不是预约、费用或患者数据为空。App IIFE 在调用 `App()` 前先登记与 `globalData` 共用同一引用的启动容器，
`onLaunch` 将该容器显式传给全局资料初始化；页面 bundle 进入正常生命周期后仍优先使用微信返回的完整 App 实例。
这样既覆盖 `getApp()` 尚未可用的启动窗口，也不会创建第二份全局状态。

本次新增的运行时测试结果为 `3 pass / 0 fail`；连同全局资料和会话事件回归为 `17 pass / 0 fail / 69 expect()`。
运行包 `dist/app.js` 已确认包含启动容器登记和显式初始化调用，不能继续使用开发者工具中缓存的旧增量包；重新打开
`apps/miniprogram/dist/` 并普通编译后再生成真机二维码，才属于当前候选入口。
