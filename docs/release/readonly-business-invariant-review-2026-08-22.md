...
> `ba1dd23e0f40191745a60997939b31b4c47795cd`。本次不把历史 journald 窗口升级为当前业务证据。
...
- 小程序运行包来源：`ba1dd23e0f40191745a60997939b31b4c47795cd`，14 个页面入口完整，`dist/` 不含 `*.test.js` 或 `*.spec.js`。
...
`ba1dd23e0f40191745a60997939b31b4c47795cd`；真实微信、患者切换和只读业务三层证据仍未产生。
...
`build-info.json.sourceRevision=ba1dd23e0f40191745a60997939b31b4c47795cd`，再进行任何真机业务操作。
这样可以避免旧端窗口继续加载旧页面，或把旧增量索引误认为新运行包问题。

## 5. 下一步准入顺序

1. 在微信开发者工具关闭旧真机调试，切换到已打开的 `apps/miniprogram/` 新项目窗口，普通编译并确认 `dist/build-info.json` 来源为 `ba1dd23e0f40191745a60997939b31b4c47795cd`。
