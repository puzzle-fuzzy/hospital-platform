...
> `7f09bbb2cf32d4753795bcbc91fe23ec05eeeee6`。本次不把历史 journald 窗口升级为当前业务证据。
...
- 小程序运行包来源：`7f09bbb2cf32d4753795bcbc91fe23ec05eeeee6`，14 个页面入口完整，`dist/` 不含 `*.test.js` 或 `*.spec.js`。
...
`7f09bbb2cf32d4753795bcbc91fe23ec05eeeee6`；真实微信、患者切换和只读业务三层证据仍未产生。
...
- 文档链接审计：Markdown 文档无断链；发布基线指向服务端 `84370077024762d92050cf077c27f3c60302e8f8` 和小程序 `7f09bbb`。
...
- `pnpm release:baseline:audit` 与 `pnpm docs:audit`：均通过，当前来源为服务端 `84370077`、小程序 `7f09bbb`。
...
`build-info.json.sourceRevision=7f09bbb2cf32d4753795bcbc91fe23ec05eeeee6`，再进行任何真机业务操作。
这样可以避免旧端窗口继续加载旧页面，或把旧增量索引误认为新运行包问题。

## 5. 下一步准入顺序

1. 在微信开发者工具关闭旧真机调试，切换到已打开的 `apps/miniprogram/` 新项目窗口，普通编译并确认 `dist/build-info.json` 来源为 `7f09bbb2cf32d4753795bcbc91fe23ec05eeeee6`。
