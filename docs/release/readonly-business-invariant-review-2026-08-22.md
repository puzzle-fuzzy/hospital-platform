...
- 小程序运行包来源：`34f0fd21aac33214e991de561d37dfd7071013bf`，40 个页面入口完整，`dist/` 不含 `*.test.js` 或 `*.spec.js`。
...
`34f0fd21aac33214e991de561d37dfd7071013bf`；真实微信、患者切换和只读业务三层证据仍未产生。
...
- 文档链接审计：Markdown 文档无断链；发布基线指向服务端 `1bc8b0a85f21cb58205a99ce4de0de6afe9bf240` 和小程序 `34f0fd21`（完整来源 `34f0fd21aac33214e991de561d37dfd7071013bf`）。
...
- `pnpm release:baseline:audit` 与 `pnpm docs:audit`：均通过，当前来源为服务端 `1bc8b0a8` 与小程序 `34f0fd21`；本轮无开发者工具或真机业务会话。
...
1. 在微信开发者工具关闭旧真机调试，切换到已打开的 `apps/miniprogram/` 新项目窗口，普通编译并确认 `dist/build-info.json` 来源为 `34f0fd21aac33214e991de561d37dfd7071013bf`。
