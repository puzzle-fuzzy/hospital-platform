...
> 下方 2026-08-22 的候选只作历史追溯；当前执行使用顶部 `8eb51b5f` 服务端 + `13f597e` 小程序分层基线。


> 当前完整小程序来源校验值：`cac6561b3f4ebbae2de8c632b052837fe7bc28b6`；当前服务端 release：`5738a71e0bcddaa8849106754baf5b296427bed7`。

> 当前发布基线：服务端 `8eb51b5ffe85b0b8f8a032783f893117d3df549d`；小程序运行包来源
...
- 服务端当前已验证 release：`5738a71e0bcddaa8849106754baf5b296427bed7`。
- 小程序运行包来源：`cac6561b3f4ebbae2de8c632b052837fe7bc28b6`，40 个页面入口完整，`dist/` 不含 `*.test.js` 或 `*.spec.js`。
...
`cac6561b3f4ebbae2de8c632b052837fe7bc28b6`；真实微信、患者切换和只读业务三层证据仍未产生。
...
- 文档链接审计：Markdown 文档无断链；发布基线指向服务端 `5738a71e0bcddaa8849106754baf5b296427bed7` 和小程序 `cac6561`（完整来源 `cac6561b3f4ebbae2de8c632b052837fe7bc28b6`）。
...
- `pnpm release:baseline:audit` 与 `pnpm docs:audit`：均通过，当前来源为服务端 `5738a71e` 与小程序 `cac6561`；本轮无开发者工具或真机业务会话。
...
`build-info.json.sourceRevision=4ba492a3fdae8283409bd2ab4a0a45247c46600c`，再进行任何真机业务操作。
这样可以避免旧端窗口继续加载旧页面，或把旧增量索引误认为新运行包问题。

## 5. 下一步准入顺序

1. 在微信开发者工具打开 `apps/miniprogram/` 新项目窗口，普通编译并确认 `dist/build-info.json` 来源为 `cac6561b3f4ebbae2de8c632b052837fe7bc28b6`。
