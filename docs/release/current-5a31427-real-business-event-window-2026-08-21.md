...
| 当前运行包构建时间 | `2026-08-21T00:48:02.507Z`（08:48:02 CST） |
...
当前 `acf5a85` 运行包构建时间为 08:48 CST，而本窗口中的业务事件约在 08:16 CST 已发生。即使事件本身真实，也无法从服务端事件名称反推出手机使用的是哪个小程序运行包；因此不能把它回填为 `acf5a85` 的页面、客户端 HTTP 和服务端三层证据。

在当前运行包重新生成、二维码重新打开后，08:45–08:55 CST 没有新的微信、患者、预约历史、门诊费用或普通资料事件。下一次扫码必须重新建立窄时间窗口，并同时记录：
