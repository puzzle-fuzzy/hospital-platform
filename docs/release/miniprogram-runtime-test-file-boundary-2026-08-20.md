...
下一步必须使用最新 `6ce1272` 运行包取得真机页面、HTTP 请求链和服务端低敏日志三层证据；支付、医保、退款和 HIS 写入继续保持关闭。

## 当前候选更新（2026-08-21）

选择页和普通资料页会话生命周期修正后，当前真机候选已推进到 `9340846`，完整运行包来源为
`93408462f3eeadffed172f1ea3b10c043d461b1b`；重新构建和 `runtime:verify` 均通过，运行包仍为 14 个页面且不包含测试脚本。
前文 `7f157d4`、`6e6604f` 只保留为历史运行包证据，不能继续生成当前真机二维码。
