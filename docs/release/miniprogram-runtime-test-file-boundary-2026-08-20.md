...
下一步必须使用最新 `7a6f4df` 运行包取得真机页面、HTTP 请求链和服务端低敏日志三层证据；支付、医保、退款和 HIS 写入继续保持关闭。

## 当前候选更新（2026-08-21）

只读页面跨会话本地事件修正后，当前真机候选已推进到 `7a6f4df`，完整运行包来源为
`7a6f4df34fac5975c6012a30d2c137953a892059`；重新构建和 `runtime:verify` 均通过，运行包仍为 14 个页面且不包含测试脚本。
前文 `9340846`、`7f157d4`、`6e6604f` 只保留为历史运行包证据，不能继续生成当前真机二维码。
