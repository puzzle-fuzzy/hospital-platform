...
下一步必须使用最新 `3a6bf3e` 运行包取得真机页面、HTTP 请求链和服务端低敏日志三层证据；支付、医保、退款和 HIS 写入继续保持关闭。

## 当前候选更新（2026-08-21）

选择页和普通资料页会话生命周期修正后，当前真机候选已推进到 `3a6bf3e`，完整运行包来源为
`3a6bf3ea3d2b8944e05ffcad254a37afdbca2aab`；重新构建和 `runtime:verify` 均通过，运行包仍为 14 个页面且不包含测试脚本。
