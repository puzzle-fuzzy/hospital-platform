...
下一步必须使用最新 `39e2caa` 运行包取得真机页面、HTTP 请求链和服务端低敏日志三层证据；支付、医保、退款和 HIS 写入继续保持关闭。

## 当前候选更新（2026-08-21）

选择页会话生命周期修正后，当前真机候选已推进到 `39e2caa`，完整运行包来源为
`39e2caac27ec4bf569f5debc8edb47947bc398d2`；重新构建和 `runtime:verify` 均通过，运行包仍为 14 个页面且不包含测试脚本。
