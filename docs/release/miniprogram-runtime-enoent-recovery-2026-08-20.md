...
> 本文记录的 `6e6604f`、`02c18af`、`1d161b7` 及更早候选均为历史窗口。当前真机入口是 [`candidate-6ce1272-local-build-2026-08-21.md`](candidate-6ce1272-local-build-2026-08-21.md)，ENOENT 修复原则不变，但二维码必须从当前候选重新生成。
...
选择页和普通资料页会话生命周期修正后，当前小程序候选已推进为 `6ce1272`，完整运行包来源为
`6ce12729c3e112a6cb8333c5132c23713d1cb1ec`。该候选已重新构建并通过 `runtime:verify`；
...
因此，当前只能使用 `6ce1272` 重新生成的新二维码进行真机验收，必须从“微信登录 → `/me` → 患者同步”开始；
