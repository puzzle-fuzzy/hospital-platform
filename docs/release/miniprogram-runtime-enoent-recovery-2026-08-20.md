...
> 本文记录的 `6e6604f`、`02c18af`、`1d161b7` 及更早候选均为历史窗口。当前真机入口是 [`candidate-d4d65b7-local-build-2026-08-21.md`](candidate-d4d65b7-local-build-2026-08-21.md)，ENOENT 修复原则不变，但二维码必须从当前候选重新生成。
...
选择页和普通资料页会话生命周期修正后，当前小程序候选已推进为 `9340846`，完整运行包来源为
`93408462f3eeadffed172f1ea3b10c043d461b1b`。该候选已重新构建并通过 `runtime:verify`；
`dist/` 仍不存在 `single-flight.test.js` 或其它 `*.test.js`/`*.spec.js`。后续真机调试必须以该候选
重新普通编译和生成二维码，前文 `7f157d4`、`6e6604f` 及更早候选只用于历史追溯。
...
## 当前工作树复核（2026-08-21，`d4d65b7`）

当前小程序运行输入已推进到 `d4d65b7`，完整来源为
`d4d65b735da8630e9b6795d9e105192713297474`。重新检查后，`dist/services/single-flight.js`
