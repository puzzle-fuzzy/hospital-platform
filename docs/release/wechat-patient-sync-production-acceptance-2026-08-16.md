...
- 观测窗口：2026-08-16 23:08:45-23:38:00（Asia/Shanghai）。
- 日志来源：服务器 `hospital-platform-api-v2.service` journald，只读查询。
- 当前服务：`/home/ps/code/hospital-platform/current -> releases/a11f117`，新 API `18081`；旧 Python `8001` 仍在监听。
...
7. 新 API 与旧 Python 服务继续共存，日志显示新 API 使用 production mode 和 `a11f117` release。

排班请求中的 `appointment.schedule_snapshots.failed` 不等于 Provider 目录失败：本次 Provider
返回有效排班，HTTP 仍为 200；但短期快照没有落库，所以该排班只能用于只读展示，不能作为未来锁号或预约
写入的前置事实。新日志的 `snapshotPersistenceStatus=unavailable` 用于避免把两条事实链混为一谈。
...
- 预约科室和排班已有真实 Provider 结果，但本次排班快照持久化暂时不可用；预约历史、报告和门诊费用仍未取得真实 Provider 结果；
...
2. 先重复验证 MySQL/快照持久化恢复，再确认 `snapshotPersistenceStatus=persisted`；
