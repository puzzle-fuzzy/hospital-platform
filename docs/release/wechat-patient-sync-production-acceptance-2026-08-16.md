...
- 观测窗口：2026-08-16 23:08:45-23:18:00（Asia/Shanghai）。
...
6. 新 API 与旧 Python 服务继续共存，日志显示新 API 使用 production mode 和 `a11f117` release。
...
- 尚未取得预约科室、排班、预约历史、报告和门诊费用的真实 Provider 结果；
...
2. 获取/准备第二条可验证的就诊人目录样例后，验收患者选择页切换与 owner-scoped 业务查询；
3. 按预约目录 → 预约历史 → 门诊费用 → 报告的顺序进行只读 Provider 验收；
4. 新 Provider 文档到达后，先登记来源、版本、hash、环境、错误样例和状态机，再更新公共 contract；
5. 以上只读证据全部通过后，才讨论预约写入；支付、医保、退款和 HIS 仍按最后阶段处理。
