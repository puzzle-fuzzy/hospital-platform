...
> 本手册用于当前 `687690e` 服务端与配套小程序候选的真实微信小程序验收。小程序运行包来源指纹必须为
> `01b184d9a6e37f7045b0cf62ecbf685cf0fc482c`。它覆盖微信登录、普通资料、患者上下文、预约历史、爽约记录和门诊费用只读查询；不包含预约下单、微信支付、医保结算、退款或 HIS 写入。
>
