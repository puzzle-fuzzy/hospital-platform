...
> `7f09bbb2cf32d4753795bcbc91fe23ec05eeeee6`。小程序当前本地运行包仍需从正确项目普通编译后再生成二维码；没有三层证据不得放行。

> 历史候选：服务端 release `7181e99e3a352244102f5591279528b3b66332c9`；小程序运行包来源 `4e1b2e224964797c103eba832323ee7074c7ad2b`（提交 `4e1b2e2`）。

> 下文的 `7181e99e + 4e1b2e2` 仅是历史配套基线；当前验收必须使用本页顶部声明的 `84370077 + 7f09bbb`。

> 本手册的验收顺序仍可复用，但真实执行必须使用当前 `84370077` 服务端和 `7f09bbb` 运行包。它覆盖微信登录、普通资料、患者上下文、预约目录/历史、报告目录/LIS 详情、爽约记录和门诊费用只读查询；不包含预约下单、微信支付、医保结算、退款或 HIS 写入，也不开放二维码协议。
>
