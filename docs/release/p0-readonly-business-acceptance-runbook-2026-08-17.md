...
> 本手册用于当前 `5a31427` 服务端与配套小程序候选的真实微信小程序验收。小程序运行包来源指纹必须为
> `b02594a521cae2d12b991d2361c80224572c79b0`（当前本地候选 `b02594a`，尚未上传线上）。它覆盖微信登录、普通资料、患者上下文、预约目录/历史、报告目录/LIS 详情、爽约记录和门诊费用只读查询；不包含预约下单、微信支付、医保结算、退款或 HIS 写入，也不开放二维码协议。
>
...
[`真机三层证据记录模板`](miniprogram-real-device-evidence-template-b02594a.md)；空白模板不构成验收证据。
...
1. 当前服务端基线为 `5a31427`，当前本地真机候选来源为
