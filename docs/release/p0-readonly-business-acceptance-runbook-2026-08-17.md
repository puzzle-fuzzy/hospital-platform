...
> `6e6604f8089e45ceeaaf4bcbbd57065174a59a31`（当前本地候选 `6e6604f`，尚未上传线上）。它覆盖微信登录、普通资料、患者上下文、预约目录/历史、报告目录/LIS 详情、爽约记录和门诊费用只读查询；不包含预约下单、微信支付、医保结算、退款或 HIS 写入，也不开放二维码协议。
>
...
[`真机三层证据记录模板`](miniprogram-real-device-evidence-template-6e6604f.md)；空白模板不构成验收证据。
...
1. 当前服务端基线为 `5a31427`，当前本地真机候选来源为
   `6e6604f8089e45ceeaaf4bcbbd57065174a59a31`（提交 `6e6604f`）；截至本记录，公网 live/ready 和未登录 401 已复核，
