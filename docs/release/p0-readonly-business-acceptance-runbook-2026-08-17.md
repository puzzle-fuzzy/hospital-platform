...
> 下文的 `7181e99e + 4e1b2e2` 仅是历史配套基线；当前验收必须使用本页顶部声明的 `9f479c9a + 41c708e1`。

> 本手册的验收顺序仍可复用，但真实执行必须使用当前 `9f479c9a` 服务端和 `41c708e1` 运行包。它覆盖微信登录、普通资料、患者上下文、预约目录/历史、报告目录/LIS 详情、爽约记录和门诊费用只读查询；不包含预约下单、微信支付、医保结算、退款或 HIS 写入，也不开放二维码协议。
>
