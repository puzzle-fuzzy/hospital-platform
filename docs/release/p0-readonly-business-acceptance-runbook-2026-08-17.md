...
> 本手册用于当前 `b7c9451` 服务端与配套小程序候选的真实微信小程序验收。小程序运行包来源指纹必须为
> `534871549517080807c7e5c1375247477f422750`（当前本地候选 `5348715`，尚未上传线上）。它覆盖微信登录、普通资料、患者上下文、预约目录/历史、报告目录/LIS 详情、爽约记录和门诊费用只读查询；不包含预约下单、微信支付、医保结算、退款或 HIS 写入，也不开放二维码协议。
>
...
1. 当前服务端基线为 `b7c9451`，当前本地真机候选为
   `534871549517080807c7e5c1375247477f422750`（提交 `5348715`）；截至本记录，公网 live/ready 和未登录 401 已复核，
