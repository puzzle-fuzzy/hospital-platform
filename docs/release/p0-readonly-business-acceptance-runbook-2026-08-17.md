...
> 本手册用于当前 `3ab0a6c` 生产版本的真实微信小程序验收。它只覆盖微信登录、患者上下文、预约历史、爽约记录和门诊费用只读查询；不包含预约下单、微信支付、医保结算、退款或 HIS 写入。
>
...
1. 当前 release 切换后尚未产生新的真实微信登录、患者同步、预约历史或门诊费用事件，因此本手册的结果尚待真机操作填充。
   2026-08-17 14:37 CST 的服务器只读观察确认当前进程仍没有 `appointment.records.*` 或 `outpatient.payment.records.*`
   事件；详见 [`当前服务器 P0 只读观察`](current-server-p0-observation-2026-08-17.md)。
