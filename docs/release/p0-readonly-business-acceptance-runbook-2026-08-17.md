...
> 下文的 `7181e99e + 4e1b2e2` 仅是历史配套基线；当前验收必须使用本页顶部声明的 `28a5c0c1 + 13f597e`。

> 本手册的验收顺序仍可复用，但真实执行必须使用当前 `28a5c0c1` 服务端和 `13f597e` 运行包。它覆盖微信登录、普通资料、患者上下文、预约目录/历史、报告目录/LIS 详情、爽约记录和门诊费用只读查询；不包含预约下单、微信支付、医保结算、退款或 HIS 写入，也不开放二维码协议。
>
...
1. 当前服务端基线为 `c8eef370`，当前本地真机候选来源为
   `02c18af2e658507a0fa5182368235cf62cd348c7`（提交 `02c18af`）；截至本记录，公网 live/ready 和未登录 401 已复核，
