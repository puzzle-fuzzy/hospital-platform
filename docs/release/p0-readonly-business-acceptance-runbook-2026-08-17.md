...
> 当前配套小程序运行包（2026-08-27）：本地 live `dist` 的 sourceRevision 为 `cac6561b3f4ebbae2de8c632b052837fe7bc28b6`（`cac6561`），共 40 个页面；当前没有运行中的微信开发者工具或真机会话，九个真机证据域仍为 `pending`。本文下方历史候选仅作追溯。

> 当前配套小程序运行包来源（2026-08-27）：`cac6561b3f4ebbae2de8c632b052837fe7bc28b6`（`cac6561`）；当前没有开发者工具或真机会话，九个真机证据域仍为 `pending`。本文下方更早候选仅作历史追溯。

> 当前服务端配套发布更新（2026-08-26）：线上服务端 release 为 `8eb51b5ffe85b0b8f8a032783f893117d3df549d`；当前线上小程序运行包来源为 `13f597ea9ee3f65b9be858117826d948339d904a`（提交 `13f597e`）。本地 `ed20c52` 仍是未发布 pending 候选，下方历史候选仅供追溯，本行优先。
...
> 当前发布基线更新（2026-08-26）：线上服务端 release 为 `8eb51b5ffe85b0b8f8a032783f893117d3df549d`；当前小程序运行包来源为 `13f597ea9ee3f65b9be858117826d948339d904a`（提交 `13f597e`）。真机业务三层证据仍待。
> 本段优先于本文下方旧日期、旧 release 或旧运行包叙述；旧值只作为历史记录，不作为当前验收入口。
> 下方历史 release 与运行包只作追溯；当前执行使用顶部 `8eb51b5f` 服务端 + `13f597e` 小程序分层基线。
...
> 当前完整小程序来源校验值：`13f597ea9ee3f65b9be858117826d948339d904a`；当前服务端 release：`8eb51b5ffe85b0b8f8a032783f893117d3df549d`。

> 当前发布基线（2026-08-26）：服务端 `8eb51b5ffe85b0b8f8a032783f893117d3df549d`；小程序运行包来源
> `13f597ea9ee3f65b9be858117826d948339d904a`。小程序当前本地运行包仍需从正确项目普通编译后再生成二维码；没有三层证据不得放行。

> 历史候选：服务端 release `7181e99e3a352244102f5591279528b3b66332c9`；小程序运行包来源 `4e1b2e224964797c103eba832323ee7074c7ad2b`（提交 `4e1b2e2`）。

> 下文的 `7181e99e + 4e1b2e2` 仅是历史配套基线；当前验收必须使用本页顶部声明的 `8eb51b5f + 13f597e`。

> 本手册的验收顺序仍可复用，但真实执行必须使用当前 `8eb51b5f` 服务端和 `13f597e` 运行包。它覆盖微信登录、普通资料、患者上下文、预约目录/历史、报告目录/LIS 详情、爽约记录和门诊费用只读查询；不包含预约下单、微信支付、医保结算、退款或 HIS 写入，也不开放二维码协议。
>
...
[`真机三层证据记录模板`](miniprogram-real-device-evidence-template-13f597e.md)；空白模板不构成验收证据。
...
1. 当前线上服务端基线为 `8eb51b5f`，当前本地真机候选来源为
   `7f7a7a1844f5269c88f814d7d97d805fe4b8aeca`（提交 `7f7a7a18`，尚未发布）；截至本记录，公网 live/ready 和未登录 401 已复核，
...
> 当前统一发布基线补充（2026-08-27）：服务端 release 为 `5738a71e0bcddaa8849106754baf5b296427bed7`；小程序本地 live 运行包来源为 `cac6561b3f4ebbae2de8c632b052837fe7bc28b6`，共 40 个页面。本文更早版本仅作历史追溯，真机证据仍为 pending；旧 Python `8001` 未修改。
