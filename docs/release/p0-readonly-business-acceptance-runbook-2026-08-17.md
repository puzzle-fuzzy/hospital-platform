...
> `e050fa026f2cb5b2de4af4de98024cfdb946229c`（当前本地候选 `e050fa0`，尚未上传线上）。它覆盖微信登录、普通资料、患者上下文、预约目录/历史、报告目录/LIS 详情、爽约记录和门诊费用只读查询；不包含预约下单、微信支付、医保结算、退款或 HIS 写入，也不开放二维码协议。
>
...
1. 当前服务端基线为 `398be8e`，当前本地真机候选来源为
   `e050fa026f2cb5b2de4af4de98024cfdb946229c`（提交 `e050fa0`）；截至本记录，公网 live/ready 和未登录 401 已复核，
