# 历史小程序 pending 运行包候选（fc70fa0b）

> 本记录只作历史追溯，已由 `7627843a` 替代；不代表当前 pending、微信开发者工具、线上服务或真机验收状态。

## 候选事实

| 项目 | 当前值 |
| --- | --- |
| 小程序源码提交 | `fc70fa0b78a29f564c4e3e4b35ab28ae4afb3fde` |
| pending 位置 | `.local/hospital-miniprogram/pending/` |
| 页面数量 | 20 |
| 运行包来源 | `build-info.json.sourceRevision` |
| 真机证据清单 | [`device-evidence-fc70fa0-pending.json`](device-evidence-fc70fa0-pending.json)，9 个域均为 `pending` |
| 服务端候选 | `b42922f`，尚未与线上旧 release 切换 |

## 本地验证

- `pnpm --filter @hospital/miniprogram typecheck`：通过；
- 当前小程序测试：`272 pass / 0 fail / 2679 expect()`；
...
本候选只修正入口状态页对未知/过期 `feature` 的错误语义，不打开预约写入、患者新增绑定、二维码真实生成、临床问卷、外部 WebView、支付、医保或 HIS 回写；这些业务继续遵守各自的 contract 和 release gate。
