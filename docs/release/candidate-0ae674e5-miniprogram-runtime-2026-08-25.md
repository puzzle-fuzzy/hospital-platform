# 当前小程序 pending 运行包候选（0ae674e5）

> 本记录描述 `0ae674e5` 生成的 pending 运行包。它尚未发布到微信开发者工具、线上服务或真机，不代表真实业务验收通过。

## 候选事实

| 项目 | 当前值 |
| --- | --- |
| 小程序源码提交 | `0ae674e5ceb353434ad2e52e375927e4b788222e` |
| pending 位置 | `.local/hospital-miniprogram/pending/` |
| 页面数量 | 20 |
| pending 来源 | `build-info.json.sourceRevision` |
| 真机证据清单 | [`device-evidence-0ae674e-pending.json`](device-evidence-0ae674e-pending.json)，9 个域均为 `pending` |
| 服务端候选 | `b42922f`，尚未与线上旧 release 切换 |
| 当前 live `dist` | `fcc6630ebfa7b0697cbd03a5e376ce6765d1643b`，仍被微信开发者工具占用 |

## 本批迁移覆盖变更

- 64 个旧入口现在全部进入 A–F 批次或明确排除；A=21、B=4、C=4、D=23、E=4、F=7，排除 1 个开发辅助页；
- 统一状态页继续展示迁移阶段、旧端入口、业务域、所属 A–F 批次和该批次下一步输入；
- 逐页批次只用于迁移编排，不会把状态页、静态页面或安全子集误报为业务完成；
...
- 当前小程序测试：`281 pass / 0 fail / 3037 expect()`；
- `pnpm migration:readiness`：通过结构审计，64 个旧入口未分配批次为 0；
...
发布后才能使用本候选重新生成二维码并采集九域真机证据。旧 live `dist/` 未被替换前，不能把本候选代码测试、pending 验证或页面状态页当作微信业务完成证据。
