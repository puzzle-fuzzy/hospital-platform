# 小程序当前候选真机三层证据记录模板（`5e43aed`）

> 当前服务端 release 为 `1e58bb66bf24021d2b680eb5fd03abfec467989a`；小程序完整运行包来源为
> `5e43aed0e026cd48d980d58c468223b9a5ee8744`。空白模板不代表真机或业务验收通过。
...
页面、客户端 HTTP 和服务端日志必须来自同一次扫码会话，并且使用本表声明的两份来源指纹。只看到二维码、模拟器或健康检查，
不足以完成真实业务验收。不得记录 openid、session_key、完整身份证、完整卡号、token、支付密钥或 Provider 原始报文。
