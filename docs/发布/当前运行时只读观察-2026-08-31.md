...
仓库当前 migration head 为 `0017_outbox_manual_review_state`，而线上已部署 release 的 preflight 仍以 `0016_patient_directory_sync_owner_index` 为期望 head。结合 [`server-runtime-drift-audit-2026-08-31.md`](服务端运行时漂移审计-2026-08-31.md) 中的 16 个未部署运行时文件，服务端 release 漂移是已取得线上证据的真实阻塞，不应通过修改审计器、跳过 migration 或拆分发布来绕过。
