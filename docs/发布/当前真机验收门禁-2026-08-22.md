...
``miniprogram-devtools-reimport-2026-08-22-1314.md``。该动作仍只属于真机准入，
...
报告目录默认使用当前日前一个中国标准时间日历月窗口（月底沿用旧端 JavaScript 日历月溢出），先通过 owner-scoped 患者映射取得 `his-patient` 引用，再访问众阳
...
规则和回归证据见 ``device-evidence-distinct-chain-audit-2026-08-22.md``。
...
``7181e99e-production-acceptance-2026-08-22.md``。
