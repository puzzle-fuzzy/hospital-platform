...
- `pnpm provider:audit` 通过，当前仓库登记了 3 份 Provider 接收记录、26 个 `documentId`；
  `docs/提供商接入/` 中没有报告目录专用的正式接收记录、脱敏响应样例或错误样例。
- `docs/Provider契约-v1.md` 只冻结了平台侧的候选 endpoint 和公开字段边界，不能代替报告 Provider 的版本、环境、权限、
...
[`../Provider文档接入流程.md`](../Provider文档接入流程.md) 登记，再更新本文的字段差异和验收证据。
...
历史线上发布和运行观察见 ``687690e-production-acceptance-2026-08-18.md``；
