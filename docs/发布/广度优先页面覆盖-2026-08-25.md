...
> **当前事实源补充（2026-09-03）**：本页下方的旧候选段落保留历史交接信息；当前逐页机器台账以
> `pnpm migration:audit` 为准：64 个旧页面、42 个原生页面，状态分布为
> `partial=31`、`replaced=9`、`surface-only=21`、`blocked-payment=0`、
> `blocked-provider=1`、`blocked-external=1`、`excluded=1`。
> 这份覆盖证据证明入口没有遗漏，不证明阻塞入口的真实业务已经开放。
> 当前工作区源码已注册 42 个原生页面；由于本轮代码仍未提交，sourceRevision 待提交后重新生成；
