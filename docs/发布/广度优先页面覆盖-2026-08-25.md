...
> **当前事实源补充（2026-09-06）**：本页下方的旧候选段落保留历史交接信息；当前逐页机器台账以
> `pnpm migration:audit` 为准：64 个旧页面、43 个原生页面，状态分布为
> `partial=33`、`replaced=10`、`surface-only=18`、`blocked-payment=0`、
> `blocked-provider=1`、`blocked-external=1`、`excluded=1`。
> 这份覆盖证据证明入口没有遗漏，不证明阻塞入口的真实业务已经开放。
> 当前工作区源码已注册 43 个原生页面；当前小程序源码与 live 运行输入为 `f3f2bc665cb0ed5d727936e7918b1dacbd2eccfc`（发布前仍需重新构建并校验运行包）；
...
| 分类 | 数量 | 含义 |
| --- | ---: | --- |
| `replaced` | 9 | 已由原生页面或等价静态能力替换，不代表旧端所有隐含 provider 能力都存在 |
| `partial` | 31 | 已有安全只读/静态子集，详情、写入、支付或外部回写仍关闭 |
| `surface-only` | 21 | 已有原生页面外壳和稳定关闭态，真实业务读取或写入仍未开放 |
