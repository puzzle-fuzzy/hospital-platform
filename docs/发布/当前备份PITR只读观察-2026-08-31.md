# 生产备份与 PITR 只读观测（2026-09-01 复核）

本文记录 2026-09-01 对 `ps@192.168.112.172` 的只读复核结果，用于判断备份、binlog/PITR、恢复
...
| 检查 | 结果 | 能否作为医院业务备份证据 |
| --- | --- | --- |
| systemd 中备份相关 unit | 发现 `dpkg-db-backup.service` / `dpkg-db-backup.timer`，另有未启用的 `pg_basebackup@.timer`；本次复核仍只有系统包数据库备份定时器 | 不能；它们不是医院 MySQL 业务库备份任务 |
...
本轮复核再次确认并关闭了“当前 binlog 运行参数未知”这一事实缺口，不能关闭生产备份与恢复 TODO。
