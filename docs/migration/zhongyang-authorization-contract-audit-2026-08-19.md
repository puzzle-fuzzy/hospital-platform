...
> 当前验收配套基线：服务端 `5a31427`；小程序候选 `03d9d25`；完整运行包来源
> `03d9d25d80d5a5d872a9137c7df0aa19a91ba38f`。这只是统一证据来源，不代表 Provider 鉴权或真机业务已经完成。
...
当前下一步是先在服务端 `5a31427` 上用小程序候选 `03d9d25` 取得新平台真机会话，再按患者同步 → 预约记录 → 门诊费用 → 报告
