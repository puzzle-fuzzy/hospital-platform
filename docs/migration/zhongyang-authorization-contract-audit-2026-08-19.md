...
> 当前验收配套基线：服务端 `0e360d3`；小程序候选 `0dccf54`；完整运行包来源
> `0dccf545ef65a905b58a27f38e787daea250fa54`。这只是统一证据来源，不代表 Provider 鉴权或真机业务已经完成。
...
当前下一步是先用与服务端 `0e360d3` 配套的小程序候选 `0dccf54` 取得新平台真机会话，再按患者同步 → 预约记录 → 门诊费用 → 报告
