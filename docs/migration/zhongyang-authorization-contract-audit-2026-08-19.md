...
> 当前验收配套基线：服务端 `5a31427`；小程序候选 `ec3e5f09`；完整运行包来源
> `ec3e5f094ecd0a85bbf98d4fc6ff9f14b27bb9ac`。这只是统一证据来源，不代表 Provider 鉴权或真机业务已经完成。
...
当前下一步是先在服务端 `5a31427` 上用小程序候选 `ec3e5f09` 取得新平台真机会话，再按患者同步 → 预约记录 → 门诊费用 → 报告
