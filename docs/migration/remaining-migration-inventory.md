...
> **最新候选覆盖补充（2026-08-25）**：当前小程序功能候选为 `3cf828ed`，原生页面注册数为 20 个，健康百科目录、搜索、疾病/药品详情三个只读落点、客户端运行时响应校验、分类级空态修复、互联网医院安全壳迁移分类、全局资料授权跨 bundle 单飞与异常恢复、首页二维码入口状态、跨会话授权拒绝保护、健康知识错误码文案、未知/过期入口错误语义、迁移入口覆盖视图、A–F 迁移批次展示和统一页面滚动边界均已纳入；pending 候选位于 `.local/hospital-miniprogram/pending/`，当前源码回归为 `284 pass / 0 fail / 3129 expect()`，且首页/我的 31 个可见 action 与全部 20 个页面 WXML 事件均通过广度审计。页面接入不等于健康内容开放，正式审核 bundle、发布/撤回演练和真机证据仍待。

> 当前事实源（2026-08-25，优先于下方历史交接）：小程序功能源码候选为 `3cf828ed185f0e745138db04011d26c6db62fa8a`，当前运行输入/pending 来源一致，位于 `.local/hospital-miniprogram/pending/`；
> 当前 live `dist` 仍为 `fcc6630ebfa7b0697cbd03a5e376ce6765d1643b`，因为开发者工具锁定目录尚未发布。本轮小程序回归为 `284 pass / 0 fail / 3129 expect()`；更早候选仅作历史交接。
