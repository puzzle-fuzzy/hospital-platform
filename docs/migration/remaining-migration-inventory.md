...
> 2026-08-21 当前候选代码门禁复核：`f9833c1` 小程序测试 `182 pass`、`1445 expects`，构建、运行包来源和全仓门禁均通过；
...
> 当前发布基线补充（2026-08-21）：服务端为 `5a31427`；小程序候选为 `f9833c1`，完整运行包来源为
> `f9833c11ef0d49591fd14f99ce60ad92d800156a`，尚未上传线上。
>
...
> 当前配套小程序候选构建来源为 `f9833c11ef0d49591fd14f99ce60ad92d800156a`（提交 `f9833c1`），尚未上传线上；用户已有的开发者工具配置修改不属于本次候选代码。
...
> `7a6f4df34fac5975c6012a30d2c137953a892059`。代码、架构、迁移清单、Provider intake、文档、类型、测试均通过；小程序构建门禁随后以当前候选重新执行，
> 379 篇 Markdown 文档、Biome、9 个 workspace 的类型检查/测试/构建均通过；当前 API 测试 188 项通过，小程序测试 178 项通过、1418 个断言通过，
> 配置包新增的空白上游地址回退测试也通过。
...
- 当前小程序运行输入来源为 `7a6f4df`，本轮完整构建已生成并通过 `runtime:verify`；`dist/build-info.json` 的来源指纹为
  `7a6f4df34fac5975c6012a30d2c137953a892059`，注册页面和生成脚本均为 14 个；本轮患者上下文
