...
- 当前架构边界审计为 26 条规则，并扫描 `apps/miniprogram/src` 全部生产文本源码；它会阻止 provider
 ...
 - 最近一次生产切换证据为：`current=9833a01`、新 API `18081` active、旧 Python `8001` 继续监听、Worker
  `inactive/disabled`；公网和内网 readiness 均为 `200`，数据库/Redis/schema 为 `ok`，公网保留
  `Cache-Control: no-store`。完整证据见 [`../release/9833a01-production-acceptance-2026-08-17.md`](../release/9833a01-production-acceptance-2026-08-17.md)。
...
| --- | --- | --- | --- |
| 微信登录与平台会话 | `auth`、Redis session | 服务端真实登录、`/me` 会话恢复和单患者同步已有历史 journald 证据，当前 API 已切换到 `9833a01`；当前版本业务链路待重新取证 | Redis 实际 TTL、多就诊人切换、完整真机网络对齐和其他业务仍未完成 |
