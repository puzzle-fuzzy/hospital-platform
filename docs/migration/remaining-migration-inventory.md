...
> 当前本地未发布候选的运行输入为 `3b8a04e59ae0c733e3cbb6d425119e498a8d79c7`（提交 `3b8a04e5`）。该候选继续使用原生共享四 Tab，并把开发者工具 watcher 根、`src/`/`scripts/` 源码监听隔离、运行包来源日志、native 图标缓存隔离、81×81 图标尺寸门禁、当前 Tab 重复导航 no-op、主 Tab 前四项注册约束、预约标签重载范围一致性和 `dist` 独立运行工程一致性纳入门禁；真机控制台可通过启动日志核对候选来源。
...
> 当前本地小程序候选（2026-08-25）为 `3b8a04e59ae0c733e3cbb6d425119e498a8d79c7`，`dist/` 已原子发布并通过 `runtime:verify`，16 个页面脚本完整；开发者工具应直接打开 `apps/miniprogram/dist/` 的独立工程。之前的 `0f40ab9`、`4ea15b8`、`4e8f6877`、`0bf2bf8`、`7a85dce8`、`f4c844c1`、`ecff1f9`、`49c641b6`、`e039e99f` 仅为历史来源；线上仍是 `13f597e`，两者不能混用。
...
> 当前执行基线（2026-08-25）：线上新 API 为 `8eb51b5ffe85b0b8f8a032783f893117d3df549d`，线上配套小程序运行包为 `13f597e`（来源 `13f597ea9ee3f65b9be858117826d948339d904a`）；本轮 adapter 门禁已随服务端切换进入线上，渠道 4 “全部挂号”查询仍待真机三层证据，普通资料真实写入/409 也仍待同一候选下的受控验收。本地候选 `3b8a04e59ae0c733e3cbb6d425119e498a8d79c7` 只用于代码和运行包验证，尚未替换线上小程序运行包。
...
> 本轮继续只修改新项目。当前本地小程序候选为 `3b8a04e59ae0c733e3cbb6d425119e498a8d79c7`，
> 已完成原生 Tab 运行包重建、`runtime:verify`、小程序 `239 pass / 0 fail`、文档链接和迁移台账审计。
...
| P1 | 真机只读与普通资料受控验收 | 新小程序运行包来源与已验证服务端配套；微信登录、患者同步/切换、预约历史、门诊费用、普通资料 GET 分别取得页面、HTTP requestId/Provider requestId、Pino 日志三层证据；普通资料 PUT/409 仅使用明确授权的可恢复测试值 | 当前进行中：线上真机配套运行包为 `13f597ea9ee3f65b9be858117826d948339d904a`，本地未发布候选为 `3b8a04e59ae0c733e3cbb6d425119e498a8d79c7`；`dist` 原子构建、独立工程和运行包校验已通过，但显式患者切换、页面截图、预约历史、门诊费用和普通资料三层证据仍待，真实写入尚未执行，详见 [`../release/next-profile-business-acceptance-plan-2026-08-24.md`](../release/next-profile-business-acceptance-plan-2026-08-24.md) |
...
| 门诊费用 | `payments/outpatient/records` | 只读目录已实现，查询时间显式使用 `Asia/Shanghai`；待缴/已缴各一次 `requested → loaded` 的空列表观察属于历史 release，线上 `13f597ea` 仍需重新取得业务证据，页面展示合法空态并保留患者更换入口 | 真实微信真机证据、费用详情、金额非空样例、支付、医保、结算回写和退费未开放；空列表不能替代费用字段和支付链路验收 |
