...
> 当前本地未发布候选的运行输入为 `0dd1b65b`。该候选使用微信官方共享 `custom-tab-bar`，并把开发者工具 watcher 根、`src/`/`scripts/` 源码监听隔离、运行包来源日志、图标缓存隔离、81×81 图标尺寸门禁、当前 Tab 重复导航 no-op、主 Tab 前四项注册约束、预约标签重载范围一致性和 `dist` 独立运行工程一致性纳入门禁；真机控制台可通过启动日志核对候选来源。
> 本地 `dist/` 已完成原子构建，`build-info.json.sourceRevision=0dd1b65b`；构建后的运行包已通过 `runtime:verify`，不能再把旧 `0f40ab9`、`bdf4ac57` 或 `4ea15b8` 写成当前候选。
> 当前本地源码的 `app.json` 已注册 16 个页面，其中四个主入口由唯一 `custom-tab-bar` 组件统一维护；这个数字不应与线上历史运行包 `13f597e` 的 14 页记录混用。
...
> 本轮继续只修改新项目。当前本地小程序候选为 `0dd1b65b`，
