...
> **当前回归复核（2026-08-27）**：当前 live 小程序全量回归为 `338 pass / 0 fail / 3712 expect()`；下方旧段落中仍出现的旧测试数字仅是前一轮候选记录，不覆盖本条当前事实。

> **最新横向迁移补充（2026-08-27）**：患者签名和消息订阅已经完成原生安全展示层迁移；前一批补齐旧端采血预约、电子锦旗和健康表扬信的当前就诊人、错误重试、统一选人入口和明确关闭态；本轮又统一临床/服务入口和我的问诊入口的当前就诊人上下文，并补齐患者绑定/签名页查看协议原文的安全入口、公共日期窗口边界、微信资料拒绝后的设置页重试和选择页刷新并发门禁；最新候选又统一便民记录区域的加载、错误、未开放三态和固定高度，并收紧就诊二维码安全壳的有效会话门禁、固定本地健康数值工具规则版本，同时在会话失效时清理旧 owner 的微信资料缓存。最新运行包来源为 `413cbea13f022831f63e9c750661eeabbffc68d5`，已通过发布前静态校验和发布后 live 校验；本轮另修正健康百科和报告详情迁移台账映射、报告/费用底层患者范围输入，并修正就诊/互联网医院关闭态卡片纵向居中和预约查询底层输入边界。共享上下文只读取 owner-scoped 脱敏患者目录，不触发 Provider 或写入；采血旧端没有号源请求，因此不伪造号源、预约写入或取消；锦旗/表扬信和问诊入口不伪造公开记录、会话或提交。九个真机证据域仍未完成。详细边界见 [`../release/candidate-413cbea-miniprogram-runtime-2026-08-27.md`](../release/candidate-413cbea-miniprogram-runtime-2026-08-27.md)。

> **当前功能候选事实（2026-08-27）**：本轮小程序业务功能和 live 运行包来源为 `413cbea13f022831f63e9c750661eeabbffc68d5`；候选包含全量迁移入口覆盖视图、A–F 批次展示、契约族边界、逐入口说明、前序业务候选、协议静态页及其安全入口、健康自测安全数值子集、临床/外部安全页面、预约 Provider 入口、统一当前就诊人上下文、我的问诊患者作用域、旧端“我的快递”、采血预约、电子锦旗和健康表扬信安全页面迁移、公共日期窗口边界、微信资料拒绝后的设置页重试、选择页刷新并发门禁、我的快递三态、便民记录三态、报告/费用底层患者范围输入以及共享构建/门禁输入和关闭态卡片纵向居中修正。服务端当前生产 release 为 `b44421cd`，线上小程序 `13f597e` 仅作历史记录；后续源码或构建输入更新时必须重新生成来源记录；当前运行包证据见 [`../release/candidate-413cbea-miniprogram-runtime-2026-08-27.md`](../release/candidate-413cbea-miniprogram-runtime-2026-08-27.md)。
> 小程序 live 运行包来源为 `413cbea13f022831f63e9c750661eeabbffc68d5`，同源 pending 候选已在释放微信开发者工具锁后原子发布，pending 目录已清理；live 包包含 40 个页面。本地源码、live dist 和线上微信包仍必须分开记录。

> 当前功能里程碑为 live 运行包来源 `413cbea13f022831f63e9c750661eeabbffc68d5`，包含 40 个页面脚本和最新源码 `340 pass / 0 fail / 3726 expect()`。本轮统一便民记录区域状态，保留临床/服务入口和我的问诊入口当前患者展示、显式患者切换链路和错误重试，并补齐协议原文安全入口、公共日期窗口边界、微信资料拒绝后的设置页重试、选择页刷新并发门禁和我的快递三态；项目固定使用微信原生 `tabBar`，四个主入口、普通图标和选中图标只在 `app.json.tabBar.list` 声明一次；页面不渲染 `custom-tab-bar`，也不手动同步 selected。当前运行包继续把开发者工具 watcher 根、`src`/`scripts` 源码监听隔离、运行包来源日志、81×81 图标尺寸门禁、当前 Tab 重复导航 no-op、主 Tab 前四项注册约束、预约标签重载范围一致性、可见 action 分发回归、40 个页面 WXML 事件方法闭环、就诊记录分批展示和 `dist` 独立运行工程一致性纳入门禁。
> 紧随其后的 `485c0892`、`296516a5`、`39cbf021`、`8bc649f`、`1404a03`、`e5345c4`、`f97f9f0`、`fc70fa0b`、`7627843a`、`cd26a01`、`ad793c80`、`7f7a7a18`、`3b42b86` 和 `77cebe5` 构建记录是历史候选；当前验收、发布和回滚只认本页顶部 live `413cbea`，发布后没有 pending 目录。
...
> 当前本地 live 运行包来源校验值：`413cbea13f022831f63e9c750661eeabbffc68d5`，40 个页面，`runtime:verify` 已通过；此前因微信开发者工具占用 live `dist` 的同源 pending 候选已在锁释放后完成原子发布，pending 目录已清理。线上配套小程序仍为历史来源 `13f597ea9ee3f65b9be858117826d948339d904a`；线上服务端 release：`b44421cd321ff9ff23eeb49b12641d1772d2bdc1`。三者为有意分层发布，不要求同源。
...
> 当前执行基线（2026-08-27）：线上新 API 为 `b44421cd`，线上历史配套小程序运行包为 `13f597e`（来源 `13f597ea`）；本轮 adapter 门禁已随服务端切换进入线上，渠道 4 “全部挂号”查询仍待真机三层证据，普通资料真实写入/409 也仍待同一候选下的受控验收。最新本地业务源码和 live 运行输入为 `413cbea`（完整来源 `413cbea13f022831f63e9c750661eeabbffc68d5`），不等于线上小程序上传。
