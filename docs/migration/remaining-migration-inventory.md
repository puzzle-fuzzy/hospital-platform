...
> 当前发布基线（2026-08-22）：服务端上一次已验证 `2a2acd9bcc89c35988b75fc03304dbd48078c9d5`；小程序运行包来源
> `dc8cd5b8bbf99411831cc5112bc39683108cd990`。P0 运行层切换已完成，P1 真机/Provider 三层证据仍未完成。
...
| 优先级 | 当前动作 | 放行条件 | 当前决定 |
| --- | --- | --- | --- |
| P0 | 恢复受控发布链 | 阿里云 SSH 可用；服务端候选可在不停止旧 Python `8001` 的情况下切换；公网 live/ready、旧端口和新端口均有证据 | 已完成：`2a2acd9` 原子切换，新 API 重启，旧 Python PID 不变；详见 [`../release/2a2acd9b-production-acceptance-2026-08-22.md`](../release/2a2acd9b-production-acceptance-2026-08-22.md) |
| P1 | 真机只读验收 | 新小程序运行包来源与已验证服务端配套；微信登录、患者同步/切换、预约历史、门诊费用分别取得页面、HTTP requestId/Provider requestId、Pino 日志三层证据 | 当前进行中：`dc8cd5b8` 已构建、运行包校验和二维码入口已恢复，但尚无手机页面与当前服务端的完整三层业务证据，不能宣称业务完成 |
...
当前服务端上一次已验证 release 为 `2a2acd9bcc89c35988b75fc03304dbd48078c9d5`，小程序运行包来源为
`dc8cd5b8bbf99411831cc5112bc39683108cd990`。因此可以继续进行真机准入，但只有页面、客户端 HTTP 和 Pino 同链证据齐全时
才能把只读业务标记为已验收。

2026-08-22 09:18 CST 线上低敏观察显示已验证 release 曾出现微信登录、患者目录和预约历史事件，但门诊费用事件为 0；
