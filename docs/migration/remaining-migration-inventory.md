...
> 当前本地未发布候选的运行输入为 `ecff1f9ca97a1fb47ee090810a92a5fe533779f9`（提交 `ecff1f9`）。该候选继续使用原生共享四 Tab，并把开发者工具 watcher 根、`src/`/`scripts/` 源码监听隔离、运行包来源日志、native 图标缓存隔离、当前 Tab 重复导航 no-op 和 `dist` 独立运行工程一致性纳入门禁；真机控制台可通过启动日志核对候选来源。
...
> 当前本地小程序候选（2026-08-25）为 `ecff1f9ca97a1fb47ee090810a92a5fe533779f9`，`dist/` 已原子发布并通过 `runtime:verify`，16 个页面脚本完整；开发者工具已切换到直接打开 `apps/miniprogram/dist/` 的独立工程。之前的 `0f40ab9`、`4ea15b8`、`4e8f6877`、`0bf2bf8`、`7a85dce8`、`f4c844c1` 仅为历史来源；线上仍是 `13f597e`，两者不能混用。
...
> 当前执行基线（2026-08-25）：线上新 API 为 `8eb51b5ffe85b0b8f8a032783f893117d3df549d`，线上配套小程序运行包为 `13f597e`（来源 `13f597ea9ee3f65b9be858117826d948339d904a`）；本轮 adapter 门禁已随服务端切换进入线上，渠道 4 “全部挂号”查询仍待真机三层证据，普通资料真实写入/409 也仍待同一候选下的受控验收。未发布的本地候选 `ecff1f9ca97a1fb47ee090810a92a5fe533779f9` 只用于代码和运行包验证。
