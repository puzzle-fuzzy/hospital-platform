...
> **当前功能候选事实（2026-08-27）**：本轮小程序业务功能和 live 运行包来源为 `0be59f966de2c3a0861cb44e9a526a1ef557f6c7`；候选包含全量迁移入口覆盖视图、A–F 批次展示、契约族边界、逐入口说明、前序业务候选、协议静态页及其安全入口、健康自测安全数值子集、临床/外部安全页面、预约 Provider 入口、统一当前就诊人上下文、我的问诊患者作用域、旧端“我的快递”、采血预约、电子锦旗和健康表扬信安全页面迁移、公共日期窗口边界、微信资料拒绝后的设置页重试、选择页刷新并发门禁、我的快递三态、便民记录三态以及共享构建/门禁输入。服务端当前生产 release 为 `1107a78a`，线上小程序 `13f597e` 仅作历史记录；后续源码或构建输入更新时必须重新生成来源记录；当前运行包证据见 [`../release/candidate-0be59f96-miniprogram-runtime-2026-08-26.md`](../release/candidate-0be59f96-miniprogram-runtime-2026-08-26.md)。
> 小程序 live 运行包来源为 `0be59f966de2c3a0861cb44e9a526a1ef557f6c7`，本轮构建验证了同源 pending 候选，但因微信开发者工具仍占用 `dist`，pending 尚未发布替换 live；live 与 pending 均包含 40 个页面。本地源码、live dist 和线上微信包仍必须分开记录。
...
> 当前本地 live 运行包来源校验值：`0be59f966de2c3a0861cb44e9a526a1ef557f6c7`，40 个页面，`runtime:verify` 已通过；本轮再次构建时因微信开发者工具占用 live `dist`，同源 pending 候选已保留，待完全退出后执行 `runtime:publish-pending`。线上配套小程序仍为历史来源 `13f597ea9ee3f65b9be858117826d948339d904a`；线上服务端 release：`1107a78a47ac2fbe0557958251d66da9effc66de`。三者为有意分层发布，不要求同源。
...
> 当前执行基线（2026-08-27）：线上新 API 为 `1107a78a`，线上历史配套小程序运行包为 `13f597e`（来源 `13f597ea`）；本轮 adapter 门禁已随服务端切换进入线上，渠道 4 “全部挂号”查询仍待真机三层证据，普通资料真实写入/409 也仍待同一候选下的受控验收。最新本地业务源码和 live 运行输入为 `0be59f96`，不等于线上小程序上传。
...
> 本表的当前验收基线为服务端 `1107a78a47ac2fbe0557958251d66da9effc66de` 与小程序 `0be59f966de2c3a0861cb44e9a526a1ef557f6c7`（`0be59f96`）的分层配套。表内更早提交或历史线上窗口只用于说明实现来源，
