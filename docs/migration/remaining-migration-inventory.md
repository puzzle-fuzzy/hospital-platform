...
> 2026-08-21 当前候选代码门禁复核：`ec3e5f09` 小程序测试 `196 pass`、`1490 expects`，构建、运行包来源和运行包门禁均通过；
...
> 当前发布基线补充（2026-08-21）：服务端为 `5a31427`；小程序候选为 `ec3e5f09`，完整运行包来源为
> `ec3e5f094ecd0a85bbf98d4fc6ff9f14b27bb9ac`，尚未上传线上。
>
...
> 当前配套小程序候选构建来源为 `ec3e5f094ecd0a85bbf98d4fc6ff9f14b27bb9ac`（提交 `ec3e5f09`），尚未上传线上；用户已有的开发者工具配置修改不属于本次候选代码。
...
- 当前小程序运行输入来源为 `ec3e5f09`，本轮完整构建已生成并通过 `runtime:verify`；`dist/build-info.json` 的来源指纹为
  `ec3e5f094ecd0a85bbf98d4fc6ff9f14b27bb9ac`，注册页面和生成脚本均为 14 个；本轮患者上下文
