...
小程序测试为 `259 pass / 0 fail / 2445 expect()`；候选构建已确认 20 个页面脚本完整，预约记录增加未来/历史摘要的 8 条分批展开，待发布到被锁定的 `dist/` 后再生成真机入口。旧端二维码实际使用
...
> 已完成原生 tabBar 运行包重建、全局启动资料初始化、授权点击修复、未迁移入口统一状态页、首页悬浮客服入口收口、预约/报告/门诊缴费/新增就诊人二级动作状态路由、就诊预约记录未来/历史摘要的 8 条分批展示、候选构建和小程序 `259 pass / 0 fail / 2445 expect()`；候选目前因开发者工具锁定 `dist/` 暂存 pending，上一候选 custom-tab-bar 在真机未呈现，已撤回。全局用户资料仓库同时完成多 Tab 单飞和冻结快照引用一致性回归。
...
> **当前本地源码候选纠正（2026-08-25，优先级高于本文下方旧交接段落）**：最新本地候选为 `99c7e8fd76bd7b38de50d1c5cfdbc7002cba4a15`，原生页面 20 个，小程序回归为 `259 pass / 0 fail / 2445 expect()`；候选仍在 pending，`dist` 被开发者工具锁定。本文下方 `baa31df0`、17 页和 `258 pass` 的表述均为历史交接，不得作为当前真机入口。
