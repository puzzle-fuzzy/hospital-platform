...
> 当前功能里程碑为 `7bc595673a6ba41503a751cd38f0beb15ee8f23f`；当前 pending 运行输入来源为 `e5345c423a4cd44801e0b3e0a202063cad882c50`，包含 20 个页面脚本和当前源码 266 项小程序测试。项目固定使用微信原生 `tabBar`，四个主入口、普通图标和选中图标只在 `app.json.tabBar.list` 声明一次；页面不渲染 `custom-tab-bar`，也不手动同步 selected。当前候选继续把开发者工具 watcher 根、`src/`/`scripts/` 源码监听隔离、运行包来源日志、81×81 图标尺寸门禁、当前 Tab 重复导航 no-op、主 Tab 前四项注册约束、预约标签重载范围一致性、可见 action 分发回归、就诊记录 8 条分批展示和 `dist` 独立运行工程一致性纳入门禁。
...
> **最新候选覆盖补充（2026-08-25）**：当前小程序业务代码基线为 `7bc5956`，运行输入/pending 来源为 `e5345c4`，原生页面注册数为 20 个，健康百科目录、搜索、疾病/药品详情三个只读落点、客户端运行时响应校验、分类级空态修复、互联网医院安全壳迁移分类、全局资料授权跨 bundle 单飞、首页二维码入口状态和跨会话授权拒绝保护均已纳入；pending 候选位于 `.local/hospital-miniprogram/pending/`，当前源码回归为 266 项通过。页面接入不等于健康内容开放，正式审核 bundle、发布/撤回演练和真机证据仍待。
...
> 当前 live `dist` 仍为 `fcc6630ebfa7b0697cbd03a5e376ce6765d1643b`，因为开发者工具锁定目录尚未发布。本轮小程序回归为 `266 pass / 0 fail / 2548 expect()`；更早候选仅作历史交接。
