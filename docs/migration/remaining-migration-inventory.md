> 当前服务端 release 为 `8eb51b5ffe85b0b8f8a032783f893117d3df549d`（提交 `8eb51b5f`）；线上小程序运行包来源仍为 `13f597ea9ee3f65b9be858117826d948339d904a`（提交 `13f597e`）。本地最新业务源码候选为 pending `ded78c58`，live 运行输入仍为 `02dbf10`，两者不能互相替代。

> **当前回归复核（2026-08-26）**：最新 pending 候选小程序全量回归为 `329 pass / 0 fail / 3632 expect()`；下方旧段落中仍出现的旧测试数字仅是前一轮候选记录，不覆盖本条当前事实。

> **最新横向迁移补充（2026-08-26）**：患者签名和消息订阅已经完成原生安全展示层迁移；前一批补齐旧端采血预约、电子锦旗和健康表扬信的当前就诊人、错误重试、统一选人入口和明确关闭态；本轮又统一临床/服务入口和我的问诊入口的当前就诊人上下文，并补齐患者绑定/签名页查看协议原文的安全入口、公共日期窗口边界、微信资料拒绝后的设置页重试和选择页刷新并发门禁；最新候选又统一便民记录区域的加载、错误、未开放三态和固定高度。最新候选为 pending `ded78c58c53923ecf5232a8035b3e790e5959216`，回归为 `329 pass / 0 fail / 3632 expect()`，已通过 pending 运行包校验；当前 live 仍为 `02dbf10`。本轮另修正健康百科和报告详情迁移台账映射。共享上下文只读取 owner-scoped 脱敏患者目录，不触发 Provider 或写入；采血旧端没有号源请求，因此不伪造号源、预约写入或取消；锦旗/表扬信和问诊入口不伪造公开记录、会话或提交。九个真机证据域仍未完成。详细边界见 [`../release/candidate-ded78c58-miniprogram-runtime-2026-08-26.md`](../release/candidate-ded78c58-miniprogram-runtime-2026-08-26.md)。

> **当前功能候选事实（2026-08-26）**：本轮小程序业务功能候选为 pending `ded78c58c53923ecf5232a8035b3e790e5959216`，当前 live 运行输入为 `02dbf10419740d96c4445493df019021ac22bcfa`；候选包含全量迁移入口覆盖视图、A–F 批次展示、契约族边界、逐入口说明、前序业务候选、协议静态页及其安全入口、健康自测安全数值子集、临床/外部安全页面、预约 Provider 入口、统一当前就诊人上下文、我的问诊患者作用域、旧端“我的快递”、采血预约、电子锦旗和健康表扬信安全页面迁移、公共日期窗口边界、微信资料拒绝后的设置页重试、选择页刷新并发门禁、我的快递三态、便民记录三态以及共享构建/门禁输入。服务端当前生产 release 仍为 `8eb51b5f`，本地工作树不代表已部署 release；后续源码或构建输入更新时必须重新生成来源记录；当前运行包证据见 [`../release/candidate-ded78c58-miniprogram-runtime-2026-08-26.md`](../release/candidate-ded78c58-miniprogram-runtime-2026-08-26.md)。
> 小程序 live 运行包来源为 `02dbf10419740d96c4445493df019021ac22bcfa`，pending 候选来源为 `ded78c58c53923ecf5232a8035b3e790e5959216`，均包含 40 个页面；pending 校验已通过，live dist 尚未切换。本地源码、live dist 和线上包仍必须分开记录。

> 当前功能里程碑为 pending 业务源码候选 `ded78c58c53923ecf5232a8035b3e790e5959216`；当前 live 运行输入为 `02dbf10419740d96c4445493df019021ac22bcfa`，包含 40 个页面脚本和最新源码 `329 pass / 0 fail / 3632 expect()`。本轮统一便民记录区域状态，保留临床/服务入口和我的问诊入口当前患者展示、显式患者切换链路和错误重试，并补齐协议原文安全入口、公共日期窗口边界、微信资料拒绝后的设置页重试、选择页刷新并发门禁和我的快递三态；项目固定使用微信原生 `tabBar`，四个主入口、普通图标和选中图标只在 `app.json.tabBar.list` 声明一次；页面不渲染 `custom-tab-bar`，也不手动同步 selected。当前候选继续把开发者工具 watcher 根、`src`/`scripts` 源码监听隔离、运行包来源日志、81×81 图标尺寸门禁、当前 Tab 重复导航 no-op、主 Tab 前四项注册约束、预约标签重载范围一致性、可见 action 分发回归、40 个页面 WXML 事件方法闭环、就诊记录分批展示和 `dist` 独立运行工程一致性纳入门禁。
> 紧随其后的 `485c0892`、`296516a5`、`39cbf021`、`8bc649f`、`1404a03`、`e5345c4`、`f97f9f0`、`fc70fa0b`、`7627843a`、`cd26a01`、`ad793c80`、`7f7a7a18`、`3b42b86` 和 `77cebe5` 构建记录是历史候选；当前验收、发布和回滚只认本页顶部 pending `ded78c58` 与 live `02dbf10` 的明确分层。
...
> 当前事实源（2026-08-26，优先于下方历史交接）：小程序最新功能源码候选为 pending `ded78c58c53923ecf5232a8035b3e790e5959216`，当前 live 运行输入为 `02dbf10419740d96c4445493df019021ac22bcfa`；pending 运行包校验已通过但尚未切换 live。本轮小程序回归为 `329 pass / 0 fail / 3632 expect()`；更早候选仅作历史交接。
...
> 当前本地 live 运行包来源校验值：`02dbf10419740d96c4445493df019021ac22bcfa`，40 个页面，`runtime:verify` 已通过；最新 pending 来源为 `ded78c58c53923ecf5232a8035b3e790e5959216`，pending 校验已通过但尚未切换。线上配套小程序仍为 `13f597ea9ee3f65b9be858117826d948339d904a`；线上服务端 release：`8eb51b5ffe85b0b8f8a032783f893117d3df549d`。三者为有意分层发布，不要求同源。
...
> 当前执行基线（2026-08-26）：线上新 API 为 `8eb51b5ffe85b0b8f8a032783f893117d3df549d`，线上配套小程序运行包为 `13f597e`（来源 `13f597ea`）；本轮 adapter 门禁已随服务端切换进入线上，渠道 4 “全部挂号”查询仍待真机三层证据，普通资料真实写入/409 也仍待同一候选下的受控验收。最新本地业务源码候选为 pending `ded78c58`，当前 live 运行输入为 `02dbf104`，不等于线上小程序上传。
...
> 本轮继续只修改新项目；最新 pending 小程序功能源码为 `ded78c58c53923ecf5232a8035b3e790e5959216`，当前本地 live 运行输入仍为 `02dbf10419740d96c4445493df019021ac22bcfa`。
> 候选已完成原生 tabBar 运行包重建、全局启动资料初始化、授权点击修复、未迁移入口统一状态页、首页悬浮客服入口收口、预约/报告/门诊缴费/新增就诊人二级动作状态路由、就诊预约记录今日/未来/历史摘要的 8 条分批展示、健康百科分类级空态修复、互联网医院安全壳迁移分类修正、全局资料授权跨 bundle 单飞、首页二维码入口状态路由、跨会话授权拒绝状态保护、健康知识错误码契约、未知/过期入口错误语义、迁移入口覆盖视图、健康自测 BMI/血压安全数值子集、临床内容/外部入口/预约 Provider 页面外壳、统一临床/服务入口当前就诊人上下文、旧端“我的快递”和采血预约真实空态、电子锦旗和健康表扬信安全页面、患者协议原文安全入口及患者切换链路、公共日期窗口边界、微信资料拒绝后的设置页重试、选择就诊人刷新并发门禁、我的快递三态、便民记录三态与固定高度、候选构建和小程序 `329 pass / 0 fail / 3632 expect()`；pending 运行包校验已通过，尚未替换 live，上一候选 custom-tab-bar 在真机未呈现，已撤回。全局用户资料仓库同时完成多 Tab 单飞和冻结快照引用一致性回归。
...
| P1 | 真机只读与普通资料受控验收 | 新小程序运行包来源与已验证服务端配套；微信登录、患者同步/切换、预约历史、门诊费用、普通资料 GET 分别取得页面、HTTP requestId/Provider requestId、Pino 日志三层证据；普通资料 PUT/409 仅使用明确授权的可恢复测试值 | 当前进行中：线上真机配套运行包为 `13f597ea9ee3f65b9be858117826d948339d904a`，最新本地 pending 运行输入为 `ded78c58c53923ecf5232a8035b3e790e5959216`，当前 live 仍为 `02dbf10`；显式患者切换、页面截图、预约历史、门诊费用和普通资料三层证据仍待，真实写入尚未执行，详见 [`../release/next-readonly-business-acceptance-plan-2026-08-26.md`](../release/next-readonly-business-acceptance-plan-2026-08-26.md) |
...
> **当前事实源覆盖旧段落（2026-08-26）**：请以 pending 业务源码 `ded78c58c53923ecf5232a8035b3e790e5959216`、live 运行输入 `02dbf104`、40 个原生页面、`329 pass / 0 fail / 3632 expect()` 和 pending 运行包校验为准；本页前置的 `baa31df0`/17 页文字只作历史交接。
> **当前本地源码候选纠正（2026-08-26，优先级高于本文下方旧交接段落）**：最新业务源码候选为 pending `ded78c58c53923ecf5232a8035b3e790e5959216`，原生页面 40 个，当前工作树复跑为 `329 pass / 0 fail / 3632 expect()`；当前 live 仍为 `02dbf10`，候选等待释放 `dist` 锁后原子发布，后续真机必须从新 live `dist` 普通编译并生成新二维码。本文下方旧候选、17 页和旧测试数字的表述均为历史交接，不得作为当前真机入口。
