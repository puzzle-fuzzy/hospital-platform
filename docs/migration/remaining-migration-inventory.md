...
- 患者同步的 durable operation ledger、租约代次和重放分支已经在代码与 `0015_patient_directory_sync_operations` 中实现；当前仍缺生产 migration/schema probe、真实并发和公网验收，未完成这些证据前不能宣称线上已具备 durable 幂等，具体状态机见 [`patient-sync-idempotency-contract.md`](patient-sync-idempotency-contract.md)。
