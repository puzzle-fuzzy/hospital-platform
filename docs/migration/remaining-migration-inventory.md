...
> **最新横向迁移补充（2026-08-26）**：患者签名和消息订阅已经完成原生安全展示层迁移；前一批补齐旧端采血预约、电子锦旗和健康表扬信的当前就诊人、错误重试、统一选人入口和明确关闭态；本轮又统一临床/服务入口的当前就诊人上下文。当前候选为 `c7db7f04a0dee5a8aad6ba149f17bd413b0d1d5d`（`c7db7f04`），回归为 `307 pass / 0 fail / 3501 expect()`。共享上下文只读取 owner-scoped 脱敏患者目录，不触发 Provider 或写入；采血旧端没有号源请求，因此不伪造号源、预约写入或取消；锦旗/表扬信不伪造公开记录或提交。构建候选位于 `.local/hospital-miniprogram/pending/`，`dist/` 仍被开发者工具锁定，真机证据未完成。详细边界见 [`patient-surface-context-migration-2026-08-26.md`](patient-surface-context-migration-2026-08-26.md) 和 [`../release/candidate-c7db7f04-miniprogram-runtime-2026-08-26.md`](../release/candidate-c7db7f04-miniprogram-runtime-2026-08-26.md)。

> **当前功能候选事实（2026-08-26）**：本轮小程序功能候选为 `c7db7f04a0dee5a8aad6ba149f17bd413b0d1d5d`，当前运行输入/pending 来源与该候选一致；后者包含全量迁移入口覆盖视图、A–F 批次展示、契约族边界、逐入口说明、前序业务候选、协议静态页、健康自测安全数值子集、临床/外部安全页面、预约 Provider 入口、统一当前就诊人上下文、旧端“我的快递”、采血预约、电子锦旗和健康表扬信安全页面迁移以及共享构建/门禁输入。服务端当前生产 release 仍为 `8eb51b5f`，本地工作树不代表已部署 release；后续文档提交不改变已构建的 pending 运行包，源码或构建输入更新时必须重新生成来源记录；当前候选证据见 [`../release/candidate-c7db7f04-miniprogram-runtime-2026-08-26.md`](../release/candidate-c7db7f04-miniprogram-runtime-2026-08-26.md)。
> 小程序 pending 运行包来源为 `c7db7f04a0dee5a8aad6ba149f17bd413b0d1d5d`，包含 40 个页面；
> `apps/miniprogram/dist/` 仍由开发者工具锁定，当前 live dist 来源为 `fcc6630e`。本地候选、pending、live dist 和线上包必须分开记录。

> 当前功能里程碑为 `c7db7f04a0dee5a8aad6ba149f17bd413b0d1d5d`；当前 pending 运行输入来源一致，包含 40 个页面脚本和当前源码 `307 pass / 0 fail / 3501 expect()`。本轮统一临床/服务入口当前患者展示、显式患者切换链路和错误重试；项目固定使用微信原生 `tabBar`，四个主入口、普通图标和选中图标只在 `app.json.tabBar.list` 声明一次；页面不渲染 `custom-tab-bar`，也不手动同步 selected。当前候选继续把开发者工具 watcher 根、`src`/`scripts` 源码监听隔离、运行包来源日志、81×81 图标尺寸门禁、当前 Tab 重复导航 no-op、主 Tab 前四项注册约束、预约标签重载范围一致性、可见 action 分发回归、40 个页面 WXML 事件方法闭环、就诊记录分批展示和 `dist` 独立运行工程一致性纳入门禁。
> 紧随其后的 `485c0892`、`296516a5`、`39cbf021`、`8bc649f`、`1404a03`、`e5345c4`、`f97f9f0`、`fc70fa0b`、`7627843a`、`cd26a01`、`ad793c80`、`7f7a7a18`、`3b42b86` 和 `77cebe5` 构建记录是历史候选；当前验收、发布和回滚只认本页顶部的 `c7db7f04` 运行包候选。
...
> **当前事实覆盖（2026-08-26，优先于下方历史交接）**：源码已注册 40 个原生页面，健康自测中的 BMI/血压安全数值子集已从关闭外壳升级为 `partial`；当前旧端 64 页统计为 `replaced=8 / partial=19 / surface-only=29 / blocked-payment=7 / excluded=1`；本轮新增临床/服务入口共享当前就诊人上下文、重试和选择入口，但健康内容、协议同意/撤回/审计、临床 Provider、外部会话、患者域真实 contract 和支付医保仍未开放。pending 运行包来源为 `c7db7f04a0dee5a8aad6ba149f17bd413b0d1d5d`，但发布仍受微信开发者工具锁定影响。
...
> 当前事实源（2026-08-26，优先于下方历史交接）：小程序功能源码候选为 `c7db7f04a0dee5a8aad6ba149f17bd413b0d1d5d`，当前运行输入/pending 来源一致，位于 `.local/hospital-miniprogram/pending/`；
> 当前 live `dist` 仍为 `fcc6630ebfa7b0697cbd03a5e376ce6765d1643b`，因为开发者工具锁定目录尚未发布。本轮小程序回归为 `307 pass / 0 fail / 3501 expect()`；更早候选仅作历史交接。
...
> 当前本地最新运行包来源校验值：`c7db7f04a0dee5a8aad6ba149f17bd413b0d1d5d`，候选暂存于 pending；被开发者工具锁定的当前 `dist/` 仍为 `fcc6630ebfa7b0697cbd03a5e376ce6765d1643b`。线上配套小程序仍为 `13f597ea9ee3f65b9be858117826d948339d904a`；线上服务端 release：`8eb51b5ffe85b0b8f8a032783f893117d3df549d`。两者为有意分层发布，不要求同源。
...
> 当前执行基线（2026-08-26）：线上新 API 为 `8eb51b5ffe85b0b8f8a032783f893117d3df549d`，线上配套小程序运行包为 `13f597e`（来源 `13f597ea`）；本轮 adapter 门禁已随服务端切换进入线上，渠道 4 “全部挂号”查询仍待真机三层证据，普通资料真实写入/409 也仍待同一候选下的受控验收。本地最新运行包候选为 `c7db7f04`，尚未替换当前被工具锁定的 `dist/` 或线上小程序运行包。
...
> 本轮继续只修改新项目；当前小程序功能候选为 `c7db7f04`，最新本地运行输入/pending 候选为 `c7db7f04a0dee5a8aad6ba149f17bd413b0d1d5d`，
> 已完成原生 tabBar 运行包重建、全局启动资料初始化、授权点击修复、未迁移入口统一状态页、首页悬浮客服入口收口、预约/报告/门诊缴费/新增就诊人二级动作状态路由、就诊预约记录今日/未来/历史摘要的 8 条分批展示、健康百科分类级空态修复、互联网医院安全壳迁移分类修正、全局资料授权跨 bundle 单飞、首页二维码入口状态路由、跨会话授权拒绝状态保护、健康知识错误码契约、未知/过期入口错误语义、迁移入口覆盖视图、健康自测 BMI/血压安全数值子集、临床内容/外部入口/预约 Provider 页面外壳、统一临床/服务入口当前就诊人上下文、旧端“我的快递”和采血预约真实空态、电子锦旗和健康表扬信安全页面及患者切换链路、候选构建和小程序 `307 pass / 0 fail / 3501 expect()`；候选目前因开发者工具锁定 `dist/` 暂存 pending，上一候选 custom-tab-bar 在真机未呈现，已撤回。全局用户资料仓库同时完成多 Tab 单飞和冻结快照引用一致性回归。
...
> **当前事实源覆盖旧段落（2026-08-26）**：请以运行输入/pending 来源 `c7db7f04`、40 个原生页面、`307 pass / 0 fail / 3501 expect()` 和 pending 运行包为准；本页前置的 `baa31df0`/17 页文字只作历史交接。
> **当前本地源码候选纠正（2026-08-26，优先级高于本文下方旧交接段落）**：最新运行输入/pending 来源为 `c7db7f04a0dee5a8aad6ba149f17bd413b0d1d5d`，原生页面 40 个，当前工作树复跑为 `307 pass / 0 fail / 3501 expect()`；候选仍在 pending，`dist` 被微信开发者工具锁定。本文下方旧候选、17 页和 `258 pass` 的表述均为历史交接，不得作为当前真机入口。
