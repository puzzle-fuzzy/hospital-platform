...
| 预约科室/排班 | `appointments/departments`、`schedules` | `a11f117` 已取得真实 Provider 科室/排班只读结果；页面两列级联和排班分批渲染正常，日志已区分 Provider 读取与快照持久化状态 | 快照持久化恢复、多次稳定观察、公网/真机网络证据仍待；不能锁号、不能把 `scheduleId` 当成写入授权 |
