...
> 当前发布基线（2026-08-22 15:31 CST）：服务端已验证 `1e58bb66bf24021d2b680eb5fd03abfec467989a`；小程序运行包来源
> `5e43aed0e026cd48d980d58c468223b9a5ee8744`。P0 运行层切换已完成，P1 真机/Provider 三层证据仍未完成。
...
| P1 | 真机只读验收 | 新小程序运行包来源与已验证服务端配套；微信登录、患者同步/切换、预约历史、门诊费用分别取得页面、HTTP requestId/Provider requestId、Pino 日志三层证据 | 当前进行中：`5e43aed` 已构建，运行包校验通过，但尚无手机页面与当前服务端的完整三层业务证据，不能宣称业务完成 |
...
`5e43aed0e026cd48d980d58c468223b9a5ee8744`。因此可以继续进行真机准入，但只有页面、客户端 HTTP 和 Pino 同链证据齐全时
才能把只读业务标记为已验收。
...
> 2026-08-22 15:31 CST 当前工作树补充：小程序提交 `5e43aed` 的运行包已重新构建，二维码患者上下文门禁已收口；门诊费用继续只读，未开放支付、医保、结算或 HIS 写回。`dist/` 中测试脚本为 0，`runtime:verify` 通过。详细证据见 [`../release/candidate-5e43aed-local-build-2026-08-22.md`](../release/candidate-5e43aed-local-build-2026-08-22.md)。
...
下方内容保留过去的候选、线上观察和失败原因，仅用于追溯；不得覆盖本页顶部及“当前执行决策”中的 `1e58bb66 + 5e43aed` 基线。

除本页顶部“2026-08-22 当前执行决策”外，下方所有按历史日期记录的“当前”均只表示当时窗口的当前事实；
它们不能覆盖本页当前的 `1e58bb66 + 5e43aed` 基线。
