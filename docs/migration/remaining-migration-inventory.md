...
> **当前功能候选事实（2026-08-25）**：本轮小程序功能候选为 `7627843aa48ffe18651a5e5162202cbd0fd5d594`，完整构建后当前运行输入/pending 来源与该候选一致；后者包含全量迁移入口覆盖展示、前序业务候选以及共享构建/门禁输入。服务端本地候选按 `apps/api` 最新提交为 `b42922f4`；后续文档提交不改变已构建的 pending 运行包，源码或构建输入更新时必须重新生成来源记录；
> 小程序 pending 运行包来源为 `7627843aa48ffe18651a5e5162202cbd0fd5d594`，包含 20 个页面；
> `apps/miniprogram/dist/` 仍由开发者工具锁定，当前 live dist 来源为 `fcc6630e`。本地候选、pending、live dist 和线上包必须分开记录。

> 当前功能里程碑为 `7627843aa48ffe18651a5e5162202cbd0fd5d594`；当前 pending 运行输入来源一致，包含 20 个页面脚本和当前源码 `276 pass / 0 fail / 2915 expect()`。项目固定使用微信原生 `tabBar`，四个主入口、普通图标和选中图标只在 `app.json.tabBar.list` 声明一次；页面不渲染 `custom-tab-bar`，也不手动同步 selected。当前候选继续把开发者工具 watcher 根、`src/`/`scripts/` 源码监听隔离、运行包来源日志、81×81 图标尺寸门禁、当前 Tab 重复导航 no-op、主 Tab 前四项注册约束、预约标签重载范围一致性、可见 action 分发回归、20 个页面 WXML 事件方法闭环、就诊记录 8 条分批展示和 `dist` 独立运行工程一致性纳入门禁。
> 紧随其后的 `485c0892`、`296516a5`、`39cbf021`、`8bc649f`、`1404a03`、`e5345c4`、`f97f9f0` 和 `fc70fa0b` 构建记录是历史候选；当前验收、发布和回滚只认本页顶部的 `7627843a` 运行包候选。
...
> **最新候选覆盖补充（2026-08-25）**：当前小程序功能候选为 `7627843a`，原生页面注册数为 20 个，健康百科目录、搜索、疾病/药品详情三个只读落点、客户端运行时响应校验、分类级空态修复、互联网医院安全壳迁移分类、全局资料授权跨 bundle 单飞、首页二维码入口状态、跨会话授权拒绝保护、健康知识错误码文案、未知/过期入口错误语义和迁移入口覆盖信息均已纳入；pending 候选位于 `.local/hospital-miniprogram/pending/`，当前源码回归为 `276 pass / 0 fail / 2915 expect()`，且首页/我的 31 个可见 action 与全部 20 个页面 WXML 事件均通过广度审计。页面接入不等于健康内容开放，正式审核 bundle、发布/撤回演练和真机证据仍待。

> 当前事实源（2026-08-25，优先于下方历史交接）：小程序功能源码候选为 `7627843aa48ffe18651a5e5162202cbd0fd5d594`，当前运行输入/pending 来源一致，位于 `.local/hospital-miniprogram/pending/`；
> 当前 live `dist` 仍为 `fcc6630ebfa7b0697cbd03a5e376ce6765d1643b`，因为开发者工具锁定目录尚未发布。本轮小程序回归为 `276 pass / 0 fail / 2915 expect()`；更早候选仅作历史交接。

> 本候选新增旧端 64 页面逐页机器台账、健康百科客户端运行时响应校验、分类级空态修复和迁移入口覆盖信息，不改变线上服务或旧项目；当前交接与运行包来源详情见
> [`full-migration-handoff-2026-08-25.md`](full-migration-handoff-2026-08-25.md) 和
> [`../release/candidate-7627843-miniprogram-runtime-2026-08-25.md`](../release/candidate-7627843-miniprogram-runtime-2026-08-25.md)。
...
> 当前本地最新运行包来源校验值：`7627843aa48ffe18651a5e5162202cbd0fd5d594`，候选暂存于 pending；被开发者工具锁定的当前 `dist/` 仍为 `fcc6630ebfa7b0697cbd03a5e376ce6765d1643b`。线上配套小程序仍为 `13f597ea9ee3f65b9be858117826d948339d904a`；线上服务端 release：`8eb51b5ffe85b0b8f8a032783f893117d3df549d`。两者为有意分层发布，不要求同源。
...
> 当前执行基线（2026-08-25）：线上新 API 为 `8eb51b5ffe85b0b8f8a032783f893117d3df549d`，线上配套小程序运行包为 `13f597e`（来源 `13f597ea9ee3f65b9be858117826d948339d904a`）；本轮 adapter 门禁已随服务端切换进入线上，渠道 4 “全部挂号”查询仍待真机三层证据，普通资料真实写入/409 也仍待同一候选下的受控验收。本地最新运行包候选为 `7627843a`，尚未替换当前被工具锁定的 `dist/` 或线上小程序运行包。
...
> 本轮继续只修改新项目；当前小程序功能候选为 `7627843a`，最新本地运行输入/pending 候选为 `7627843a`，
> 已完成原生 tabBar 运行包重建、全局启动资料初始化、授权点击修复、未迁移入口统一状态页、首页悬浮客服入口收口、预约/报告/门诊缴费/新增就诊人二级动作状态路由、就诊预约记录未来/历史摘要的 8 条分批展示、健康百科分类级空态修复、互联网医院安全壳迁移分类修正、全局资料授权跨 bundle 单飞、首页二维码入口状态路由、跨会话授权拒绝状态保护、健康知识错误码契约、未知/过期入口错误语义、迁移入口覆盖信息、候选构建和小程序 `276 pass / 0 fail / 2915 expect()`；候选目前因开发者工具锁定 `dist/` 暂存 pending，上一候选 custom-tab-bar 在真机未呈现，已撤回。全局用户资料仓库同时完成多 Tab 单飞和冻结快照引用一致性回归。
...
> **当前事实源覆盖旧段落（2026-08-25）**：请以运行输入/pending 来源 `7627843a`、20 个原生页面、`276 pass / 0 fail / 2915 expect()` 和 pending 运行包为准；本页前置的 `baa31df0`/17 页文字只作历史交接。
> **当前本地源码候选纠正（2026-08-25，优先级高于本文下方旧交接段落）**：最新运行输入/pending 来源为 `7627843aa48ffe18651a5e5162202cbd0fd5d594`，原生页面 20 个，当前工作树复跑为 `276 pass / 0 fail / 2915 expect()`；候选仍在 pending，`dist` 被开发者工具锁定。本文下方旧候选、17 页和 `258 pass` 的表述均为历史交接，不得作为当前真机入口。
