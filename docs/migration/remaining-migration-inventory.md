...
> 当前事实源（2026-08-26，优先于下方历史交接）：小程序功能源码候选与 pending 运行输入均为 `731c95718d26bcf2826987b72f79295413b203d7`，位于 `.local/hospital-miniprogram/pending/`；
...
> 历史执行基线（2026-08-25 早前窗口）为 `45742ff4450b223b8db3b36e4a3859e3fc86e1c5`，随后曾短暂验证 `ad7b079`；这些候选以及此前的 `7fc22fae` 均为历史来源，当前验收以本页顶部业务源码与 pending 运行输入 `731c9571` 为准，live `dist` 仍是 `fcc6630e`。早前候选已经通过当时的 `runtime:verify`，但不能继续作为当前运行包来源；开发者工具应直接打开 `apps/miniprogram/dist/` 的独立工程。之前的 `0f40ab9`、`4ea15b8`、`4e8f6877`、`0bf2bf8`、`7a85dce8`、`f4c844c1`、`ecff1f9`、`49c641b6`、`e039e99f`、`3b8a04e5` 和 `77cebe5` 仅为历史来源；线上仍是 `13f597e`，两者不能混用。
...
> 当前本地 pending 运行包来源校验值：`731c95718d26bcf2826987b72f79295413b203d7`，候选暂存于 pending；被开发者工具锁定的当前 `dist/` 仍为 `fcc6630ebfa7b0697cbd03a5e376ce6765d1643b`。线上配套小程序仍为 `13f597ea9ee3f65b9be858117826d948339d904a`；线上服务端 release：`8eb51b5ffe85b0b8f8a032783f893117d3df549d`。三者为有意分层发布，不要求同源。
...
> 当前执行基线（2026-08-26）：线上新 API 为 `8eb51b5ffe85b0b8f8a032783f893117d3df549d`，线上配套小程序运行包为 `13f597e`（来源 `13f597ea`）；本轮 adapter 门禁已随服务端切换进入线上，渠道 4 “全部挂号”查询仍待真机三层证据，普通资料真实写入/409 也仍待同一候选下的受控验收。本地业务源码与 pending 运行输入为 `731c9571`，尚未替换当前被工具锁定的 `dist/` 或线上小程序运行包。
...
> 本轮继续只修改新项目；当前小程序功能源码与最新本地运行输入/pending 候选均为 `731c95718d26bcf2826987b72f79295413b203d7`，
...
> **当前事实源覆盖旧段落（2026-08-26）**：请以业务源码与运行输入/pending 来源 `731c9571`、40 个原生页面、`312 pass / 0 fail / 3548 expect()` 和 pending 运行包为准；本页前置的 `baa31df0`/17 页文字只作历史交接。
> **当前本地源码候选纠正（2026-08-26，优先级高于本文下方旧交接段落）**：业务源码与最新运行输入/pending 来源均为 `731c95718d26bcf2826987b72f79295413b203d7`，原生页面 40 个，当前工作树复跑为 `312 pass / 0 fail / 3548 expect()`；候选仍在 pending，`dist` 被微信开发者工具锁定。本文下方旧候选、17 页和 `258 pass` 的表述均为历史交接，不得作为当前真机入口。
