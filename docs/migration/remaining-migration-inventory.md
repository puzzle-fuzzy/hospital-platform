...
> 本轮继续只修改新项目；当前小程序功能源码和本地 live 运行输入为 `6f47c6408fe5b62025bd74fa66893f306eb7b9aa`。
> 候选已完成原生 tabBar 运行包重建、全局启动资料初始化、授权点击修复、未迁移入口统一状态页、首页悬浮客服入口收口、预约/报告/门诊缴费/新增就诊人二级动作状态路由、就诊预约记录今日/未来/历史摘要的 8 条分批展示、健康百科分类级空态修复、互联网医院安全壳迁移分类修正、全局资料授权跨 bundle 单飞、首页二维码入口状态路由、跨会话授权拒绝状态保护、健康知识错误码契约、未知/过期入口错误语义、迁移入口覆盖视图、健康自测 BMI/血压安全数值子集、临床内容/外部入口/预约 Provider 页面外壳、统一临床/服务入口当前就诊人上下文、旧端“我的快递”和采血预约真实空态、电子锦旗和健康表扬信安全页面、患者协议原文安全入口及患者切换链路、公共日期窗口边界、微信资料被拒绝后的设置页重试、选择就诊人刷新并发门禁、我的快递三态、便民记录三态与固定高度、候选构建和小程序 `336 pass / 0 fail / 3698 expect()`；运行包已完成校验并原子切换到 live，上一候选 custom-tab-bar 在真机未呈现，已撤回。全局用户资料仓库同时完成多 Tab 单飞和冻结快照引用一致性回归，会话失效时旧 owner 微信资料缓存也会被清理。
