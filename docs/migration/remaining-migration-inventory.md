...
> **最新候选覆盖补充（2026-08-25）**：当前小程序业务代码基线为 `7bc5956`，运行输入/pending 来源为 `e5345c4`，原生页面注册数为 20 个，健康百科目录、搜索、疾病/药品详情三个只读落点、客户端运行时响应校验、分类级空态修复、互联网医院安全壳迁移分类、全局资料授权跨 bundle 单飞、首页二维码入口状态和跨会话授权拒绝保护均已纳入；pending 候选位于 `.local/hospital-miniprogram/pending/`，当前源码回归为 267 项通过，且首页/我的 31 个可见 action 已通过分发审计。页面接入不等于健康内容开放，正式审核 bundle、发布/撤回演练和真机证据仍待。
