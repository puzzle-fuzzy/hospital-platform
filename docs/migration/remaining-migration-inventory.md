...
| 优先级 | 当前动作 | 放行条件 | 当前决定 |
| --- | --- | --- | --- |
| P0 | 恢复受控发布链 | 阿里云 SSH 可用；服务端候选可在不停止旧 Python `8001` 的情况下切换；公网 live/ready、旧端口和新端口均有证据 | 当前阻塞在 SSH 公钥被拒绝，不能上传、重启或改线上配置 |
| P1 | 真机只读验收 | 新小程序运行包来源与已部署服务端配套；微信登录、患者同步/切换、预约历史、门诊费用分别取得页面、HTTP requestId/Provider requestId、Pino 日志三层证据 | 代码门禁已通过，但当前没有本轮候选的完整三层证据，不能宣称业务完成 |
...
当前本地代码候选为服务端 `4f2d890d`，小程序运行包来源为 `b0e093565493285de07fe549879f8b87eda649cc7`；线上仍为服务端
`7181e99e`，配套历史小程序候选为 `4e1b2e2`。因此本地候选只能继续做代码和门禁验证，不能直接进行真机或 Provider 业务验收。
...
SSH 恢复前，任何需要线上环境的步骤都停止在只读检查；旧项目、旧服务、旧数据库和旧 Redis 不作修改。

> 当前候选：服务端 release `7181e99e`（完整提交 `7181e99e3a352244102f5591279528b3b66332c9`）；小程序运行包来源为 `4e1b2e224964797c103eba832323ee7074c7ad2b`（提交 `4e1b2e2`），真机前必须重新构建并核对来源。

> 2026-08-22 07:32 CST 工作树补充：当前本地服务端/小程序候选为 `b0e093565493285de07fe549879f8b87eda649cc7`，
