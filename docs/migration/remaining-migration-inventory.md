...
> 当前本地未发布候选的运行输入为 `45742ff4450b223b8db3b36e4a3859e3fc86e1c5`（提交 `45742ff4`）。该候选继续使用原生共享四 Tab，并把开发者工具 watcher 根、`src/`/`scripts/` 源码监听隔离、运行包来源日志、native 图标缓存隔离、81×81 图标尺寸门禁、当前 Tab 重复导航 no-op、主 Tab 前四项注册约束、预约标签重载范围一致性和 `dist` 独立运行工程一致性纳入门禁；真机控制台可通过启动日志核对候选来源。
...
> 本轮继续只修改新项目。当前本地小程序候选为 `45742ff4450b223b8db3b36e4a3859e3fc86e1c5`，
