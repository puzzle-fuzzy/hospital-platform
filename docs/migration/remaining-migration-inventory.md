...
> 2026-08-21 当前候选代码门禁复核：`cde7bc9` 小程序测试 `186 pass`、`1461 expects`，构建、运行包来源和全仓门禁均通过；
...
> 当前发布基线补充（2026-08-21）：服务端为 `5a31427`；小程序候选为 `cde7bc9`，完整运行包来源为
> `cde7bc90a23698398e9474944adf42b36f37982c`，尚未上传线上。
>
...
> 当前配套小程序候选构建来源为 `cde7bc90a23698398e9474944adf42b36f37982c`（提交 `cde7bc9`），尚未上传线上；用户已有的开发者工具配置修改不属于本次候选代码。
...
> `cde7bc90a23698398e9474944adf42b36f37982c`。代码、架构、迁移清单、Provider intake、文档、类型、测试均通过；小程序构建门禁随后以当前候选重新执行，
> 395 篇 Markdown 文档、Biome、9 个 workspace 的类型检查/测试/构建均通过；当前 API 测试 199 项通过，小程序测试 186 项通过、1461 个断言通过，
> 配置包新增的空白上游地址回退测试也通过。
...
- 当前小程序运行输入来源为 `cde7bc9`，本轮完整构建已生成并通过 `runtime:verify`；`dist/build-info.json` 的来源指纹为
  `cde7bc90a23698398e9474944adf42b36f37982c`，注册页面和生成脚本均为 14 个；本轮患者上下文
