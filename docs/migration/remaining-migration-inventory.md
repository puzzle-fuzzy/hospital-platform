...
> 未授权患者/预约接口均返回 `401 unauthorized`。本地 `ce8d68b` 尚未部署；这次没有 Provider 调用、服务重启或
...
- 当前小程序运行输入来源为 `ce8d68b`，本轮完整构建已生成并通过 `runtime:verify`；`dist/build-info.json` 的来源指纹为
  `ce8d68b15fc0c6a813fbf7a95d36610167874b8e`，注册页面和生成脚本均为 14 个；本轮患者上下文
