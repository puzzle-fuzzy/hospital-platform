...
仍缺真实证据：Redis 实际 TTL、多就诊人切换/失效恢复、众阳预约历史/报告/门诊费用、公网分域真机页面和生产回归；预约科室/排班已有真实只读结果，但本轮排班快照持久化曾暂时不可用
```
...
- 预约目录已取得一次真实 Provider、内网 API 和调试窗口只读证据，但快照持久化出现暂时不可用；预约历史、报告、门诊费用仍需分别完成 provider、内网 API、公网 HTTPS 和真机四层证据。
