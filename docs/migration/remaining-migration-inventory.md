...
> 当前执行基线（2026-08-27）：线上新 API 为 `5738a71e`，线上历史配套小程序运行包为 `13f597e`（来源 `13f597ea`）；本轮 adapter 门禁已随服务端切换进入线上，渠道 4 “全部挂号”查询仍待真机三层证据，普通资料真实写入/409 也仍待同一候选下的受控验收。当前本地 live 运行输入为 `1bc5bf6`（完整来源 `1bc5bf6f7cc4d38fad29fbf7e8aca3f65c46b916`），启动保护候选 `1bc5bf6` 已完成 pending 校验但尚未覆盖 live，不等于线上小程序上传。
...
`1bc5bf6f7cc4d38fad29fbf7e8aca3f65c46b916`；启动保护候选 `1bc5bf6f7cc4d38fad29fbf7e8aca3f65c46b916` 尚未覆盖 live。因此可以继续进行真机准入，但必须先发布候选并重新编译，且只有页面、客户端 HTTP 和 Pino 同链证据齐全时
才能把只读业务标记为已验收；本地测试仍不能代替部署后的页面和 Provider 证据。

> 当前小程序运行包来源为 `1bc5bf6f7cc4d38fad29fbf7e8aca3f65c46b916`；待发布启动保护候选为
> `1bc5bf6f7cc4d38fad29fbf7e8aca3f65c46b916`，只能通过 pending 运行包校验和发布器进入 live，不能在真机证据中混用两套来源。
