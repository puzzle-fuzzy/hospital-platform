...
> `171a8743185fb4ecc1696851662659c1a0ee7ebf`。P0 运行层切换已完成，P1 真机/Provider 三层证据仍未完成。
...
| P1 | 真机只读验收 | 新小程序运行包来源与已验证服务端配套；微信登录、患者同步/切换、预约历史、门诊费用分别取得页面、HTTP requestId/Provider requestId、Pino 日志三层证据 | 当前进行中：`171a874` 与 `0e2a366e` 已配套发布，运行层 smoke 通过，但尚无手机页面与当前服务端的完整业务三层证据，不能宣称业务完成 |
...
`171a8743185fb4ecc1696851662659c1a0ee7ebf`。因此可以继续进行真机准入，但只有页面、客户端 HTTP 和 Pino 同链证据齐全时
才能把只读业务标记为已验收。
...
> 2026-08-22 17:50 CST 历史工作树补充：小程序提交 `a64fe023` 的运行包已重新构建，患者范围页面入口门禁已收口；门诊费用继续只读，未开放支付、医保、结算或 HIS 写回。该记录不覆盖当前 `171a874`，详细证据见 [`../release/candidate-a64fe023-local-build-2026-08-22.md`](../release/candidate-a64fe023-local-build-2026-08-22.md)。
...
下方内容保留过去的候选、线上观察和失败原因，仅用于追溯；不得覆盖本页顶部及“当前执行决策”中的 `0e2a366e + 171a874` 基线。

除本页顶部“2026-08-22 当前执行决策”外，下方所有按历史日期记录的“当前”均只表示当时窗口的当前事实；
它们不能覆盖本页当前的 `0e2a366e + 171a874` 基线。
...
> `171a8743185fb4ecc1696851662659c1a0ee7ebf`（`171a874`）。表内更早提交或历史线上窗口只用于说明实现来源，
...
| 预约科室/排班 | `appointments/departments`、`schedules` | 历史 `41c9c18` 曾取得真实 Provider 科室/排班只读结果并出现 `snapshotPersistenceStatus=persisted`；当前代码仍只接受已确认的 `usableSourceNum`，页面两列级联和排班分批渲染正常 | `0e2a366e + 171a874` 尚待重新取得当前候选的多次稳定、公网/真机网络证据；缺少 `usableSourceNum` 的响应会 fail-closed；不能锁号、不能把 `scheduleId` 当成写入授权 |
| 预约历史/爽约筛选 | `appointments/records`、`missed-appointments` | contract、服务端状态映射、挂号记录页和 `missed` 派生页已实现；历史 release 曾观察到预约记录与爽约筛选结果，当前 `0e2a366e + 171a874` 仍需重新取得同一业务域证据；在线渠道固定 `requestChannel=3`，在线标签只排除服务端明确的 `cancelled`，爽约页只接受 `missed`；全部渠道标签保留位置但 fail-closed 提示迁移中 | 全部渠道仍缺独立 `requestChannel=4` contract、Provider 字段、公网和真机业务证据；未知状态不能推导为爽约；缺口审计见 [`request-channel-4-all-records-contract-audit-2026-08-18.md`](request-channel-4-all-records-contract-audit-2026-08-18.md) |
...
| 门诊费用 | `payments/outpatient/records` | 只读目录已实现，查询时间显式使用 `Asia/Shanghai`；待缴/已缴各一次 `requested → loaded` 的空列表观察属于历史 release，当前 `0e2a366e + 171a874` 仍需重新取得业务证据，页面展示合法空态并保留患者更换入口 | 真实微信真机证据、费用详情、金额非空样例、支付、医保、结算回写和退费未开放；空列表不能替代费用字段和支付链路验收 |
