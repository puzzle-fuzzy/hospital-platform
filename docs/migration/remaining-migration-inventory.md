...
> 当前发布基线补充（2026-08-21）：服务端为 `5a31427`；小程序候选为 `f66514d`，完整运行包来源为
> `f66514de81c051cb8ade1477f758700b2837b9b7`，尚未上传线上。
>
...
> 当前配套小程序候选构建来源为 `f66514de81c051cb8ade1477f758700b2837b9b7`（提交 `f66514d`），尚未上传线上；用户已有的开发者工具配置修改不属于本次候选代码。
...
> `f66514de81c051cb8ade1477f758700b2837b9b7`。代码、架构、迁移清单、Provider intake、文档、类型、测试均通过；小程序构建门禁随后以当前候选重新执行，
> 343 篇 Markdown 文档、Biome、9 个 workspace 的类型检查/测试/构建均通过；当前 API 测试 188 项通过，小程序测试 174 项通过、1378 个断言通过，
> 配置包新增的空白上游地址回退测试也通过。
...
- 当前小程序运行输入来源为 `f66514d`，本轮完整构建已生成并通过 `runtime:verify`；`dist/build-info.json` 的来源指纹为
  `f66514de81c051cb8ade1477f758700b2837b9b7`，注册页面和生成脚本均为 14 个；本轮患者上下文
