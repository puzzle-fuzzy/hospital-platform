...
> 当前本地未发布候选的运行输入为 `39b50d5c4287f54ecc24e8564e2dc811a55c1d1b`（提交 `39b50d5c`）。该候选继续使用原生共享四 Tab，并把开发者工具私有配置、`src/`/`scripts/` 源码监听隔离、运行包来源日志、native 图标缓存隔离、当前 Tab 重复导航 no-op 和 `dist` 运行包一致性纳入门禁；真机控制台可通过启动日志核对候选来源。
...
> 当前本地小程序候选（2026-08-24）为 `39b50d5c4287f54ecc24e8564e2dc811a55c1d1b`，`dist/` 已原子发布并通过 `runtime:verify`，16 个页面脚本完整。之前的 `0f40ab9`、`4ea15b8`、`4e8f6877`、`0bf2bf8`、`7a85dce8`、`f4c844c1` 仅为历史来源；线上仍是 `13f597e`，两者不能混用。
...
> 当前执行基线（2026-08-24）：线上新 API 为 `8eb51b5ffe85b0b8f8a032783f893117d3df549d`，线上配套小程序运行包为 `13f597e`（来源 `13f597ea9ee3f65b9be858117826d948339d904a`）；本轮 adapter 门禁已随服务端切换进入线上，渠道 4 “全部挂号”查询仍待真机三层证据。未发布的本地候选 `39b50d5c4287f54ecc24e8564e2dc811a55c1d1b` 只用于代码和运行包验证。
...
| P1 | 真机只读验收 | 新小程序运行包来源与已验证服务端配套；微信登录、患者同步/切换、预约历史、门诊费用分别取得页面、HTTP requestId/Provider requestId、Pino 日志三层证据 | 当前进行中：线上真机配套运行包为 `13f597ea9ee3f65b9be858117826d948339d904a`，本地未发布候选为 `39b50d5c4287f54ecc24e8564e2dc811a55c1d1b`；`dist` 原子构建和运行包校验已通过，但显式患者切换、页面截图、预约历史和门诊费用三层证据仍待，详见 [`../release/current-real-device-login-patient-observation-2026-08-24.md`](../release/current-real-device-login-patient-observation-2026-08-24.md) |
