...
> 当前执行基线（2026-08-25）：线上新 API 为 `8eb51b5ffe85b0b8f8a032783f893117d3df549d`，线上配套小程序运行包为 `13f597e`（来源 `13f597ea9ee3f65b9be858117826d948339d904a`）；本轮 adapter 门禁已随服务端切换进入线上，渠道 4 “全部挂号”查询仍待真机三层证据，普通资料真实写入/409 也仍待同一候选下的受控验收。本地候选当前以本页顶部的 `21af93b4` 为准，只用于代码和运行包验证，尚未替换线上小程序运行包。
...
| P1 | 真机只读与普通资料受控验收 | 新小程序运行包来源与已验证服务端配套；微信登录、患者同步/切换、预约历史、门诊费用、普通资料 GET 分别取得页面、HTTP requestId/Provider requestId、Pino 日志三层证据；普通资料 PUT/409 仅使用明确授权的可恢复测试值 | 当前进行中：线上真机配套运行包为 `13f597ea9ee3f65b9be858117826d948339d904a`，本地未发布候选为 `21af93b4d398e3d424aba9506c7bfbef9a3cfbd8`；`dist` 原子构建、独立工程和运行包校验已通过，但显式患者切换、页面截图、预约历史、门诊费用和普通资料三层证据仍待，真实写入尚未执行，详见 [`../release/next-profile-business-acceptance-plan-2026-08-24.md`](../release/next-profile-business-acceptance-plan-2026-08-24.md) |
