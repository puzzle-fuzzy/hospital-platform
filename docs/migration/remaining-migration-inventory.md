...
## 当前 release 基线（2026-08-18 11:09 CST）
...
- 下一步固定使用服务端 `c63dba9` + 小程序客户端 `055a784` 的验收组合；当前 `dist/` 已重新构建并验证 14 个页面脚本存在，
  小程序 106 项测试、932 个断言通过。复用有效微信会话，按“刷新/显式切换就诊人 → 我的挂号 → 爽约记录 → 门诊待缴/已缴”取得
