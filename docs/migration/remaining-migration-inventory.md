...
> 当前发布基线（2026-08-22）：服务端 `49f74e0209778836db41bef6249758b4f590792a`；小程序运行包来源
> `b0e093565493285e07fe549879f8b87eda649cc7`。P0 运行层切换已完成，P1 真机/Provider 三层证据仍未完成。
...
| 优先级 | 当前动作 | 放行条件 | 当前决定 |
| --- | --- | --- | --- |
| P0 | 恢复受控发布链 | 阿里云 SSH 可用；服务端候选可在不停止旧 Python `8001` 的情况下切换；公网 live/ready、旧端口和新端口均有证据 | 已完成：`49f74e0` 原子切换，新 API 重启，旧 Python PID 不变；详见生产验收记录 |
| P1 | 真机只读验收 | 新小程序运行包来源与已部署服务端配套；微信登录、患者同步/切换、预约历史、门诊费用分别取得页面、HTTP requestId/Provider requestId、Pino 日志三层证据 | 当前进行中：构建和工具缓存已恢复，但尚无当前 `49f74e0` 的完整三层业务证据，不能宣称业务完成 |
...
当前服务端 release 为 `49f74e0209778836db41bef6249758b4f590792a`，小程序运行包来源为
...
> 当前候选：服务端 release `49f74e0`（完整提交 `49f74e0209778836db41bef6249758b4f590792a`）；小程序运行包来源为 `b0e093565493285e07fe549879f8b87eda649cc7`（提交 `b0e0935`），真机前必须重新普通编译并核对来源。
