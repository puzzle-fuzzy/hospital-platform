...
> 当前本地未发布候选的运行输入为 `0bf2bf828b107c7b65b26ac5457edfa2b4e1ac9e`（提交 `0bf2bf8`）。该候选继续使用原生共享四 Tab，并把开发者工具私有配置、运行包来源日志和 `dist` 运行包一致性纳入门禁；真机控制台可通过启动日志核对候选来源。
...
> 当前本地小程序候选（2026-08-24）为 `0bf2bf828b107c7b65b26ac5457edfa2b4e1ac9e`，`dist/` 已原子发布并通过 `runtime:verify`，16 个页面脚本完整。之前的 `0f40ab9`、`4ea15b8`、`4e8f6877` 仅为历史来源；线上仍是 `13f597e`，两者不能混用。
