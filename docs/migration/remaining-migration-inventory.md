...
> 当前配套小程序候选构建来源为 `3a89312cd982ee2fc490b75515cdb6c7d58d513e`（提交 `3a89312`），尚未上传线上；用户已有的开发者工具配置修改不属于本次候选代码。
...
> 未授权患者/预约接口均返回 `401 unauthorized`。本地 `3a89312` 尚未部署；这次没有 Provider 调用、服务重启或
...
- 当前小程序运行输入来源为 `3a89312`，本轮完整构建已生成并通过 `runtime:verify`；`dist/build-info.json` 的来源指纹为
  `3a89312cd982ee2fc490b75515cdb6c7d58d513e`，注册页面和生成脚本均为 14 个；本轮患者上下文
