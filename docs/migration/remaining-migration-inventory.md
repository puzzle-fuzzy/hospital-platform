...
> 当前本地未发布候选的运行输入基线为 `2bf9d8d9f67521067d761b48cc2bfec449ef1348`（提交 `2bf9d8d9`；功能修正基线为 `e48cfe42`）。该候选在收紧微信开发者工具单一项目边界的基础上，关闭公共/本机热重载并在构建时校验四项 Tab 的页面、普通图标和选中图标，保留共享四 Tab、挂号卡片层级、爽约患者上下文边界和多页面查询状态外壳。
...
| P1 | 真机只读验收 | 新小程序运行包来源与已验证服务端配套；微信登录、患者同步/切换、预约历史、门诊费用分别取得页面、HTTP requestId/Provider requestId、Pino 日志三层证据 | 当前进行中：线上真机配套运行包为 `13f597ea9ee3f65b9be858117826d948339d904a`，已取得微信登录/患者同步的服务端与真机调试网络观察；本地未发布页面候选为 `2bf9d8d9f67521067d761b48cc2bfec449ef1348`。显式患者切换、页面截图、预约历史和门诊费用三层证据仍待，详见 [`../release/current-real-device-login-patient-observation-2026-08-24.md`](../release/current-real-device-login-patient-observation-2026-08-24.md) |
