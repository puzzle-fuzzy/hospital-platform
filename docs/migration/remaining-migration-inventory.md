...
> 当前执行基线（2026-08-24）：线上新 API 为 `28a5c0c131794ce9dcc5f94bd3809402188ac87a`，小程序运行包为 `13f597e`（来源 `13f597ea9ee3f65b9be858117826d948339d904a`）；本轮 adapter 门禁已随服务端切换进入线上，渠道 4 “全部挂号”查询仍待真机三层证据。
...
| P1 | 真机只读验收 | 新小程序运行包来源与已验证服务端配套；微信登录、患者同步/切换、预约历史、门诊费用分别取得页面、HTTP requestId/Provider requestId、Pino 日志三层证据 | 当前进行中：小程序 `13f597e`（来源 `13f597ea`）对应线上服务端 `28a5c0c1`；尚无手机页面与当前服务端的完整业务三层证据 |
