...
| 项目 | 当前状态 | 证据 |
| --- | --- | --- |
| 仓库代码候选 | 最新已验证的实现提交为 `29d4213`，在 `c660462` 的“我的”页失败收敛基础上，修正患者选择页在医院目录同步完成前仍可点击返回的竞态；旧端支付调起台账已按当前源代码稳定为 2 个文件；尚未部署线上，仓库实际 HEAD 以 Git history 为准 | Git history；不得用仓库代码或文档 HEAD 代替线上 release |
