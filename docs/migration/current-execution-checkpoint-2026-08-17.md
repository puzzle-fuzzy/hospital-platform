...
> 当前基线更新：服务端 `5a31427`；小程序候选 `f66514d`；完整运行包来源 `f66514de81c051cb8ade1477f758700b2837b9b7`。下文更早候选只作历史追溯。

本文是新会话继续迁移时的短入口。它不替代逐域 contract，而是把当前线上事实、剩余范围、
下一步顺序和停止条件固定下来，避免在 Provider 文档不足时凭旧页面猜实现。

> 当前发布基线补充（2026-08-21）：服务端 `5a31427`；小程序候选 `f66514d`；完整运行包来源 `f66514de81c051cb8ade1477f758700b2837b9b7`，尚未上传线上。

> 当前配套小程序候选为 `f66514d`，构建来源为 `f66514de81c051cb8ade1477f758700b2837b9b7`；尚未上传线上，真实微信与真机证据仍需独立采集。
...
> `f66514de81c051cb8ade1477f758700b2837b9b7`（当前本地候选 `f66514d`，尚未上传线上）。真实微信、患者上下文和 P0 只读验收的操作顺序统一见
