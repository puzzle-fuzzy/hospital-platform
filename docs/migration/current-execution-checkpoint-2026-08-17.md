...
| 项目 | 当前状态 | 证据 |
| --- | --- | --- |
| 仓库代码候选 | 最新已验证的实现提交为 `5a58774`，包含门诊费用 `tradeStatus` 状态一致性校验、排班字段边界修正、小程序运行包独立验证门禁，以及匿名“新增就诊人”登录成功后自动进入选择页；尚未部署线上，仓库实际 HEAD 以 Git history 为准 | Git history；不得用仓库代码或文档 HEAD 代替线上 release |
...
- 本地 `pnpm test`、`pnpm typecheck` 和小程序构建均通过；9 个 workspace 包测试成功，原生小程序
  ...
  Provider 业务证据。
- 当前最新已验证实现提交 `5a58774` 尚未发布；该提交在 `c72232b` 的门诊费用状态一致性校验、`f268be0` 的排班 adapter
...
- 本轮尝试只读连接 `ps@192.168.112.172` 获取当前 release 和业务日志时，SSH 返回
  ...
  Redis TTL 采样，再决定是否发布当前代码候选 `5a58774`。
