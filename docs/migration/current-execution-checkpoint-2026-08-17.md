...
| 项目 | 当前状态 | 证据 |
| --- | --- | --- |
| 仓库代码候选 | 最新已验证的实现提交为 `ef6f34c`，包含门诊费用 `tradeStatus` 状态一致性校验、排班字段边界修正、小程序运行包独立验证门禁、匿名“新增就诊人”登录成功后自动进入选择页，以及个人中心未开放入口的逐项关闭边界；尚未部署线上，仓库实际 HEAD 以 Git history 为准 | Git history；不得用仓库代码或文档 HEAD 代替线上 release |
