...
> 当前配套小程序候选为 `e050fa0`，构建来源为 `e050fa026f2cb5b2de4af4de98024cfdb946229c`；尚未上传线上，真实微信与真机证据仍需独立采集。

> 截至 2026-08-19，本地仓库 `main` 的当前 HEAD 以 Git history 为准，线上 API release 已切换为 `398be8e`；本轮已完成候选上传、隔离 smoke 和只重启新 API；新 Bun/Elysia API
> 监听 `10.0.0.3:18081`，旧 Python API 继续监听 `8001`。此前的 `b3c9a99`、`5f5915e`、`bf67b96` 等内容均为历史段落，
> 不能继续当作当前线上事实。当前 release 的发布和业务证据见
> [`../release/398be8e-production-acceptance-2026-08-19.md`](../release/398be8e-production-acceptance-2026-08-19.md)；配套小程序构建来源为
> `e050fa026f2cb5b2de4af4de98024cfdb946229c`（当前本地候选 `e050fa0`，尚未上传线上）。真实微信、患者上下文和 P0 只读验收的操作顺序统一见
...
| 项目 | 当前状态 | 证据 |
| --- | --- | --- |
| 仓库代码基线 | `main` 当前 HEAD（以 Git history 为准）；生产运行 bundle 的代码来源为 `398be8e`，仓库 HEAD 不能替代线上 release | Git history；线上 bundle provenance 见最新发布记录 |
| 线上新 API | `398be8e`，监听 `10.0.0.3:18081`，由 `hospital-platform-api-v2.service` 托管 | [`../release/398be8e-production-acceptance-2026-08-19.md`](../release/398be8e-production-acceptance-2026-08-19.md) |
