...
| 项目 | 当前状态 | 证据 |
| --- | --- | --- |
| 仓库代码候选 | 最新已验证的实现提交为 `144fc5a`，在 `e1fcc2d` 的预约历史失败日志闭环基础上，补充门诊费用失败事件的内部 `patientId` 关联字段；旧端支付调起台账已按当前源代码稳定为 2 个文件；尚未部署线上，仓库实际 HEAD 以 Git history 为准 | Git history；不得用仓库代码或文档 HEAD 代替线上 release |
