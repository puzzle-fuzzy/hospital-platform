...
| 项目 | 当前状态 | 证据 |
| --- | --- | --- |
| 仓库代码候选 | 最新已验证的实现提交为 `e1fcc2d`，在 `c291b93` 的预约历史状态边界基础上，补齐输入校验、依赖缺失、owner 映射和 Provider 失败的 `appointment.records.failed` 日志，并区分 Provider 明确空数组的 `appointment.records.synced(itemCount=0)`；旧端支付调起台账已按当前源代码稳定为 2 个文件；尚未部署线上，仓库实际 HEAD 以 Git history 为准 | Git history；不得用仓库代码或文档 HEAD 代替线上 release |
