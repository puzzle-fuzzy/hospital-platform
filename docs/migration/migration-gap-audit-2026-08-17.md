...
> `171a8743185fb4ecc1696851662659c1a0ee7ebf`。当前 P0 运行层切换已完成，P1 真机与 Provider 三层证据仍未完成。
...
> 当前配套小程序候选构建来源为 `171a8743185fb4ecc1696851662659c1a0ee7ebf`（提交 `171a874`），服务端 `0e2a366e` 已完成配套发布；历史候选只用于追溯。
...
- 2026-08-17 复核旧仓库时发现 `module_common` 从原台账的 34 个漂移到 38 个挂载路由，旧服务
  ...
  `171a8743185fb4ecc1696851662659c1a0ee7ebf`（当前候选 `171a874`，已具备配套发布基线）；旧 Python 服务继续监听 `8001`，旧服务、
