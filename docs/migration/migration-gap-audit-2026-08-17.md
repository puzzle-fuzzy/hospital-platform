...
> 当前配套小程序候选构建来源为 `aafccf53d1e0eaac5ddaf98b1d850b084e4f78ed`（提交 `aafccf5`），尚未上传线上；历史候选只用于追溯。
...
- 当前线上 release 为 `5a31427`，已完成 production preflight、隔离 runtime smoke 和只重启新 API 的无损切换；旧 Python `8001` 继续运行。`6038560`、`0e360d3`、`398be8e`、`968af78`、`b7c9451`、`c26e696` 及此前 release 均按历史切换记录理解。
...
- 2026-08-17 复核旧仓库时发现 `module_common` 从原台账的 34 个漂移到 38 个挂载路由，旧服务
  ...
  支付、医保、退款或 HIS 回写接口。
  - 当前线上新 API release 为 `5a31427`，监听 `18081`；配套小程序构建来源为
  `aafccf53d1e0eaac5ddaf98b1d850b084e4f78ed`（当前本地候选，尚未上传线上）；旧 Python 服务继续监听 `8001`，旧服务、
...
- 当前切换后 SSH 核对确认新 Bun API 监听 `10.0.0.3:18081`、旧 Python API 监听 `0.0.0.0:8001`，
  `hospital-platform-api-v2.service` 为 active/running，服务器 current 指向 `5a31427`，Worker 仍 inactive。这补强运行层共存证据，
