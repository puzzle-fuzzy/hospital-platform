...
- 当前线上新 API release 为 `ca5a372`，监听 `18081`；旧 Python 服务继续监听 `8001`，旧服务、
...
- 同日切换后 SSH 核对确认新 Bun API 监听 `10.0.0.3:18081`、旧 Python API 监听 `0.0.0.0:8001`，
  `hospital-platform-api-v2.service` 为 active/running，服务器 current 指向 `ca5a372`，Worker 仍 inactive。这补强运行层共存证据，
...
- `ca5a372` 已完成真实生产 preflight、临时端口候选 smoke、原子切换和公网 runtime smoke；公网 readiness 连续 6/6、no-store、system ping 和未登录 401 均通过。其新增认证顺序回归也已用缺少业务参数的四条公网请求验证。该稳定性证据已解除“候选运行前置未验证”阻断，但不解除真实微信、多患者、Provider 或真机业务门禁，详见 [`../release/ca5a372-production-acceptance-2026-08-17.md`](../release/ca5a372-production-acceptance-2026-08-17.md)。
