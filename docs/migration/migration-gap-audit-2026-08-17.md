...
> 当前配套小程序候选构建来源为 `3a66d125b0c1ca53879dd88a3661e3025fb7dd3d`，尚未上传线上；历史候选只用于追溯。
...
- 2026-08-17 复核旧仓库时发现 `module_common` 从原台账的 34 个漂移到 38 个挂载路由，旧服务
  ...
  - 当前线上新 API release 为 `b7c9451`，监听 `18081`；配套小程序构建来源为
    `3a66d125b0c1ca53879dd88a3661e3025fb7dd3d`（当前本地候选，尚未上传线上）；旧 Python 服务继续监听 `8001`，旧服务、
