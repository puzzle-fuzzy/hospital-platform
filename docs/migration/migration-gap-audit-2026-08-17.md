...
> `a33416d8417661fa5256deb22df55a97456cc608`。当前 P0 运行层切换已完成，P1 真机与 Provider 三层证据仍未完成。
...
> 当前配套小程序候选构建来源为 `a33416d8417661fa5256deb22df55a97456cc608`（提交 `a33416d8`），服务端已完成配套发布；历史候选只用于追溯。
...
- 2026-08-17 复核旧仓库时发现 `module_common` 从原台账的 34 个漂移到 38 个挂载路由，旧服务
  ...
  `a33416d8417661fa5256deb22df55a97456cc608`（当前候选 `a33416d8`，已具备配套发布基线）；旧 Python 服务继续监听 `8001`，旧服务、
