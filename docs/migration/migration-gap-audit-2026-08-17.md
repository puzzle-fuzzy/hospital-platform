...
- 旧端扫描基线为 64 个页面，原生小程序当前注册 14 个 TypeScript 页面；旧 FastAPI 与旧小程序
 ...
 - 当前线上新 API release 为 `5c4e7cf`，监听 `18081`；旧 Python 服务继续监听 `8001`，旧服务、
...
- 同日切换后 SSH 核对确认新 Bun API 监听 `10.0.0.3:18081`、旧 Python API 监听 `0.0.0.0:8001`，
  `hospital-platform-api-v2.service` 为 active/running，服务器 current 指向 `5c4e7cf`，Worker 仍 inactive。这补强运行层共存证据，
