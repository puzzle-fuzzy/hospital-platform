...
> 历史发布基线（2026-08-24 13:01 CST）：线上服务端 release 为 `28a5c0c131794ce9dcc5f94bd3809402188ac87a`；当时小程序运行包来源为 `13f597ea9ee3f65b9be858117826d948339d904a`（提交 `13f597e`）。服务端独立只读 adapter 发布已完成，真机业务三层证据仍待。
> 历史配套小程序构建来源（2026-08-26）：`0be59f966de2c3a0861cb44e9a526a1ef557f6c7`，仅表示当时本地 live 候选，未证明微信线上版本或真机业务已验收；当前入口以本文顶部和当前项目基线为准。
...
> 历史分层基线：服务端 release 为 `28a5c0c131794ce9dcc5f94bd3809402188ac87a`；当时小程序运行包来源为 `13f597ea9ee3f65b9be858117826d948339d904a`（提交 `13f597e`）。两者为当时服务端独立只读 adapter 发布后的有意分层，真实真机与 Provider 三层证据仍待。
...
> 下方带历史日期的候选只作追溯，当前执行入口以本文顶部 `28a5c0c1` 服务端 + `13f597e` 小程序分层基线为准。


> 当前完整小程序来源校验值：`13f597ea9ee3f65b9be858117826d948339d904a`；当前线上服务端 release：`28a5c0c131794ce9dcc5f94bd3809402188ac87a`。

> 历史发布基线（2026-08-22 18:55 CST）：服务端 `0e2a366efcca8da25d7edd4a286781f2d3dfdbec`；小程序运行包来源
> `13f597ea9ee3f65b9be858117826d948339d904a`。当前 P0 运行层切换已完成，P1 真机与 Provider 三层证据仍未完成。
...
> 当前小程序候选构建来源为 `13f597ea9ee3f65b9be858117826d948339d904a`（提交 `13f597e`），线上服务端 release 为 `28a5c0c1`；本轮是服务端独立只读 adapter 发布，历史候选只用于追溯。
...
- 当前线上 release 为 `28a5c0c1`，已完成 production preflight、隔离 runtime smoke 和只重启新 API 的无损切换；旧 Python `8001` 继续运行。`c8eef370`、`5a31427`、`6038560`、`0e360d3`、`398be8e`、`968af78`、`b7c9451`、`c26e696` 及此前 release 均按历史切换记录理解。
...
- 2026-08-17 复核旧仓库时发现 `module_common` 从原台账的 34 个漂移到 38 个挂载路由，旧服务
  ...
  支付、医保、退款或 HIS 回写接口。
  - 当前线上新 API release 为 `5738a71e`，监听 `18081`；配套小程序构建来源为
  `cac6561b3f4ebbae2de8c632b052837fe7bc28b6`（当前候选 `cac6561`，已具备配套发布基线）；旧 Python 服务继续监听 `8001`，旧服务、
