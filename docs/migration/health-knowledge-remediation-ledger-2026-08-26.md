...
健康内容。台账 schema 已升级到 v2，并与审核队列共用
`tools/health-knowledge-review-gates.mjs` 中定义的六个 gate ID。
...
| staging 事务导入 | 必须在 staging 完成 bundle 校验、单事务导入和失败回滚 | 不能 |
| 发布与撤回演练 | 必须演练重复导入、重叠版本、同版本读取和撤回 | 不能 |
