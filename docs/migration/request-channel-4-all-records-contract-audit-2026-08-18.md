...
> **2026-08-24 当前修正：**本文以下“当前不能开放渠道 4”的结论是 2026-08-18 的历史审计快照，不能覆盖当前
> `13f597ea` 的实现和发布证据。当前新端已经将公网 `scope=all` 映射为服务端固定的
> `requestChannel=4`，省略日期参数并独立读取完整历史；生产只读核对已确认渠道 4 的成功包络和历史返回。
> 当前尚未完成的是同一 13f 真机点击“全部挂号”的页面、客户端 requestId、服务端 trace 和 Provider requestId
> 四方关联证据，不是继续把渠道 4 当作代码未实现。预约详情、取消、预问诊、预约写入、支付和 HIS 回写仍保持关闭。
