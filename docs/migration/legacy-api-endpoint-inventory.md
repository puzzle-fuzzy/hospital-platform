...
| `GET /msun-middle-aggregate-patient/v1/patInfosFind` | `api/modules/ZY.ts` | 代码已迁移（服务端临床映射，只读） | 新服务按目录返回的卡号+姓名查询 `success=true` 的 `data.patId`，仅保存为 owner-scoped `his-patient` 引用，不进入小程序、日志或公共 contract；`patCardVOList` 只用于卡片归属/卡号一致性校验。`invalidFlag`、锁定/卡片状态、机构边界、Provider 鉴权和真实业务验收仍待正式 contract，不能据此开放写入、二维码或支付。字段级审计见 [`patinfosfind-archive-field-audit-2026-08-19.md`](patinfosfind-archive-field-audit-2026-08-19.md)。 |
