...
当前新端只保留两个正式主 Tab 的稳定页面和迁移状态，继续禁止未经 contract 证明的副作用、外部跳转和实时数据展示。
...
2. 用固定高度的状态外壳展示“正在迁移”，避免页面高度因加载/空态切换发生跳变；
3. 不调用旧 WebSocket、不读取 provider 患者号、不直连队列接口、不打开外部 WebView、不接受任意 URL。

这不是业务完成，而是 fail-closed 的迁移边界。两个页面均继续等待各自 contract、服务端实现、公网证据和真机证据。
...
1. 先从现有众阳文档/服务端 contract 中确认 C-08 的预约历史只读字段，若不能证明实时事件，则继续只做预约历史独立页面，不实现“今日就诊”；
2. 单独收集互联网医院正式入口的 audience、域名和回跳协议，不与 AI/客服 WebView 共用配置；
3. contract 冻结后再设计服务端 adapter、脱敏读模型、日志字段和小程序页面状态机；
4. 取得公网、服务端日志和真机三层配对证据后，才把对应页面从“迁移中”改为“只读已实现”或“外部入口已验收”。
