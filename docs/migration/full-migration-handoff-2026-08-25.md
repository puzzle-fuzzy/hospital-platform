...
> **最新候选事实（2026-08-26）**：提交 `b4120bac` 在既有全量 64 个旧入口 A–F 批次覆盖上新增四个临床 `surface-only` 页面外壳和共享页面工厂构建门禁；当前 pending 来源为 `b4120bac70b081c235ad9d50423712b2d782d57d`，25 个页面，`293 pass / 0 fail / 3269 expect()`。构建已生成并通过 pending 校验，但 live `dist` 仍受微信开发者工具锁定，发布返回 `EBUSY`；此前候选仅作历史追溯。
...
> **当前仓库事实（2026-08-26）**：本轮小程序功能候选为 `b4120bac`，当前小程序运行输入/pending 来源为 `b4120bac70b081c235ad9d50423712b2d782d57d`；该运行输入包含前序业务候选、全量迁移入口覆盖视图、A–F 批次展示、契约族边界、逐入口说明、四个临床页面外壳、共享 Tab、统一页面滚动边界、全局资料授权边界、owner 回调保护和协议静态页。微信开发者工具仍锁定 live `dist`，没有发布线上服务，也没有改变旧 Python 服务。历史候选编号只用于追溯，不能替代当前运行包、线上 release 或真机证据。
...
| 小程序业务代码候选 | `b4120bac70b081c235ad9d50423712b2d782d57d` |
| 小程序 pending 运行包 | `.local/hospital-miniprogram/pending/`，`build-info.sourceRevision=b4120bac70b081c235ad9d50423712b2d782d57d` |
