...
5. `pnpm release:baseline:audit` 当前因 `main` 中的 `apps/api/src/modules/knowledge/index.ts`、`service.ts` 尚未部署而失败；候选提交以 `git rev-parse HEAD` 固定，完成新的 API-only 原子发布并取得远端证据后，才可更新当前 release 基线。不能通过修改审计器或只部署半套代码来“变绿”。
