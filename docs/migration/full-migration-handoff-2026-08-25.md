...
> **最新候选纠正（2026-08-26）**：当前小程序候选已推进到 `0d28b72`，完整 sourceRevision 为 `0d28b7241f40de95f4049d7d2a18e07f6f162268`，pending 运行包包含 20 个页面，源码回归为 `288 pass / 0 fail / 3225 expect()`。本轮修复就诊页跨中国标准时间零点的业务日快照一致性；下文 `a6319d7` 及更早候选只作历史追溯。live `dist` 仍为 `fcc6630e`，微信开发者工具锁定导致发布返回 `EBUSY`，旧 Python 服务、线上服务和另一会话的众阳预约适配器均未修改。
