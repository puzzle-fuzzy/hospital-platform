...
> **最新候选纠正（2026-08-26）**：当前源码已注册 40 个页面，健康自测中的 BMI/血压安全数值子集已进入 `partial`，就诊页今日预约摘要已补齐但实时叫号仍关闭；当前统计为 `replaced=8 / partial=19 / surface-only=29 / blocked-payment=7 / excluded=1`。最新 pending 运行包来源为 `c7db7f04a0dee5a8aad6ba149f17bd413b0d1d5d`，但仍受微信开发者工具锁定影响，需要重新发布；本轮又统一了临床/服务入口的当前就诊人上下文、重试和选择入口，真实 Provider/临床/外部/患者写入业务仍未开放。协议版本、同意记录、撤回和审计仍关闭，正式健康审核 bundle 仍缺失；旧 Python 服务、线上服务和另一会话的众阳预约适配器均未修改。
...
> **最新候选事实（2026-08-26）**：功能工作树在既有全量 64 个旧入口 A–F 批次覆盖上继续完成当前就诊人上下文的横向迁移；当前 `c7db7f04` 的 40 页 pending 运行包已经生成并通过静态校验，页面只迁移 owner-scoped 患者展示、重试、选择入口和明确关闭态，不虚构临床 Provider、物流、采血号源或公开记录 provider。发布仍须先处理微信开发者工具对 live `dist` 的锁定，不能把旧 pending 或旧 live 包写成新候选真机证据。当前候选证据见 [`../release/candidate-c7db7f04-miniprogram-runtime-2026-08-26.md`](../release/candidate-c7db7f04-miniprogram-runtime-2026-08-26.md)。
...
> **当前仓库事实（2026-08-26）**：本轮小程序功能候选为 `c7db7f04`，当前小程序运行输入/pending 来源为 `c7db7f04a0dee5a8aad6ba149f17bd413b0d1d5d`；该运行输入包含前序业务候选、全量迁移入口覆盖视图、A–F 批次展示、契约族边界、逐入口说明、健康自测安全数值子集、临床/外部安全页面、预约 Provider 入口、统一当前就诊人上下文、共享 Tab、统一页面滚动边界、全局资料授权边界、owner 回调保护、协议静态页以及旧端“我的快递”、采血预约、电子锦旗和健康表扬信安全页面迁移。微信开发者工具仍锁定 live `dist`，没有发布线上服务，也没有改变旧 Python 服务。历史候选编号只用于追溯，不能替代当前运行包、线上 release 或真机证据。

## 1. 当前真实基线

| 项目 | 当前事实 |
| --- | --- |
| 功能候选代码基线 | `c7db7f04`（共享当前就诊人上下文与既有迁移边界） |
| 当前功能基线 | `c7db7f04`（文档更新不改变 live `dist`） |
| 小程序业务代码候选 | 功能提交 `c7db7f04`；运行来源 `c7db7f04a0dee5a8aad6ba149f17bd413b0d1d5d` |
| 小程序 pending 运行包 | `.local/hospital-miniprogram/pending/`，`build-info.sourceRevision=c7db7f04a0dee5a8aad6ba149f17bd413b0d1d5d` |
| 当前源码页面数 | 40 个；每个页面具备 TypeScript 源码和页面配置 |
| pending 页面数 | 40 个；`runtime:verify:pending` 已通过 |
| 小程序回归 | `307 pass / 0 fail / 3501 expect()`；入口分发审计通过 |
...
2. 使用 [`device-evidence-c7db7f0-pending.json`](../release/device-evidence-c7db7f0-pending.json) 作为 9 个只读验收域的起始清单；一次记录成功、空结果、401、依赖不可用、Provider 超时和患者切换边界。
   `pnpm device:evidence:audit --file docs/release/device-evidence-c7db7f0-pending.json` 在全部域仍为 `pending` 时会先完成清单、候选指纹和脱敏边界审计，但总结果仍为 `passed=false`；一旦出现 `passed/failed` 真实链路结果，线上 release 基线必须先通过，否则工具直接拒绝纳入验收。
...
```powershell
$env:HOSPITAL_MINIPROGRAM_EXPECTED_SOURCE_REVISION = "c7db7f04a0dee5a8aad6ba149f17bd413b0d1d5d"
