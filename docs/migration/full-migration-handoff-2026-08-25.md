...
> **最新候选事实（2026-08-25）**：提交 `7f7a7a18` 补充迁移状态页逐入口说明，并保留前序全量 64 个旧入口 A–F 批次覆盖；当前 pending 来源为 `7f7a7a1844f5269c88f814d7d97d805fe4b8aeca`，20 个页面，`285 pass / 0 fail / 3211 expect()`。构建已生成 pending，但 live `dist` 仍为 `fcc6630e`，微信开发者工具锁定导致发布返回 `EBUSY`；此前候选仅作历史追溯。

> **2026-08-25 续做记录**：提交 `163d696b` 补强了跨患者、预约、报告和费用模块共用的 `AdapterCallContext` 失败日志兜底；提交 `f97f9f03` 补齐健康知识错误码的服务端、客户端和文档契约；提交 `fc70fa0b` 修正未知/过期 `feature` 进入状态页时被误归类为“医疗记录”的错误语义；提交 `7627843a` 补齐状态页的迁移阶段，提交 `cd26a01` 补齐旧入口、业务域和下一步准入展示。以上候选均保留作历史追溯；当前 pending 小程序运行包为 `7f7a7a18`（20 页），live `dist` 仍为旧来源 `fcc6630e`，因此没有覆盖 `dist`、没有发布微信运行包，也没有修改旧服务、旧数据库、旧 Redis 或另一会话的众阳预约适配器。

> **当前仓库事实（2026-08-25）**：本轮小程序功能候选为 `7f7a7a18`，当前小程序运行输入/pending 来源为 `7f7a7a1844f5269c88f814d7d97d805fe4b8aeca`；该运行输入包含前序业务候选、全量迁移入口覆盖视图、A–F 批次展示、契约族边界、逐入口说明、共享 Tab、统一页面滚动边界和全局资料授权边界。微信开发者工具仍锁定 live `dist`，没有发布线上服务，也没有改变旧 Python 服务。服务端本地候选仍以 `b42922f4` 为准。历史候选编号只用于追溯，不能替代当前运行包、线上 release 或真机证据。

## 1. 当前真实基线

| 项目 | 当前事实 |
| --- | --- |
| 功能候选代码基线 | `7f7a7a18`（补充迁移状态页逐入口说明） |
| 当前功能基线 | `7f7a7a18`（文档更新不改变 live `dist`） |
| 小程序业务代码候选 | `7f7a7a1844f5269c88f814d7d97d805fe4b8aeca` |
| 小程序 pending 运行包 | `.local/hospital-miniprogram/pending/`，`build-info.json.sourceRevision=7f7a7a1844f5269c88f814d7d97d805fe4b8aeca` |
| pending 页面数 | 20 个；每个页面具备 `.js/.json/.wxml/.wxss` |
| 小程序回归 | 当前源码 285 pass / 0 fail / 3211 expect()；pending 已包含当前运行输入；入口分发广度审计通过 |
...
2. 使用 [`device-evidence-7f7a7a1-pending.json`](../release/device-evidence-7f7a7a1-pending.json) 作为 9 个只读验收域的起始清单；一次记录成功、空结果、401、依赖不可用、Provider 超时和患者切换边界。
   `pnpm device:evidence:audit --file docs/release/device-evidence-7f7a7a1-pending.json` 在全部域仍为 `pending` 时会先完成清单、候选指纹和脱敏边界审计，但总结果仍为 `passed=false`；一旦出现 `passed/failed` 真实链路结果，线上 release 基线必须先通过，否则工具直接拒绝纳入验收。
