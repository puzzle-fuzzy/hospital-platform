...
> **最新候选纠正（2026-08-26）**：源码和 pending 运行包已更新到 `b4120bac`，当前注册/构建页面均为 25 个，回归为 `293 pass / 0 fail / 3269 expect()`；四个临床页面为 `surface-only` 外壳，真实 Provider/临床业务仍未开放。pending 已通过 `runtime:verify:pending`，但尚未替换被微信开发者工具锁定的 live `dist`，因此尚未产生真机证据。协议版本、同意记录、撤回和审计仍关闭，正式健康审核 bundle 仍缺失；旧 Python 服务、线上服务和另一会话的众阳预约适配器均未修改。
...
> **最新候选事实（2026-08-26）**：功能提交 `b4120bac` 在既有全量 64 个旧入口 A–F 批次覆盖上新增四个临床 `surface-only` 页面外壳和共享页面工厂构建门禁；当前 pending 来源为 `25cd7c4a323c9767b9367d319be14a82f681b37b`，25 个页面，`293 pass / 0 fail / 3269 expect()`。构建已生成并通过 pending 校验，但 live `dist` 仍受微信开发者工具锁定，发布返回 `EBUSY`；此前候选仅作历史追溯。
...
> **当前仓库事实（2026-08-26）**：本轮小程序功能候选为 `b4120bac`，当前小程序运行输入/pending 来源为 `25cd7c4a323c9767b9367d319be14a82f681b37b`；该运行输入包含前序业务候选、全量迁移入口覆盖视图、A–F 批次展示、契约族边界、逐入口说明、四个临床页面外壳、共享 Tab、统一页面滚动边界、全局资料授权边界、owner 回调保护和协议静态页。微信开发者工具仍锁定 live `dist`，没有发布线上服务，也没有改变旧 Python 服务。历史候选编号只用于追溯，不能替代当前运行包、线上 release 或真机证据。

## 1. 当前真实基线

| 项目 | 当前事实 |
| --- | --- |
| 功能候选代码基线 | `b4120bac`（临床页面外壳与构建门禁） |
| 当前功能基线 | `b4120bac`（文档更新不改变 live `dist`） |
| 小程序业务代码候选 | 功能提交 `b4120bac`；运行来源 `25cd7c4a323c9767b9367d319be14a82f681b37b` |
| 小程序 pending 运行包 | `.local/hospital-miniprogram/pending/`，`build-info.sourceRevision=25cd7c4a323c9767b9367d319be14a82f681b37b` |
| 当前源码页面数 | 25 个；每个页面具备 TypeScript 源码和页面配置 |
| pending 页面数 | 25 个；已包含本轮四个临床页面外壳 |
...
2. 使用 [`device-evidence-3b42b86-pending.json`](../release/device-evidence-3b42b86-pending.json) 作为 9 个只读验收域的起始清单；一次记录成功、空结果、401、依赖不可用、Provider 超时和患者切换边界。
   `pnpm device:evidence:audit --file docs/release/device-evidence-3b42b86-pending.json` 在全部域仍为 `pending` 时会先完成清单、候选指纹和脱敏边界审计，但总结果仍为 `passed=false`；一旦出现 `passed/failed` 真实链路结果，线上 release 基线必须先通过，否则工具直接拒绝纳入验收。
...
```powershell
$env:HOSPITAL_MINIPROGRAM_EXPECTED_SOURCE_REVISION = "3b42b867ae19f6dd23bacd88648d1f5917dabf26"
