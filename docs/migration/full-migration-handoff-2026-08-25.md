> **当前候选同步（2026-08-28）**：服务端 release `5738a71e0bcddaa8849106754baf5b296427bed7`；本地小程序 live 仍为 `87eb00f0aa0c6d46b61364449d375df5105c80d6`，pending 候选 sourceRevision 为 `8182a8774bf0d6ad6f62d928693add952ddff034`；历史段落只作追溯。
...
> **最新候选纠正（2026-08-28）**：当前源码已注册 38 个页面，健康自测中的 BMI/血压安全数值子集已进入 `partial`，就诊页今日预约摘要已补齐但实时叫号仍关闭；采血预约、我的快递、患者签名展示和消息订阅展示也已进入 `partial`，当前统计为 `replaced=8 / partial=23 / surface-only=23 / blocked-payment=7 / excluded=1`。当前 38 页运行相关源码候选 `8182a8774bf0d6ad6f62d928693add952ddff034` 已完成构建并保存在 pending，尚未切换到本地 live `dist`；九个真机证据域仍为 `pending`，真实 Provider/临床/外部/患者写入业务仍未开放。协议版本、同意记录、撤回和审计仍关闭，正式健康审核 bundle 仍缺失；本候选补充“我的问诊”演示数据边界和众阳 TLS 诊断，不新增 Provider 请求或写入；旧 Python 服务未修改，线上新 API 仍以已部署 release `5738a71e0bcddaa8849106754baf5b296427bed7` 为准。
...
> **最新候选事实（2026-08-28）**：功能工作树在既有全量 64 个旧入口 A–F 批次覆盖上继续完成当前就诊人上下文的横向迁移；38 页运行相关源码候选 `8182a877` 已完成构建、静态校验并保存在 pending，尚未切换到 live `dist`。页面只迁移 owner-scoped 患者展示、重试、选择入口、协议原文只读和明确关闭态，并补齐共享患者外壳的 owner 证明、会话代际、账号切换清理以及会话失效时旧 owner 微信资料缓存清理；同时保留微信资料拒绝后的设置页重试、选择页刷新并发门禁以及“我的快递”加载/错误/未开放状态边界，并统一电子锦旗和表扬信记录区域的加载、错误、未开放三态及固定高度，不虚构临床 Provider、问诊会话、物流、采血号源或公开记录 provider。当前候选还收紧预约排班和预约历史底层请求字段、日期和范围关系，并修复 App.onLaunch 全局资料初始化时序；同时确认旧端“我的问诊”不能以演示数据充当真实来源，并新增众阳 TLS 证书审计。当前证据清单仍为九域 `pending`，需待候选发布后逐域取证。当前候选证据见 [`device-evidence-8182a877-pending.json`](../release/device-evidence-8182a877-pending.json)；历史候选的 live 包和证据清单不能直接作为本轮真机完成证据。
...
| `surface-only` | 23 | 页面外壳、必要的患者选择入口和关闭态已迁移；真实业务仍按对应 contract 阻塞 |
| `blocked-provider` | 1 | Provider 入口已先迁移安全壳，真实读取仍未开放 |
...
| `blocked-external` | 1 | 外部入口已先迁移安全壳，WebView、客服、问诊、分享、订阅仍未开放 |
...
1. 当前候选 `8182a877` 已完成 pending 构建和 `runtime:verify:pending`，但尚未切换到 live `dist`；后续发布时仍必须先确认锁状态并沿用原子发布器，不能删除或覆盖 live `dist`。
2. 当前候选 `8182a877` 的九域真机清单为 [`device-evidence-8182a877-pending.json`](../release/device-evidence-8182a877-pending.json)，已绑定 `8182a8774bf0d6ad6f62d928693add952ddff034`；九个域全部为 `pending`，一次记录成功、空结果、401、依赖不可用、Provider 超时、患者切换和账号切换边界。旧候选的 `device-evidence-02dbf10-pending.json`、`device-evidence-731c957-pending.json`、`device-evidence-de5dea8-pending.json` 和 `device-evidence-ed63a8e-pending.json` 只作历史模板，不能直接提交本轮证据。
   执行 `pnpm device:evidence:audit --file docs/release/device-evidence-8182a877-pending.json` 时，在全部域仍为 `pending` 的情况下总结果仍为 `passed=false`；一旦出现 `passed/failed` 真实链路结果，线上 release 基线必须先通过，否则工具直接拒绝纳入验收。
...
5. `pnpm release:baseline:audit` 当前未通过：仓库多份当前发布文档仍引用上一候选 `1bc5bf6`，且服务端 release `5738a71e0bcddaa8849106754baf5b296427bed7` 之后仍有未部署的运行时代码；不能通过修改审计器或只部署半套代码来“变绿”。

本轮共享基础设施修正已经完成，后续工作回到广度队列：A 批次需先发布 `8182a877` pending 运行包，并在众阳证书恢复可信后采集九个只读域证据；B 批次等待内容责任人审核 bundle；C/D/E 批次分别等待临床、患者写入和外部入口 contract；F 批次继续最后处理支付、医保和 HIS 回写。业务代码不能因为一个共享日志问题已修复就提前打开这些阻断域。
