...
> **最新候选事实（2026-08-25）**：提交 `3cf828ed` 修复普通资料暂时异常时的微信授权边界，并保留前序全量 64 个旧入口 A–F 批次覆盖；当前 pending 来源为 `3cf828ed185f0e745138db04011d26c6db62fa8a`，20 个页面，`284 pass / 0 fail / 3129 expect()`。构建已生成 pending，但 live `dist` 仍为 `fcc6630e`，微信开发者工具锁定导致发布返回 `EBUSY`；此前候选仅作历史追溯。
...
> **当前仓库事实（2026-08-25）**：本轮小程序功能候选为 `3cf828ed`，当前小程序运行输入/pending 来源为 `3cf828ed185f0e745138db04011d26c6db62fa8a`；该运行输入包含前序业务候选、全量迁移入口覆盖视图、A–F 批次展示、共享 Tab、统一页面滚动边界和全局资料授权边界。微信开发者工具仍锁定 live `dist`，没有发布线上服务，也没有改变旧 Python 服务。服务端本地候选仍以 `b42922f4` 为准。历史候选编号只用于追溯，不能替代当前运行包、线上 release 或真机证据。

## 1. 当前真实基线

| 项目 | 当前事实 |
| --- | --- |
| 功能候选代码基线 | `68902677`（统一 20 个页面的显式内容滚动边界） |
| 当前功能基线 | `68902677`（文档更新不改变 live `dist`） |
| 小程序业务代码候选 | `689026770a9f68837ba60f1d7bf1ad9f073d547e` |
| 小程序 pending 运行包 | `.local/hospital-miniprogram/pending/`，`build-info.json.sourceRevision=689026770a9f68837ba60f1d7bf1ad9f073d547e` |
| pending 页面数 | 20 个；每个页面具备 `.js/.json/.wxml/.wxss` |
| 小程序回归 | 当前源码 282 pass / 0 fail / 3119 expect()；pending 已包含当前运行输入；入口分发广度审计通过 |
