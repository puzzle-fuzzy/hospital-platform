...
> **最新候选纠正（2026-08-26）**：当前小程序候选已推进到 `896a83cf`，完整 sourceRevision 为 `896a83cfb9d8b4350664cfe97f8bee643cbca434`，pending 运行包包含 20 个页面，源码回归为 `288 pass / 0 fail / 3229 expect()`。本轮迁移首页二维码安全展示壳，真实二维码因旧端第三方生成方式不具备医院扫码 contract 而保持关闭；就诊页业务日快照修正见前一候选。live `dist` 仍为 `fcc6630e`，微信开发者工具锁定导致发布返回 `EBUSY`，旧 Python 服务、线上服务和另一会话的众阳预约适配器均未修改。
