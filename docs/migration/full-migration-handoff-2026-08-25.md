...
> **最新候选事实（2026-08-25）**：提交 `68902677` 统一了 20 个页面的显式内容滚动边界，并保留前序全量 64 个旧入口 A–F 批次覆盖；当前 pending 来源为 `689026770a9f68837ba60f1d7bf1ad9f073d547e`，20 个页面，`282 pass / 0 fail / 3119 expect()`。构建已生成 pending，但 live `dist` 仍为 `fcc6630e`，微信开发者工具锁定导致发布返回 `EBUSY`；此前候选仅作历史追溯。
...
> **当前仓库事实（2026-08-25）**：本轮小程序功能候选为 `68902677`，当前小程序运行输入/pending 来源为 `689026770a9f68837ba60f1d7bf1ad9f073d547e`；该运行输入包含前序业务候选、全量迁移入口覆盖视图、A–F 批次展示、共享 Tab 和统一页面滚动边界。微信开发者工具仍锁定 live `dist`，没有发布线上服务，也没有改变旧 Python 服务。服务端本地候选仍以 `b42922f4` 为准。历史候选编号只用于追溯，不能替代当前运行包、线上 release 或真机证据。
