...
> **最新候选事实（2026-08-25）**：提交 `cd26a01` 补齐了统一状态页的旧入口、业务域和下一步准入展示；当前 pending 来源为 `cd26a01febf81729516185c79ee76f250b4541c0`，20 个页面，`277 pass / 0 fail / 2918 expect()`。构建已生成 pending，但 live `dist` 仍为 `fcc6630e`，微信开发者工具锁定导致发布返回 `EBUSY`；上一候选 `7627843a` 仅作历史追溯。
...
> **当前仓库事实（2026-08-25）**：本轮小程序功能候选为 `cd26a01`，当前小程序运行输入/pending 来源为 `cd26a01febf81729516185c79ee76f250b4541c0`；该运行输入包含前序业务候选、健康知识错误码契约、未知入口错误语义、迁移入口覆盖视图和共享门禁输入。微信开发者工具仍锁定 live `dist`，没有发布线上服务，也没有改变旧 Python 服务。服务端本地候选仍以 `b42922f4` 为准。历史候选编号只用于追溯，不能替代当前运行包、线上 release 或真机证据。

## 1. 当前真实基线

| 项目 | 当前事实 |
| --- | --- |
| 功能候选代码基线 | `cd26a01`（迁移状态页补齐旧入口与业务域覆盖视图） |
| 当前功能基线 | `cd26a01`（文档更新不改变 live `dist`） |
| 小程序业务代码候选 | `cd26a01febf81729516185c79ee76f250b4541c0` |
| 小程序 pending 运行包 | `.local/hospital-miniprogram/pending/`，`build-info.json.sourceRevision=cd26a01febf81729516185c79ee76f250b4541c0` |
| pending 页面数 | 20 个；每个页面具备 `.js/.json/.wxml/.wxss` |
| 小程序回归 | 当前源码 277 pass / 0 fail / 2918 expect()；pending 已包含当前运行输入；入口分发广度审计通过 |
