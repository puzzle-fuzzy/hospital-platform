...
> **最新候选事实（2026-08-26）**：功能工作树在既有全量 64 个旧入口 A–F 批次覆盖上继续新增健康自测安全数值子集、临床/外部/预约 Provider 页面外壳和共享页面工厂构建门禁；当前 `d204425` 的 40 页 pending 运行包已经生成并通过静态校验，今日预约摘要边界已纳入运行包。发布仍须先处理微信开发者工具对 live `dist` 的锁定，不能把旧 pending 或旧 live 包写成新候选真机证据。
...
> **当前仓库事实（2026-08-26）**：本轮小程序功能候选为 `d204425`，当前小程序运行输入/pending 来源为 `d204425420ddc8633260053e32ff042e2b0635c7`；该运行输入包含前序业务候选、全量迁移入口覆盖视图、A–F 批次展示、契约族边界、逐入口说明、健康自测安全数值子集、今日预约摘要边界、6 个临床内容、3 个外部入口、2 个预约 Provider 和 3 个患者域页面外壳、共享 Tab、统一页面滚动边界、全局资料授权边界、owner 回调保护和协议静态页。微信开发者工具仍锁定 live `dist`，没有发布线上服务，也没有改变旧 Python 服务。历史候选编号只用于追溯，不能替代当前运行包、线上 release 或真机证据。

## 1. 当前真实基线

| 项目 | 当前事实 |
| --- | --- |
| 功能候选代码基线 | `d204425`（今日预约摘要边界与既有迁移边界） |
| 当前功能基线 | `d204425`（文档更新不改变 live `dist`） |
| 小程序业务代码候选 | 功能提交 `d204425`；运行来源 `d204425420ddc8633260053e32ff042e2b0635c7` |
| 小程序 pending 运行包 | `.local/hospital-miniprogram/pending/`，`build-info.sourceRevision=d204425420ddc8633260053e32ff042e2b0635c7` |
| 当前源码页面数 | 40 个；每个页面具备 TypeScript 源码和页面配置 |
| pending 页面数 | 40 个；`runtime:verify:pending` 已通过 |
| 小程序回归 | `298 pass / 0 fail / 3394 expect()`；入口分发审计通过 |
...
2. 使用 [`device-evidence-d204425-pending.json`](../release/device-evidence-d204425-pending.json) 作为 9 个只读验收域的起始清单；一次记录成功、空结果、401、依赖不可用、Provider 超时和患者切换边界。
   `pnpm device:evidence:audit --file docs/release/device-evidence-d204425-pending.json` 在全部域仍为 `pending` 时会先完成清单、候选指纹和脱敏边界审计，但总结果仍为 `passed=false`；一旦出现 `passed/failed` 真实链路结果，线上 release 基线必须先通过，否则工具直接拒绝纳入验收。
...
```powershell
$env:HOSPITAL_MINIPROGRAM_EXPECTED_SOURCE_REVISION = "d204425420ddc8633260053e32ff042e2b0635c7"
