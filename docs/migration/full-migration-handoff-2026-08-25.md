...
> **最新候选事实（2026-08-26）**：提交 `3b42b86` 在既有全量 64 个旧入口 A–F 批次覆盖上新增协议静态页契约关联修正；当前 pending 来源为 `3b42b867ae19f6dd23bacd88648d1f5917dabf26`，21 个页面，`293 pass / 0 fail / 3237 expect()`。构建已生成 pending，但 live `dist` 仍为 `fcc6630e`，微信开发者工具锁定导致发布返回 `EBUSY`；此前候选仅作历史追溯。
...
> **当前仓库事实（2026-08-26）**：本轮小程序功能候选为 `3b42b86`，当前小程序运行输入/pending 来源为 `3b42b867ae19f6dd23bacd88648d1f5917dabf26`；该运行输入包含前序业务候选、全量迁移入口覆盖视图、A–F 批次展示、契约族边界、逐入口说明、共享 Tab、统一页面滚动边界、全局资料授权边界、owner 回调保护和协议静态页。微信开发者工具仍锁定 live `dist`，没有发布线上服务，也没有改变旧 Python 服务。服务端本地候选仍以 `b42922f4` 为准。历史候选编号只用于追溯，不能替代当前运行包、线上 release 或真机证据。

## 1. 当前真实基线

| 项目 | 当前事实 |
| --- | --- |
| 功能候选代码基线 | `3b42b86`（协议静态页契约关联修正） |
