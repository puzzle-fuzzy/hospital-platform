...
| 旧个人资料、头像、患者新增/绑卡和协议入口 | 独立 profile/consent/binding contract | 不把旧 `/system/user/current/info/update`、`/system/user/current/avatar/upload`、`patients`、`patCards` 直接暴露给小程序 | 仍待实现；旧端混用实名资料、微信身份和 provider 患者号，重复绑定、超时建档、协议同意和资源授权必须先冻结；患者绑定草案见 [`patient-binding-contract-draft.md`](patient-binding-contract-draft.md)，总边界见 [`patient-center-and-external-entry-boundaries.md`](patient-center-and-external-entry-boundaries.md) |
...
| 小程序预约/科室/报告接口 | `/api/v1/appointments`、`/api/v1/reports`、`GET /api/v1/reports/:reportId` | 以患者端业务模型重组，不按旧 provider URL 透传；爽约记录是预约历史 `status=missed` 的派生筛选，不新增 provider endpoint | 科室/排班已有服务器到真实 provider 的只读回归；预约历史/爽约筛选、报告真实 provider、公网/真机证据仍待完成；预约写入、锁号、支付和体检报告未迁移 |
