...
| 小程序预约/科室/报告接口 | `/api/v1/appointments`、`/api/v1/reports`、`GET /api/v1/reports/:reportId` | 以患者端业务模型重组，不按旧 provider URL 透传；爽约记录是预约历史 `status=missed` 的派生筛选，不新增 provider endpoint；科室/排班 adapter 拒绝重复主键，页面列表 key 不承担业务身份 | 科室/排班已有服务器到真实 provider 的只读回归；预约历史/爽约筛选、报告真实 provider、公网/真机证据仍待完成；预约写入、锁号、支付和体检报告未迁移 |
