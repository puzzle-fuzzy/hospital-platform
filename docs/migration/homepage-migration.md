...
- 首页顶部的“新增就诊人”和“更换就诊人”在已有平台会话时统一进入
  `pages/patient-select/patient-select`；不能只在首页刷新目录或静默继续使用当前用户。选择页负责目录读取、临床映射同步和
  未开放绑定能力的提示。未登录状态由首页先发起微信登录，登录成功后自动继续进入选择页；这一条路径只建立会话，
  不在首页重复同步患者，避免首页和选择页同时请求 provider。其他业务入口的登录仍保留首页患者初始化。
