...
## 当前分层候选意见反馈复核（2026-08-24）

当前线上服务端为 `28a5c0c131794ce9dcc5f94bd3809402188ac87a`，小程序提交为 `13f597e`。本轮只读对照旧端
`pagesB/user/feedback.vue` 与原生 `pages/feedback/feedback`，确认该页面没有被误接成“提交成功”的假业务：

- 旧端“意见反馈”点击处理实际只有 Toast，真正的提交页面跳转代码处于注释状态；新端保留同样的 Toast，
  不发起 API、不写入 MySQL/Redis，也不提示工单已受理；
- “咨询电话”仍需用户在弹窗中确认后才调用 `wx.makePhoneCall`，页面加载和点击反馈不会自动拨号；
- 热点问题是旧端内置的静态说明，不是服务端 FAQ，也不应被页面文案误解为医疗诊断或退费承诺；
- 在线反馈要真正迁移，必须另行确认字段、内容安全、登录/匿名权限、持久化、工单状态、重复提交幂等、
  审计日志和客服处理回执。在这些 contract 完成前，保留当前静态说明和关闭态是正确行为。

因此本轮没有新增反馈 API，也没有修改旧项目、旧服务或线上配置；该页面属于静态/电话入口已迁移，在线工单提交仍未迁移。

## 当前 13f 健康内容与自测复核（2026-08-24）

本轮对照旧端健康百科、健康自测配置、BMI 和血压计算器，并核对新端健康知识导入器与只读 service。
健康百科的 domain、repository、版本化 schema 和导入门禁已经具备，但仓库没有真实脱敏内容 bundle、临床审核记录
或 staging 发布/撤回证据；自测评分、BMI/血压规则还存在版本、适用人群和输入边界缺口。因此本轮不注册健康知识/自测
患者路由、不导入旧正文或评分配置、不用 fixture 让页面显示“成功”。具体门禁见
[`health-content-and-self-test-audit-2026-08-24.md`](health-content-and-self-test-audit-2026-08-24.md)。
