...
| `pages/appointment-records/appointment-records` | `只读已实现` | 按当前就诊人读取前后各 90 天预约历史，并翻译服务端的预约、取消、完成、爽约、停诊、替诊和已登记状态；在线范围固定读取渠道 3，全部范围通过独立 `scope=all` 读取渠道 4 并保留取消记录；查询由 dashboard service 的统一构造器生成并覆盖自然日/内部患者校验；页面首批渲染 10 条，本地继续展开；已按旧端恢复全宽就诊人/院区信息行、状态标签、列表背景、预约状态图标和院内导航弹窗。当前院区仍是单院区静态展示，不渲染未实现的动态选择入口。布局和差异边界见 [`personal-center-visual-contract.md`](personal-center-visual-contract.md)。 | 本地分批不是 provider 分页；`missed` 只能来自服务端明确状态；页面不自行推断爽约，不展示 provider 订单或支付事实；详情、预问诊、取消、退号和支付仍不能伪造；小程序只能提交 `scope=online|all`，不能透传渠道码。当前候选已完成代码和生产只读核对，但尚未切换线上。 | 先受控发布预约范围候选，再取得当前 release 的真实 provider、公网/真机三层证据；动态院区、取消/退号、详情和预问诊另立 contract。 |
