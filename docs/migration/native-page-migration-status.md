...
| `pages/patient-express/patient-express` | `页面外壳已迁移` | 已迁移“我的快递”入口、患者选择入口和稳定关闭态；旧端实际只有预留空列表，当前不把空列表当查询成功。 | 快递记录必须按 owner/就诊人过滤；单号等字段需要脱敏；成功空、拒绝、超时和 provider 异常必须区分。 | 物流来源、患者归属、字段白名单、状态枚举、查询失败语义和低敏日志。 |
