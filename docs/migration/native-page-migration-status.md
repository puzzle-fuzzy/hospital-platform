...
| `pages/appointment-records/appointment-records` | `只读已实现` | 按当前就诊人读取前后各 90 天预约历史，并翻译服务端的预约、取消、完成、爽约、停诊、替诊和已登记状态；在线范围固定读取渠道 3，全部范围通过独立 `scope=all` 读取渠道 4 并保留取消记录；查询由 dashboard service 的统一构造器生成并覆盖自然日/内部患者校验；页面首批渲染 10 条，本地继续展开；已按旧端恢复全宽就诊人/院区信息行、状态标签、列表背景、预约状态图标和院内导航弹窗；本轮将挂号卡片按类型/状态、科室、就诊时间和操作重新分层，并让 loading/空数据/错误共用稳定状态容器。当前院区仍是单院区静态展示，不渲染未实现的动态选择入口。布局和差异边界见 [`personal-center-visual-contract.md`](personal-center-visual-contract.md)。 | 本地分批不是 provider 分页；`missed` 只能来自服务端明确状态；页面不自行推断爽约，不展示 provider 订单或支付事实；详情、预问诊、取消、退号和支付仍不能伪造；小程序只能提交 `scope=online|all`，不能透传渠道码。当前候选已完成代码和生产只读核对，但尚未切换线上。 | 先受控发布预约范围候选，再取得当前 release 的真实 provider、公网/真机三层证据；动态院区、取消/退号、详情和预问诊另立 contract。 |
| `pages/missed-appointments/missed-appointments` | `部分迁移` | 从预约历史 `status=missed` 派生的过去 90 天只读筛选页，首批渲染 10 条并支持本地继续展开；“我的”页入口只要求已验证平台会话，缺少患者上下文时留在本页稳定错误态并允许重试；只有用户在页面内明确点击“更换就诊人”才进入统一选择页；爽约空态只代表已确认患者没有记录。 | 空列表不等于没有历史；未知状态不能变成爽约；本地分批不是 provider 分页；不得通过客户端传入状态制造记录；Provider/网络/持久化错误不能误导成选择患者，也不能自动打开选择模块；显式更换患者时仍须消费统一导航结果，不能把被同步门禁拦截的入口显示成永久加载。 | provider 状态映射和真实账号证据。 |
