...
| `pages/patient-signature/patient-signature` | `页面外壳已迁移` | 已迁移旧端患者列表层级、owner-scoped 脱敏患者目录、当前选中态、统一选择页、顶部错误/重试和稳定空态；不复用旧端假患者列表、不调用硬编码外部小程序。 | 签名必须绑定用途、当前 owner 和当前就诊人；文件不能跨账号读取；当前只更新页面选中态，不上传、不跳转、不展示签名结果。 | 用途与同意、文件安全、短期访问、撤回、医护侧读取和审计 contract。 |
| `pages/patient-express/patient-express` | `页面外壳已迁移` | 已按旧端真实行为迁移“当前就诊人 + 未查询到相关记录”空态，并支持返回统一就诊人选择页；旧端没有真实物流请求，当前不把预留空列表当查询成功。 | 未来物流记录必须按 owner/就诊人过滤；单号等字段需要脱敏；成功空、拒绝、超时和 provider 异常必须区分。 | 物流来源、患者归属、字段白名单、状态枚举、查询失败语义和低敏日志。 |
...
| `pages/patient-subscription/patient-subscription` | `页面外壳已迁移` | 已迁移旧端橙色提示、当前就诊人入口、标题搜索、四类提醒、折叠布局、固定底部动作和顶部错误/重试；开关固定只读并标记“暂未接入”。 | 本地展示和开关不能代表微信授权、服务端保存或消息发送；授权拒绝/过期/撤销不能显示为已订阅。 | 模板 ID、授权回执、患者/账号作用域、业务事件、发送结果、重试、撤销和审计。 |
| `pages/blood-appointment/blood-appointment` | `页面外壳已迁移` | 已按旧端真实行为迁移当前就诊人、院区位置、无可预约项目空态、顶部错误/重试和统一患者选择；旧端没有采血号源请求，不把硬编码院区当动态事实。 | 不读取普通门诊号源，不提交预约，不把空号源当成功。 | 采血号源、患者映射、状态枚举、预约写入、取消和最终查询。 |
