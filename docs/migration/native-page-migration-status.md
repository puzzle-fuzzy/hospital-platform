...
| `pages/patient-signature/patient-signature` | `页面外壳已迁移` | 已迁移患者签名入口、患者选择入口和稳定关闭态；不复用旧端假患者列表、不调用硬编码外部小程序。 | 签名必须绑定用途、当前 owner 和当前就诊人；文件不能跨账号读取；当前不上传或展示签名。 | 用途与同意、文件安全、短期访问、撤回、医护侧读取和审计 contract。 |
...
| `pages/patient-subscription/patient-subscription` | `页面外壳已迁移` | 已迁移消息订阅入口和授权关闭态，不修改微信订阅状态。 | 本地勾选不等于微信授权，授权拒绝/过期/撤销不能显示为已订阅。 | 模板 ID、授权回执、业务事件、发送结果、重试、撤销和审计。 |
