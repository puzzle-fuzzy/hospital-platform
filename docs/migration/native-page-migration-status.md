...
| `pages/missed-appointments/missed-appointments` | `部分迁移` | 从预约历史 `status=missed` 派生的过去 90 天只读筛选页，首批渲染 10 条并支持本地继续展开；患者上下文明确失效时直接进入统一就诊人选择页；同步阻塞导航时退出跳转 loading 并给出可重试错误；爽约空态只代表已确认患者没有记录。 | 空列表不等于没有历史；未知状态不能变成爽约；本地分批不是 provider 分页；不得通过客户端传入状态制造记录；Provider/网络/持久化错误不能误导成选择患者；统一导航结果必须被页面消费，不能把被同步门禁拦截的入口显示成永久加载。 | provider 状态映射和真实账号证据。 |
