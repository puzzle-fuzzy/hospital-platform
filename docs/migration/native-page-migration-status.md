...
| `pages/consult/consult` | `部分迁移` | 旧端“智能陪诊”入口的安全静态说明和迁移状态已接入正式主 Tab；页面不调用旧端 WebSocket 或队列接口。 | 静态说明不代表已经取得叫号、排队、就诊动态或患者临床状态；不能把患者 ID、队列位置或旧端消息字段直接迁入页面。 | 陪诊/叫号 Provider contract、患者与就诊事件映射、实时连接生命周期、断线重连和真机证据。 |
| `pages/hospital/hospital` | `待 contract` | 正式主 Tab 入口已注册，并明确展示互联网医院业务迁移状态。 | 旧端依赖外部 WebView；未冻结 audience、域名 allowlist、短期引用、回调和退出策略前，不加载外部 URL，不把打开页面解释成互联网医院登录或服务成功。 | 外部入口安全 contract、主体授权、短期会话、回调审计、失败/撤回策略和真机证据。 |
