...
| `pages/gift-banner/gift-banner` | `页面外壳已迁移` | 已迁移电子锦旗提交、列表和详情的统一原生入口外壳。 | 不上传文件或公开患者内容；必须先审核、脱敏，且支持撤回和失效。 | 内容审核状态机、文件安全、公开视图、幂等、撤回和审计日志。 |
| `pages/health-praise/health-praise` | `页面外壳已迁移` | 已迁移表扬信提交、列表和详情的统一原生入口外壳。 | 表扬信不是医疗证明；正文、附件和医护信息不能未经审核公开。 | 内容审核、附件安全、脱敏、患者授权、幂等、撤回和留存周期。 |
