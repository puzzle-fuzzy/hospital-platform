...
| `pages/feature-status/feature-status` | `迁移状态页` | 为旧端仍可见但尚未完成真实 contract 的入口提供统一、可解释的页面落点；固定 key 目录不接受外部 URL 或 provider 参数。 | 页面存在不代表对应业务完成；不读取旧缓存、不调用未确认 provider、不发起支付。 | 对应业务域完成 contract、adapter、API、日志和真机验收后，替换状态页入口。 |
