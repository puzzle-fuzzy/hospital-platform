...
| `pages/patient-select/patient-select` | `只读已实现` | owner-scoped 患者列表、选择、刷新和失效选择处理；可展示记录与临床可用记录分开标记。 | 页面只保存内部 `patientId`；只有 `clinicalAccess=ready` 才能选择并进入业务页；关系/证件号文案来自服务端规范化读模型；新增家属和绑卡仍关闭。 | 真实账号同步、失效/恢复、真机切换患者证据；新增绑定 provider contract。 |
...
| `pages/appointment-records/appointment-records` | `只读已实现` | 按当前就诊人读取前后各 90 天预约历史，并翻译服务端的预约、取消、完成、爽约、停诊、替诊和已登记状态；查询由 dashboard service 的统一构造器生成并覆盖自然日/内部患者校验；页面首批渲染 10 条，本地继续展开；已按旧端恢复全宽就诊人/院区信息行、状态标签、列表背景、预约状态图标和院内导航弹窗。当前院区仍是单院区静态展示，不渲染未实现的动态选择入口。布局和差异边界见 [`personal-center-visual-contract.md`](personal-center-visual-contract.md)。 | 本地分批不是 provider 分页；`missed` 只能来自服务端明确状态；页面不自行推断爽约，不展示 provider 订单或支付事实；详情、预问诊、取消、退号和支付仍不能伪造；“全部挂号”继续等待独立 `requestChannel=4` 合同。 | 真实 provider 患者映射、公网/真机证据；`requestChannel=4` 合同见 [`request-channel-4-all-records-contract-audit-2026-08-18.md`](request-channel-4-all-records-contract-audit-2026-08-18.md)，动态院区、取消/退号、详情和预问诊另立 contract。 |
| `pages/missed-appointments/missed-appointments` | `部分迁移` | 从预约历史 `status=missed` 派生的过去 90 天只读筛选页，首批渲染 10 条并支持本地继续展开。 | 空列表不等于没有历史；未知状态不能变成爽约；本地分批不是 provider 分页；不得通过客户端传入状态制造记录。 | provider 状态映射和真实账号证据。 |
...
| `pages/profile/profile` | `只读已实现` | 普通昵称、性别、年龄、邮箱资料及版本并发更新；会话失效或账号切换时清理旧资料展示并回到未加载态。 | 普通资料不等于实名资料、微信身份、手机号或患者档案；409 冲突不能静默覆盖；未确认当前会话时不能保留上一账号资料。 | 真实微信会话读写、409 真机验收；头像/实名另立 contract。 |
