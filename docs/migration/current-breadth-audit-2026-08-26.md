...
2026-08-26 最新横向补充：患者签名和消息订阅已从通用状态壳升级为原生安全展示页，分别补齐 owner-scoped 患者目录、统一就诊人切换、搜索、分类折叠、固定状态容器和顶部错误区；签名外部跳转、微信订阅授权和本地保存仍关闭。当前源码与 pending 运行包为 `c7220d733b95d393030af9826c2ca305a7fc1f8e`（`c7220d73`），小程序回归为 `301 pass / 0 fail / 3435 expect()`，不改变 64 个旧入口的状态分布。
...
| 当前小程序源码 | `ed20c525de0f0ae0ed3b047b95b7365b39c4dec9`（旧端“我的快递”真实空态迁移 + 既有全量页面覆盖） | 40 页，`299 pass / 0 fail / 3407 expect()` |
| 当前 pending 运行包 | `ed20c525de0f0ae0ed3b047b95b7365b39c4dec9` | 40 页，`runtime:verify:pending` 通过；尚未发布到 live `dist`，不能作为真机完成证据 |
