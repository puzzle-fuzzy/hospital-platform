...
| 当前小程序运行相关源码候选 | live `d4f67485a34195a2e1e392071502cf2a7006dd27`；当前无 pending 目录 | 40 页；运行包校验通过；核心回归、App.onLaunch 初始化时序、共享患者会话边界、规则版本、关闭态布局和预约请求边界测试通过 |
| 当前 live 运行包 | `d4f67485a34195a2e1e392071502cf2a7006dd27`（`d4f6748`） | 40 页，`runtime:verify` 通过；九个真机证据域仍为 pending，不能作为真机完成证据 |
...
当前 `pnpm check` 已包含 `pnpm release:baseline:audit`，并以线上 `b44421cd321ff9ff23eeb49b12641d1772d2bdc1` 与本地 live 小程序 `d4f6748` 的来源一致性为准；当前发布基线已经通过。后续运行时代码发生变化时，必须重新生成候选并完成完整发布协调，不允许用门禁豁免代替部署。
