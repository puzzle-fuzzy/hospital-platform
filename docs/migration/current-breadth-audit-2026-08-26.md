...
2026-08-26 最新横向补充：患者签名和消息订阅已从通用状态壳升级为原生安全展示页；采血预约、电子锦旗和表扬信已补齐真实可确认的患者入口、列表/记录关闭态、错误重试和统一状态说明；本批又把临床/服务入口以及我的问诊统一到共享当前就诊人上下文，并补充患者协议原文只读入口。签名外部跳转、微信订阅授权、采血号源、锦旗/表扬信 Provider、临床 Provider、问诊 Provider 和写入仍关闭。当前源码为 `de9c5b99`，本地小程序回归为 `309 pass / 0 fail / 3522 expect()`；待发布 pending 运行包仍为前一候选 `ad7bd1f7148463be5b2f48e6b389108e7ce43531`（`ad7bd1f`），不包含本轮日期窗口边界修正。源码与运行包尚未重新构建/发布前，不能把本地回归写成真机证据，也不改变 64 个旧入口的业务状态分布。
...
| 当前小程序源码 | `ad7bd1f7148463be5b2f48e6b389108e7ce43531`（我的问诊患者作用域 + 共享当前就诊人上下文 + 锦旗/表扬信安全页面 + 采血预约真实旧行为空态 + 签名/订阅安全页 + 患者协议原文只读入口 + 既有全量页面覆盖） | 40 页，`307 pass / 0 fail / 3513 expect()` |
| 当前 pending 运行包 | `ad7bd1f7148463be5b2f48e6b389108e7ce43531` | 40 页，`runtime:verify:pending` 通过；尚未发布到 live `dist`，不能作为真机完成证据 |
