...
> **当前候选覆盖（2026-08-27）**：小程序运行包已切换为 `02865d3`（完整来源 `02865d385a9c09876dc51da1ffb71183139a559b`），40 页，核心回归、App.onLaunch 全局资料初始化时序、共享患者会话边界、规则版本、关闭态布局和预约请求边界测试通过；就诊二维码安全壳已增加有效会话门禁，BMI/血压安全数值工具已固定规则版本，会话失效时旧 owner 的微信资料缓存会被清理。发布后 `runtime:verify` 通过；九个真机证据域仍为 pending。详情见 [`../release/candidate-02865d3-miniprogram-runtime-2026-08-27.md`](../release/candidate-02865d3-miniprogram-runtime-2026-08-27.md)。

> 本文是本轮广度迁移的事实记录，不把“入口已经有落点”写成“业务已经完成”。
> 旧 Python 服务、旧数据库、旧 Redis、线上旧进程和另一会话维护的众阳预约适配器不在本轮修改范围内。

2026-08-26 最新横向补充：患者签名和消息订阅已从通用状态壳升级为原生安全展示页；采血预约、电子锦旗和表扬信已补齐真实可确认的患者入口、列表/记录关闭态、错误重试和统一状态说明；本批又把临床/服务入口以及我的问诊统一到共享当前就诊人上下文，并补充患者协议原文只读入口；微信资料被拒绝后的重试已接入 `wx.openSetting` 用户点击链路，选择页刷新并发门禁已补齐，“我的快递”已区分患者加载、失败和未开放三态。本轮又把电子锦旗和表扬信记录区域统一为加载/失败/未开放三态，补齐就诊二维码安全壳的有效会话门禁，并给 BMI/血压安全数值工具增加固定规则版本；最新候选继续收紧共享患者外壳的 owner 证明、会话代际和账号切换清理，并修复 App.onLaunch 阶段全局资料初始化时序。签名外部跳转、微信订阅授权、采血号源、锦旗/表扬信 Provider、临床 Provider、问诊 Provider 和写入仍关闭。当前 40 页运行相关源码和 live 运行包来源为 `02865d385a9c09876dc51da1ffb71183139a559b`，发布前 pending 校验和发布后 `runtime:verify` 均通过；九个真机证据域仍为 `pending`。本轮另修正健康百科和报告详情迁移台账映射，并补齐预约请求底层输入校验。源码回归和运行包校验不能写成真机证据，也不改变 64 个旧入口的业务状态分布。
...
| 当前小程序运行相关源码候选 | live `02865d385a9c09876dc51da1ffb71183139a559b`；当前无 pending 目录 | 40 页；运行包校验通过；核心回归、App.onLaunch 初始化时序、共享患者会话边界、规则版本、关闭态布局和预约请求边界测试通过 |
| 当前 live 运行包 | `02865d385a9c09876dc51da1ffb71183139a559b`（`02865d3`） | 40 页，`runtime:verify` 通过；九个真机证据域仍为 pending，不能作为真机完成证据 |
| 线上服务端 | `0aaa13b53cb6e21b59b332dbd4e2b982a5aba1e7` | 与旧 Python `8001` 共存，未修改旧服务 |
...
`0aaa13b5` 已包含本轮健康知识运行时代码。旧 Python `8001` 继续共存。剩余阻断来自真实
...
当前 `pnpm check` 已包含 `pnpm release:baseline:audit`，并以线上 `0aaa13b53cb6e21b59b332dbd4e2b982a5aba1e7` 与本地 live 小程序 `02865d3` 的来源一致性为准；当前发布基线已经通过。后续运行时代码发生变化时，必须重新生成候选并完成完整发布协调，不允许用门禁豁免代替部署。
