...
> **当前候选覆盖（2026-08-27）**：小程序运行包已切换为 `6f47c64`（完整来源 `6f47c6408fe5b62025bd74fa66893f306eb7b9aa`），40 页，核心回归、共享患者会话边界、规则版本和关闭态布局测试通过；就诊二维码安全壳已增加有效会话门禁，BMI/血压安全数值工具已固定规则版本，会话失效时旧 owner 的微信资料缓存会被清理。发布后 `runtime:verify` 通过；九个真机证据域仍为 pending。详情见 [`../release/candidate-6f47c64-miniprogram-runtime-2026-08-27.md`](../release/candidate-6f47c64-miniprogram-runtime-2026-08-27.md)。
