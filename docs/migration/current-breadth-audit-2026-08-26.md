...
2026-08-26 最新横向补充：患者签名和消息订阅已从通用状态壳升级为原生安全展示页；本轮又把旧端采血预约的真实当前就诊人、院区占位、无可预约项目空态、顶部错误区和统一就诊人切换补齐。签名外部跳转、微信订阅授权、采血号源和预约写入仍关闭。当前源码与 pending 运行包为 `34d86d918ecaf09d0adec9a6a5cf3e225add0722`（`34d86d9`），小程序回归为 `302 pass / 0 fail / 3450 expect()`，不改变 64 个旧入口的业务状态分布。
...
| 当前小程序源码 | `34d86d918ecaf09d0adec9a6a5cf3e225add0722`（采血预约真实旧行为空态迁移 + 签名/订阅安全页 + 既有全量页面覆盖） | 40 页，`302 pass / 0 fail / 3450 expect()` |
| 当前 pending 运行包 | `34d86d918ecaf09d0adec9a6a5cf3e225add0722` | 40 页，`runtime:verify:pending` 通过；尚未发布到 live `dist`，不能作为真机完成证据 |
