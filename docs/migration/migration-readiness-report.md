...
> **当前运行事实（2026-08-26）**：最新 pending 小程序来源为 `3b42b867ae19f6dd23bacd88648d1f5917dabf26`（提交 `3b42b86`），21 个页面；当前 live `dist` 仍为 `fcc6630e`，微信开发者工具锁定导致 pending 尚未发布。当前源码回归为 `293 pass / 0 fail / 3237 expect()`，真机证据清单为 `device-evidence-3b42b86-pending.json`，9 个域仍为 `pending`。协议静态页已迁移，但同意/撤回/审计能力仍关闭。
...
- live `dist` 来源为 `fcc6630e`，当前 pending 来源为 `3b42b86`；两者不一致，所以待发布候选仍需在微信开发者工具释放目录锁后原子发布。`3b42b86` 还包含旧端使用条款原文只读页及其静态/同意 contract 边界；当前小程序回归为 `293 pass / 0 fail / 3237 expect()`。
