...
- 关闭态审计器已在 `2580a1fe` 修正模板插值扫描边界：模板普通文案仍不参与调用匹配，`${...}` 内的可执行表达式不会被剥离；当前 `1bc5bf6` 修复 App.onLaunch 全局资料初始化时序，不改变任何业务放行状态。
