...
> **当前运行事实（2026-08-26）**：最新 pending 小程序来源为 `25cd7c4a323c9767b9367d319be14a82f681b37b`（功能提交 `b4120bac`），25 个页面；当前 live `dist` 仍为旧来源，微信开发者工具锁定导致 pending 尚未发布。当前源码回归为 `293 pass / 0 fail / 3269 expect()`；四个临床页面是 `surface-only` 外壳，真实 Provider/临床业务仍关闭，真机证据尚未采集。协议静态页已迁移，但同意/撤回/审计能力仍关闭。
...
| `migrationBreadth` | `migration-breadth-audit.mjs` 的结果 | 入口广度审计是否已经纳入总结构准入；任一可见 action 缺少固定分发，或 25 个页面的 WXML 事件缺少 TS 方法/共享页面工厂注册时，`structuralAuditPassed` 直接为 `false` | 不代表入口背后的 Provider、临床或支付业务已经完成 |
...
- 原生小程序注册 25 个页面，四个主入口继续使用微信原生 `tabBar`；四个临床入口当前为 `surface-only`。
...
- live `dist` 仍为旧来源，当前 pending 来源为 `b4120bac`；两者不一致，所以待发布候选仍需在微信开发者工具释放目录锁后原子发布。`b4120bac` 包含四个临床页面外壳及其关闭态边界；当前小程序回归为 `293 pass / 0 fail / 3269 expect()`。
