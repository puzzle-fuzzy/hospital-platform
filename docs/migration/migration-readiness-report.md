...
> **当前运行事实（2026-08-25）**：pending 小程序来源为 `ad793c80b0d05b74ee0cee76058b32bd25b4ce38`（提交 `ad793c80`），20 个页面；当前 live `dist` 仍为 `fcc6630e`，微信开发者工具锁定导致 pending 尚未发布。当前源码回归为 `279 pass / 0 fail / 3031 expect()`，真机证据清单为 `device-evidence-ad793c8-pending.json`，9 个域仍为 `pending`。
...
- live `dist` 来源为 `fcc6630e`，当前 pending 来源为 `ad793c80`；两者不一致，所以待发布候选仍需在微信开发者工具释放目录锁后原子发布。`ad793c80` 包含统一状态页迁移阶段、旧入口覆盖数量、业务域、所属 A–F 批次和下一步材料；当前小程序回归为 `279 pass / 0 fail / 3031 expect()`。
