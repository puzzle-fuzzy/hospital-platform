...
> 状态：已实现代码契约和本地测试；真实生产 schema、公网接口和真机证据仍待单独验收。
>
...
5. 日志只记录事件名、traceId、修改字段数量、版本和错误类型；禁止记录昵称、邮箱和完整请求体。
...
生产启用前必须先应用 0014 migration、通过 schema probe、确认回滚策略，再做公网 HTTPS 和真机
验收；未通过这些证据前不能把文档中的“代码已实现”写成“生产已完成”。
