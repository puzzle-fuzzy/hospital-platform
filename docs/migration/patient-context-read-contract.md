...
其中门诊费用、预约记录和报告目录的加载器还必须在小程序服务层再次调用同一个 patientId
形状校验，不能因为页面已经展示过患者卡片就把空白、控制字符或超过 128 个 UTF-16 code unit
的标识交给 API。这样可以把患者上下文错误在网络请求之前收敛为统一的
`patient-selection-required`，也避免为无效查询制造业务日志；服务端仍然保留 owner 归属校验，
客户端前置校验不是安全边界的替代品。
...
### 2.2 首页会话状态必须绑定当前目录读取结果

首页的 `/patients` 读取还承担“当前会话是否已经恢复到可进入业务入口”的显示状态收敛，
因此不能只看 Promise 是否 resolved。`loadPatients()` 的 `loaded` 表示本轮目录确实提交到当前页面，
`superseded` 表示它已经被新的页面读取或生命周期淘汰；后者不能把 `sessionStatus` 改成“已恢复会话”。
`onShow` 必须同时检查目录结果、页面生命周期和会话 guard；下拉刷新如果淘汰了 `onShow` 的读取，
也必须由自己的当前周期在真正 `loaded` 后再恢复状态。这样不会出现“旧回调宣称会话有效、最新患者目录仍在加载”
的窗口。该状态门禁只控制小程序展示和入口，不替代服务端 `/me`、owner 和患者映射校验。
...
- 首页状态门禁必须确保 `onShow` 和下拉刷新消费 `PatientDirectoryLoadResult`，只有 `loaded` 才能写入“已恢复会话”，
  并在回调中检查页面生命周期和 session guard；不能用 `Promise.resolve` 或 `superseded` 冒充当前目录成功；
