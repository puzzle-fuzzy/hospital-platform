...
### 7.4 我的医生：旧接口、外部头像和索引兜底都未形成稳定契约
...
因此当前不能只把页面改成新端列表就宣称“我的医生”已迁移。正式实现必须先明确医生关系的来源、有效状态、owner 范围、稳定引用、详情字段和隐私边界；头像也必须使用新端受控资源引用。没有这些资料时，继续保持关闭/迁移提示。
...
本轮结论：上述五类页面没有发现可以安全转化为真实业务的隐藏 P0 子集；协议正文和关注公众号宣传页仍可作为明确标注的静态内容，协议同意、订阅保存、患者签名、医生关系和 WebView 登录态继续等待契约。该结论与本文件的迁移矩阵和 `pnpm migration:audit` 门禁保持一致。
