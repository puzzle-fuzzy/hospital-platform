 ...
 * 历史；线上和全部范围都必须由服务端各自的已确认查询 contract 决定。
 */
 ...
 * 读取患者端业务页当前可用的就诊人。
 *
 * 这里读取的是当前 owner 的实时众阳目录：服务端 `/patients` 会先请求众阳，
 * 再返回经过校验的最小读模型，并把内部镜像只用于稳定的 patientId 和后续
 * 业务关联。页面不需要自行拼 Provider 参数，也不能把本地旧目录当成最新事实。
 * 预约记录、爽约记录、报告和门诊费用页仍只需要在发起各自查询前确认最新目录
 * 与本地显式选择保持一致。
 *
 ...
 * 生成预约记录的唯一查询契约。
 *
 * “我的挂号”页面的两个标签共用服务端完整挂号记录，在线标签只在已归一化
 * 读模型上做筛选；这样切换标签不会出现两套 Provider 查询结果互相不一致。
 * “爽约记录”仍使用过去 90 天的有限窗口，因为它是独立的派生页面。
 * 把窗口和范围选择集中在这里，页面就不能因为复制代码而改变业务意图。
 */
export function createAppointmentRecordQuery(
	patientId: string,
	now?: Date,
	window?: AppointmentRecordQueryWindow,
	scope?: "online",
): {
	patientId: string;
	scope: "online";
...
export function createAppointmentRecordQuery(
	...
	scope: AppointmentRecordScope = "online",
): AppointmentRecordQuery {
