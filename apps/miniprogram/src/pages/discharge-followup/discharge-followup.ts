...
export const DISCHARGE_FOLLOWUP_ITEMS: ReadonlyArray<FollowupItem> =
	Object.freeze([
		{
			title: "高平市人民医院一级随访记录表",
			tableName: "高平市人民医院一级随访记录表",
		},
		{
			title: "高平市人民医院四级手术随访记录表",
			tableName: "高平市人民医院四级手术随访记录表",
		},
		{
			title: "心内科四级手术随访记录表",
			tableName: "心内科四级手术随访记录表",
		},
		{
			title: "神经内科四级手术随访记录表",
			tableName: "神经内科四级手术随访记录表",
		},
		{
			title: "高平市人民医院二级回访登记表",
			tableName: "高平市人民医院二级回访登记表",
		},
		{
			title: "高平市人民医院日间手术随访记录表",
			tableName: "高平市人民医院日间手术随访记录表",
		},
		{
			title: "高平市人民医院四级手术二次回访登记表",
			tableName: "高平市人民医院四级手术二次回访登记表",
		},
		{
			title: "高平市人民医院日间手术二级回访登记表",
			tableName: "高平市人民医院日间手术二级回访登记表",
		},
	]);
...
Page<DischargePageData, DischargePageMethods>({
	...
	async loadPage() {
		...
		try {
			...
			const hasCompletedVisit = records.some(
	...
	onFollowupTap(event) {
		...
		const tableName = String(event.currentTarget.dataset.tableName ?? title);
		if (!title) return;
		wx.navigateTo({
			url: `/pages/discharge-followup-detail/discharge-followup-detail?tableName=${encodeURIComponent(tableName)}&displayTitle=${encodeURIComponent(title)}`,
		});
	},
