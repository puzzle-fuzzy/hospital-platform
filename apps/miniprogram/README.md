...
“智能导诊”已经替换为原生 `pages/smart-guide/smart-guide`：支持文字、最长 60 秒的长按录音、多轮会话、
免责声明、科室推荐和进入挂号。小程序每次发送只提供新的微信临时 code；旧服务 JWT 和原始
`conversation_id` 始终留在服务端，平台只下发 owner-scoped 会话引用。音频与症状正文不进入客户端
预览或 Provider 原始日志。该代码边界不等于旧模型、语音识别、真实微信账号和真机已经验收。
...
本候选还将四组共享 Tab 普通态/选中态图标固定为微信建议的
`81×81` PNG；真实手机选中态仍需重新普通编译后复核。若工具标题或控制台来源 revision 不符合本候选，
先关闭错误工程并按上述缓存步骤重开，再进行真机预览。

四个主入口使用微信原生 tabBar，四项路由、图标、选中图标和顺序唯一声明在
`src/app.json` 的 `tabBar.list` 中，并显式设置 `custom: false`、`position: bottom`；
页面代码不渲染第二份底栏，也不维护第二份 selected 状态。自定义 `custom-tab-bar` 已移除，
避免真机组件层未挂载时出现底栏完全消失。
...
主 Tab 页面使用 `disableScroll: true` 和独立 `scroll-view`；原生底栏固定在窗口底部，
统一的 `padding-bottom` 只属于内容滚动区域，因此只有内容区域滚动。
后续替换图标时必须同时保留普通态和选中态两份实际不同的资源，并确保
	原生 `tabBar` 的 `selectedIconPath` 不复用 `iconPath`；构建脚本仍会检查根配置中的两组图标路径和文件字节。
...
“我的挂号”进入 `pages/appointment-records/appointment-records`，按当前选择的内部 `patientId` 查询当前中国标准时间日前后各三个月记录；
...
正式主小程序在预约写入后进入 `pages/registration-payment/registration-payment`，提供“医保支付”和“微信支付”两条入口；纯医保入口不对用户展示，历史纯医保订单仍保留恢复能力。
...
医保支付的微信现金段使用 2.6.65.2 `result` 返回并经服务端校验的 APIv2/MD5 参数；
