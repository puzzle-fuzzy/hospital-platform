...
test("native my page separates ordinary profile from family patient selection", async () => {
	...
	// 四个主入口交给微信官方 custom-tab-bar 管理；页面自身不能复制底栏，
	// 激活态由组件根据当前 route 统一维护，避免每个页面各自创建底栏。
	...
	const customTabBar = await source("custom-tab-bar/index.wxml");
	const customTabBarScript = await source("custom-tab-bar/index.ts");
	const customTabBarStyle = await source("custom-tab-bar/index.wxss");
	expect(customTabBar).toContain('class="legacy-tabbar"');
	expect(customTabBar).toContain("legacy-tabbar-item-active");
	expect(customTabBarScript).toContain("resolveSelectedTab");
	expect(customTabBarScript).toContain("wx.switchTab");
	expect(customTabBarScript).toContain(
		"this.setData({ selected: index, switching: true })",
	);
	expect(customTabBarScript).toContain("路由失败时回滚视觉状态");
	expect(customTabBarStyle).toContain("position: fixed");
	expect(customTabBarStyle).toContain("env(safe-area-inset-bottom)");
...
test("shared custom primary tabs keep one stable selected bar", async () => {
	...
	// 页面只负责自己的内容；底栏由微信官方 custom-tab-bar 统一渲染。
