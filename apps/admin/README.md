...
新服务 Admin 查询地址和服务间令牌；建议只监听回环地址并通过 Nginx 提供 HTTPS。

浏览器只访问本项目的同源 `/api`；Bun 服务端代理仅开放验证码、登录、退出和 1101 四个固定
入口，不接受任意上游路径。查询代理只发送固定的 `X-Admin-Query-Token`，不会把新服务令牌
下发浏览器。账号密码、访问令牌、身份证号和医保返回内容不会写入应用日志，登录状态只保存在
当前标签页的 `sessionStorage`。
