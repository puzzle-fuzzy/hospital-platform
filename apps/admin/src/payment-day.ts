...
function recordTime(message: JsonObject, envelope: JsonObject): string | undefined {
...
export function parsePaymentJournal(serialized: string): PaymentJournalRecord[] {
...
function orderSeed(record: PaymentJournalRecord): string | undefined {
	...
	return orderId && event && (ORDER_SEED_EVENTS.has(event) || PAYMENT_EVENT.test(event))
		? orderId
		: undefined;
}

function createOrder(record: PaymentJournalRecord, orderId: string): PaymentOrder {
	return {
		orderId,
		appointmentId: stringValue(record.message.appointmentId),
...
function addRecord(order: PaymentOrder, record: PaymentJournalRecord): void {
	...
	if (!order.appointmentId) {
		order.appointmentId = stringValue(record.message.appointmentId);
	}
...
function selectTimelineOrder(
	...
	for (let index = 0; index < timeline.length; index += 1) {
		...
		const nextStart = timeline[index + 1]
			? Date.parse(timeline[index + 1].startedAt)
...
export function collectPaymentOrders(records: PaymentJournalRecord[]): PaymentOrder[] {
	...
	for (const record of records) {
		const orderId = stringValue(record.message.orderId) || stringValue(record.message.taskId);
	...
	for (const record of records) {
		...
		const candidates = ordered.filter((order) => order.appointmentId === appointmentId);
...
export function paymentDayWindow(date: string): PaymentDayWindow {
	...
	if (Number.isNaN(start.getTime())) throw new Error("payment-day-date-invalid");
...
function boundaryFor(timestamp: string, window: PaymentDayWindow): PaymentBoundary {
...
): { entries: RawLogEntry[]; reasons: Map<RawLogEntry, Attribution>; warningCount: number } {
	const reasons = new Map<RawLogEntry, Attribution>();
	const selected: RawLogEntry[] = [];
	let warningCount = 0;
	for (const entry of entries) {
		const correlated = allOrders.filter((candidate) => hasIdentifier(entry, candidate));
...
function statusForOrder(
	...
	if (interfaces.some((item) => item.request && item.response)) return "OBSERVED";
	return "INCOMPLETE";
}
...
function buildInternalFlow(
	...
	const primary = invocations.filter((invocation) => invocation.layer === "transport");
			...
			return {
				...
				...(invocation.request?.statusCode !== undefined
					...
					: invocation.response?.statusCode !== undefined
						...
						: {}),
				complete: Boolean(invocation.request?.complete && invocation.response?.complete),
				attribution,
...
export function buildPaymentDaySnapshot(
	...
	if (new TextEncoder().encode(serialized).byteLength > PAYMENT_MAX_JOURNAL_BYTES) {
...
export function publicPaymentDay(snapshot: PaymentDaySnapshot): PaymentDayResult {
	return {
		...snapshot,
		orders: snapshot.orders.map((order) => ({
			...order,
			interfaces: order.interfaces.map(({ request: _request, response: _response, ...summary }) => summary),
		})),
	};
}
...
export function paymentInterfaceDetail(
	...
	const interfaceItem = flow?.interfaces.find((item) => item.ordinal === ordinal);
