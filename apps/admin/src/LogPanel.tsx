...
function recordText(record: AdminLogRecord): string {
	const endpoint = [record.method || "EVENT", record.path || record.event || "-"]
		.join(" ")
		.trim();
	const status = record.statusCode === undefined ? "-" : String(record.statusCode);
	const duration =
...
export function LogPanel({
	...
	useEffect(() => {
		...
		if (!terminal || !followBottomRef.current) return;
	...
	return (
		<main className="console-content terminal-page">
			<div className="terminal-toolbar">
				...
				<Tag color={error ? "error" : "green"}>{error ? "连接异常" : "LIVE"}</Tag>
			</div>
			<div
				ref={terminalRef}
			...
			>
				<div className="terminal-line terminal-line-system">
					<span className="terminal-prompt">$</span> hospital-admin tail --follow
				</div>
				{records.length === 0 ? (
				...
				{error ? (
					<div className="terminal-line terminal-line-error">[poll] {error}</div>
				) : null}
			</div>
			<div className="terminal-status">
				<span>{records.length} 条可见记录</span>
				<span>
					{lastUpdated ? `最后更新 ${formatTime(lastUpdated.toISOString())}` : "正在连接…"}
				</span>
			</div>
		</main>
	);
