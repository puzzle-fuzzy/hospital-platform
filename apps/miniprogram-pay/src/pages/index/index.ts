...
async function releaseAfterPaymentExit(
	page: PageInstance,
	pending: ReturnType<typeof readPendingPayment>,
	message: string,
): Promise<void> {
	if (!pending) {
		page.setData({ hasPendingPayment: false, error: "", message });
		return;
	}
	try {
		...
		page.setData({
			...
			message: "支付已取消，但订单或号源尚未释放，请点击原支付方式重试清理",
		});
	}
}

async function showNavigationCancelled(page: PageInstance): Promise<void> {
	await releaseAfterPaymentExit(
		page,
		readPendingPayment(),
		"已取消医保授权，订单已失效，号源已释放，请重新预约",
	);
}

async function showWechatPaymentCancelled(page: PageInstance): Promise<void> {
	await releaseAfterPaymentExit(
		page,
		readPendingPayment(),
		"已取消支付，订单已失效，号源已释放，请重新预约",
	);
}
...
>({
	...
	onShow() {
		...
		if (pending?.phase === "self_payment_cancelled") {
			this.setData({
				...
				hasPendingPayment: true,
				error: "",
				message: "正在释放已取消的自费订单和号源",
			});
			void showWechatPaymentCancelled(this).finally(() =>
		...
		if (!authCode && pending?.phase === "cash_payment" && !resumingPayment) {
			...
			const paymentMode = pending.mode === "medical" ? "medical" : "mixed";
			this.setData({
				busy: true,
				hasPendingPayment: true,
				selectedMode: paymentMode,
		...
		if (!authCode && pending) {
			...
			this.setData({
				...
				message:
					pending.phase === "cash_payment"
						? pendingMode === "medical"
							? "检测到未完成的纯医保支付，请选择纯医保支付继续"
							: "检测到未完成的医保混合支付，请选择医保混合支付继续"
						: pending.phase === "self_payment"
	...
	async bootstrap() {
		...
		try {
			...
			const pending = readPendingPayment();
			const defaultIndex = initialPaymentPatientIndex(
				patients,
				pending?.patientId,
			);
			const defaultPatient = patients[defaultIndex];
			const pendingPatientUnavailable = Boolean(pending && !defaultPatient);
			this.setData({
				patients,
				patientNames: patients.map((item) => item.displayName),
				patientIndex: defaultIndex,
				hasPendingPayment: Boolean(pending),
				...(pendingPatientUnavailable
					? {
							error:
								"待支付订单绑定的就诊人已不在当前账号目录，已禁止切换后继续支付，请先恢复该就诊人或由工作人员处理原订单",
						}
					: {}),
				...(defaultPatient
					? {
	...
	onRegisterAndPay(event: PaymentModeEvent) {
		...
		if (
			...
			this.setData({
				error:
					"待支付订单与当前就诊人不一致，已禁止继续支付，请先恢复原就诊人或由工作人员处理原订单",
				message: "",
			});
			return;
		}
					...
					this.setData({
						message:
							expectedMode === "medical"
								? "当前是纯医保订单，请选择纯医保支付继续确认"
								: "当前是医保混合订单，请选择医保混合支付继续确认",
						error: "",
					});
					return;
				}
