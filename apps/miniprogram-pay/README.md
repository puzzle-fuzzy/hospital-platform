...
> **已退役（2026-09-17）**：本项目不再使用，也不再接受功能开发、问题修复、构建、验收或发布；目录暂时保留用于历史追溯，后续择机删除。现行支付入口和流程请查看 [`../../docs/迁移/miniprogram-pay退役说明.md`](../../docs/迁移/miniprogram-pay退役说明.md) 及主项目文档。
>
> 下方链路、配置项和构建命令均为退役前记录，仅用于历史追溯，不应据此新增功能、构建运行包或执行发布。
...
支付，不能回退到唯一剩余的其他就诊人。亲属代付时，服务端把当前微信账号下唯一“本人”
档案作为 `payer`，把当前订单绑定的选中就诊人作为 `relative`，并将两者的实名信息从
服务端档案重新读取；小程序不能提交或替换这两个身份。
