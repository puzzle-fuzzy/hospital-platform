...
export const READ_ONLY_DOMAIN_CATALOG = [
	...
	{
		id: "outpatient-medical-records",
		name: "门诊就诊摘要目录",
		operationClass: "read-only",
		pages: ["pages/medical-record/medical-record"],
		publicRoutes: ["GET /api/v2/medical-records"],
		internalRouteTokens: ["/medical-records"],
		apiModule: "apps/api/src/modules/medical-records/index.ts",
		serviceFiles: ["apps/api/src/modules/medical-records/service.ts"],
		domainFiles: ["packages/domain/src/medical-records.ts"],
		adapterFiles: ["packages/adapters/src/zhongyang-medical-records.ts"],
		documentation: [
			"docs/迁移/病案目录契约草案.md",
			"docs/迁移/临床查询状态一致性-2026-08-28.md",
		],
		logEvents: [
		...
		boundary:
			"只读近 30 天门诊就诊摘要；病历正文、详情、附件和住院病历继续关闭。",
		semanticStates: [
			"requesting",
			"success-non-empty",
			"success-empty",
		...
		emptyResult: {
			...
			mustNotMaskError: true,
			meaning:
				"Provider 明确确认当前就诊人在近 30 天内没有可展示的门诊就诊摘要",
		},
		errorCodes: [
